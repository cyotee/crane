...
/// @notice Phase 1 greenfield factories: Crane core + ERC20Permit + Uni V2/V4 + Permit2.
///
...
///      - `script/output/battlechain-sepolia/greenfield-phase1.latest.json` (runtime copy)
///      Tell an agent "Phase 1 greenfield deployed" — they read the JSON and refresh
...
contract Script_BC_Phase1_Factories is BCPhaseScriptBase {
    using BetterEfficientHashLib for bytes;

    /// @dev Greenfield agreement salt — must not reuse Wave A promo salt (collision risk).
    bytes32 internal constant AGREEMENT_SALT = keccak256("crane-indexedex-bc-greenfield-v1");

    string internal constant SAMPLE_TOKEN_NAME = "Crane BC Greenfield Token";
    string internal constant SAMPLE_TOKEN_SYMBOL = "CBCG";
    ...
    string internal constant MANIFEST_RUNTIME_JSON = "script/output/battlechain-sepolia/greenfield-phase1.latest.json";
    ...
    function _protocolName() internal pure override returns (string memory) {
        return "Crane DeFi Ports - BC Greenfield";
    }
    ...
    /// @notice Deploy without start/stopBroadcast (for FullStack composition).
    ...
    function run() external {
        ...
        console2.log("Tell agent: Phase 1 greenfield BattleChain deploy complete; refresh Deployed Addresses from JSON.");
    }
    ...
    function _bindBattlechainProvided() internal {
        ...
        if (block.chainid == 627) {
            require(weth.code.length > 0, "Script_Phase1: BC WETH has no code");
            require(uniV3Factory.code.length > 0, "Script_Phase1: BC Uni V3 Factory has no code");
            require(uniV3SwapRouter.code.length > 0, "Script_Phase1: BC Uni V3 SwapRouter has no code");
            require(uniV3Npm.code.length > 0, "Script_Phase1: BC Uni V3 NPM has no code");
        }
    }
    ...
    function _logAddresses() internal view {
        console2.log("=== BattleChain Phase 1 Greenfield ===");
    ...
    function _writeJsonManifest(string memory path, address deployer) internal {
        ...
        vm.writeLine(path, '  "product": "Crane DeFi Ports - BC Greenfield",');
        vm.writeLine(path, '  "script": "scripts/foundry/bc/Script_BC_Phase1_Factories.s.sol",');
        vm.writeLine(path, '  "agreementSalt": "crane-indexedex-bc-greenfield-v1",');
    ...
    function _writeTableManifest(string memory path) internal {
        vm.writeFile(path, "<!-- GENERATED by Script_BC_Phase1_Factories - do not edit by hand -->\n");
        ...
        _writeTableRow(path, "Sample permit token (CBCG)", samplePermitToken);
