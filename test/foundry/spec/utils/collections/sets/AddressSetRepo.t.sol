...
/// forge-lint: disable-next-line(unaliased-plain-import)
import "src/utils/collections/sets/AddressSetRepo.sol";
...
contract AddressSetRepoTest is Test {
    ...
    function test_addExclusive(address value) public {
        testInstance._addExclusive(value);
        assertEq(value, testInstance.values[testInstance.indexes[value] - 1]);
    }

    function test_addExclusive(address[] calldata values) public {
        for (uint256 cursor = 0; values.length > cursor; cursor++) {
