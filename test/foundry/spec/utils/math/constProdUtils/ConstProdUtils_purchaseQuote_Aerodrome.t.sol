...
contract ConstProdUtils_purchaseQuote_Aerodrome is TestBase_ConstProdUtils_Aerodrome {
    ...
    function _testPurchaseQuote_Aerodrome_AtoB(
        ...
        TestData memory data;
        {
            ...
            data.expectedInput = ConstProdUtils._purchaseQuote(
                data.desiredOutput, data.reserveA, data.reserveB, AERO_FEE_PERCENT, 10000
            );
        ...
        if (data.expectedInput > 0) {
            emit log_named_uint(
                "pair.getAmountOut(expectedInput-1)", pair.getAmountOut(data.expectedInput - 1, address(tokenB))
            );
        }
        ...
        aerodromeRouter.swapExactTokensForTokens(
            data.amountIn, data.desiredOutput, routes, address(this), block.timestamp + 300
        );
    ...
    function _testPurchaseQuote_Aerodrome_BtoA(
        ...
        TestData memory data;
        {
            ...
            data.expectedInput = ConstProdUtils._purchaseQuote(
                data.desiredOutput, data.reserveB, data.reserveA, AERO_FEE_PERCENT, 10000
            );
            data.amountIn = data.expectedInput;
        }
        ...
        if (data.expectedInput > 0) {
            emit log_named_uint(
                "pair.getAmountOut(expectedInput-1)", pair.getAmountOut(data.expectedInput - 1, address(tokenA))
            );
        }
