...
contract ConstProdUtils_swapDepositSaleAmt_Uniswap is TestBase_ConstProdUtils_Uniswap {
    ...
    function test_swapDepositSaleAmt_Uniswap_unbalancedPool() public {
        ...
        (uint256 reserveA, uint256 reserveB) = sortedReserves(address(uniswapUnbalancedTokenA), uniswapUnbalancedPair);
        uint256 feePercent = 300; // Uniswap V2 default fee in PPHK (0.3% == 300)
        ...
        uint256 expectedLPTokens = ConstProdUtils._depositQuote(
            remainingTokenA,
            tokenBReceived,
            uniswapUnbalancedPair.totalSupply(),
            reserveA + saleAmt, // New reserveA after swap
            reserveB - tokenBReceived // New reserveB after swap
        );

        // Execute the deposit directly to the unbalanced pair and mint
        uniswapUnbalancedTokenA.transfer(address(uniswapUnbalancedPair), remainingTokenA);
        uniswapUnbalancedTokenB.transfer(address(uniswapUnbalancedPair), tokenBReceived);
        uint256 actualLPTokens = uniswapUnbalancedPair.mint(address(this));
    ...
    function test_swapDepositSaleAmt_Uniswap_extremeUnbalancedPool() public {
        ...
        (uint256 reserveA, uint256 reserveB) = sortedReserves(address(uniswapExtremeTokenA), uniswapExtremeUnbalancedPair);
        uint256 feePercent = 300; // Uniswap V2 default fee in PPHK (0.3% == 300)
        ...
        uint256 expectedLPTokens = ConstProdUtils._depositQuote(
            remainingTokenA,
            tokenBReceived,
            uniswapExtremeUnbalancedPair.totalSupply(),
            reserveA + saleAmt, // New reserveA after swap
            reserveB - tokenBReceived // New reserveB after swap
        );

        // Execute the deposit directly to the extreme pair and mint
        uniswapExtremeTokenA.transfer(address(uniswapExtremeUnbalancedPair), remainingTokenA);
        uniswapExtremeTokenB.transfer(address(uniswapExtremeUnbalancedPair), tokenBReceived);
        uint256 actualLPTokens = uniswapExtremeUnbalancedPair.mint(address(this));
