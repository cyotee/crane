...
contract HookMinerTest is Test {
    function tryFind(uint16 flags, uint256 number) external view returns (address, bytes32) {
    ...
    function test_fuzz_hookMiner(uint16 flags, uint256 number) public {
        address addr;
        bytes32 salt;
        // ~1/18,000 (flags, number) pairs have no salt within HookMiner.MAX_LOOP; reject those.
        try this.tryFind(flags, number) returns (address a, bytes32 s) {
            (addr, salt) = (a, s);
        } catch {
            vm.assume(false);
        }
