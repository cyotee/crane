...
contract WstETHHookTest is Test, Deployers {
    ...
    function setUp() public {
        ...
        deployCodeTo(
            "contracts/protocols/dexes/uniswap/v4/hooks/public/WstETHHook.sol:WstETHHook",
            abi.encode(manager, wstETH),
            address(hook)
        );

        wrapZeroForOne = address(stETH) < address(wstETH);

        // Create pool key for stETH/wstETH using sorted currency order
        poolKey = PoolKey({
            currency0: wrapZeroForOne ? Currency.wrap(address(stETH)) : Currency.wrap(address(wstETH)),
            currency1: wrapZeroForOne ? Currency.wrap(address(wstETH)) : Currency.wrap(address(stETH)),
    ...
    function test_unwrap_exactInput() public {
        ...
        router.swap(
            ...
            SwapParams({
                ...
                sqrtPriceLimitX96: wrapZeroForOne ? TickMath.MAX_SQRT_PRICE - 1 : TickMath.MIN_SQRT_PRICE + 1
            }),
            ""
        );
