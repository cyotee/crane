 ...
 * deprecated AerodromeService library. DO NOT copy usage patterns from here.
 *
 * For new code, use the specialized libraries instead:
 *   - AerodromeServiceVolatile.sol - For volatile pools (xy = k curve)
 *   - AerodromeServiceStable.sol   - For stable pools (x³y + xy³ = k curve)
 *
 * See AerodromeServiceVolatile.t.sol for the canonical test patterns.
 *
 ...
 *   3. Validate the migration guide in AerodromeService.sol
 */
/* -------------------------------------------------------------------------- */
...
/**
 * @title AerodromeService_Test
 * @notice DEPRECATED: Backward compatibility tests for the legacy AerodromeService library
 * @dev For new volatile pool tests, see AerodromeServiceVolatile.t.sol
 *      For new stable pool tests, see AerodromeServiceStable.t.sol
 */
contract AerodromeService_Test is TestBase_Aerodrome_Pools {
    ...
    function test_swap_normalSwap_returnsExpectedOutput() public {
        ...
        AerodromeService.SwapParams memory params = AerodromeService.SwapParams({
        ...
        uint256 amountOut = AerodromeService._swap(params);
    ...
    function test_swapDepositVolatile_balancedPool_mintsLP() public {
        ...
        uint256 lpAmount = AerodromeService._swapDepositVolatile(params);
    ...
    function test_withdrawSwapVolatile_fullWithdrawal_returnsTargetToken() public {
        ...
        uint256 amountOut = AerodromeService._withdrawSwapVolatile(params);
    ...
    function test_quoteSwapDepositSaleAmt_returnsPositiveAmount() public {
        ...
        uint256 saleAmt = AerodromeService._quoteSwapDepositSaleAmt(params);
    ...
    function test_integration_swapDepositWithdrawSwap_roundTrip() public {
        ...
        AerodromeService.SwapDepositVolatileParams memory depositParams = AerodromeService.SwapDepositVolatileParams({
            router: IRouter(address(aerodromeRouter)),
            factory: IPoolFactory(address(aerodromePoolFactory)),
            pool: IPool(address(aeroBalancedPool)),
            token0: token0Ierc20,
        ...
        uint256 lpAmount = AerodromeService._swapDepositVolatile(depositParams);
        ...
        AerodromeService.WithdrawSwapVolatileParams memory withdrawParams = AerodromeService.WithdrawSwapVolatileParams({
            aerodromeRouter: IRouter(address(aerodromeRouter)),
            pool: IPool(address(aeroBalancedPool)),
            factory: IPoolFactory(address(aerodromePoolFactory)),
            tokenOut: IERC20(address(aeroBalancedTokenA)),
        ...
        uint256 amountBack = AerodromeService._withdrawSwapVolatile(withdrawParams);
