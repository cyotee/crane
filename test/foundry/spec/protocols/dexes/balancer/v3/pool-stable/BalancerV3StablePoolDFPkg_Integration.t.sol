...
/**
 * @title BalancerV3StablePoolDFPkg_Integration_Test
 * @notice Integration test for stable pool DFPkg deployment + vault registration.
 */
contract BalancerV3StablePoolDFPkg_Integration_Test is CraneTest {
    BalancerV3StablePoolDFPkg internal pkg;
    ...
    BalancerV3StablePoolFacet internal stableFacet;
    ...
    function _deployRealFacets() internal {
        ...
        stableFacet = new BalancerV3StablePoolFacet();
        ...
        vm.label(address(stableFacet), "BalancerV3StablePoolFacet");
        vm.label(address(poolInfoFacet), "MockPoolInfoFacet");
    }

    function _deployPkg() internal {
        pkg = new BalancerV3StablePoolDFPkg(
            IBalancerV3StablePoolDFPkg.PkgInit({
                ...
                balancerV3StablePoolFacet: IFacet(address(stableFacet)),
                balancerV3Vault: IVault(address(mockVault)),
                diamondFactory: diamondFactory,
                poolFeeManager: poolManager
            })
        );
        vm.label(address(pkg), "BalancerV3StablePoolDFPkg");
    }
    ...
    function test_postDeploy_triggersVaultRegistration() public {
        ...
        bytes memory pkgArgs = abi.encode(
            IBalancerV3StablePoolDFPkg.PkgArgs({
                tokenConfigs: configs, amplificationParameter: 100, hooksContract: address(0)
            })
        );
