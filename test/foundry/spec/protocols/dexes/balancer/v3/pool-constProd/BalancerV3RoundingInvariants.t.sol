...
contract BalancerV3RoundingInvariants_Test is Test {
    ...
    /* -------------------------------------------------------------------------- */
    /*              CRANE-109: 2-Token Pool Guardrail Tests                        */
    /* -------------------------------------------------------------------------- */

    /**
     * @notice computeInvariant reverts for 0-token array.
     */
    function test_computeInvariant_revert_zeroTokens() public {
        uint256[] memory balances = new uint256[](0);
        vm.expectRevert(abi.encodeWithSelector(IBalancerV3Pool.BalancerV3Pool_RequiresTwoTokens.selector, 0));
        pool.computeInvariant(balances, Rounding.ROUND_DOWN);
    }

    /**
     * @notice computeInvariant reverts for 1-token array.
     */
    function test_computeInvariant_revert_oneToken() public {
        uint256[] memory balances = new uint256[](1);
        balances[0] = 1000e18;
        vm.expectRevert(abi.encodeWithSelector(IBalancerV3Pool.BalancerV3Pool_RequiresTwoTokens.selector, 1));
        pool.computeInvariant(balances, Rounding.ROUND_DOWN);
    }

    /**
     * @notice computeInvariant reverts for 3-token array.
     */
    function test_computeInvariant_revert_threeTokens() public {
        uint256[] memory balances = new uint256[](3);
        balances[0] = 1000e18;
        balances[1] = 1000e18;
        balances[2] = 1000e18;
        vm.expectRevert(abi.encodeWithSelector(IBalancerV3Pool.BalancerV3Pool_RequiresTwoTokens.selector, 3));
        pool.computeInvariant(balances, Rounding.ROUND_DOWN);
    }

    /**
     * @notice computeInvariant reverts for 4-token array.
     */
    function test_computeInvariant_revert_fourTokens() public {
        uint256[] memory balances = new uint256[](4);
        balances[0] = 500e18;
        balances[1] = 500e18;
        balances[2] = 500e18;
        balances[3] = 500e18;
        vm.expectRevert(abi.encodeWithSelector(IBalancerV3Pool.BalancerV3Pool_RequiresTwoTokens.selector, 4));
        pool.computeInvariant(balances, Rounding.ROUND_UP);
    }

    /**
     * @notice computeInvariant still works with exactly 2 tokens.
     */
    function test_computeInvariant_twoTokens_succeeds() public view {
        uint256[] memory balances = new uint256[](2);
        balances[0] = 1000e18;
        balances[1] = 1000e18;
        uint256 invariant = pool.computeInvariant(balances, Rounding.ROUND_DOWN);
        assertTrue(invariant > 0, "2-token invariant should be positive");
    }

    /**
     * @notice computeBalance reverts for 0-token array.
     */
    function test_computeBalance_revert_zeroTokens() public {
        uint256[] memory balances = new uint256[](0);
        vm.expectRevert(abi.encodeWithSelector(IBalancerV3Pool.BalancerV3Pool_RequiresTwoTokens.selector, 0));
        pool.computeBalance(balances, 0, 1e18);
    }

    /**
     * @notice computeBalance reverts for 1-token array.
     */
    function test_computeBalance_revert_oneToken() public {
        uint256[] memory balances = new uint256[](1);
        balances[0] = 1000e18;
        vm.expectRevert(abi.encodeWithSelector(IBalancerV3Pool.BalancerV3Pool_RequiresTwoTokens.selector, 1));
        pool.computeBalance(balances, 0, 1e18);
    ...
    /**
     * @notice computeBalance reverts for 3-token array.
     */
    function test_computeBalance_revert_threeTokens() public {
        uint256[] memory balances = new uint256[](3);
        balances[0] = 1000e18;
        balances[1] = 1000e18;
        balances[2] = 1000e18;
    ...
    /**
     * @notice computeBalance reverts for out-of-bounds tokenInIndex.
     */
    function test_computeBalance_revert_tokenIndexOutOfBounds() public {
        uint256[] memory balances = new uint256[](2);
        balances[0] = 1000e18;
        balances[1] = 1000e18;
        vm.expectRevert(abi.encodeWithSelector(IBalancerV3Pool.BalancerV3Pool_TokenIndexOutOfBounds.selector, 2));
        pool.computeBalance(balances, 2, 1e18);
    }

    /**
     * @notice computeBalance reverts for large tokenInIndex.
     */
    function test_computeBalance_revert_tokenIndexLarge() public {
        uint256[] memory balances = new uint256[](2);
        balances[0] = 1000e18;
        balances[1] = 1000e18;
        vm.expectRevert(abi.encodeWithSelector(IBalancerV3Pool.BalancerV3Pool_TokenIndexOutOfBounds.selector, 100));
        pool.computeBalance(balances, 100, 1e18);
    }

    /**
     * @notice computeBalance succeeds with valid inputs (index 0 and 1).
     */
    function test_computeBalance_validInputs_succeeds() public view {
        uint256[] memory balances = new uint256[](2);
        balances[0] = 1000e18;
        balances[1] = 1000e18;
        uint256 newBalance0 = pool.computeBalance(balances, 0, 1.1e18);
        uint256 newBalance1 = pool.computeBalance(balances, 1, 1.1e18);
        assertTrue(newBalance0 > 0, "computeBalance with index 0 should return positive");
        assertTrue(newBalance1 > 0, "computeBalance with index 1 should return positive");
    }

    /**
     * @notice Fuzz: computeInvariant reverts for any non-2 length.
     */
    function testFuzz_computeInvariant_revert_wrongLength(uint256 length) public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IBalancerV3Pool.BalancerV3Pool_RequiresTwoTokens.selector, length));
        pool.computeInvariant(balances, Rounding.ROUND_DOWN);
    }

    /**
     * @notice Fuzz: computeBalance reverts for out-of-bounds index.
     */
    function testFuzz_computeBalance_revert_badIndex(uint256 index) public {
        index = bound(index, 2, type(uint256).max);
        uint256[] memory balances = new uint256[](2);
        balances[0] = 1000e18;
        balances[1] = 1000e18;
        vm.expectRevert(abi.encodeWithSelector(IBalancerV3Pool.BalancerV3Pool_TokenIndexOutOfBounds.selector, index));
        pool.computeBalance(balances, index, 1e18);
    }

    /* -------------------------------------------------------------------------- */
    /*        CRANE-108: Large Balance Overflow Protection Tests (mulDiv)          */
    /* -------------------------------------------------------------------------- */

    /**
     * @notice Verify EXACT_IN handles balances large enough to overflow raw multiplication.
     * @dev With balances near 1e38, Y * dx would overflow uint256 (>2^256) using raw `*`.
     *      Math.mulDiv uses 512-bit intermediate, so it succeeds.
     */
    function test_onSwap_exactIn_largeBalances_noOverflow() public view {
        uint256[] memory balances = new uint256[](2);
        balances[0] = 5e38; // ~5 * 10^38 (near uint256 overflow for product)
        balances[1] = 5e38;

        PoolSwapParams memory params = PoolSwapParams({
            kind: SwapKind.EXACT_IN,
            amountGivenScaled18: 1e38,
            balancesScaled18: balances,
            indexIn: 0,
            indexOut: 1,
            router: address(0),
        ...
        // This would overflow with raw (Y * dx) since 5e38 * 1e38 = 5e76 > 2^256
        uint256 amountOut = pool.onSwap(params);
        assertTrue(amountOut > 0, "Large balance EXACT_IN should not overflow");
        assertTrue(amountOut < balances[1], "Output must be less than reserve");
    }

    /**
     * @notice Verify EXACT_OUT handles balances large enough to overflow raw multiplication.
     * @dev With balances near 1e38, X * dy would overflow uint256 using raw `*`.
     */
    function test_onSwap_exactOut_largeBalances_noOverflow() public view {
        uint256[] memory balances = new uint256[](2);
        balances[0] = 5e38;
        balances[1] = 5e38;
        ...
        PoolSwapParams memory params = PoolSwapParams({
            kind: SwapKind.EXACT_OUT,
            amountGivenScaled18: 1e37, // Request 10% of a smaller amount
            balancesScaled18: balances,
            indexIn: 0,
            indexOut: 1,
            router: address(0),
        ...
        // This would overflow with raw (X * dy) since 5e38 * 1e37 = 5e75 > 2^256
        uint256 amountIn = pool.onSwap(params);
        assertTrue(amountIn > 0, "Large balance EXACT_OUT should not overflow");
    }

    /**
     * @notice Verify computeBalance handles large invariants that would overflow on squaring.
     * @dev newInvariant^2 can overflow uint256 with raw `*` when newInvariant > ~2^128.
     *      Math.mulDiv uses 512-bit intermediate, so it succeeds.
     *
     *      With balances 3e38 each (near FixedPoint.mulDown limit of ~3.4e38):
     *        mulDown = 3e38 * 3e38 / 1e18 = 9e58
     *        sqrt(9e58) = 3e29, invariant = 3e29 * 1e9 = 3e38
     *        After mulDown(2e18): newInvariant = 6e38
     *        newInvariant^2 = 3.6e77, which exceeds 2^256 (~1.16e77)
     *        Raw `*` would overflow; Math.mulDiv handles it via 512-bit intermediate.
     */
    function test_computeBalance_largeInvariant_noOverflow() public view {
        uint256[] memory balances = new uint256[](2);
        balances[0] = 3e38; // Near max for FixedPoint.mulDown without overflow
        balances[1] = 3e38;

        uint256 ratio = 2e18; // 2x increase pushes newInvariant^2 past 2^256

        uint256 newBalance = pool.computeBalance(balances, 0, ratio);
        assertTrue(newBalance > 0, "Large invariant computeBalance should not overflow");
    }
    ...
    function testFuzz_onSwap_exactIn_largeBalances(
        ...
        // Should not overflow thanks to Math.mulDiv
        uint256 amountOut = pool.onSwap(params);
        assertTrue(amountOut > 0, "Large balance fuzz: EXACT_IN should succeed");
        assertTrue(amountOut < balances[1], "Output must be less than reserve");
    }
    ...
    function testFuzz_onSwap_exactOut_largeBalances(
        ...
        // Should not overflow thanks to Math.mulDiv
        uint256 amountIn = pool.onSwap(params);
        assertTrue(amountIn > 0, "Large balance fuzz: EXACT_OUT should succeed");
    }
}
