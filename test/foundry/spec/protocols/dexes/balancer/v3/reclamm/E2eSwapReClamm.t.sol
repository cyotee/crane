...
contract E2eSwapReClammTest is E2eSwapFuzzPoolParamsHelper, E2eSwapTest {
        ...
        returns (E2eTestState memory)
    {
        ...
        (poolInitAmountTokenA, poolInitAmountTokenB) = _fuzzPoolParams(
            ReClammPoolMock(payable(pool)), params, getRate(tokenA), getRate(tokenB), decimalsTokenA, decimalsTokenB
        );
    ...
    function _computeReClammSwapLimits() internal view returns (SwapLimits memory swapLimits) {
        (swapLimits.minTokenA, swapLimits.minTokenB, swapLimits.maxTokenA, swapLimits.maxTokenB) =
