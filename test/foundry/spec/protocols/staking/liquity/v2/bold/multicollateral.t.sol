...
contract CsBold013 is TestAccounts {
    ...
    function setUp() external {
        ...
        for (uint256 i = 0; i < _branches.length; ++i) {
            ...
            for (uint256 j = 0; j < accountsList.length; j++) {
                ...
                if (address(_branches[i].collToken) != address(weth)) {
                    vm.prank(accountsList[j]);
    ...
    function openTroveWithExactICRAndDebt(
        ...
        vm.prank(account);
        troveId = branches[collIndex].borrowerOperations
            .openTrove(account, index, coll, borrow, 0, 0, interestRate, upfrontFee, address(0), address(0), address(0));
    }
