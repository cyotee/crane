...
contract PoolEModeTests is TestnetProcedures {
    ...
    function test_reenterSameEmode_shouldSucceedAndDoNothing() public {
        ...
        vm.prank(alice);
        pool.setUserEMode(0);
        (, uint256 assetBaseLTV, uint256 assetBaseLT,,,,,,,) =
    ...
    function test_setUserEmode_shouldAllowSwitchingWhenAssetIsBorrowableInEmode(uint104 amount) public {
    ...
    function test_borrowing_nonBorrowableOutsideEmode(uint256 amount) public {
        ...
        contracts.poolConfiguratorProxy.setEModeCategory(1, 9000, 9200, liquidationBonus, "usdx eMode low");
        contracts.poolConfiguratorProxy.setAssetCollateralInEMode(tokenList.usdx, 1, true);
    ...
    function test_borrowing_nonBorrowableOutsideEmode_shouldRevertIfNotInsideEmode(uint256 amount) public {
        amount = bound(amount, 1 ether, type(uint104).max);
        vm.startPrank(poolAdmin);
        uint16 liquidationBonus = 10050;
        contracts.poolConfiguratorProxy.setEModeCategory(1, 9000, 9200, liquidationBonus, "usdx eMode low");
    ...
    function test_liquidations_shouldApplyEModeLBForEmodeAssets(uint256 amount) public {
        ...
        contracts.poolProxy.liquidationCall(tokenList.usdx, tokenList.wbtc, alice, type(uint256).max, false);

        DataTypes.ReserveConfigurationMap memory reserveConfig = contracts.poolProxy.getConfiguration(tokenList.usdx);
    ...
    function _borrowMaxLt(address erc20, address user) internal {
        (uint256 totalCollateralBase,,, uint256 currentLt,,) = contracts.poolProxy.getUserAccountData(user);
