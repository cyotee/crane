...
import {
    PoolLogic,
    ReserveLogic
} from "@crane/contracts/protocols/lending/aave/v3.6/protocol/libraries/logic/PoolLogic.sol";
...
contract PoolLogicInitReservesTests is TestnetProcedures {
    ...
    function test_reverts_initReserves_reserveAlreadyAdded() public {
        DataTypes.InitReserveParams memory params =
            DataTypes.InitReserveParams(address(new Mock()), report.aToken, report.variableDebtToken, 0, 10);
    ...
    function test_initReserves_return_false_after_dropped_reserve() public {
        DataTypes.InitReserveParams memory params1 =
            DataTypes.InitReserveParams(address(new Mock()), report.aToken, report.variableDebtToken, 0, 10);

        DataTypes.InitReserveParams memory params2 =
            DataTypes.InitReserveParams(address(new Mock()), report.aToken, report.variableDebtToken, 1, 10);

        DataTypes.InitReserveParams memory params3 =
            DataTypes.InitReserveParams(address(new Mock()), report.aToken, report.variableDebtToken, 2, 10);
    ...
    function test_reverts_initReserves_max() public {
        DataTypes.InitReserveParams memory params1 =
            DataTypes.InitReserveParams(address(new Mock()), report.aToken, report.variableDebtToken, 0, 0);
