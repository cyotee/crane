...
import {
    TestnetERC20,
    IERC20WithPermit
} from "@crane/contracts/protocols/lending/aave/v3.6/utils/mocks/testnet-helpers/TestnetERC20.sol";
...
contract PoolRoundingTests is TestnetProcedures {
    ...
    function test_debtBalanceInBaseCurrencyShouldRoundUp() external {
        ...
        vm.prank(user);
        contracts.poolProxy.borrow({asset: asset, amount: 1, interestRateMode: 2, referralCode: 0, onBehalfOf: user});

        (, uint256 totalDebtBase,,,, uint256 healthFactor) = contracts.poolProxy.getUserAccountData(user);
    ...
    function test_supplyCapShouldRoundDown() external {
        ...
        contracts.poolConfiguratorProxy.setSupplyCap(asset, supplyAmount / 10 ** IERC20Metadata(asset).decimals());
        vm.stopPrank();
    ...
    function test_borrowCapShouldRoundUp() external {
        ...
        vm.startPrank(poolAdmin);
        contracts.poolConfiguratorProxy.setBorrowCap(asset, borrowAmount / 10 ** IERC20Metadata(asset).decimals());
        vm.stopPrank();
        ...
        vm.startPrank(user);
        contracts.poolProxy
            .borrow({asset: asset, amount: borrowAmount - 5, interestRateMode: 2, referralCode: 0, onBehalfOf: user});
        ...
        contracts.poolProxy
            .borrow({asset: asset, amount: borrowAmount, interestRateMode: 2, referralCode: 0, onBehalfOf: user});
    }
}
