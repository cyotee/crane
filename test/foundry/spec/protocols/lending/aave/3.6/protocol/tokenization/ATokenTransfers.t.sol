...
import {
    IncentivizedERC20
} from "@crane/contracts/protocols/lending/aave/v3.6/protocol/tokenization/base/IncentivizedERC20.sol";
...
import {MathUtils} from "@crane/contracts/protocols/lending/aave/v3.6/protocol/libraries/math/MathUtils.sol";
...
import {MockAToken} from "@crane/contracts/protocols/lending/aave/v3.6/utils/mocks/tokens/MockAToken.sol";
...
contract ATokenTransferTests is TestnetProcedures {
    ...
    function setUp() public {
        ...
        mockAToken = new MockAToken(IPool(report.poolProxy), report.rewardsControllerProxy, report.treasury);
    ...
    function test_atoken_alice_transfer_to_herself() public {
        ...
        emit IERC20Events.Transfer(alice, alice, 120e6);
    ...
    function test_atoken_alice_transfer_to_herself_zero() public {
        vm.expectEmit(address(aToken));
        emit IERC20Events.Transfer(alice, alice, 0);
    ...
    function test_atoken_alice_transfer_to_bob() public {
        ...
        vm.expectEmit(address(aToken));
        emit IERC20Events.Transfer(alice, bob, transferAmount);

        vm.prank(alice);
        // forge-lint: disable-next-line(erc20-unchecked-transfer)
        aToken.transfer(bob, transferAmount);

        (uint256 bobCollateralBalance,,,,,,,,) = contracts.protocolDataProvider.getUserReserveData(tokenList.usdx, bob);
        (uint256 aliceCollateralBalance,,,,,,,,) =
            contracts.protocolDataProvider.getUserReserveData(tokenList.usdx, alice);
        assertEq(bobCollateralBalance, transferAmount, "Bob balance should be equal alice transfer to bob");
    ...
    function test_atoken_alice_transfer_all_to_bob() public {
        ...
        assertEq(aToken.balanceOf(bob), transferAmount, "Bob balance should be equal alice transfer to bob");
    ...
    function test_atoken_alice_transfer_to_bob_zero() public {
        vm.expectEmit(address(aToken));
        emit IERC20Events.Transfer(alice, bob, 0);
    ...
    function test_atoken_multiple_transfers() public {
        vm.expectEmit(address(aToken));
        emit IERC20Events.Transfer(alice, bob, 10_000e6);
        ...
        vm.expectEmit(address(aToken));
        emit IERC20Events.Transfer(bob, alice, 444e6);
        ...
        vm.expectEmit(address(aToken));
        emit IERC20Events.Transfer(bob, carol, 555e6);
    ...
    function test_reverts_atoken_transfer_all_collateral_from_bob_borrower_to_alice() public {
        vm.expectEmit(address(aToken));
        emit IERC20Events.Transfer(alice, bob, 50_000e6);
        ...
        vm.expectRevert(abi.encodeWithSelector(Errors.HealthFactorLowerThanLiquidationThreshold.selector));
        vm.prank(bob);
        // forge-lint: disable-next-line(erc20-unchecked-transfer)
    ...
    function test_atoken_transfer_some_collateral_from_bob_borrower_to_alice() public {
        ...
        vm.prank(bob);
        contracts.poolProxy.borrow(tokenList.wbtc, 1e8, 2, 0, bob);
        (,, uint256 borrowBalance,,,,,,) = contracts.protocolDataProvider.getUserReserveData(tokenList.wbtc, bob);
        assertEq(borrowBalance, 1e8, "borrowBalance should match borrowed amount");

        uint256 transferAmount = aToken.balanceOf(bob) / 4;

        (uint256 carolCollateralBalanceBefore,,,,,,,,) =
            contracts.protocolDataProvider.getUserReserveData(tokenList.usdx, carol);

        vm.prank(bob);
        // forge-lint: disable-next-line(erc20-unchecked-transfer)
        aToken.transfer(carol, transferAmount);

        (uint256 carolCollateralBalance,,,,,,,,) =
            contracts.protocolDataProvider.getUserReserveData(tokenList.usdx, carol);
    ...
    function test_atoken_alice_transfer_to_carol_accrues_interests() public {
        ...
        uint256 cumulatedLiquidityInterest =
            MathUtils.calculateLinearInterest(reserveData.currentLiquidityRate, reserveData.lastUpdateTimestamp);
        ...
        vm.expectEmit(address(aToken));
        emit IERC20Events.Transfer(alice, carol, transferAmount);
        ...
        assertEq(aliceCollateralBalance, aliceBalanceBefore - transferAmount);
        assertEq(aToken.balanceOf(carol), carolBalanceBefore + transferAmount + recipientBalanceIncrease);
        assertEq(aToken.balanceOf(alice), aliceBalanceBefore - transferAmount);
    }

    function test_atoken_transfer_sets_enabled_as_collateral(uint256 timePassed, uint256 amount) public {
        ...
        (,,,,,,,, bool collateralEnabled) =
            contracts.protocolDataProvider.getUserReserveData(tokenList.usdx, mockReceiver);
    ...
    function test_scaled_balance_token_base_alice_transfer_to_bob_accrues_interests() public {
        ...
        mockAToken.setStorage(alice, bob, previousIndex, aliceScaledBalanceBefore, bobScaledBalanceBefore);

        uint256 senderBalanceIncrease =
            aliceScaledBalanceBefore.rayMulFloor(expectedIndex) - aliceScaledBalanceBefore.rayMulFloor(previousIndex);
        uint256 recipientBalanceIncrease =
            bobScaledBalanceBefore.rayMulFloor(expectedIndex) - bobScaledBalanceBefore.rayMulFloor(previousIndex);

        vm.expectEmit(address(mockAToken));
        emit IERC20Events.Transfer(address(0), alice, senderBalanceIncrease);
        vm.expectEmit(address(mockAToken));
        emit IScaledBalanceToken.Mint(alice, alice, senderBalanceIncrease, senderBalanceIncrease, expectedIndex);
        vm.expectEmit(address(mockAToken));
        emit IERC20Events.Transfer(address(0), bob, recipientBalanceIncrease);
        vm.expectEmit(address(mockAToken));
        emit IScaledBalanceToken.Mint(alice, bob, recipientBalanceIncrease, recipientBalanceIncrease, expectedIndex);
        vm.expectEmit(address(mockAToken));
        emit IERC20Events.Transfer(alice, bob, transferAmount);
    ...
    function test_approve_and_renounceAllowance() public {
        ...
        vm.expectEmit(address(aToken));
        emit IERC20Events.Approval(alice, bob, approveAmount);
        ...
        vm.expectEmit(address(aToken));
        emit IERC20Events.Approval(alice, bob, 0);
