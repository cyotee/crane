...
contract PercentageMathTests is Test {
    ...
    function test_percentMul_fuzz(uint256 value, uint256 percentage) public {
        if (
            (percentage == 0
                        || (value > (type(uint256).max - PercentageMath.HALF_PERCENTAGE_FACTOR) / percentage) == false)
                == false
        ) {
            vm.expectRevert();
            ...
            assertEq(
                PercentageMath.percentMul(value, percentage),
                ((value * percentage) + PercentageMath.HALF_PERCENTAGE_FACTOR) / (PercentageMath.PERCENTAGE_FACTOR)
            );
        }
    }

    function test_percentDiv_fuzz(uint256 value, uint256 percentage) public {
        if (percentage == 0 || value > (type(uint256).max - (percentage / 2)) / PercentageMath.PERCENTAGE_FACTOR) {
            vm.expectRevert();
    ...
    function test_percentMul() external pure {
        ...
        assertEq(PercentageMath.percentMul(14.2515e18, 74_42), 10.6059663e18);
