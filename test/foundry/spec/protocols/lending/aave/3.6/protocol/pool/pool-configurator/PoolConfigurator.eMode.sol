...
contract PoolConfiguratorEModeConfigTests is TestnetProcedures {
    ...
    function test_updateEModeCategory() public {
        ...
        EModeCategoryInput memory updatedCategory = EModeCategoryInput(ct.id, 90_00, 92_00, 101_00, "GROUP_B");

        vm.prank(poolAdmin);
        vm.expectEmit(address(contracts.poolConfiguratorProxy));
        emit IPoolConfigurator.EModeCategoryAdded(
            ct.id, updatedCategory.ltv, updatedCategory.lt, updatedCategory.lb, address(0), updatedCategory.label
        );

        contracts.poolConfiguratorProxy
            .setEModeCategory(ct.id, updatedCategory.ltv, updatedCategory.lt, updatedCategory.lb, updatedCategory.label);
    ...
    function test_reverts_setEmodeCategory_liquidation_threshold_doesnt_match_bonus() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(Errors.InvalidEmodeCategoryParams.selector));
        vm.prank(poolAdmin);
        contracts.poolConfiguratorProxy.setEModeCategory(1, 90_00, liquidationThreshold, liquidationBonus, "LABEL");
    }
    ...
    function test_removeCollateralFromEmode_underlyingLtvzero_shouldRevert_dueToNonZeroSupply() public {
        EModeCategoryInput memory prevCt = _genCategoryOne();
        test_setAssetCollateralInEMode();
        vm.prank(poolAdmin);
    ...
    function test_setAssetLtvzeroInEMode_shouldRevertIfNotCollateral() public {
        ...
        vm.prank(poolAdmin);
        vm.expectRevert(abi.encodeWithSelector(Errors.MustBeEmodeCollateral.selector, tokenList.usdx, input.id));
    ...
    function _getFullEMode(uint8 eMode) internal view returns (DataTypes.EModeCategory memory) {
        ...
        DataTypes.CollateralConfig memory cfg = contracts.poolProxy.getEModeCategoryCollateralConfig(eMode);
