...
contract PoolSupplyTests is TestnetProcedures {
    ...
    function test_supplyWithPermit(uint128 userPk, uint120 supplyAmount, uint128 underlyingBalance) public {
        vm.assume(userPk != 0);
        ...
        bytes32 digest = EIP712SigUtils.getTypedDataHash(
            permit, bytes(TestnetERC20(tokenList.wbtc).name()), bytes("1"), tokenList.wbtc
        );
        ...
        vm.prank(user);
        contracts.poolProxy.supplyWithPermit(tokenList.wbtc, supplyAmount, user, 0, permit.deadline, v, r, s);
    ...
    function test_supplyWithPermit_should_revert_if_permit_is_less_then_supply_amount(
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IERC20Errors.ERC20InsufficientAllowance.selector, address(contracts.poolProxy), 0, supplyAmount
            )
        );
        contracts.poolProxy.supplyWithPermit(tokenList.wbtc, supplyAmount, user, 0, permit.deadline, v, r, s);
    }
