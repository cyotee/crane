...
contract PoolConfiguratorInitReservesTest is TestnetProcedures {
    ...
    function test_initReserves_validNumberOfAssets(TestVars[128] memory t, uint8 length) public {
        ...
        ConfiguratorInputTypes.InitReserveInput[] memory input = new ConfiguratorInputTypes.InitReserveInput[](length);
        for (uint256 i = 0; i < length; i++) {
            ...
            emit IPoolConfigurator.ReserveInitialized(
                input[i].underlyingAsset, address(0), address(0), address(0), report.defaultInterestRateStrategy
            );
        }
        // Perform action
        vm.prank(poolAdmin);
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            // Perform assertions
            {
                (address aTokenProxy,, address variableDebtProxy) =
                    contracts.protocolDataProvider.getReserveTokensAddresses(initConfig.underlyingAsset);

                assertEq(AToken(aTokenProxy).name(), initConfig.aTokenName);
                assertEq(AToken(aTokenProxy).symbol(), initConfig.aTokenSymbol);
                assertEq(AToken(aTokenProxy).decimals(), TestnetERC20(initConfig.underlyingAsset).decimals());
                ...
                assertEq(AToken(variableDebtProxy).decimals(), TestnetERC20(initConfig.underlyingAsset).decimals());
                assertEq(AToken(variableDebtProxy).UNDERLYING_ASSET_ADDRESS(), initConfig.underlyingAsset);
                assertEq(address(AToken(variableDebtProxy).getIncentivesController()), report.rewardsControllerProxy);
            }
            // Perform default asset checks
            TestReserveConfig memory c = _getReserveConfig(initConfig.underlyingAsset, report.protocolDataProvider);
    ...
    function test_initReserves_notEnoughDecimal(TestVars memory t) public {
        ...
        ConfiguratorInputTypes.InitReserveInput[] memory input = _generateInitConfig(t, report, poolAdmin, false);
