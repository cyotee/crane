...
contract PoolTests is TestnetProcedures {
    ...
    function test_noop_setUserUseReserveAsCollateralOnBehalfOf_true_when_already_is_activated() public {
        test_setUserUseReserveAsCollateralOnBehalfOf_true();

        pool.setUserUseReserveAsCollateralOnBehalfOf(tokenList.usdx, true, alice);
    }

    function test_reverts_setUserUseReserveAsCollateralOnBehalfOf_caller_not_position_manager(address caller) public {
    ...
    function test_getters_getUserAccountData() public {
        ...
        (uint256 totalCollateralBase, uint256 totalDebtBase,, uint256 currentLiquidationThreshold, uint256 ltv,) =
    ...
    function test_setUserEmode_twice_inconsistent_category() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(Errors.InvalidDebtInEmode.selector, tokenList.weth, ct2.id));
    ...
    function test_reverts_setUserEmode_0_bad_hf() public {
        ...
        stdstore.target(IAaveOracle(report.aaveOracle).getSourceOfAsset(tokenList.wbtc)).sig("_latestAnswer()")
            .checked_write(_calcPrice(IAaveOracle(report.aaveOracle).getAssetPrice(tokenList.wbtc), 70_00));
