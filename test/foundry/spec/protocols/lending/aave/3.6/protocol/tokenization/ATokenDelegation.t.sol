...
} from "@crane/contracts/protocols/lending/aave/v3.6/instances/ATokenWithDelegationInstance.sol";
import {IATokenWithDelegation} from "@crane/contracts/protocols/lending/aave/v3.6/interfaces/IATokenWithDelegation.sol";
...
import {
    IBaseDelegation
} from "@crane/contracts/protocols/lending/aave/v3.6/protocol/tokenization/delegation/interfaces/IBaseDelegation.sol";
...
abstract contract BaseATokenDelegationTest is TestnetProcedures {
    ...
    function _updateATokens() private {
        ATokenWithDelegationInstanceNext aTokenImplementation =
            new ATokenWithDelegationInstanceNext(contracts.poolProxy, report.rewardsControllerProxy, report.treasury);
        ...
        for (uint256 i = 0; i < reserves.length; ++i) {
            ...
            contracts.poolConfiguratorProxy
                .updateAToken(
    ...
    function test_initial_state_without_delegation() public view {
        address votingDelegatee = aToken.getDelegateeByType(alice, IBaseDelegation.GovernancePowerType.VOTING);
        address propositionDelegatee = aToken.getDelegateeByType(alice, IBaseDelegation.GovernancePowerType.PROPOSITION);

        uint256 aliceVotingPower = aToken.getPowerCurrent(alice, IBaseDelegation.GovernancePowerType.VOTING);
        uint256 alicePropositionPower = aToken.getPowerCurrent(alice, IBaseDelegation.GovernancePowerType.PROPOSITION);

        uint256 bobVotingPower = aToken.getPowerCurrent(bob, IBaseDelegation.GovernancePowerType.VOTING);
        uint256 bobPropositionPower = aToken.getPowerCurrent(bob, IBaseDelegation.GovernancePowerType.PROPOSITION);
    ...
    function _performTransfersAndChecks(address caller, address from, address to) private {
        ...
        if (from != address(0)) {
            _delegateByType({from: from, to: user1, powerType: IBaseDelegation.GovernancePowerType.VOTING});
            _delegateByType({from: from, to: user2, powerType: IBaseDelegation.GovernancePowerType.PROPOSITION});
        }

        // to delegates to user3 and user4
        if (to != address(0)) {
            _delegateByType({from: to, to: user3, powerType: IBaseDelegation.GovernancePowerType.VOTING});
            _delegateByType({from: to, to: user4, powerType: IBaseDelegation.GovernancePowerType.PROPOSITION});
        }
        ...
        if (from != address(0)) {
            _delegateByType({from: from, to: user3, powerType: IBaseDelegation.GovernancePowerType.VOTING});
            _delegateByType({from: from, to: user4, powerType: IBaseDelegation.GovernancePowerType.PROPOSITION});
        }
        ...
        // from delegates to user3 and user4
        if (from != address(0)) {
            _delegateByType({from: from, to: user4, powerType: IBaseDelegation.GovernancePowerType.VOTING});
            _delegateByType({from: from, to: user3, powerType: IBaseDelegation.GovernancePowerType.PROPOSITION});
        }
    ...
    function test_getDelegates() public {
        ...
        _delegateByType({from: alice, to: carol, powerType: IBaseDelegation.GovernancePowerType.VOTING});
        ...
        _delegateByType({from: alice, to: address(0), powerType: IBaseDelegation.GovernancePowerType.PROPOSITION});
    ...
    function test_getPowersCurrent() public {
        ...
        assertEq(propositionPower, aToken.getPowerCurrent(bob, IBaseDelegation.GovernancePowerType.PROPOSITION));
        ...
        _delegateByType({from: bob, to: carol, powerType: IBaseDelegation.GovernancePowerType.PROPOSITION});
        ...
        _delegateByType({from: bob, to: address(0), powerType: IBaseDelegation.GovernancePowerType.VOTING});
    ...
    function test_getPowersCurrent_with_index_growth() public {
        ...
        assertApproxEqAbs(
            votingPower,
            _getDownscaleVotingPower(alice) + aToken.scaledBalanceOf(bob) + _getDownscaleVotingPower(carol),
            1,
            "1"
        );
        assertEq(votingPower, aToken.getPowerCurrent(bob, IBaseDelegation.GovernancePowerType.VOTING));
        assertApproxEqAbs(propositionPower, _getDownscaleVotingPower(alice) + aToken.scaledBalanceOf(bob), 1, "2");
        assertEq(propositionPower, aToken.getPowerCurrent(bob, IBaseDelegation.GovernancePowerType.PROPOSITION));
        ...
        assertEq(votingPower, aToken.getPowerCurrent(bob, IBaseDelegation.GovernancePowerType.VOTING));
        assertApproxEqAbs(propositionPower, _getDownscaleVotingPower(alice) + aToken.scaledBalanceOf(bob), 1, "4");
        assertEq(propositionPower, aToken.getPowerCurrent(bob, IBaseDelegation.GovernancePowerType.PROPOSITION));
    }
    ...
    function test_precision() public {
        ...
        IATokenWithDelegation usdxAToken = IATokenWithDelegation(contracts.poolProxy.getReserveAToken(tokenList.usdx));
        IATokenWithDelegation wbtcAToken = IATokenWithDelegation(contracts.poolProxy.getReserveAToken(tokenList.wbtc));
        IATokenWithDelegation wethAToken = IATokenWithDelegation(contracts.poolProxy.getReserveAToken(tokenList.weth));
    ...
    function _checkDelegationResult(address delegator, address newDelegatee) private {
        address previousVotingDelegatee =
            aToken.getDelegateeByType(delegator, IBaseDelegation.GovernancePowerType.VOTING);
        address previousPropositionDelegatee =
            aToken.getDelegateeByType(delegator, IBaseDelegation.GovernancePowerType.PROPOSITION);
        ...
        if (previousVotingDelegatee != formattedNewDelegatee) {
            ...
            emit IBaseDelegation.DelegateChanged(
                delegator, formattedNewDelegatee, IBaseDelegation.GovernancePowerType.VOTING
            );
        }
        ...
        if (previousPropositionDelegatee != formattedNewDelegatee) {
            vm.expectEmit(address(aToken));
            emit IBaseDelegation.DelegateChanged(
                delegator, formattedNewDelegatee, IBaseDelegation.GovernancePowerType.PROPOSITION
            );
        }
        ...
        address votingDelegatee = aToken.getDelegateeByType(delegator, IBaseDelegation.GovernancePowerType.VOTING);
        ...
        uint256 delegatorVotingPower = aToken.getPowerCurrent(delegator, IBaseDelegation.GovernancePowerType.VOTING);
        uint256 delegatorPropositionPower =
            aToken.getPowerCurrent(delegator, IBaseDelegation.GovernancePowerType.PROPOSITION);
        ...
        if (newDelegatee == alice || newDelegatee == address(0)) {
            ...
            uint256 newDelegateeVotingPower =
                aToken.getPowerCurrent(newDelegatee, IBaseDelegation.GovernancePowerType.VOTING);
            uint256 newDelegateePropositionPower =
                aToken.getPowerCurrent(newDelegatee, IBaseDelegation.GovernancePowerType.PROPOSITION);
    ...
    function _checkTransferResult(address caller, address from, address to, uint256 amount) private {
        ...
        params.votingPowerDelegateeFromBefore =
            aToken.getPowerCurrent(params.votingDelegateeFrom, IBaseDelegation.GovernancePowerType.VOTING);
        params.propositionPowerDelegateeFromBefore =
            aToken.getPowerCurrent(params.propositionDelegateeFrom, IBaseDelegation.GovernancePowerType.PROPOSITION);
        params.votingPowerDelegateeToBefore =
            aToken.getPowerCurrent(params.votingDelegateeTo, IBaseDelegation.GovernancePowerType.VOTING);
        params.propositionPowerDelegateeToBefore =
            aToken.getPowerCurrent(params.propositionDelegateeTo, IBaseDelegation.GovernancePowerType.PROPOSITION);
            ...
            if (caller == report.poolProxy) {
                if (from == address(0)) {
                    ...
                    aToken.transferOnLiquidation({
                        from: from, to: to, amount: amount, scaledAmount: amount.rayDivCeil(index), index: index
                    });
                }
            } else {
                vm.prank(from);
        ...
        if (params.propositionDelegateeFrom != params.propositionDelegateeTo) {
            ...
            assertEq(params.propositionPowerDelegateeToAfter, params.propositionPowerDelegateeFromAfter, "8");
        }
    }
    ...
    function _delegate(address from, address to) private {
        if (usePermits) {
            EIP712SigUtils.Delegate memory delegation = EIP712SigUtils.Delegate({
                delegator: from, delegatee: to, nonce: aToken.nonces(from), deadline: block.timestamp
            });

            bytes32 digest =
                EIP712SigUtils.getDelegateTypedDataHash(delegation, bytes(aToken.name()), bytes("1"), address(aToken));

            (uint8 v, bytes32 r, bytes32 s) = vm.sign(_getPrivateKey(from), digest);

            aToken.metaDelegate({delegator: from, delegatee: to, deadline: block.timestamp, v: v, r: r, s: s});
        } else {
            vm.prank(from);
            aToken.delegate(to);
        }
    ...
    function _delegateByType(address from, address to, IBaseDelegation.GovernancePowerType powerType) private {
        if (usePermits) {
            ...
            bytes32 digest = EIP712SigUtils.getDelegateByTypeTypedDataHash(
                delegation, bytes(aToken.name()), bytes("1"), address(aToken)
            );

            (uint8 v, bytes32 r, bytes32 s) = vm.sign(_getPrivateKey(from), digest);

            aToken.metaDelegateByType({
                delegator: from, delegatee: to, delegationType: powerType, deadline: block.timestamp, v: v, r: r, s: s
            });
        } else {
            vm.prank(from);
