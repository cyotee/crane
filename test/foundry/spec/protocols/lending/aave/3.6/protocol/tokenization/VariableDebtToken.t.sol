...
} from "@crane/contracts/protocols/lending/aave/v3.6/interfaces/IAaveIncentivesController.sol";
import {
    IInitializableDebtToken,
    IScaledBalanceToken
} from "@crane/contracts/protocols/lending/aave/v3.6/interfaces/IVariableDebtToken.sol";
import {
...
contract VariableDebtTokenEventsTests is TestnetProcedures {
    ...
    function test_new_VariableDebtToken_implementation() public returns (VariableDebtTokenInstance) {
        VariableDebtTokenInstance varDebtToken =
            new VariableDebtTokenInstance(IPool(report.poolProxy), report.rewardsControllerProxy);
    ...
    function test_initialize_VariableDebtToken(TestVars memory t) public returns (VariableDebtTokenInstance) {
        VariableDebtTokenInstance varDebtTokenImpl = test_new_VariableDebtToken_implementation();
        ConfiguratorInputTypes.InitReserveInput memory listing = _generateInitReserveInput(t, report, poolAdmin, true);
        ...
        VariableDebtTokenInstance varDebtToken = VariableDebtTokenInstance(
            address(
                new TransparentUpgradeableProxy(
                    ...
                    makeAddr("variable-debt-token-proxy-admin"),
    ...
    function test_renounceDelegation() public {
        ...
        emit ICreditDelegationToken.BorrowAllowanceDelegated(
            alice, bob, variableDebtToken.UNDERLYING_ASSET_ADDRESS(), approveDelegationAmount
        );
        ...
        vm.expectEmit(address(variableDebtToken));
        emit ICreditDelegationToken.BorrowAllowanceDelegated(
            alice, bob, variableDebtToken.UNDERLYING_ASSET_ADDRESS(), 0
        );
    ...
    function test_partial_burn_variableDebt(TestVars memory t) public {
        ...
        vm.expectEmit(address(debtToken));
        emit IScaledBalanceToken.Mint(alice, alice, balanceScaled.rayMulCeil(supplyIndex), 0, supplyIndex);
        ...
        vm.expectEmit(address(debtToken));
        emit IScaledBalanceToken.Burn(alice, address(0), previousBalance - nextBalance, balanceIncrease, newIndex);
    ...
    function test_scaledBalanceOf() public {
        ...
        assertEq(
            variableDebtToken.scaledBalanceOf(alice), aliceAmount1.rayDivCeil(index1) + aliceAmount2.rayDivCeil(index2)
        );
    }
    ...
    function test_delegationWithSig() public {
        ...
        EIP712SigUtils.CreditDelegation memory delegation =
            EIP712SigUtils.CreditDelegation({delegatee: bob, value: amount, nonce: 0, deadline: deadline});

        bytes32 digest = EIP712SigUtils.getCreditDelegationTypedDataHash(
            delegation, bytes(variableDebtToken.name()), bytes("1"), address(variableDebtToken)
        );

        (uint8 v, bytes32 r, bytes32 s) = vm.sign(alicePrivateKey, digest);

        vm.expectEmit(address(variableDebtToken));
        emit ICreditDelegationToken.BorrowAllowanceDelegated(alice, bob, address(tokenList.usdx), amount);
    ...
    function test_cancel_delegationWithSig() public {
        ...
        EIP712SigUtils.CreditDelegation memory delegation =
            EIP712SigUtils.CreditDelegation({delegatee: bob, value: 0, nonce: 1, deadline: deadline});
        ...
        assertEq(variableDebtToken.borrowAllowance(alice, bob), 0, "Alice allowance to bob should be zero");
