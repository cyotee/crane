...
contract PoolDeficitTests is TestnetProcedures {
    ...
    function test_eliminateReserveDeficit_exactDeficit(address coverageAdmin, uint120 borrowAmount) public {
        ...
        DataTypes.UserConfigurationMap memory userConfigBefore = contracts.poolProxy.getUserConfiguration(coverageAdmin);
    ...
    function test_eliminateReserveDeficit_fullUserBalance(address coverageAdmin, uint120 borrowAmount) public {
        _filterAddresses(coverageAdmin);
        uint256 currentDeficit = _createReserveDeficit(borrowAmount, tokenList.usdx);
        ...
        DataTypes.UserConfigurationMap memory userConfigAfter = contracts.poolProxy.getUserConfiguration(coverageAdmin);
        assertEq(userConfigAfter.isUsingAsCollateral(reserveData.id), false);

        _checkInterestRates(tokenList.usdx);
    }

    function test_eliminateReserveDeficit_surplus(address coverageAdmin, uint120 borrowAmount) public {
        _filterAddresses(coverageAdmin);
        uint256 currentDeficit = _createReserveDeficit(borrowAmount, tokenList.usdx);
        ...
        DataTypes.ReserveDataLegacy memory reserveData = contracts.poolProxy.getReserveData(tokenList.usdx);
        DataTypes.UserConfigurationMap memory userConfig = contracts.poolProxy.getUserConfiguration(coverageAdmin);
        assertEq(userConfig.isUsingAsCollateral(reserveData.id), true);

        _checkInterestRates(tokenList.usdx);
    }

    function test_eliminateReserveDeficit_partial(address coverageAdmin, uint120 borrowAmount, uint120 amountToCover)
        public
    {
        _filterAddresses(coverageAdmin);
        uint256 currentDeficit = _createReserveDeficit(borrowAmount, tokenList.usdx);
    ...
    function test_reverts_eliminateReserveDeficit_invalid_caller(address caller, uint120 borrowAmount) public {
    ...
    function test_reverts_eliminateReserveDeficit_invalid_amount(address coverageAdmin, uint120 borrowAmount) public {
        _filterAddresses(coverageAdmin);
    ...
    function _checkIrInvariant(address asset) internal view {
        ...
        assertLt(
            ...
            reserveData.currentVariableBorrowRate * IERC20(reserveData.variableDebtTokenAddress).totalSupply()
        );
    }

    function _createReserveDeficit(uint256 borrowAmount, address borrowAsset, bool mintBorrowableAssets)
        internal
        returns (uint256)
    {
    ...
    function _createReserveDeficit(uint120 borrowAmount, address borrowAsset) internal returns (uint256) {
