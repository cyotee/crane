...
import {
    ...
    IPoolConfigurator
} from "@crane/contracts/protocols/lending/aave/v3.6/protocol/pool/PoolConfigurator.sol";
import {MockAToken} from "@crane/contracts/protocols/lending/aave/v3.6/utils/mocks/tokens/MockAToken.sol";
import {
    MockVariableDebtToken
} from "@crane/contracts/protocols/lending/aave/v3.6/utils/mocks/tokens/MockDebtTokens.sol";
...
contract PoolConfiguratorUpgradeabilityTests is TestnetProcedures {
    ...
    function test_setReserveInterestRateData() public {
        address currentInterestRateStrategy =
        ...
        emit IPoolConfigurator.ReserveInterestRateDataChanged(
            tokenList.usdx, currentInterestRateStrategy, newInterestRateData
        );

        vm.prank(poolAdmin);
        contracts.poolConfiguratorProxy
            .setReserveInterestRateData(tokenList.usdx, _getDefaultInterestRatesStrategyData());

        address newInterestRateStrategy = contracts.protocolDataProvider.getInterestRateStrategyAddress(tokenList.usdx);
        address newInterestRateStrategyFromGetReserveData =
            contracts.poolProxy.getReserveData(tokenList.usdx).interestRateStrategyAddress;
    ...
    function test_updateAToken() public {
        ConfiguratorInputTypes.UpdateATokenInput memory input = ConfiguratorInputTypes.UpdateATokenInput({
        ...
        address previousImplementation = SlotParser.loadAddressFromSlot(
            aTokenProxy, bytes32(uint256(keccak256("eip1967.proxy.implementation")) - 1)
        );
    ...
    function test_updateVariableDebtToken() public {
        ...
        ConfiguratorInputTypes.UpdateDebtTokenInput memory input = ConfiguratorInputTypes.UpdateDebtTokenInput({
            ...
            implementation: address(new MockVariableDebtToken(IPool(report.poolProxy), report.rewardsControllerProxy)),
            params: bytes("")
        });

        address previousImplementation = SlotParser.loadAddressFromSlot(
            variableDebtProxy, bytes32(uint256(keccak256("eip1967.proxy.implementation")) - 1)
        );
        ...
        vm.expectEmit(address(contracts.poolConfiguratorProxy));
        emit IPoolConfigurator.VariableDebtTokenUpgraded(tokenList.usdx, variableDebtProxy, input.implementation);
        ...
        assertEq(address(VariableDebtToken(variableDebtProxy).getIncentivesController()), report.rewardsControllerProxy);
    }
