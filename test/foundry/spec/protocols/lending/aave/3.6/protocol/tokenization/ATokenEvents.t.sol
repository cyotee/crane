...
import {
    ...
    IERC20,
    IScaledBalanceToken
} from "@crane/contracts/protocols/lending/aave/v3.6/interfaces/IAToken.sol";
...
contract ATokenEventsTests is TestnetProcedures {
    ...
    function _expectATokenSupplyEvents(
        ...
        DataTypes.ReserveDataLegacy memory reserveData = contracts.poolProxy.getReserveData(underlyingToken);
        ...
        if (checkInterestsNonZero) {
            assertTrue(balanceIncrease > 0, "Intention failed: balanceIncrease should be greater than zero");
        }

        vm.expectEmit(address(underlyingToken));
        emit IERC20Events.Transfer(user, aTokenAddress, amount);
        vm.expectEmit(address(aToken));
        emit IERC20Events.Transfer(address(0), onBehalfOf, mintAmount);
        vm.expectEmit(address(aToken));
    ...
    function _expectATokenWithdrawEvents(
        ...
        if (checkAmountLessThanInterests) {
            assertTrue(balanceIncrease > amount, "Intention failed: balanceIncrease should be greater than amount");
        } else {
            assertTrue(amount > balanceIncrease, "Intention failed: amount should be greater than balanceIncrease");
        }
        if (checkInterestsNonZero) {
            assertTrue(balanceIncrease > 0, "Intention failed: balanceIncrease should be greater than zero");
        }
        if (deltaAmount > 0) {
            vm.expectEmit(address(aToken));
            // forge-lint: disable-next-line(unsafe-typecast)
            emit IERC20Events.Transfer(address(0), user, uint256(deltaAmount));
            vm.expectEmit(address(aToken));
            // forge-lint: disable-next-line(unsafe-typecast)
            emit IScaledBalanceToken.Mint(user, user, uint256(deltaAmount), balanceIncrease, newIndex);
            vm.expectEmit(address(underlyingToken));
            emit IERC20Events.Transfer(aTokenAddress, user, amount);
        } else {
            vm.expectEmit(address(aToken));
            // forge-lint: disable-next-line(unsafe-typecast)
            emit IERC20Events.Transfer(user, address(0), uint256(-deltaAmount));
            vm.expectEmit(address(aToken));
            // forge-lint: disable-next-line(unsafe-typecast)
    ...
    function test_atoken_burnEvents_singleWithdraw_noInterests() public {
        ...
        _expectATokenWithdrawEvents(tokenList.usdx, address(aToken), alice, alice, supplyAmount, false, false);
        vm.prank(alice);
    ...
    function test_atoken_burnEvents_singleWithdraw_WithInterests() public {
        ...
        _expectATokenWithdrawEvents(tokenList.usdx, address(aToken), alice, alice, supplyAmount, false, true);
        vm.prank(alice);
        contracts.poolProxy.withdraw(tokenList.usdx, supplyAmount, alice);
    }
    ...
    function test_atoken_burnEvents_multipleWithdrawals_withInterests() public {
        ...
        _expectATokenWithdrawEvents(tokenList.usdx, address(aToken), alice, alice, partialWithdrawAmount, false, true);
        vm.prank(alice);
        ...
        _expectATokenWithdrawEvents(
            tokenList.usdx, address(aToken), alice, alice, secondPartialWithdrawAmount, false, true
        );
        vm.prank(alice);
    ...
    function test_atoken_burnEvents_fullBalance() public {
        ...
        _expectATokenWithdrawEvents(tokenList.usdx, address(aToken), alice, alice, fullBalance, false, true);
        vm.prank(alice);
    ...
    function test_mintToTreasury_events() public {
        ...
        vm.expectEmit(address(aToken));
        emit IERC20Events.Transfer(address(0), aToken.RESERVE_TREASURY_ADDRESS(), amountToMint);

        vm.expectEmit(address(aToken));
        emit IScaledBalanceToken.Mint(
            address(contracts.poolProxy), aToken.RESERVE_TREASURY_ADDRESS(), amountToMint, 0, 1e27
        );
    ...
    function test_allowance_events_in_approve_function() public {
        vm.expectEmit(address(aToken));
        emit IERC20Events.Approval(alice, bob, 1e18);
        vm.prank(alice);
        ...
        vm.expectEmit(address(aToken));
        emit IERC20Events.Approval(alice, bob, type(uint256).max);
        vm.prank(alice);
    ...
    function test_allowance_events_in_increaseAllowance_function() public {
        ...
        vm.expectEmit(address(aToken));
        emit IERC20Events.Approval(alice, bob, 1e18 + 0.5e18);
        vm.prank(alice);
    ...
    function test_allowance_events_in_decreaseAllowance_function() public {
        ...
        vm.expectEmit(address(aToken));
        emit IERC20Events.Approval(alice, bob, 0.5e18);
        vm.prank(alice);
