...
contract PoolConfiguratorBorrowCapTests is TestnetProcedures {
    ...
    function test_borrow_lt_cap() public {
        ...
        assertEq(IERC20(tokenList.usdx).balanceOf(alice), 1250e6, "Alice balance should match borrow amount");
    }
    ...
    function test_borrow_eq_cap() public {
        ...
        assertEq(IERC20(tokenList.usdx).balanceOf(alice), 5000e6, "Alice balance should match borrow amount");

        uint256 variableDebt = IERC20(contracts.poolProxy.getReserveVariableDebtToken(tokenList.usdx)).totalSupply();

        assertEq(variableDebt, borrowCapUsdx * 10 ** 6, "Borrow Cap should match same amount than total debt");
    }
    ...
    function test_setBorrowCap_them_setBorrowCap_zero() public {
        ...
        assertEq(IERC20(tokenList.usdx).balanceOf(alice), 5000e6, "Alice borrowed balance should match borrow amount");
    }
