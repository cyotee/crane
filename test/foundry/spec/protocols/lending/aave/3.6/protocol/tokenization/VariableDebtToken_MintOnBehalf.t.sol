...
contract VariableDebtToken_MintOnBehalfTests is TestnetProcedures {
    ...
    function setUp() public {
        ...
        variableDebtToken = VariableDebtTokenInstance(contracts.poolProxy.getReserveVariableDebtToken(token));
    }
    ...
    function test_mint_shouldRevertIfSenderIsNotApproved() external {
        ...
        uint256 amount = 100;
        vm.expectRevert(
            abi.encodeWithSelector(ICreditDelegationToken.InsufficientBorrowAllowance.selector, sender, 0, amount)
        );
    ...
    function test_mint_shouldRevertIfSenderInsufficientAllowance() external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ICreditDelegationToken.InsufficientBorrowAllowance.selector, sender, amount - 1, amount
            )
        );
        vm.prank(address(contracts.poolProxy));
        variableDebtToken.mint(sender, owner, amount, amount, 1e27);
    ...
    function test_mint(uint128 variableBorrowIndex, uint256 approval) external {
        ...
        variableDebtToken.approveDelegation(
            sender, bound(approval, amount, amount.rayDivCeil(variableBorrowIndex).rayMulCeil(variableBorrowIndex))
        );
        ...
        vm.prank(address(contracts.poolProxy));
        variableDebtToken.mint(sender, owner, amount, amount.rayDivCeil(variableBorrowIndex), variableBorrowIndex);
        ...
        uint256 upscaledDiff = (ownerScaledBalanceAfter - ownerScaledBalanceBefore).rayMulFloor(variableBorrowIndex);
