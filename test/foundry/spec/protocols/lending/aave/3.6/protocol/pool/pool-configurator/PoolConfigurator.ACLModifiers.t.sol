...
import {ConfiguratorInputTypes} from "@crane/contracts/protocols/lending/aave/v3.6/protocol/pool/PoolConfigurator.sol";
...
contract PoolConfiguratorACLModifiersTest is TestnetProcedures {
    ...
    function test_reverts_notAdmin_initReserves(TestVars memory t, address caller) public {
        ...
        vm.assume(
            !contracts.aclManager.isPoolAdmin(caller) && !contracts.aclManager.isAssetListingAdmin(caller)
    ...
    function test_reverts_notAdmin_dropReserve(address caller) public {
        vm.assume(!contracts.aclManager.isPoolAdmin(caller) && caller != address(contracts.poolAddressesProvider));
    ...
    function test_reverts_notRiskAdmin_setReserveBorrowing(address caller) public {
        vm.assume(
            !contracts.aclManager.isPoolAdmin(caller) && !contracts.aclManager.isRiskAdmin(caller)
                && caller != address(contracts.poolAddressesProvider)
        );
    ...
    function test_reverts_notRiskOrPoolOrEmergencyAdmin_setReserveFreeze(address caller) public {
        vm.assume(
            !contracts.aclManager.isPoolAdmin(caller) && !contracts.aclManager.isRiskAdmin(caller)
                && !contracts.aclManager.isEmergencyAdmin(caller) && caller != address(contracts.poolAddressesProvider)
        );
    ...
    function test_reverts_setReservePause_on_unauth(address caller, address asset, bool paused, uint40 gracePeriod)
        public
    {
        vm.assume(
            !contracts.aclManager.isPoolAdmin(caller) && !contracts.aclManager.isEmergencyAdmin(caller)
                && caller != address(contracts.poolAddressesProvider)
        );
    ...
    function test_reverts_setReservePause_off_unauth(address caller, address asset, bool paused, uint40 gracePeriod)
        public
    {
        vm.assume(
            !contracts.aclManager.isPoolAdmin(caller) && !contracts.aclManager.isEmergencyAdmin(caller)
    ...
    function test_reverts_setReservePause_noGracePeriod_off_unauth(address caller, address asset, bool paused) public {
        vm.assume(
            !contracts.aclManager.isPoolAdmin(caller) && !contracts.aclManager.isEmergencyAdmin(caller)
                && caller != address(contracts.poolAddressesProvider)
        );
    ...
    function test_reverts_disableLiquidationGracePeriod_on_unauth(address caller, address asset) public {
        vm.assume(
            !contracts.aclManager.isPoolAdmin(caller) && !contracts.aclManager.isEmergencyAdmin(caller)
                && caller != address(contracts.poolAddressesProvider)
        );
    ...
    function test_reverts_setPoolPause_unauth(address caller, bool paused, uint40 gracePeriod) public {
        vm.assume(
            !contracts.aclManager.isPoolAdmin(caller) && !contracts.aclManager.isEmergencyAdmin(caller)
                && caller != address(contracts.poolAddressesProvider)
        );
