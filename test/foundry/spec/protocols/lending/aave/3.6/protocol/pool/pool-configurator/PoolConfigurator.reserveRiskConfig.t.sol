...
contract PoolConfiguratorReserveRiskConfigs is TestnetProcedures {
    ...
    function test_enableBorrowing(TestVars memory t) public {
        ConfiguratorInputTypes.InitReserveInput[] memory input = _generateInitConfig(t, report, poolAdmin, true);

        // Perform action
        vm.prank(poolAdmin);
        contracts.poolConfiguratorProxy.initReserves(input);
        (,,,,,, bool borrowingEnabledDefault,,,) =
            contracts.protocolDataProvider.getReserveConfigurationData(input[0].underlyingAsset);
    ...
    function test_setCollateralConfig(TestVars memory t) public {
        ...
        emit IPoolConfigurator.CollateralConfigurationChanged(
            input[0].underlyingAsset, ltv, liquidationThreshold, liquidationBonus
        );

        vm.prank(poolAdmin);
        contracts.poolConfiguratorProxy
            .configureReserveAsCollateral(input[0].underlyingAsset, ltv, liquidationThreshold, liquidationBonus);
        (, uint256 ltvConfig, uint256 liqThreshold, uint256 liqBonus,,,,,,) =
            contracts.protocolDataProvider.getReserveConfigurationData(input[0].underlyingAsset);
    ...
    function test_reverts_setCollateralConfig_invalidParams(TestVars memory t) public {
        ...
        vm.prank(poolAdmin);
        contracts.poolConfiguratorProxy.configureReserveAsCollateral(input[0].underlyingAsset, 95_00, 90_00, 105_00);

        // Revert due LIQ BONUS < PERCENTAGE_FACTOR
        vm.expectRevert(abi.encodeWithSelector(Errors.InvalidReserveParams.selector));

        vm.prank(poolAdmin);
        contracts.poolConfiguratorProxy.configureReserveAsCollateral(input[0].underlyingAsset, 90_00, 95_00, 99_00);

        // Revert due LQT does not cover bonus
        vm.expectRevert(abi.encodeWithSelector(Errors.InvalidReserveParams.selector));

        vm.prank(poolAdmin);
        contracts.poolConfiguratorProxy.configureReserveAsCollateral(input[0].underlyingAsset, 90_00, 95_00, 200_00);

        // Revert due LQT == 0 == LQ_BONUS
        vm.expectRevert(abi.encodeWithSelector(Errors.InvalidReserveParams.selector));

        vm.prank(poolAdmin);
        contracts.poolConfiguratorProxy.configureReserveAsCollateral(input[0].underlyingAsset, 90_00, 0, 1);
    ...
    function test_setReserveActive_false() public {
        ...
        vm.prank(poolAdmin);
        contracts.poolConfiguratorProxy.setReserveActive(tokenList.usdx, false);
        (,,,,,,,, bool isActive,) = contracts.protocolDataProvider.getReserveConfigurationData(tokenList.usdx);
    ...
    function test_PoolAdminSetReserveFreeze_true() public {
        ...
        (,,,,,,,,, bool isFrozen) = contracts.protocolDataProvider.getReserveConfigurationData(tokenList.usdx);
    ...
    function test_setReserveFreeze_false() public {
        ...
        (,,,,,,,,, isFrozen) = contracts.protocolDataProvider.getReserveConfigurationData(tokenList.usdx);
    ...
    function test_setBorrowableInIsolation_true() public {
        ...
        DataTypes.ReserveConfigurationMap memory config = contracts.poolProxy.getConfiguration(tokenList.usdx);
    ...
    function test_setReserveFactor() public {
        ...
        (,,,, uint256 reserveFactor,,,,,) = contracts.protocolDataProvider.getReserveConfigurationData(tokenList.usdx);
    ...
    function test_dropReserve() public {
        DataTypes.ReserveDataLegacy memory reserveDataUsdx = contracts.poolProxy.getReserveData(tokenList.usdx);
        vm.expectEmit(address(contracts.poolConfiguratorProxy));
        emit IPoolConfigurator.ReserveDropped(tokenList.usdx);

        (address pA, address pS, address pV) = contracts.protocolDataProvider.getReserveTokensAddresses(tokenList.usdx);
        ...
        (address a, address s, address v) = contracts.protocolDataProvider.getReserveTokensAddresses(tokenList.usdx);
        {
            (
                uint256 decimals,
                uint256 ltv,
    ...
    function _getFullReserveData(address asset) internal view returns (DataTypes.ReserveData memory) {
        DataTypes.ReserveDataLegacy memory reserveDataLegacy = contracts.poolProxy.getReserveData(asset);
        ...
        tempReserveData.__deprecatedInterestRateStrategyAddress = reserveDataLegacy.interestRateStrategyAddress;
        ...
        tempReserveData.virtualUnderlyingBalance = uint128(contracts.poolProxy.getVirtualUnderlyingBalance(asset));
