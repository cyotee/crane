...
} from "@crane/contracts/protocols/lending/aave/v3.6/utils/mocks/flashloan/MockFlashLoanReceiver.sol";
import {
    MockFlashLoanSimpleReceiver
} from "@crane/contracts/protocols/lending/aave/v3.6/utils/mocks/flashloan/MockSimpleFlashLoanReceiver.sol";
import {
...
} from "@crane/contracts/protocols/lending/aave/v3.6/interfaces/IReserveInterestRateStrategy.sol";
import {DataTypes} from "@crane/contracts/protocols/lending/aave/v3.6/protocol/libraries/types/DataTypes.sol";
import {IERC20} from "@crane/contracts/interfaces/IERC20.sol";
import {
    MockFlashLoanATokenReceiver
} from "@crane/contracts/protocols/lending/aave/v3.6/utils/mocks/MockFlashLoanATokenReceiver.sol";
import {
    MockFlashLoanBorrowInsideFlashLoan
} from "@crane/contracts/protocols/lending/aave/v3.6/utils/mocks/MockFlashLoanBorrowInsideFlashLoan.sol";
import {
    TestnetProcedures,
    TestReserveConfig
} from "@crane/test/foundry/spec/protocols/lending/aave/3.6/utils/TestnetProcedures.sol";
...
contract PoolFlashLoansTests is TestnetProcedures {
    ...
    function test_reverts_flashLoan_invalid_return() public {
        (address[] memory assets, uint256[] memory amounts, uint256[] memory modes, bytes memory emptyParams) =
            _defaultInput();
        ...
        vm.prank(alice);
        contracts.poolProxy.flashLoan(address(mockFlashReceiver), assets, amounts, modes, alice, emptyParams, 0);
    }
    ...
    function test_reverts_supply_flashloan_transfer_withdraw() public {
        ...
        MockFlashLoanATokenReceiver newMockReceiver =
            new MockFlashLoanATokenReceiver(IPoolAddressesProvider(report.poolAddressesProvider), aUSDX);
        ...
        contracts.poolProxy
            .flashLoan(address(newMockReceiver), assets, amounts, modes, address(newMockReceiver), emptyParams, 0);
    }
    ...
    function test_reverts_supply_flashloan_simple_transfer_withdraw() public {
        ...
        vm.expectRevert(stdError.arithmeticError);
        contracts.poolProxy.flashLoanSimple(address(newMockReceiver), tokenList.usdx, 10e6, emptyParams, 0);
    }

    function test_flashloan_simple() public {
        uint256 virtualUnderlyingBalanceBefore = contracts.poolProxy.getVirtualUnderlyingBalance(tokenList.usdx);
        ...
        vm.prank(alice);
        contracts.poolProxy.flashLoanSimple(address(mockFlashSimpleReceiver), tokenList.usdx, amount, "0x", 0);

        uint256 virtualUnderlyingBalanceAfter = contracts.poolProxy.getVirtualUnderlyingBalance(tokenList.usdx);
        assertEq(virtualUnderlyingBalanceBefore + (amount * totalFee) / 1e4, virtualUnderlyingBalanceAfter);
    }
    ...
    function test_flashloan_simple_rounding_premium() public {
        ...
        vm.prank(alice);
        contracts.poolProxy.flashLoanSimple(address(mockFlashSimpleReceiver), tokenList.usdx, 1, "0x", 0);
    }
    ...
    function test_flashloan_rounding_premium() public {
        ...
        vm.prank(alice);
        contracts.poolProxy.flashLoan(address(mockFlashReceiver), assets, amounts, modes, alice, "0x", 0);
    }
    ...
    function test_flashloan_rounding_accruedToTreasury() public {
        ...
        contracts.poolProxy.supply({asset: tokenList.usdx, amount: supplyAmount, onBehalfOf: alice, referralCode: 0});

        contracts.poolProxy
            .borrow({
                asset: tokenList.usdx, amount: supplyAmount / 5, interestRateMode: 2, referralCode: 0, onBehalfOf: alice
            });

        vm.warp(block.timestamp + 4500000 days);

        contracts.poolProxy
            .repay({asset: tokenList.usdx, amount: supplyAmount / 5, interestRateMode: 2, onBehalfOf: alice});

        vm.stopPrank();

        // check liquidity index

        DataTypes.ReserveDataLegacy memory reserveData = contracts.poolProxy.getReserveData(tokenList.usdx);
    ...
    function test_flashloan_simple_2() public {
        ...
        vm.prank(alice);
        contracts.poolProxy.flashLoanSimple(address(mockFlashSimpleReceiver), tokenList.wbtc, 3e8, "0x", 0);
    ...
    function test_flashloan() public {
        ...
        uint256 virtualUnderlyingBalanceBefore = contracts.poolProxy.getVirtualUnderlyingBalance(assets[0]);
        uint256 totalFee = contracts.poolProxy.FLASHLOAN_PREMIUM_TOTAL();

        vm.prank(alice);
        contracts.poolProxy.flashLoan(address(mockFlashReceiver), assets, amounts, modes, alice, emptyParams, 0);

        uint256 virtualUnderlyingBalanceAfter = contracts.poolProxy.getVirtualUnderlyingBalance(assets[0]);
        assertEq(virtualUnderlyingBalanceBefore + (amounts[0] * totalFee) / 1e4, virtualUnderlyingBalanceAfter);
    }
    ...
    function test_flashloan_multiple() public {
        ...
        uint256 virtualUnderlyingBalanceBefore0 = contracts.poolProxy.getVirtualUnderlyingBalance(assets[0]);
        uint256 virtualUnderlyingBalanceBefore1 = contracts.poolProxy.getVirtualUnderlyingBalance(assets[1]);
        uint256 totalFee = contracts.poolProxy.FLASHLOAN_PREMIUM_TOTAL();

        vm.prank(alice);
        contracts.poolProxy.flashLoan(address(mockFlashReceiver), assets, amounts, modes, alice, emptyParams, 0);

        uint256 virtualUnderlyingBalanceAfter0 = contracts.poolProxy.getVirtualUnderlyingBalance(assets[0]);
        uint256 virtualUnderlyingBalanceAfter1 = contracts.poolProxy.getVirtualUnderlyingBalance(assets[1]);

        assertEq(virtualUnderlyingBalanceBefore0 + (amounts[0] * totalFee) / 1e4, virtualUnderlyingBalanceAfter0);
        assertEq(virtualUnderlyingBalanceBefore1 + (amounts[1] * totalFee) / 1e4, virtualUnderlyingBalanceAfter1);
    }
    ...
    function test_flashloan_simple_borrow_inside_flashloan_and_check_rate_after() public {
        MockFlashLoanBorrowInsideFlashLoan receiver =
            new MockFlashLoanBorrowInsideFlashLoan(contracts.poolAddressesProvider);
        ...
        contracts.poolProxy
            .borrow({
                asset: asset, amount: underlyingBalance / 5, interestRateMode: 2, referralCode: 0, onBehalfOf: carol
            });
        // forge-lint: disable-next-line(erc20-unchecked-transfer)
        IERC20(contracts.poolProxy.getReserveAToken(asset)).transfer(address(receiver), underlyingBalance / 2);
        vm.stopPrank();
        ...
        contracts.poolProxy
            .flashLoanSimple({
                receiverAddress: address(receiver), asset: asset, amount: amount, params: "", referralCode: 0
            });
    ...
    function test_flashloan_borrow_inside_flashloan_and_check_rate_after() public {
        ...
        for (uint256 i = 0; i < assets.length; ++i) {
            ...
            // forge-lint: disable-next-line(erc20-unchecked-transfer)
            IERC20(contracts.poolProxy.getReserveAToken(assets[i]))
                .transfer(address(receiver), underlyingBalances[i] / 2);
            vm.stopPrank();
            vm.prank(address(receiver));
            ...
            DataTypes.ReserveDataLegacy memory reserveData = contracts.poolProxy.getReserveData(assets[i]);
    ...
    function _defaultInput() internal returns (address[] memory, uint256[] memory, uint256[] memory, bytes memory) {
        return _defaultInput(false, 0);
    }

    function _defaultInput(bool checkEvents, uint256 mode)
        internal
    ...
    function _defaultMultipleInput(bool checkEvents)
        internal
        returns (address[] memory, uint256[] memory, uint256[] memory, bytes memory)
    {
        address[] memory assets = new address[](2);
    ...
    function _checkFlashLoanEvents(address[] memory assets, uint256[] memory amounts, uint256[] memory modes) internal {
        uint256 totalFee = contracts.poolProxy.FLASHLOAN_PREMIUM_TOTAL();
