...
contract ATokenPermitTests is TestnetProcedures {
    ...
    function test_submitPermit() public {
        EIP712SigUtils.Permit memory permit = EIP712SigUtils.Permit({
            owner: alice, spender: bob, value: 1e6, nonce: 0, deadline: vm.getBlockTimestamp() + 1 days
        });
        bytes32 digest = EIP712SigUtils.getTypedDataHash(permit, bytes(aToken.name()), bytes("1"), address(aToken));

        (uint8 v, bytes32 r, bytes32 s) = vm.sign(alicePrivateKey, digest);

        assertEq(aToken.allowance(alice, bob), 0, "Alice allowance to bob should be zero before permit");

        vm.prank(bob);
        aToken.permit(alice, bob, permit.value, permit.deadline, v, r, s);
        assertEq(aToken.allowance(alice, bob), permit.value, "Alice allowance does not match permit.value");
    ...
    function test_cancelPermit() public {
        ...
        EIP712SigUtils.Permit memory permit = EIP712SigUtils.Permit({
            owner: alice, spender: bob, value: 0, nonce: 1, deadline: vm.getBlockTimestamp() + 1 days
        });
        bytes32 digest = EIP712SigUtils.getTypedDataHash(permit, bytes(aToken.name()), bytes("1"), address(aToken));

        (uint8 v, bytes32 r, bytes32 s) = vm.sign(alicePrivateKey, digest);

        assertEq(aToken.allowance(alice, bob), 1e6, "Alice allowance to bob should be 1e6 before the cancel the permit");
    ...
    function test_revert_submitPermit_0_expiration() public {
        EIP712SigUtils.Permit memory permit =
            EIP712SigUtils.Permit({owner: alice, spender: bob, value: 1e6, nonce: 0, deadline: 0});
        bytes32 digest = EIP712SigUtils.getTypedDataHash(permit, bytes(aToken.name()), bytes("1"), address(aToken));
    ...
    function test_revert_submitPermit_invalid_nonce() public {
        EIP712SigUtils.Permit memory permit = EIP712SigUtils.Permit({
            owner: alice, spender: bob, value: 1e6, nonce: 10, deadline: vm.getBlockTimestamp() + 1 days
        });
        bytes32 digest = EIP712SigUtils.getTypedDataHash(permit, bytes(aToken.name()), bytes("1"), address(aToken));
    ...
    function test_revert_submitPermit_InvalidExpiration_previosCurrentBlock() public {
        EIP712SigUtils.Permit memory permit =
            EIP712SigUtils.Permit({owner: alice, spender: bob, value: 1e6, nonce: 0, deadline: 1});
        bytes32 digest = EIP712SigUtils.getTypedDataHash(permit, bytes(aToken.name()), bytes("1"), address(aToken));
    ...
    function test_revert_submitPermit_InvalidSignature() public {
        EIP712SigUtils.Permit memory permit =
            EIP712SigUtils.Permit({owner: alice, spender: bob, value: 1e6, nonce: 0, deadline: 1 days});
        bytes32 digest = EIP712SigUtils.getTypedDataHash(permit, bytes(aToken.name()), bytes("1"), address(aToken));
    ...
    function test_chain_fork_calculateDomainSeparator() public {
        ...
        assertEq(aToken.DOMAIN_SEPARATOR(), cachedDomainSeparator, "domain separator should match cached");
    }
}
