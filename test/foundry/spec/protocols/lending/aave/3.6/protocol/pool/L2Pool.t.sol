...
import {L2Encoder} from "@crane/contracts/protocols/lending/aave/v3.6/helpers/L2Encoder.sol";
import {IL2Pool} from "@crane/contracts/protocols/lending/aave/v3.6/interfaces/IL2Pool.sol";
import {
...
contract L2PoolTests is PoolTests {
    ...
    function test_l2_supply_permit(uint128 userPk, uint120 supplyAmount) public {
        ...
        (bytes32 encodedInput1, bytes32 encodedInput2, bytes32 encodedInput3) =
            l2Encoder.encodeSupplyWithPermitParams(tokenList.usdx, permit.value, 0, permit.deadline, v, r, s);
    ...
    function test_l2_repay_permit(
        ...
        (bytes32 encodedInput1, bytes32 encodedInput2, bytes32 encodedInput3) =
            l2Encoder.encodeRepayWithPermitParams(tokenList.usdx, permit.value, 2, permit.deadline, v, r, s);
    ...
    function test_l2_liquidationCall() public {
        ...
        stdstore.target(IAaveOracle(report.aaveOracle).getSourceOfAsset(tokenList.wbtc)).sig("_latestAnswer()")
            .checked_write(_calcPrice(IAaveOracle(report.aaveOracle).getAssetPrice(tokenList.wbtc), 30_00));
        ...
        (bytes32 encodedInput1, bytes32 encodedInput2) =
            l2Encoder.encodeLiquidationCall(tokenList.wbtc, tokenList.usdx, alice, UINT256_MAX, false);
