...
import {ISequencerOracle} from "@crane/contracts/protocols/lending/aave/v3.6/interfaces/ISequencerOracle.sol";
...
import {
    BorrowLogic,
    IERC20
} from "@crane/contracts/protocols/lending/aave/v3.6/protocol/libraries/logic/BorrowLogic.sol";
...
contract PoolRepayTests is TestnetProcedures {
    ...
    function test_repay_full_variable_borrow() public {
        ...
        emit IPool.Repay(tokenList.usdx, alice, alice, IERC20(address(varDebtUSDX)).balanceOf(alice), false);
    ...
    function test_repayWithATokens_full_variable_borrow() public {
        ...
        vm.expectEmit(report.poolProxy);
        emit IPool.Repay(tokenList.usdx, alice, alice, IERC20(address(varDebtUSDX)).balanceOf(alice), true);
    ...
    function test_repayWithPermit_should_revert_if_permit_is_less_then_repay_amount(
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IERC20Errors.ERC20InsufficientAllowance.selector, address(contracts.poolProxy), 0, repayAmount
            )
        );
        contracts.poolProxy.repayWithPermit(tokenList.usdx, repayAmount, 2, user, permit.deadline, v, r, s);
        vm.stopPrank();
    ...
    function test_partial_repay_borrow_variable_in_isolation() public {
        ...
        vm.expectEmit(report.poolProxy);
        emit IPool.IsolationModeTotalDebtUpdated(tokenList.wbtc, isolationModeTotalDebt - isolationDebtRepaid);
