...
abstract contract BaseInvariants is HandlerAggregator {
    ...
    function assert_BASE_INVARIANT_A_EXACT(IERC20 debtToken) internal {
        ...
        assertEq(IAToken(address(debtToken)).scaledTotalSupply(), sumOfUserBalances, BASE_INVARIANT_A_EXACT);
    }
    ...
    function assert_BASE_INVARIANT_B_EXACT(IERC20 aToken) internal {
        ...
        assertEq(IAToken(address(aToken)).scaledTotalSupply(), sumOfUserBalances, BASE_INVARIANT_B_EXACT);
    }

    function assert_BASE_INVARIANT_C(address asset) internal {
        uint256 liquidityAfterAllRepayments = IERC20(protocolTokens[asset].variableDebtTokenAddress).totalSupply()
    ...
    function assert_BASE_INVARIANT_F(address asset) internal {
        ...
        for (uint256 i; i < numberOfEModes; i++) {
            ...
            if (EModeConfiguration.isReserveEnabledOnBitmap({
                    bitmap: ltvzeroBitmap, reserveIndex: protocolTokens[asset].id
                })) {
                uint128 collateralBitmap = pool.getEModeCategoryCollateralBitmap(categoryId);

                assertTrue(
                    EModeConfiguration.isReserveEnabledOnBitmap({
                        bitmap: collateralBitmap, reserveIndex: protocolTokens[asset].id
                    }),
                    BASE_INVARIANT_F
                );
            }
    ...
    function assert_BORROWING_INVARIANT_B(address asset) internal {
        ...
        for (uint256 i; i < NUMBER_OF_ACTORS; i++) {
            if (IAToken(protocolToken.variableDebtTokenAddress).scaledBalanceOf(actorAddresses[i]) == 0) {
                assertFalse(
                    pool.getUserConfiguration(actorAddresses[i]).isBorrowing(protocolToken.id), BORROWING_INVARIANT_B1
                );
            } else {
                assertTrue(
                    pool.getUserConfiguration(actorAddresses[i]).isBorrowing(protocolToken.id), BORROWING_INVARIANT_B2
                );
            }
        }
    }
    ...
    function assert_BORROWING_INVARIANT_C(address asset) internal {
        for (uint256 i; i < NUMBER_OF_ACTORS; i++) {
            if (IAToken(protocolTokens[asset].aTokenAddress).scaledBalanceOf(actorAddresses[i]) == 0) {
                assertFalse(
                    pool.getUserConfiguration(actorAddresses[i]).isUsingAsCollateral(protocolTokens[asset].id),
    ...
    function assert_BORROWING_INVARIANT_D(address asset) internal {
        assertLe(pool.getLiquidationGracePeriod(asset), block.timestamp + MAX_GRACE_PERIOD, BORROWING_INVARIANT_D);
    }
    ...
    function assert_ORACLE_INVARIANT_A() internal {
        for (uint256 i; i < baseAssets.length; i++) {
            try contracts.aaveOracle.getAssetPrice(baseAssets[i]) {}
            catch {
