...
// Libraries
import {Strings} from "@crane/contracts/protocols/lending/aave/v3.6/utils/Pretty.sol";
import {
...
import {
    ILendingHandler
} from "@crane/test/foundry/spec/protocols/lending/aave/3.6/invariants/handlers/interfaces/ILendingHandler.sol";
import {
    IBorrowingHandler
} from "@crane/test/foundry/spec/protocols/lending/aave/3.6/invariants/handlers/interfaces/IBorrowingHandler.sol";
import {
    ILiquidationHandler
} from "@crane/test/foundry/spec/protocols/lending/aave/3.6/invariants/handlers/interfaces/ILiquidationHandler.sol";
import {
    IPoolHandler
} from "@crane/test/foundry/spec/protocols/lending/aave/3.6/invariants/handlers/interfaces/IPoolHandler.sol";
...
import {
    IATokenHandler
} from "@crane/test/foundry/spec/protocols/lending/aave/3.6/invariants/handlers/interfaces/IATokenHandler.sol";
...
abstract contract DefaultBeforeAfterHooks is BaseHooks {
    ...
    function _makeSnapshot(SnapshotGlobalVars storage _snapshotGlobalVars) internal {
        ...
        _makeUserSnapshot(_snapshotGlobalVars.usersInfo[address(contracts.treasury)], address(contracts.treasury));
    ...
    function _makeEModeSnapshot(EModeInfo storage _eModeSnapshot, uint256 eModeCategory) internal {
        ...
        for (uint256 i = 0; i < baseAssets.length; ++i) {
            ...
            if (EModeConfiguration.isReserveEnabledOnBitmap({
                    bitmap: _eModeSnapshot.collateralBitmap, reserveIndex: reserveData.id
                })) {
            ...
            if (EModeConfiguration.isReserveEnabledOnBitmap({
                    bitmap: _eModeSnapshot.borrowableBitmap, reserveIndex: reserveData.id
                })) {
            ...
            if (EModeConfiguration.isReserveEnabledOnBitmap({
                    bitmap: _eModeSnapshot.ltvzeroBitmap, reserveIndex: reserveData.id
                })) {
    ...
    function _resetAssetSnapshot(
        AssetInfo storage _assetSnapshot,
        address /* asset */
    )
        internal
    {
    ...
    function _makeUserSnapshot(UserInfo storage _userSnapshot, address user) internal {
        ...
        for (uint256 i = 0; i < baseAssets.length; ++i) {
            _makeUserAssetSnapshot({
                _userAssetSnapshot: _userSnapshot.userAssetsInfo[baseAssets[i]], user: user, asset: baseAssets[i]
            });
        }
    }
    ...
    function _makeUserAssetSnapshot(UserAssetInfo storage _userAssetSnapshot, address user, address asset) internal {
        ...
        _userAssetSnapshot.aTokenBalanceInBaseCurrency = (assetPrice * _userAssetSnapshot.aTokenBalance) / assetUnit;
        _userAssetSnapshot.aTokenScaledBalance = IAToken(protocolTokens[asset].aTokenAddress).scaledBalanceOf(user);
        _userAssetSnapshot.vTokenBalance = IERC20(protocolTokens[asset].variableDebtTokenAddress).balanceOf(user);
        _userAssetSnapshot.vTokenBalanceInBaseCurrency = (assetPrice * _userAssetSnapshot.vTokenBalance) / assetUnit;
        _userAssetSnapshot.vTokenScaledBalance =
            IVariableDebtToken(protocolTokens[asset].variableDebtTokenAddress).scaledBalanceOf(user);

        DataTypes.UserConfigurationMap memory userConfig = pool.getUserConfiguration(user);
        _userAssetSnapshot.isUsingAsCollateral = userConfig.isUsingAsCollateral(protocolTokens[asset].id);

        for (uint256 i = 0; i < NUMBER_OF_ACTORS; ++i) {
            _makeUserAssetAllowanceSnapshot({
                _userAssetSnapshot: _userAssetSnapshot, owner: user, recipient: actorAddresses[i], asset: asset
            });
        }
        _makeUserAssetAllowanceSnapshot({
            _userAssetSnapshot: _userAssetSnapshot, owner: user, recipient: UMBRELLA, asset: asset
        });
    }
    ...
    function _resetUserAssetSnapshot(UserAssetInfo storage _userAssetSnapshot) internal {
        ...
        for (uint256 i = 0; i < NUMBER_OF_ACTORS; ++i) {
            _resetUserAssetAllowanceSnapshot({_userAssetSnapshot: _userAssetSnapshot, recipient: actorAddresses[i]});
        }
    ...
    function _makeUserAssetAllowanceSnapshot(
        ...
        _userAssetSnapshot.aTokenAllowances[recipient] =
            IERC20(protocolTokens[asset].aTokenAddress).allowance(owner, recipient);
        _userAssetSnapshot.borrowAllowances[recipient] =
            ICreditDelegationToken(protocolTokens[asset].variableDebtTokenAddress).borrowAllowance(owner, recipient);
    }

    function _resetUserAssetAllowanceSnapshot(UserAssetInfo storage _userAssetSnapshot, address recipient) internal {
    ...
    function assert_LENDING_GPOST_C() internal {
        for (uint256 i = 0; i < baseAssets.length; ++i) {
            if (
                ...
                uint256 supplyCap = snapshotGlobalVarsAfter.assetsInfo[baseAssets[i]].configuration.getSupplyCap();
                if (supplyCap == 0) {
                    continue;
                }
    ...
    function assert_BORROWING_GPOST_H() internal {
        for (uint256 i = 0; i < baseAssets.length; ++i) {
            if (
                ...
                uint256 borrowCap = snapshotGlobalVarsAfter.assetsInfo[baseAssets[i]].configuration.getBorrowCap();
                if (borrowCap == 0) {
                    continue;
                }
    ...
    function assert_HF_GPOST_A(address user) internal {
        if (
            snapshotGlobalVarsAfter.usersInfo[user].healthFactor < snapshotGlobalVarsBefore.usersInfo[user].healthFactor
        ) {
    ...
    function assert_HF_GPOST_C(address user) internal {
        if (snapshotGlobalVarsBefore.usersInfo[user].isHealthy && !snapshotGlobalVarsAfter.usersInfo[user].isHealthy) {
