...
contract PoolPermissionedHandler is BaseHandler {
    ...
    function configureReserveAsCollateral(uint16 ltv, uint16 liquidationThreshold, uint16 liquidationBonus, uint8 i)
        external
    {
        address asset = _getRandomBaseAsset(i);

        _before();
        contracts.poolConfiguratorProxy.configureReserveAsCollateral(asset, ltv, liquidationThreshold, liquidationBonus);
        _after();
    ...
    function setEModeCategory(uint8 categoryId, uint16 ltv, uint16 liquidationThreshold, uint16 liquidationBonus)
        external
    {
        _before();
        contracts.poolConfiguratorProxy
            .setEModeCategory(categoryId, uint16(ltv), uint16(liquidationThreshold), uint16(liquidationBonus), "");
        _after();
    ...
    function setAssetLtvzeroInEMode(uint8 i, uint8 j, bool ltvzero) external {
        ...
        _before();
        contracts.poolConfiguratorProxy.setAssetLtvzeroInEMode({asset: asset, categoryId: categoryId, ltvzero: ltvzero});
        _after();
