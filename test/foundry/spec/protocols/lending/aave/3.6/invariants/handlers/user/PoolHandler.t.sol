...
contract PoolHandler is BaseHandler {
    ...
    function mintToTreasury(uint8 i) external setup {
        ...
        _before();
        (success, returnData) =
            actor.proxy(address(pool), abi.encodeWithSelector(IPool.mintToTreasury.selector, assets));
        ...
        if (success) {
            ...
            assertApproxEqAbs(
                snapshotGlobalVarsAfter.usersInfo[address(contracts.treasury)].userAssetsInfo[asset].aTokenBalance,
                snapshotGlobalVarsBefore.usersInfo[address(contracts.treasury)].userAssetsInfo[asset].aTokenBalance
                    + amountToMint,
                4,
                POOL_HSPOST_A
            );
    ...
    function setUserEMode(uint8 i) external setup {
        ...
        _before();
        (success, returnData) =
            actor.proxy(address(pool), abi.encodeWithSelector(IPool.setUserEMode.selector, eModeCategory));
        ...
        if (success) {
            ...
            if (eModeCategory != snapshotGlobalVarsBefore.usersInfo[address(actor)].userEModeCategory) {
                ...
                for (uint256 j = 0; j < baseAssets.length; j++) {
                    if (
                        snapshotGlobalVarsAfter.usersInfo[address(actor)].userAssetsInfo[baseAssets[j]].vTokenBalance
                            > 0
                    ) {
                        assertTrue(
