...
// Libraries
import "@crane/contracts/protocols/lending/aave/v3.6/protocol/libraries/types/DataTypes.sol";
import {
...
contract BorrowingHandler is BaseHandler {
    ...
    function borrow(uint256 amount, uint8 i, uint8 j) external setup {
        ...
        (success, returnData) = actor.proxy(
            ...
            abi.encodeWithSelector(
                IPool.borrow.selector, asset, amount, DataTypes.InterestRateMode.VARIABLE, 0, onBehalfOf
            )
        );
        ...
        if (success) {
            ...
            // POST-CONDITIONS
            if (eModeCategory != 0) {
                assertTrue(snapshotGlobalVarsBefore.eModesInfo[eModeCategory].isBorrowable[asset], E_MODE_HSPOST_A);
            } else {
            ...
            assertEq(
                ...
                snapshotGlobalVarsBefore.usersInfo[onBehalfOf].userAssetsInfo[asset].underlyingBalance + amount,
                BORROWING_HSPOST_I
            );

            if (address(actor) != onBehalfOf) {
                assertApproxEqAbs(
                    snapshotGlobalVarsAfter.usersInfo[onBehalfOf].userAssetsInfo[asset].borrowAllowances[address(actor)]
                        + amount,
                    snapshotGlobalVarsBefore.usersInfo[onBehalfOf].userAssetsInfo[asset].borrowAllowances[address(
                            actor
                        )],
                    2
                );
    ...
    function repay(uint256 amount, uint8 i, uint8 j) external setup {
        ...
        (success, returnData) = actor.proxy(
            address(pool),
            abi.encodeWithSelector(IPool.repay.selector, asset, amount, DataTypes.InterestRateMode.VARIABLE, onBehalfOf)
        );
    ...
    function repayWithATokens(uint256 amount, uint8 i) external setup {
        ...
        (success, returnData) = actor.proxy(
            address(pool),
            abi.encodeWithSelector(IPool.repayWithATokens.selector, asset, amount, DataTypes.InterestRateMode.VARIABLE)
        );
        ...
        if (success) {
            ...
            assertApproxEqAbs(
                ...
                snapshotGlobalVarsBefore.usersInfo[address(actor)].userAssetsInfo[asset].vTokenBalance + amount,
                2
            );
            assertGe(
                snapshotGlobalVarsAfter.usersInfo[address(actor)].userAssetsInfo[asset].vTokenBalance,
    ...
    function setUserUseReserveAsCollateral(bool useAsCollateral, uint8 i) external setup {
        ...
        _before();
        (success, returnData) = actor.proxy(
            address(pool), abi.encodeWithSelector(IPool.setUserUseReserveAsCollateral.selector, asset, useAsCollateral)
        );
