...
contract FlashLoanHandler is BaseHandler {
    ...
    function flashLoan(uint256[3] memory amounts, bool[3] memory interestRateModes, uint8 i) external setup {
        bool success;
        ...
        bytes memory calldata_ = abi.encodeWithSelector(
            IPool.flashLoan.selector, flashLoanReceiver, assets, amounts, interestRateModesUint, onBehalfOf, "", 0
        );
    ...
    function flashLoanSimple(uint256 amount, uint8 i) external setup {
        ...
        (success, returnData) = actor.proxy(
            address(pool),
            abi.encodeWithSelector(IPool.flashLoanSimple.selector, flashLoanReceiver, asset, amount, "", 0)
        );
