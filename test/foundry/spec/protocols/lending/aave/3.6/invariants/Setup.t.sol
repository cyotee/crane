...
import {Create2Factory, FactoryDeployer} from "@crane/contracts/protocols/lending/aave/v3.6/utils/Create2Factory.sol";

// Contracts
import {Actor} from "@crane/contracts/protocols/lending/aave/v3.6/utils/Actor.sol";
import {BaseTest} from "test/foundry/spec/protocols/lending/aave/3.6/invariants/base/BaseTest.t.sol";
...
contract Setup is BaseTest, DefaultMarketInput {
    ...
    function _deployProtocolCore() internal {
        ...
        MarketReport memory r =
            AaveV3BatchOrchestration.deployAaveV3(poolAdmin, roles, config, flags, deployedContracts);

        AaveV3TestListing testnetListingPayload =
            new AaveV3TestListing(IAaveV3ConfigEngine(r.configEngine), roles.poolAdmin, tokenList.weth, r);
        ...
        DataTypes.ReserveDataLegacy memory wethReserve = contracts.poolProxy.getReserveData(tokenList.weth);
        DataTypes.ReserveDataLegacy memory wbtcReserve = contracts.poolProxy.getReserveData(tokenList.wbtc);
        DataTypes.ReserveDataLegacy memory usdxReserve = contracts.poolProxy.getReserveData(tokenList.usdx);
...

        {
            ...
            priceAggregators =
                [address(mockPriceAggregatorUSDX), address(mockPriceAggregatorWBTC), address(mockPriceAggregatorWETH)];
        ...
        // Set protocol tokens
        protocolTokens[tokenList.weth] =
            ProtocolTokens(wethReserve.aTokenAddress, wethReserve.variableDebtTokenAddress, wethReserve.id);
        protocolTokens[tokenList.wbtc] =
            ProtocolTokens(wbtcReserve.aTokenAddress, wbtcReserve.variableDebtTokenAddress, wbtcReserve.id);
        protocolTokens[tokenList.usdx] =
            ProtocolTokens(usdxReserve.aTokenAddress, usdxReserve.variableDebtTokenAddress, usdxReserve.id);
