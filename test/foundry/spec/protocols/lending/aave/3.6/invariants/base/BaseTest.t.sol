...
import {IERC20Metadata as IERC20} from "@crane/contracts/interfaces/IERC20Metadata.sol";
...
import {PropertiesConstants} from "@crane/contracts/protocols/lending/aave/v3.6/utils/PropertiesConstants.sol";
import {StdAsserts} from "@crane/contracts/protocols/lending/aave/v3.6/utils/StdAsserts.sol";
...
abstract contract BaseTest is BaseStorage, PropertiesConstants, StdAsserts, StdUtils {
    ...
    function _getReserveNormalizedVariableDebt(address underlyingAsset) internal view returns (uint256) {
    ...
    function _getRealTotalSupply(address asset) internal view returns (uint256 totalSupply) {
        uint256 scaledATokenTotalSupply = IAToken(protocolTokens[asset].aTokenAddress).scaledTotalSupply();
    ...
    function _getRealTotalSupply(address asset, uint256 scaledATokenTotalSupply, uint256 accruedToTreasury)
        internal
        view
        returns (uint256)
    {
        return (scaledATokenTotalSupply + accruedToTreasury).rayMulFloor(_getReserveNormalizedIncome(asset));
    }
    ...
    function _getUserScaledDebt(address user, address asset) internal view returns (uint256) {
        return IScaledBalanceToken(protocolTokens[asset].variableDebtTokenAddress).scaledBalanceOf(user);
    }

    function _getUserAtokenScaledBalance(address user, address asset) internal view returns (uint256) {
        return IScaledBalanceToken(protocolTokens[asset].aTokenAddress).scaledBalanceOf(user);
    }

    function _getUserReserveValueInBaseCurrency(address asset, uint256 balance) internal view returns (uint256) {
    ...
    function _getUserBorrowingAssets(address user) internal view returns (address[] memory borrowingAssets) {
        DataTypes.UserConfigurationMap memory userConfig = pool.getUserConfiguration(user);
    ...
    function _getFlags(address asset) internal view returns (Flags memory flags) {
        (bool isActive, bool isFrozen, bool borrowingEnabled, bool isPaused) = pool.getConfiguration(asset).getFlags();
