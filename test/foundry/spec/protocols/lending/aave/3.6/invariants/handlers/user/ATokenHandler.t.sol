...
contract ATokenHandler is BaseHandler {
    ...
    function transfer(uint256 amount, uint8 i, uint8 j) external setup {
        ...
        _before();
        (success, returnData) = actor.proxy(aToken, abi.encodeWithSelector(IERC20.transfer.selector, recipient, amount));

        if (success) {
            _after();

            if ((amount != 0 && address(actor) != recipient) && _isUsingAsCollateral(underlyingToken, address(actor))) {
            ...
            assertApproxEqAbs(
                snapshotGlobalVarsAfter.usersInfo[address(actor)].userAssetsInfo[underlyingToken].aTokenBalance
                    + amount,
                snapshotGlobalVarsBefore.usersInfo[address(actor)].userAssetsInfo[underlyingToken].aTokenBalance,
                2,
                ERC20_HSPOST_E
            );

            assertApproxEqAbs(
                snapshotGlobalVarsBefore.usersInfo[recipient].userAssetsInfo[underlyingToken].aTokenBalance + amount,
    ...
    function transferFrom(uint256 amount, uint8 i, uint8 j, uint256 u) external setup {
        ...
        _before();
        (success, returnData) =
            actor.proxy(aToken, abi.encodeWithSelector(IERC20.transferFrom.selector, owner, recipient, amount));

        if (success) {
            _after();

            if (
                amount != 0 && owner != recipient
            ...
            if (
                snapshotGlobalVarsBefore.usersInfo[owner].userAssetsInfo[underlyingToken].underlyingAllowances[
                        address(actor)
                    ] != type(uint256).max
            ) {
                assertApproxEqAbs(
                    snapshotGlobalVarsAfter.usersInfo[owner].userAssetsInfo[underlyingToken].underlyingAllowances[address(
                                actor
                            )] + amount,
                    snapshotGlobalVarsBefore.usersInfo[owner].userAssetsInfo[underlyingToken].underlyingAllowances[address(
                            actor
                        )],
                    2,
                    ERC20_HSPOST_E
            ...
            assertApproxEqAbs(
                snapshotGlobalVarsAfter.usersInfo[owner].userAssetsInfo[underlyingToken].aTokenBalance + amount,
