...
contract LiquidationHandler is BaseHandler {
    ...
    function liquidationCall(uint256 debtToCover, bool receiveAToken, uint8 i, uint8 j, uint8 k) external setup {
        bool success;
        ...
        _before();
        {
            bytes memory callData = abi.encodeWithSelector(
                IPool.liquidationCall.selector, collateralAsset, debtAsset, borrower, debtToCover, receiveAToken
            );
            (success, returnData) = actor.proxy(address(pool), callData);
        }

        if (
            snapshotGlobalVarsBefore.assetsInfo[collateralAsset].liquidationGracePeriodUntil >= block.timestamp
        ...
        if (success) {
            ...
            if (additionalDeficit > 0) {
                assertEq(snapshotGlobalVarsAfter.usersInfo[borrower].totalCollateralBase, 0, LIQUIDATION_HSPOST_L);
                assertEq(
            ...
            uint256 liquidatedDebtAmount = snapshotGlobalVarsBefore.usersInfo[borrower].userAssetsInfo[debtAsset].vTokenBalance
                - snapshotGlobalVarsAfter.usersInfo[borrower].userAssetsInfo[debtAsset].vTokenBalance;

            uint256 liquidatedCollateralAmount = snapshotGlobalVarsBefore.usersInfo[borrower].userAssetsInfo[collateralAsset].aTokenBalance
                - snapshotGlobalVarsAfter.usersInfo[borrower].userAssetsInfo[collateralAsset].aTokenBalance;
            uint256 protocolFeeAmount = snapshotGlobalVarsAfter.usersInfo[address(
                        contracts.treasury
                    )].userAssetsInfo[collateralAsset].aTokenBalance
                - snapshotGlobalVarsBefore.usersInfo[address(
                        contracts.treasury
                    )].userAssetsInfo[collateralAsset].aTokenBalance;
            ...
            if (receiveAToken) {
                assertApproxEqAbs(
                    snapshotGlobalVarsAfter.usersInfo[address(actor)].userAssetsInfo[collateralAsset].aTokenBalance,
                    snapshotGlobalVarsBefore.usersInfo[address(actor)].userAssetsInfo[collateralAsset].aTokenBalance
                ...
                assertApproxEqAbs(
                    snapshotGlobalVarsAfter.usersInfo[address(actor)].userAssetsInfo[collateralAsset].underlyingBalance,
                    snapshotGlobalVarsBefore.usersInfo[address(actor)].userAssetsInfo[collateralAsset].underlyingBalance
                        + liquidatedCollateralAmount,
                    8,
                    LIQUIDATION_HSPOST_P
                );
            ...
            assertApproxEqAbs(
                snapshotGlobalVarsBefore.usersInfo[address(actor)].userAssetsInfo[debtAsset].underlyingBalance,
                snapshotGlobalVarsAfter.usersInfo[address(actor)].userAssetsInfo[debtAsset].underlyingBalance
            ...
            assertApproxEqAbs(
                ...
                snapshotGlobalVarsBefore.assetsInfo[debtAsset].virtualUnderlyingBalance + liquidatedDebtAmount,
                4,
                LIQUIDATION_HSPOST_R
            );
            assertApproxEqAbs(
                snapshotGlobalVarsBefore.assetsInfo[collateralAsset].virtualUnderlyingBalance,
                snapshotGlobalVarsAfter.assetsInfo[collateralAsset].virtualUnderlyingBalance + liquidatedDebtAmount,
                4,
