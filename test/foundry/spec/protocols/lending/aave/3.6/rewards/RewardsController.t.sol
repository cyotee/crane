...
import {
    RewardsController,
    IRewardsController
} from "@crane/contracts/protocols/lending/aave/v3.6/rewards/RewardsController.sol";
import {
    RewardsDistributor,
    IRewardsDistributor
} from "@crane/contracts/protocols/lending/aave/v3.6/rewards/RewardsDistributor.sol";
...
} from "@crane/contracts/protocols/lending/aave/v3.6/rewards/interfaces/ITransferStrategyBase.sol";
...
} from "@crane/contracts/protocols/lending/aave/v3.6/rewards/transfer-strategies/PullRewardsTransferStrategy.sol";
...
contract RewardsControllerTest is TestnetProcedures {
    ...
    function test_setTransferStrategy_PullRewardsTransferStrategy() public {
        ...
        ITransferStrategyBase transferStrategy =
            ITransferStrategyBase(new PullRewardsTransferStrategy(report.rewardsControllerProxy, alice, carol));
    ...
    function test_setDistributionEnd() public {
        ...
        (uint256 index, uint256 emissionPerSecond,, uint256 distributionEnd) =
            rewardsController.getRewardsData(usdxAToken, tokenList.usdx);

        vm.expectEmit(address(rewardsController));
        emit IRewardsDistributor.AssetConfigUpdated(
            usdxAToken, tokenList.usdx, emissionPerSecond, emissionPerSecond, distributionEnd, 10, index
        );
    ...
    function test_setEmissionPerSecond() public {
        ...
        vm.expectEmit(address(rewardsController));
        emit IRewardsDistributor.AssetConfigUpdated(
            usdxAToken, tokenList.usdx, emissionPerSecond, emissions[0], distributionEnd, distributionEnd, index
        );
    ...
    function test_configureAssets() public {
        PullRewardsTransferStrategy strat = new PullRewardsTransferStrategy(report.rewardsControllerProxy, alice, carol);
        ...
        vm.expectEmit(address(rewardsController));
        emit IRewardsDistributor.AssetConfigUpdated(
            usdxAToken, tokenList.usdx, 0, config[0].emissionPerSecond, 0, config[0].distributionEnd, 0
        );
    ...
    function test_accrueRewards() public {
        ...
        (uint256 newAssetIndex, uint256 expectedRewardsAccrued) =
            _calculateRewardsAccrued(usdxAToken, tokenList.usdx, alice);
        ...
        emit IRewardsDistributor.Accrued(
            usdxAToken, tokenList.usdx, alice, newAssetIndex, newAssetIndex, expectedRewardsAccrued
        );
    ...
    function _calculateRewardsAccrued(address asset, address reward, address user)
        internal
        view
        returns (uint256 index, uint256 rewards)
    {
        uint256 userIndex = rewardsController.getUserAssetIndex(user, asset, reward);
        RewardsData memory rewardData;
        (rewardData.index, rewardData.emissionPerSecond, rewardData.lastUpdateTimestamp, rewardData.distributionEnd) =
            rewardsController.getRewardsData(asset, reward);

        (, uint256 newAssetIndex) = _getAssetIndex(
            rewardData, IAToken(asset).scaledTotalSupply(), 10 ** rewardsController.getAssetDecimals(asset)
        );
    ...
    function _getAssetIndex(RewardsData memory rewardData, uint256 totalSupply, uint256 assetUnit)
        internal
        view
        returns (uint256, uint256)
    {
        ...
        if (
            emissionPerSecond == 0 || totalSupply == 0 || lastUpdateTimestamp == vm.getBlockTimestamp()
                || lastUpdateTimestamp >= distributionEnd
        ) {
            return (oldIndex, oldIndex);
        }

        uint256 currentTimestamp = vm.getBlockTimestamp() > distributionEnd ? distributionEnd : vm.getBlockTimestamp();
