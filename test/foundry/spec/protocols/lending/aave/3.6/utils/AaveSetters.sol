...
library AaveSetters {
    ...
    function setLastUpdateTimestamp(address pool, address reserve, uint40 timestamp) internal {
        ...
        uint256 currentValue = uint256(vm.load(pool, bytes32(reserveSlot)));
        bytes32 newPackedValue =
            bytes32((currentValue & (~(uint256(type(uint40).max) << 128))) | (uint256(timestamp) << 128));
        vm.store(pool, bytes32(reserveSlot), newPackedValue);
    }
    ...
    function setVariableDebtTokenBalance(address token, address user, uint256 balance, uint256 index) internal {
