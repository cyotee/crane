...
} from "@crane/contracts/protocols/lending/aave/v3.6/interfaces/IDefaultInterestRateStrategyV2.sol";
import {
...
contract ProtocolV3TestBase is Test {
    ...
    function createConfigurationSnapshot(
        ...
        vm.writeFile(path, '{ "eModes": {}, "reserves": {}, "strategies": {}, "poolConfiguration": {}, "raw": {} }');
    ...
    function _writeStrategyConfigs(string memory path, ReserveConfig[] memory configs) internal virtual {
        ...
        for (uint256 i = 0; i < configs.length; i++) {
            ...
            IDefaultInterestRateStrategyV2 strategy = IDefaultInterestRateStrategyV2(configs[i].interestRateStrategy);
            vm.serializeString(key, "baseVariableBorrowRate", vm.toString(strategy.getBaseVariableBorrowRate(asset)));
            vm.serializeString(key, "variableRateSlope1", vm.toString(strategy.getVariableRateSlope1(asset)));
            vm.serializeString(key, "variableRateSlope2", vm.toString(strategy.getVariableRateSlope2(asset)));
            vm.serializeString(key, "maxVariableBorrowRate", vm.toString(strategy.getMaxVariableBorrowRate(asset)));
            string memory object =
                vm.serializeString(key, "optimalUsageRatio", vm.toString(strategy.getOptimalUsageRatio(asset)));
    ...
    function _writeReserveConfigs(string memory path, ReserveConfig[] memory configs, IPool pool) internal virtual {
        // keys for json stringification
        ...
        for (uint256 i = 0; i < configs.length; i++) {
            ...
            ExtendedAggregatorV2V3Interface assetOracle =
                ExtendedAggregatorV2V3Interface(oracle.getSourceOfAsset(config.underlying));
            ...
            vm.serializeString(key, "variableDebtTokenSymbol", IERC20Metadata(config.variableDebtToken).symbol());
            vm.serializeString(key, "variableDebtTokenName", IERC20Metadata(config.variableDebtToken).name());
            ...
            vm.serializeString(key, "aTokenUnderlyingBalance", vm.toString(config.aTokenUnderlyingBalance));

            string memory out = vm.serializeString(
                key, "oracleLatestAnswer", vm.toString(uint256(oracle.getAssetPrice(config.underlying)))
            );
    ...
    function _writePoolConfiguration(string memory path, IPool pool) internal virtual {
        ...
        vm.serializeAddress(poolConfigKey, "priceOracleSentinel", addressesProvider.getPriceOracleSentinel());
    ...
    function _getStructReserveConfig(IPool pool, address reserve) internal view virtual returns (ReserveConfig memory) {
        ...
        (localConfig.isActive, localConfig.isFrozen, localConfig.borrowingEnabled, localConfig.isPaused) =
    ...
    function _findReserveConfigBySymbol(ReserveConfig[] memory configs, string memory symbolOfUnderlying)
        internal
        pure
        virtual
        returns (ReserveConfig memory)
        ...
        pure
    {
        ...
        require(config.underlying == expectedConfig.underlying, "_validateReserveConfig() : INVALID_UNDERLYING");
        require(config.decimals == expectedConfig.decimals, "_validateReserveConfig: InvalidDecimals()");
        ...
        require(config.reserveFactor == expectedConfig.reserveFactor, "_validateReserveConfig: InvalidReserveFactor()");
        ...
        require(config.isActive == expectedConfig.isActive, "_validateReserveConfig: INVALID_IS_ACTIVE");
        require(config.isFrozen == expectedConfig.isFrozen, "_validateReserveConfig: INVALID_IS_FROZEN");
        require(config.isSiloed == expectedConfig.isSiloed, "_validateReserveConfig: INVALID_IS_SILOED");
        require(
        ...
        require(
            config.isFlashloanable == expectedConfig.isFlashloanable, "_validateReserveConfig: INVALID_IS_FLASHLOANABLE"
        );
        require(config.supplyCap == expectedConfig.supplyCap, "_validateReserveConfig: InvalidSupplyCap()");
        require(config.borrowCap == expectedConfig.borrowCap, "_validateReserveConfig: InvalidBorrowCap()");
        require(config.debtCeiling == expectedConfig.debtCeiling, "_validateReserveConfig: InvalidDebtCeiling()");
        require(
    ...
    function _validateInterestRateStrategy(
        ...
        IDefaultInterestRateStrategyV2 strategy = IDefaultInterestRateStrategyV2(interestRateStrategyAddress);
    ...
    function _requireNoChangeInConfigs(ReserveConfig memory config1, ReserveConfig memory config2) internal pure {
        require(
        ...
        require(
            config1.aToken == config2.aToken, "_noReservesConfigsChangesApartNewListings() : UNEXPECTED_A_TOKEN_CHANGED"
        );
        require(
        ...
        require(config1.ltv == config2.ltv, "_noReservesConfigsChangesApartNewListings() : UNEXPECTED_LTV_CHANGED");
        require(
    ...
    function _validateReserveTokensImpls(ReserveConfig memory config, ReserveTokens memory expectedImpls) internal {
        require(
        ...
        require(
            ProxyHelpers.getInitializableAdminUpgradeabilityProxyImplementation(vm, config.variableDebtToken)
    ...
    function _validateAssetSourceOnOracle(
        ...
        require(
            oracle.getSourceOfAsset(asset) == expectedSource, "_validateAssetSourceOnOracle() : INVALID_PRICE_SOURCE"
        );
    }
    ...
    function _validateEmodeCategory(
        ...
        DataTypes.CollateralConfig memory cfg = IPool(poolAddress)
        ...
        require(cfg.liquidationBonus == expectedCategoryData.liquidationBonus, "_validateEmodeCategory(): INVALID_LB");
        require(
            // forge-lint: disable-next-line(unsafe-typecast)
