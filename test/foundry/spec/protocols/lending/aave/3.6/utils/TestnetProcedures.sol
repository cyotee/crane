...
import {Multicall} from "@crane/contracts/external/openzeppelin-contracts/utils/Multicall.sol";
...
import {ACLManager, Errors} from "@crane/contracts/protocols/lending/aave/v3.6/protocol/configuration/ACLManager.sol";
...
import {
    AaveV3ConfigEngine,
    IAaveV3ConfigEngine
} from "@crane/contracts/protocols/lending/aave/v3.6/extensions/v3-config-engine/AaveV3ConfigEngine.sol";
...
contract TestnetProcedures is Test, DeployUtils, FfiUtils, DefaultMarketInput {
    ...
    function _initTestEnvironment(bool mintUserTokens, bool l2) internal {
        ...
        (report, tokenList) = deployAaveV3TestnetAssets(poolAdmin, roles, config, flags, deployedContracts);
    ...
    function deployAaveV3TestnetAssets(
        ...
        AaveV3TestListing testnetListingPayload =
            new AaveV3TestListing(IAaveV3ConfigEngine(r.configEngine), roles.poolAdmin, assetsList.weth, r);
    ...
    function _getReserveConfig(address reserve, address dataProvider)
        internal
        view
        returns (TestReserveConfig memory c)
    {
        (
            uint256 decimals,
            uint256 ltv,
    ...
    function _generateInitConfig(TestVars memory t, MarketReport memory r, address poolAdminUser, bool isValidDecimals)
        internal
    ...
    function _generateInitConfig(
        ...
        ConfiguratorInputTypes.InitReserveInput[] memory configurations =
            new ConfiguratorInputTypes.InitReserveInput[](t.length);
    ...
    function _generateInitReserveInput(
        ...
        input.underlyingAsset = address(new TestnetERC20("Misc Token", "MISC", t.underlyingDecimals, poolAdminUser));
    ...
    function _calculateInterestRates(uint256 borrowAmount, address token) internal view returns (uint256) {
        ...
        DefaultReserveInterestRateStrategyV2 rateStrategy =
            DefaultReserveInterestRateStrategyV2(reserveData.interestRateStrategyAddress);
    ...
    function _deployInterestRateStrategy() internal returns (address) {
        DefaultReserveInterestRateStrategyV2 strategy =
    ...
    function _checkInterestRates(address asset) internal view {
        ...
        (uint256 nextLiquidityRate, uint256 nextVariableRate) = contracts.defaultInterestRateStrategy
