...
contract PoolOperationsComposition_gas_Tests is Testhelpers {
    ...
    function test_supplyAndBorrow() external {
        ...
        vm.snapshotGasLastCall(
            "Pool.OperationsComposition", "supplyAndBorrow: first supply->collateralEnabled, first borrow"
        );
    }
    ...
    function test_repayAndWithdraw() external {
        ...
        bytes[] memory calls = new bytes[](2);
        calls[0] = abi.encodeWithSelector(IPool.repay.selector, tokenList.usdx, type(uint256).max, 2, supplier);
        calls[1] = abi.encodeWithSelector(IPool.withdraw.selector, tokenList.weth, type(uint256).max, supplier);
        Multicall(address(contracts.poolProxy)).multicall(calls);
        vm.snapshotGasLastCall("Pool.OperationsComposition", "repayAndWithdraw: borrow disabled, collateral disabled");
    }
    ...
    function test_batchLiquidation() external {
        ...
        calls[0] = abi.encodeWithSelector(
            IPool.liquidationCall.selector, tokenList.usdx, tokenList.weth, borrower, type(uint256).max, false
        );
        calls[1] = abi.encodeWithSelector(
            IPool.liquidationCall.selector, tokenList.usdx, tokenList.weth, supplier, type(uint256).max, false
        );
        Multicall(address(contracts.poolProxy)).multicall(calls);
