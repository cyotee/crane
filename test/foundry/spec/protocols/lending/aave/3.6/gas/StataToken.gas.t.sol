...
contract StataToken_gas_Tests is Testhelpers {
    ...
    function setUp() public override {
        super.setUp();
        StataTokenFactory(report.staticATokenFactoryProxy).createStataTokens(contracts.poolProxy.getReservesList());
        stataToken = StataTokenV2(StataTokenFactory(report.staticATokenFactoryProxy).getStataToken(tokenList.usdx));
        ...
        strategy = new PullRewardsTransferStrategy(report.rewardsControllerProxy, emissionAdmin, emissionAdmin);
    ...
    function test_claimRewards() external {
        ...
        _setupEmission(rewardTokenOne, reserveData.aTokenAddress, endTimestamp, 1e14, emissionAdmin, address(strategy));
        _setupEmission(rewardTokenTwo, reserveData.aTokenAddress, endTimestamp, 1e14, emissionAdmin, address(strategy));
