...
contract AaveV3BatchTests is BatchTestProcedures {
    ...
    function setUp() public {
        ...
        (marketReportOne, gettersReportOne, poolReportOne, peripheryReportOne, miscReport, aaveV3SetupOne) =
            deployCoreAndPeriphery(roles, config, flags, deployedContracts);

        BatchTestProcedures.DeployAndSetupVariables memory deployAndSetupVariables =
    ...
    function test2AaveV3PoolDeployment() public {
        new AaveV3PoolBatch(marketReportOne.poolAddressesProvider, marketReportOne.interestRateStrategy);
    }

    function test3AaveV3L2PoolDeployment() public {
        new AaveV3L2PoolBatch(marketReportOne.poolAddressesProvider, marketReportOne.interestRateStrategy);
    }

    function test4PeripheralsRelease() public {
        new AaveV3PeripheryBatch(
            roles.poolAdmin, config, marketReportOne.poolAddressesProvider, address(aaveV3SetupOne)
        );
    }
    ...
    function test8TokensMarket() public {
        new AaveV3TokensBatch(
            setupReportTwo.poolProxy, setupReportTwo.rewardsControllerProxy, peripheryReportOne.treasury
        );
    }
    ...
    function test10StaticATokenDeployment() public {
        new AaveV3HelpersBatchTwo(setupReportTwo.poolProxy, setupReportTwo.rewardsControllerProxy, roles.poolAdmin);
    }
}
