...
contract AaveV3BatchDeployment is BatchTestProcedures {
    ...
    function testAaveV3BatchDeploymentCheck() public {
        MarketReport memory fullReport = deployAaveV3Testnet(marketOwner, roles, config, flags, deployedContracts);
        checkFullReport(config, flags, fullReport);

        AaveV3TestListing testnetListingPayload =
            new AaveV3TestListing(IAaveV3ConfigEngine(fullReport.configEngine), marketOwner, weth9, fullReport);
    ...
    function testAaveV3BatchDeploy() public {
        checkFullReport(config, flags, deployAaveV3Testnet(marketOwner, roles, config, flags, deployedContracts));
    }
