...
contract AaveV3PermissionsTest is BatchTestProcedures {
    ...
    function testCheckPermissions() public {
        ...
        MarketReport memory report = deployAaveV3Testnet(deployer, roles, config, flags, deployedContracts);

        ACLManager aclManager = ACLManager(IPoolAddressesProvider(report.poolAddressesProvider).getACLManager());
        {
            address providerOwner = Ownable(report.poolAddressesProvider).owner();
            assertEq(providerOwner, roles.marketOwner, "PoolAddressesProvider owner must be roles.marketOwner");
        }
        {
        ...
        }
        {
            ...
            assertEq(providerAclAdmin, roles.poolAdmin, "PoolAddressesProvider.getACLAdmin() must be pool admin");
        }
        {
        ...
        }
        {
            ...
            assertEq(wethGatewayOwner, roles.poolAdmin, "roles.poolAdmin must be WrappedTokenGateway owner");
        }
        {
            address rewardsControllerAdmin = RewardsController(report.rewardsControllerProxy).EMISSION_MANAGER();
            assertEq(
        ...
        }
        {
            ...
            assertEq(emissionManagerOwner, roles.poolAdmin, "EmissionManager owner does not match with roles.poolAdmin");
        }
        {
            address proxyAdminAddr = address(uint160(uint256(vm.load(report.treasury, ADMIN_SLOT))));
            address owner;
            if (proxyAdminAddr.code.length > 0) {
                owner = Ownable(proxyAdminAddr).owner();
            } else {
                owner = proxyAdminAddr;
            }
            assertEq(owner, roles.poolAdmin, "Treasury proxy admin does not match with report.proxyAdmin");
        }
    }
}
