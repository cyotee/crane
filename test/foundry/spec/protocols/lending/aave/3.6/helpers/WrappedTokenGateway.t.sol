...
import {AaveOracle} from "@crane/contracts/protocols/lending/aave/v3.6/misc/AaveOracle.sol";
import {WrappedTokenGatewayV3} from "@crane/contracts/protocols/lending/aave/v3.6/helpers/WrappedTokenGatewayV3.sol";
import {
...
import {
    VariableDebtToken
} from "@crane/contracts/protocols/lending/aave/v3.6/protocol/tokenization/VariableDebtToken.sol";
...
contract WrappedTokenGatewayTests is TestnetProcedures {
    ...
    function setUp() public {
        ...
        assertTrue(report.wrappedTokenGateway != address(0), "WrappedTokenGateway missing at deployment");
        ...
        (address aWEthAddr,, address wEthVariableDebt) =
            contracts.protocolDataProvider.getReserveTokensAddresses(tokenList.weth);
        aWEth = AToken(aWEthAddr);
        wEthVariableDebtToken = VariableDebtToken(wEthVariableDebt);
        (address aUsdxAddr,, address usdxVariableDebt) =
    ...
    function test_depositNativeEthInPool() public {
        ...
        assertEq(aWEth.balanceOf(alice), depositSize, "The a token balance should match the deposit size");
    }
    ...
    function test_withdrawEth_partial() public {
        ...
        vm.expectEmit();
        emit IPool.Withdraw(tokenList.weth, report.wrappedTokenGateway, report.wrappedTokenGateway, partialWithdraw);
        ...
        assertEq(aWEth.balanceOf(alice), partialWithdraw, "The user aToken balance should decrease by half");
    }
    ...
    function test_withdrawEth_permit() public {
        ...
        bytes32 digest = EIP712SigUtils.getTypedDataHash(permit, bytes(aWEth.name()), bytes("1"), address(aWEth));
        ...
        vm.expectEmit();
        emit IPool.Withdraw(tokenList.weth, report.wrappedTokenGateway, report.wrappedTokenGateway, withdrawAmount);

        vm.prank(alice);
        wrappedTokenGatewayV3.withdrawETHWithPermit(report.poolProxy, withdrawAmount, alice, permit.deadline, v, r, s);
    ...
    function test_withdrawEth_permit_full() public {
        ...
        vm.expectEmit();
        emit IPool.Withdraw(tokenList.weth, report.wrappedTokenGateway, report.wrappedTokenGateway, userATokenBalance);
    ...
    function test_withdrawEth_full() public {
        ...
        uint256 userEthBalanceBefore = alice.balance;
        vm.expectEmit();
        emit IPool.Withdraw(tokenList.weth, report.wrappedTokenGateway, report.wrappedTokenGateway, fullWithdraw);
    ...
    function test_borrowVariableDebtWeth_repayWithEth() public {
        ...
        assertEq(aUsdx.balanceOf(bob), usdxSize, "The deposited balance should match the user AToken balance");
        ...
        // Partial repayment
        wrappedTokenGatewayV3.repayETH{value: partialRepayment}(report.poolProxy, partialRepayment, bob);
        ...
        emit IPool.Repay(tokenList.weth, bob, address(wrappedTokenGatewayV3), partialRepayment, false);
        // Full repayment
        wrappedTokenGatewayV3.repayETH{value: partialRepayment}(report.poolProxy, type(uint256).max, bob);
    ...
    function test_borrowVariableDebtWeth_repayWithEth_mismatchedValues() public {
        ...
        wrappedTokenGatewayV3.repayETH{value: partialRepayment + 1}(report.poolProxy, partialRepayment, bob);
    ...
    function test_ownerCanRescueTokens() public {
        ...
        assertEq(usdx.balanceOf(alice), 0, "The user should have lost the funds due to the improper transfer");
    ...
    function test_ownerCanRescueEth() public {
        ...
        assertEq(alice.balance, balanceBefore + lostAmount, "The user should have recovered the lost amount");
        assertEq(
