...
import {
    ...
    IERC20AaveLM
} from "@crane/contracts/protocols/lending/aave/v3.6/extensions/stata-token/ERC20AaveLMUpgradeable.sol";
import {
    IRewardsController
...
contract ERC20AaveLMUpgradableTest is TestnetProcedures {
    ...
    function test_noRewardsInitialized() external {
        vm.expectRevert(abi.encodeWithSelector(IERC20AaveLM.RewardNotInitialized.selector, rewardToken));
    ...
    function test_claimableRewards(
        ...
        TestEnv memory env = _setupTestEnvironment(depositAmount, emissionEnd, emissionPerSecond, waitDuration);
    ...
    function test_claimRewards(uint256 depositAmount, uint32 emissionEnd, uint88 emissionPerSecond, uint32 waitDuration)
        public
    {
        _setupTestEnvironment(depositAmount, emissionEnd, emissionPerSecond, waitDuration);
        ...
        if (sendAmount > env.depositAmount) {
            vm.expectRevert(
                abi.encodeWithSelector(
                    IERC20Errors.ERC20InsufficientBalance.selector, user, env.depositAmount, sendAmount
                )
            );
            vm.prank(user);
            // forge-lint: disable-next-line(erc20-unchecked-transfer)
