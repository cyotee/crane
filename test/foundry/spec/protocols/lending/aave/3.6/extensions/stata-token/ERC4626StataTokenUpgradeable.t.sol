...
import {
    ...
    IERC4626StataToken
} from "@crane/contracts/protocols/lending/aave/v3.6/extensions/stata-token/ERC4626StataTokenUpgradeable.sol";
import {
    DataTypes
...
contract ERC4626StataTokenUpgradeableTest is TestnetProcedures {
    ...
    // ### DEPOSIT TESTS ###
    function test_depositATokens(uint128 unboundedUnderlyingBalance, uint256 unboundedAmountToDeposit, address receiver)
        public
    {
    ...
    function test_depositWithPermit_shouldRevert_emptyPermit_noPreApproval(uint128 unboundedAssets) external {
        TestEnv memory env = _setupTestEnv(unboundedAssets);
        _fundAToken(env.underlyingBalance, user);
        IERC4626StataToken.SignatureParams memory sig;
        vm.expectRevert(); // will underflow
        vm.prank(user);
        erc4626Upgradeable.depositWithPermit(env.underlyingBalance, user, vm.getBlockTimestamp() + 1000, sig, false);
    }
    ...
    function test_depositWithPermit_emptyPermit_underlying_preApproval(
        ...
        uint256 shares = erc4626Upgradeable.depositWithPermit(
            env.amountToDeposit, receiver, vm.getBlockTimestamp() + 1000, sig, true
        );
    ...
    function test_depositWithPermit_emptyPermit_aToken_preApproval(
        ...
        vm.prank(user);
        uint256 shares = erc4626Upgradeable.depositWithPermit(
            env.amountToDeposit, receiver, vm.getBlockTimestamp() + 1000, sig, false
        );
    ...
    function test_depositWithPermit_underlying(
        ...
        bytes32 permitDigest =
            SigUtils.getTypedDataHash(permit, SigUtils.PERMIT_TYPEHASH, IERC20Permit(underlying).DOMAIN_SEPARATOR());
        ...
        vm.prank(user);
        uint256 shares = erc4626Upgradeable.depositWithPermit(env.amountToDeposit, receiver, permit.deadline, sig, true);
    ...
    function test_depositWithPermit_aToken(uint128 unboundedAssets, uint256 unboundedAmountToDeposit, address receiver)
        external
    {
        _validateReceiver(receiver);
        TestEnv memory env = _setupTestEnv(unboundedAssets, unboundedAmountToDeposit);
        ...
        bytes32 permitDigest =
            SigUtils.getTypedDataHash(permit, SigUtils.PERMIT_TYPEHASH, IERC20Permit(aToken).DOMAIN_SEPARATOR());
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(userPrivateKey, permitDigest);
        IERC4626StataToken.SignatureParams memory sig = IERC4626StataToken.SignatureParams(v, r, s);
        vm.prank(user);
        uint256 shares =
            erc4626Upgradeable.depositWithPermit(env.amountToDeposit, receiver, permit.deadline, sig, false);
    ...
    function test_redeemATokens_onBehalf_shouldRevert_insufficientAllowance(uint256 assets, uint256 allowance)
        external
    {
        TestEnv memory env = _setupTestEnv(assets);
        uint256 shares = _fund4626(env.underlyingBalance, user);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IERC20Errors.ERC20InsufficientAllowance.selector, address(this), allowance, env.underlyingBalance
            )
        );
    ...
    function _setupTestEnv(uint256 underlyingBalance, uint256 amountToDeposit) internal pure returns (TestEnv memory) {
        TestEnv memory env;
        ...
        env.actualAmountToDeposit =
            env.amountToDeposit > env.underlyingBalance ? env.underlyingBalance : env.amountToDeposit;
        return env;
    }
