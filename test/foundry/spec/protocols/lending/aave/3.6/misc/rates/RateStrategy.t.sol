...
contract RateStrategyBaseTests is RateStrategyBase {
    ...
    //                                         Test Getters
    //----------------------------------------------------------------------------------------------------
    function test_getInterestRateDataRay(IDefaultInterestRateStrategyV2.InterestRateData memory rateDataToSet)
        public
        setRateParams(rateDataToSet, tokenList.usdx)
    {
        IDefaultInterestRateStrategyV2.InterestRateDataRay memory rateData =
            rateStrategy.getInterestRateData(tokenList.usdx);
    ...
    function test_getInterestRateDataBps(IDefaultInterestRateStrategyV2.InterestRateData memory rateDataToSet)
        public
        setRateParams(rateDataToSet, tokenList.usdx)
    {
        IDefaultInterestRateStrategyV2.InterestRateData memory rateData =
            rateStrategy.getInterestRateDataBps(tokenList.usdx);
    ...
    function test_getMaxVariableBorrowRate(IDefaultInterestRateStrategyV2.InterestRateData memory rateDataToSet)
        public
        setRateParams(rateDataToSet, tokenList.usdx)
    {
        ...
        assertEq(
            uint256(
                rateDataToSet.baseVariableBorrowRate + rateDataToSet.variableRateSlope1
    ...
    function test_overflow_liquidity_rates() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(SafeCast.SafeCastOverflowedUintDowncast.selector, 128, UINT256_MAX));
        vm.prank(alice);
    ...
    function test_new_SetReserveInterestRateParams_override_method(
        ...
        assertEq(rateStrategy.getOptimalUsageRatio(tokenList.usdx), uint256(rateDataToSet.optimalUsageRatio) * 1e23);
        assertEq(rateStrategy.getVariableRateSlope1(tokenList.usdx), uint256(rateDataToSet.variableRateSlope1) * 1e23);
        assertEq(rateStrategy.getVariableRateSlope2(tokenList.usdx), uint256(rateDataToSet.variableRateSlope2) * 1e23);
        assertEq(
            rateStrategy.getBaseVariableBorrowRate(tokenList.usdx), uint256(rateDataToSet.baseVariableBorrowRate) * 1e23
        );
        assertEq(
    ...
    function test_new_SetReserveInterestRateParams(IDefaultInterestRateStrategyV2.InterestRateData memory rateDataToSet)
        public
    {
        _validateSetRateParams(rateDataToSet);
    ...
    function test_reverts_SetReserveInterestRateParams_when_gt_maxRate(
        ...
        vm.assume(
            uint256(rateDataToSet.baseVariableBorrowRate) + uint256(rateDataToSet.variableRateSlope1)
                    + uint256(rateDataToSet.variableRateSlope2) > rateStrategy.MAX_BORROW_RATE()
        );
