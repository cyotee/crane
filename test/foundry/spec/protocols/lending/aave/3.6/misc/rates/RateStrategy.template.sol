...
contract RateStrategyBase is TestnetProcedures {
    ...
    modifier setRateParams(IDefaultInterestRateStrategyV2.InterestRateData memory rateData, address token) {
        _setRateParams(rateData, token);
        _;
    }

    function _validateSetRateParams(IDefaultInterestRateStrategyV2.InterestRateData memory rateData) internal view {
    ...
    function _setRateParams(IDefaultInterestRateStrategyV2.InterestRateData memory rateData, address token) internal {
    ...
    function setUp() public {
        ...
        rateStrategy = DefaultReserveInterestRateStrategyV2(address(contracts.defaultInterestRateStrategy));
        (,,,, reserveFactor,,,,,) = contracts.protocolDataProvider.getReserveConfigurationData(tokenList.usdx);

        vm.startPrank(poolAdmin);
        IPoolConfigurator(report.poolConfiguratorProxy)
