...
contract RateStrategyCalculateRatesTests is RateStrategyBase {
    ...
    {
        DataTypes.CalculateInterestRatesParams memory input =
        ...
        (params.currentLiquidityRate, params.currentVariableBorrowRate) = rateStrategy.calculateInterestRates(input);

        uint256 expectedVariableRate =
            rateStrategy.getBaseVariableBorrowRate(tokenList.usdx) + rateStrategy.getVariableRateSlope1(tokenList.usdx);
        ...
        )
    {
        ...
        (uint256 currentLiquidityRate, uint256 currentVariableBorrowRate) = rateStrategy.calculateInterestRates(input);

        uint256 expectedVariableRate = rateStrategy.getBaseVariableBorrowRate(tokenList.usdx)
            + rateStrategy.getVariableRateSlope1(tokenList.usdx) + rateStrategy.getVariableRateSlope2(tokenList.usdx);
        ...
        )
    {
        ...
        uint256 expectedVariableRate = rateStrategy.getBaseVariableBorrowRate(tokenList.usdx)
            + rateStrategy.getVariableRateSlope1(tokenList.usdx)
                .rayMul(usageRatio.rayDiv(rateStrategy.getOptimalUsageRatio(tokenList.usdx)));
        assertEq(
            currentLiquidityRate,
            expectedVariableRate.percentMul(80).percentMul(100_00 - input.reserveFactor),
            "Invalid liquidity rate"
    ...
    function test_fuzz_calculate_rates_80_percent_usage_added_and_virtual_equal(uint256 virtualBalanceAmount)
        public
        view
    {
        ...
        (uint256 currentLiquidityRateOne, uint256 currentVariableBorrowRateOne) =
        ...
        (uint256 currentLiquidityRateTwo, uint256 currentVariableBorrowRateTwo) =
            rateStrategy.calculateInterestRates(input);
    ...
    //----------------------------------------------------------------------------------------------------
    function test_calculate_rates_empty_reserve(IDefaultInterestRateStrategyV2.InterestRateData memory rateData)
        public
        setRateParams(rateData, tokenList.usdx)
    {
        DataTypes.CalculateInterestRatesParams memory input = DataTypes.CalculateInterestRatesParams({
            unbacked: 0,
            liquidityAdded: 0,
    ...
    function test_calculate_rates_below_op_usage(
        ...
        uint256 expectedVariableRate = rateStrategy.getBaseVariableBorrowRate(tokenList.usdx)
            + rateStrategy.getVariableRateSlope1(tokenList.usdx)
                .rayMul(borrowUsageRatio.rayDiv(rateStrategy.getOptimalUsageRatio(tokenList.usdx)));

        assertApproxEqAbs(params.currentVariableBorrowRate, expectedVariableRate, 100, "Invalid variable rate");

        assertApproxEqAbs(
            params.currentLiquidityRate,
            expectedVariableRate.rayMul(input.totalDebt.rayDiv(availableLiquidityPlusDebt))
                .percentMul(100_00 - input.reserveFactor),
            100,
            "Invalid liquidity rate"
        );
    }
    ...
    function test_calculate_rates_above_op_usage(
        ...
        uint256 excessBorrowUsageRatio = (borrowUsageRatio - rateStrategy.getOptimalUsageRatio(tokenList.usdx))
        .rayDiv(WadRayMath.RAY - rateStrategy.getOptimalUsageRatio(tokenList.usdx));
