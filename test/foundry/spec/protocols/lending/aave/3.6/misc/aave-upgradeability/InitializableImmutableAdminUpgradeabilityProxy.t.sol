...
import "@crane/contracts/protocols/lending/aave/v3.6/misc/aave-upgradeability/InitializableImmutableAdminUpgradeabilityProxy.sol";
...
contract InitializableImmutableAdminUpgradeabilityProxyTests is Test {
    ...
    function test_proxy_upgradeToAndCall_initialize() public returns (InitializableImmutableAdminUpgradeabilityProxy) {
        ...
        InitializableImmutableAdminUpgradeabilityProxy proxy = new InitializableImmutableAdminUpgradeabilityProxy(admin);
        address proxyAddress = address(proxy);

        bytes memory encodedCall =
            abi.encodeWithSelector(MockInitializableImple.initialize.selector, initialValue, initialText, initialList);
        ...
        assertEq(
            SlotParser.loadAddressFromSlot(
                proxyAddress, bytes32(uint256(keccak256("eip1967.proxy.implementation")) - 1)
            ),
