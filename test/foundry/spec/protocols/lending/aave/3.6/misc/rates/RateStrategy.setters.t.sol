...
contract RateStrategySettersTests is RateStrategyBase {
    ...
    function _setInterestRateParams(address token, IDefaultInterestRateStrategyV2.InterestRateData memory rateData)
        internal
        virtual
    {
    ...
    function test_new_SetReserveInterestRateParams(IDefaultInterestRateStrategyV2.InterestRateData memory rateData)
        public
    {
        _validateSetRateParams(rateData);
        ...
        assertEq(rateStrategy.getOptimalUsageRatio(tokenList.usdx), uint256(rateData.optimalUsageRatio) * 1e23);
        assertEq(rateStrategy.getVariableRateSlope1(tokenList.usdx), uint256(rateData.variableRateSlope1) * 1e23);
        assertEq(rateStrategy.getVariableRateSlope2(tokenList.usdx), uint256(rateData.variableRateSlope2) * 1e23);
        assertEq(
            rateStrategy.getBaseVariableBorrowRate(tokenList.usdx), uint256(rateData.baseVariableBorrowRate) * 1e23
        );
        assertEq(
            rateStrategy.getMaxVariableBorrowRate(tokenList.usdx),
            uint256(rateData.baseVariableBorrowRate + rateData.variableRateSlope1 + rateData.variableRateSlope2) * 1e23
        );
    }
    ...
    function test_reverts_SetReserveInterestRateParams_when_gt_maxRate(
        ...
        rateData.optimalUsageRatio = uint16(
            bound(rateData.optimalUsageRatio, rateStrategy.MIN_OPTIMAL_POINT(), rateStrategy.MAX_OPTIMAL_POINT())
        );
        rateData.variableRateSlope1 = uint32(bound(rateData.variableRateSlope1, 0, rateData.variableRateSlope2));
        vm.assume(
            uint256(rateData.baseVariableBorrowRate) + uint256(rateData.variableRateSlope1)
                    + uint256(rateData.variableRateSlope2) > rateStrategy.MAX_BORROW_RATE()
        );
