...
contract HubRemoveTest is Base {
    ...
    function test_remove_fuzz(uint256 reserveId, uint256 amount) public {
        ...
        HubActions.add({hub: hub1, assetId: assetId, caller: address(spoke1), amount: amount, user: alice});

        vm.expectEmit(address(underlying));
        emit Transfer(address(hub1), alice, amount);
        vm.expectEmit(address(hub1));
        emit IHubBase.Remove(assetId, address(spoke1), hub1.previewRemoveByAssets(assetId, amount), amount);
        ...
        assertEq(assetData.lastUpdateTimestamp, vm.getBlockTimestamp(), "asset lastUpdateTimestamp after");
    ...
    function test_remove_fuzz_multi_spoke(uint256 amount, uint256 amount2) public {
        ...
        HubActions.add({hub: hub1, assetId: assetId, caller: address(spoke1), amount: amount, user: alice});
        HubActions.add({hub: hub1, assetId: assetId, caller: address(spoke2), amount: amount2, user: alice});

        HubActions.remove({hub: hub1, assetId: assetId, caller: address(spoke1), amount: amount, to: alice});
        HubActions.remove({hub: hub1, assetId: assetId, caller: address(spoke2), amount: amount2, to: alice});
    ...
    function test_remove_fuzz_multi_spoke_with_interest(
        ...
        HubActions.draw({hub: hub1, assetId: assetId, caller: address(spoke3), amount: drawAmount, to: bob});
        skip(skipTime);
        ...
        HubActions.restoreDrawn({
            hub: hub1, assetId: assetId, caller: address(spoke3), drawnAmount: drawn, restorer: bob
        });
        ...
        HubActions.remove({hub: hub1, assetId: assetId, caller: address(spoke1), amount: spoke1Amount, to: alice});

        uint256 spoke2Amount = hub1.getSpokeAddedAssets(assetId, address(spoke2));
        HubActions.remove({hub: hub1, assetId: assetId, caller: address(spoke2), amount: spoke2Amount, to: alice});
    ...
    function test_remove_all_with_interest() public {
        ...
        (uint256 drawnRestored, uint256 premiumRestored) = hub1.getSpokeOwed(daiAssetId, address(spoke1));
        assertEq(premiumRestored, 0);
        HubActions.restoreDrawn({
            hub: hub1, assetId: daiAssetId, caller: address(spoke1), drawnAmount: drawnRestored, restorer: alice
        });
    ...
    function test_remove_fuzz_all_liquidity_with_interest(uint256 drawAmount, uint256 skipTime) public {
        ...
        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke2), amount: add2Amount, user: bob});
        ...
        AssetPosition memory asset = _getAssetPosition(hub1, daiAssetId);
        assertEq(asset.liquidity, initialLiquidity + drawnRestored + premiumRestored + add2Amount, "dai liquidity");
    ...
    function test_remove_revertsWith_InsufficientLiquidity_exceeding_added_amount() public {
        ...
        // User add
        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: amount, user: alice});
    ...
    function test_remove_revertsWith_underflow_exceeding_added_amount() public {
        ...
        // Add from spoke 2
        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke2), amount: amount, user: alice});
    ...
    function test_remove_revertsWtih_underflow_one_extra_wei() public {
        ...
        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: supplyAmount, user: alice});

        HubActions.draw({hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: supplyAmount, to: alice});
        ...
        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: supplied, user: alice});
    ...
    function test_remove_revertsWith_InsufficientLiquidity() public {
        ...
        HubActions.draw({hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: amount, to: alice});
        vm.expectRevert(abi.encodeWithSelector(IHub.InsufficientLiquidity.selector, 0));
        vm.prank(address(spoke1));
        hub1.remove(daiAssetId, amount, address(spoke1));
    }
