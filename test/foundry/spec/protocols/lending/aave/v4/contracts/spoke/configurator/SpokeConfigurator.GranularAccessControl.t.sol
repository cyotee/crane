...
contract SpokeConfiguratorGranularAccessControlTest is Base {
    ...
    function setUp() public virtual override {
        ...
        manager.setTargetFunctionRole(address(spokeConfigurator), reserveSelectors, RESERVE_MANAGER_ROLE);
        ...
        manager.setTargetFunctionRole(address(spokeConfigurator), liqSelectors, LIQUIDATION_CONFIG_MANAGER_ROLE);
        ...
        manager.setTargetFunctionRole(address(spokeConfigurator), pmSelectors, POSITION_MANAGER_ADMIN_ROLE);
    ...
    function _buildReserveManagerCalldata() internal {
        ...
        ISpoke.DynamicReserveConfig memory dynamicConfig =
            ISpoke.DynamicReserveConfig({collateralFactor: 80_00, maxLiquidationBonus: 110_00, liquidationFee: 5_00});

        reserveManagerCalldata.push(
            abi.encodeCall(ISpokeConfigurator.updateReservePriceSource, (spokeAddr, reserveId, newPriceSource))
        );
        // Skipping addReserve as it requires more complex setup
        reserveManagerCalldata.push(abi.encodeCall(ISpokeConfigurator.updatePaused, (spokeAddr, reserveId, true)));
        reserveManagerCalldata.push(abi.encodeCall(ISpokeConfigurator.updateFrozen, (spokeAddr, reserveId, true)));
        reserveManagerCalldata.push(abi.encodeCall(ISpokeConfigurator.updateBorrowable, (spokeAddr, reserveId, false)));
        reserveManagerCalldata.push(
        ...
        reserveManagerCalldata.push(
            abi.encodeCall(ISpokeConfigurator.updateMaxLiquidationBonus, (spokeAddr, reserveId, 0, 112_00))
        );
        reserveManagerCalldata.push(abi.encodeCall(ISpokeConfigurator.addLiquidationFee, (spokeAddr, reserveId, 8_00)));
        reserveManagerCalldata.push(
            abi.encodeCall(ISpokeConfigurator.updateLiquidationFee, (spokeAddr, reserveId, 0, 6_00))
        );
        reserveManagerCalldata.push(
            abi.encodeCall(ISpokeConfigurator.addDynamicReserveConfig, (spokeAddr, reserveId, dynamicConfig))
        );
        reserveManagerCalldata.push(
            abi.encodeCall(ISpokeConfigurator.updateDynamicReserveConfig, (spokeAddr, reserveId, 0, dynamicConfig))
        );
    ...
    function _buildLiquidationConfigManagerCalldata() internal {
        ...
        liquidationConfigManagerCalldata.push(
            abi.encodeCall(
                ISpokeConfigurator.updateHealthFactorForMaxBonus, (spokeAddr, HEALTH_FACTOR_LIQUIDATION_THRESHOLD / 2)
            )
        );
        liquidationConfigManagerCalldata.push(
    ...
    function test_fuzz_unauthorized_cannotCall_liquidationConfigManagerMethods(address caller) public {
        ...
        for (uint256 i = 0; i < liquidationConfigManagerCalldata.length; ++i) {
            vm.prank(caller);
            (bool ok, bytes memory ret) = address(spokeConfigurator).call(liquidationConfigManagerCalldata[i]);
            assertFalse(ok);
            assertEq(ret, abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, caller));
        }
    }
    ...
    function test_fuzz_unauthorized_cannotCall_positionManagerAdminMethods(address caller) public {
        ...
        for (uint256 i = 0; i < positionManagerAdminCalldata.length; ++i) {
            vm.prank(caller);
            (bool ok, bytes memory ret) = address(spokeConfigurator).call(positionManagerAdminCalldata[i]);
            assertFalse(ok);
            assertEq(ret, abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, caller));
        }
    }
    ...
    function test_reserveManager_cannotCall_anyLiquidationConfigMethod() public {
        for (uint256 i = 0; i < liquidationConfigManagerCalldata.length; ++i) {
            ...
            (bool ok, bytes memory ret) = address(spokeConfigurator).call(liquidationConfigManagerCalldata[i]);
            assertFalse(ok);
            assertEq(ret, abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, RESERVE_MANAGER));
        }
    }
    ...
    function test_liquidationConfigManager_cannotCall_anyReserveMethod() public {
        for (uint256 i = 0; i < reserveManagerCalldata.length; ++i) {
            ...
            assertEq(
                ret,
                abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, LIQUIDATION_CONFIG_MANAGER)
            );
        }
    }
    ...
    function test_positionManagerAdmin_cannotCall_anyReserveMethod() public {
        for (uint256 i = 0; i < reserveManagerCalldata.length; ++i) {
            ...
            assertFalse(ok);
            assertEq(
                ret, abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, POSITION_MANAGER_ADMIN)
            );
        }
    }
    ...
    function test_positionManagerAdmin_canCall_updatePositionManager() public {
        ...
        vm.prank(POSITION_MANAGER_ADMIN);
        spokeConfigurator.updatePositionManager({spoke: spokeAddr, positionManager: newPM, active: true});
