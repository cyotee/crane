...
contract SpokePositionManagerTest is Base {
    ...
    function test_onlyPositionManager_on_supply() public {
        ...
        emit Transfer(address(POSITION_MANAGER), address(hub1), amount);
        vm.expectEmit(address(spoke1));
        emit ISpoke.Supply({
            reserveId: reserveId,
        ...
        SpokeActions.supply({
            spoke: spoke1, reserveId: reserveId, caller: POSITION_MANAGER, amount: amount, onBehalfOf: alice
        });
    ...
    function test_onlyPositionManager_on_withdraw() public {
        ...
        vm.expectEmit(address(tokenList.usdx));
        emit Transfer(address(hub1), address(POSITION_MANAGER), amount);
        vm.expectEmit(address(spoke1));
    ...
    function test_onlyPositionManager_on_borrow() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: reserveId, caller: alice, amount: (amount * 3) / 2, onBehalfOf: alice
        });
    ...
    function test_onlyPositionManager_on_repay() public {
        ...
        SpokeActions.borrow({spoke: spoke1, reserveId: reserveId, caller: alice, amount: amount, onBehalfOf: alice});
        ...
        IHubBase.PremiumDelta memory expectedPremiumDelta =
            _getExpectedPremiumDeltaForRestore(spoke1, alice, reserveId, repayAmount);

        vm.expectEmit(address(tokenList.usdx));
        emit Transfer(address(POSITION_MANAGER), address(hub1), repayAmount);
        vm.expectEmit(address(spoke1));
        emit ISpoke.Repay(
            reserveId,
            POSITION_MANAGER,
        ...
        SpokeActions.repay({
            spoke: spoke1, reserveId: reserveId, caller: POSITION_MANAGER, amount: repayAmount, onBehalfOf: alice
        });
        ...
        SpokeActions.repay({
            spoke: spoke1, reserveId: reserveId, caller: POSITION_MANAGER, amount: UINT256_MAX, onBehalfOf: alice
        });
        assertEq(spoke1.getUserPosition(reserveId, POSITION_MANAGER), posBefore);
        assertEq(spoke1.getUserTotalDebt(reserveId, POSITION_MANAGER), 0);
    ...
    function test_onlyPositionManager_on_usingAsCollateral() public {
        ...
        emit ISpoke.SetUsingAsCollateral({
            reserveId: reserveId, caller: POSITION_MANAGER, user: alice, usingAsCollateral: usingAsCollateral
        });
        SpokeActions.setUsingAsCollateral({
            spoke: spoke1,
            reserveId: reserveId,
    ...
    function test_onlyPositionManager_on_updateUserRiskPremium() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _wethReserveId(spoke1), caller: alice, amount: 0.5e18, onBehalfOf: alice
        });
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: alice, amount: 1000e18, onBehalfOf: alice
        });
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _usdxReserveId(spoke1), caller: alice, amount: 1500e6, onBehalfOf: alice
        });
