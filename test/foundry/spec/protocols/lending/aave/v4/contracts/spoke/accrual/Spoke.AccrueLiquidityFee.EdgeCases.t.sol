...
contract SpokeAccrueLiquidityFeeEdgeCasesTest is Base {
    ...
    function test_accrueLiquidityFee_maxLiquidityFee_with_premium() public {
        test_accrueLiquidityFee_fuzz_maxLiquidityFee_with_premium({
            reserveId: _daiReserveId(spoke1), borrowAmount: 500e18, skipTime: 400 days, rate: 50_00
        });
    }
    ...
    function test_accrueLiquidityFee_fuzz_maxLiquidityFee_with_premium(
        ...
        assertApproxEqAbs(
            spoke1.getUserSuppliedAssets(reserveId, alice), supplyAmount, 3, "alice does not earn anything"
        );
        assertApproxEqAbs(
    ...
    function test_accrueLiquidityFee_fuzz_maxLiquidityFee_with_premium_multiple_users(
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: reserveId, caller: bob, amount: supplyAmount2, onBehalfOf: bob
        });
        SpokeActions.borrow({spoke: spoke1, reserveId: reserveId, caller: bob, amount: borrowAmount2, onBehalfOf: bob});
        ...
        assertApproxEqAbs(spoke1.getUserSuppliedAssets(reserveId, bob), supplyAmount2, 3, "bob does not earn anything");

        uint256 totalAccruedToTreasury = hub1.getSpokeAddedAssets(assetId, address(treasurySpoke));
        assertLe(
            totalAccruedToTreasury,
            spoke1.getUserTotalDebt(reserveId, alice) - borrowAmount + spoke1.getUserTotalDebt(reserveId, bob)
                - borrowAmount2,
    ...
    function test_accrueLiquidityFee_maxLiquidityFee_multi_user() public {
        ...
        for (uint256 i; i < count; ++i) {
            ...
            SpokeActions.repay({spoke: spoke1, reserveId: reserveId, caller: user, amount: 1, onBehalfOf: user}); // accrue interest & realize premium
    ...
    function test_accrueLiquidityFee_maxLiquidityFee_multi_spoke() public {
        ...
        for (uint256 i; i < count; ++i) {
            ...
            uint256 reserveId = _reserveId(spoke, assetId);
            uint256 totalOwedAfter = hub1.getAssetTotalOwed(assetId);
            SpokeActions.repay({spoke: spoke, reserveId: reserveId, caller: user, amount: 1, onBehalfOf: user}); // accrue interest & realize premium
            assertApproxEqAbs(totalOwedAfter, hub1.getAssetTotalOwed(assetId), 1);
