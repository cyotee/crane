...
contract SpokeSupplyTest is Base {
    ...
    function test_supply_revertsWith_ERC20InsufficientAllowance() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IERC20Errors.ERC20InsufficientAllowance.selector, address(spoke1), approvalAmount, amount
            )
        );
    ...
    function test_supply_fuzz_revertsWith_ERC20InsufficientBalance(uint256 amount) public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IERC20Errors.ERC20InsufficientBalance.selector, address(randomUser), 0, amount)
        );
    ...
    function test_supply_revertsWith_ReentrancyGuardReentrantCall() public {
        ...
        MockReentrantCaller reentrantCaller = new MockReentrantCaller(address(spoke1), ISpoke.supply.selector);
    ...
    function test_supply() public {
        ...
        assertEq(tokenList.dai.balanceOf(bob), MAX_SUPPLY_AMOUNT_DAI - amount, "user token balance after-supply");
        ...
        assertEq(r.reserveAfter.totalSuppliedShares, expectedShares, "reserve suppliedShares after-supply");
        assertEq(amount, hub1.getSpokeAddedAssets(daiAssetId, address(spoke1)), "spoke supplied amount after-supply");
        ...
        assertEq(r.ownerAfter.position.suppliedShares, expectedShares, "bob suppliedShares after-supply");
        assertEq(amount, spoke1.getUserSuppliedAssets(_daiReserveId(spoke1), bob), "user supplied amount after-supply");
    }
    ...
    function test_supply_fuzz_amounts(uint256 amount) public {
        ...
        assertEq(r.ownerAfter.position.suppliedShares, expectedShares, "user suppliedShares after-supply");
        assertEq(amount, spoke1.getUserSuppliedAssets(_daiReserveId(spoke1), bob), "user supplied amount after-supply");
    }
    ...
    function test_supply_index_increase_no_premium() public {
        ...
        CheckedSupplyResult memory r = _checkedSupply(
            CheckedSupplyParams({
                spoke: spoke1, reserveId: _daiReserveId(spoke1), user: carol, amount: amount, onBehalfOf: carol
            })
        );
        ...
        assertEq(tokenList.dai.balanceOf(address(hub1)), hubBalanceBefore + amount, "hub token balance after-supply");
        assertEq(tokenList.dai.balanceOf(address(spoke1)), 0, "spoke token balance after-supply");

        // reserve assertions
        IHub.SpokeData memory spokeDataAfter = hub1.getSpoke(daiAssetId, address(spoke1));
        assertEq(spokeDataAfter.drawnShares, spokeDataBefore.drawnShares, "reserve drawnShares after-supply");
        ...
        assertEq(r.ownerAfter.position.suppliedShares, expectedShares, "user suppliedShares after-supply");
        assertApproxEqAbs(
            amount, spoke1.getUserSuppliedAssets(_daiReserveId(spoke1), carol), 1, "user supplied amount after-supply"
        );
    }
    ...
    function test_supply_fuzz_index_increase_no_premium(
        ...
        vm.assume(_calcMinimumCollAmount(spoke1, _wethReserveId(spoke1), reserveId, maxSupply / 10) <= wethMaxSupply);
        ...
        _assertRefreshPremiumNotCalled(hub1);
        CheckedSupplyResult memory r = _checkedSupply(
            CheckedSupplyParams({spoke: spoke1, reserveId: reserveId, user: carol, amount: amount, onBehalfOf: carol})
        );
        ...
        // token balance
        assertEq(state.underlying.balanceOf(carol), MAX_SUPPLY_AMOUNT - amount, "user token balance after-supply");
        assertEq(state.underlying.balanceOf(address(hub1)), hubBalanceBefore + amount, "hub token balance after-supply");
        ...
        assertEq(r.ownerAfter.position.premiumShares, 0, "user premiumShares after-supply");
        assertEq(r.ownerAfter.position.premiumOffsetRay, 0, "user premiumOffsetRay after-supply");
        assertEq(r.ownerAfter.position.suppliedShares, state.expectedShares, "user suppliedShares after-supply");
    }
    ...
    function test_supply_fuzz_index_increase_with_premium(
        ...
        vm.expectEmit(address(spoke1));
        emit ISpoke.Supply({
            reserveId: reserveId, caller: carol, user: carol, suppliedShares: expectedShares, suppliedAmount: amount
        });
        _assertRefreshPremiumNotCalled(hub1);
        CheckedSupplyResult memory r = _checkedSupply(
            CheckedSupplyParams({spoke: spoke1, reserveId: reserveId, user: carol, amount: amount, onBehalfOf: carol})
        ...
        assertEq(underlying.balanceOf(address(hub1)), hubBalanceBefore + amount, "hub token balance after-supply");
    ...
    function test_supply_does_not_update_risk_premium() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: 50_000e18, onBehalfOf: bob
        ...
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _usdxReserveId(spoke1), caller: bob, amount: 10_000e6, onBehalfOf: bob
        }); // bob borrows usdx, $5k
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: 10_000e18, onBehalfOf: bob
        }); // bob borrows dai, $10k

        uint256 initialRP = _getUserRiskPremium(spoke1, bob);
        assertEq(initialRP, _calculateExpectedUserRP(spoke1, bob));

        assertGt(_getCollateralRisk(spoke1, _daiReserveId(spoke1)), _getCollateralRisk(spoke1, _wethReserveId(spoke1)));
        // bob does another supply action of the lower collateral risk reserve
        // risk premium should not be refreshed
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _wethReserveId(spoke1), caller: bob, amount: 10_000e18, onBehalfOf: bob
        });
    ...
    function _calcStoredUserRP(ISpoke spoke, uint256 reserveId, address user) internal view returns (uint256) {
