...
contract NativeTokenGatewayTest is Base {
    ...
    function test_supplyNative_fuzz(uint256 amount) public {
        ...
        assertEq(spoke1.getUserSuppliedAssets(_wethReserveId(spoke1), bob), prevUserSuppliedAmount + amount);
    ...
    function test_supplyNative_revertsWith_ReentrancyGuardReentrantCall_spokeSupply() public {
        ...
        MockReentrantCaller reentrantCaller =
            new MockReentrantCaller(address(nativeTokenGateway), INativeTokenGateway.supplyNative.selector);

        vm.mockFunction(address(spoke1), address(reentrantCaller), abi.encodeWithSelector(ISpoke.supply.selector));
    ...
    function test_supplyNative_revertsWith_ReentrancyGuardReentrantCall_hubAdd() public {
        ...
        vm.mockFunction(
            address(_hub(spoke1, _wethReserveId(spoke1))),
    ...
    function test_supplyNative_revertsWith_NotNativeWrappedAsset() public {
        ...
        vm.prank(bob);
        nativeTokenGateway.supplyNative{value: amount}(address(spoke1), _wethReserveId(spoke1) + 1, amount);
    }
    ...
    function test_supplyAndCollateralNative_fuzz(uint256 amount) public {
        ...
        vm.prank(bob);
        (returnValues.shares, returnValues.amount) =
            nativeTokenGateway.supplyAsCollateralNative{value: amount}(address(spoke1), _wethReserveId(spoke1), amount);
    ...
    function test_withdrawNative_fuzz_allBalance(uint256 supplyAmount) public {
        ...
        SpokeActions.supply({
            spoke: spoke1, reserveId: _wethReserveId(spoke1), caller: bob, amount: supplyAmount, onBehalfOf: bob
        });
        ...
        vm.expectEmit(address(spoke1));
        emit ISpoke.Withdraw(
            _wethReserveId(spoke1), address(nativeTokenGateway), bob, expectedSupplyShares, supplyAmount
        );
        vm.prank(bob);
        (returnValues.shares, returnValues.amount) =
    ...
    function test_withdrawNative_fuzz_allBalanceWithInterest(uint256 supplyAmount, uint256 borrowAmount) public {
        ...
        // Bob borrows weth
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _wethReserveId(spoke1), caller: bob, amount: borrowAmount, onBehalfOf: bob
        });
        ...
        SpokeActions.repay({
            spoke: spoke1, reserveId: _wethReserveId(spoke1), caller: bob, amount: UINT256_MAX, onBehalfOf: bob
        });
        ...
        vm.expectEmit(address(spoke1));
        emit ISpoke.Withdraw(
            _wethReserveId(spoke1), address(nativeTokenGateway), bob, expectedSupplyShares, expectedWithdrawAmount
        );
        vm.prank(bob);
        (returnValues.shares, returnValues.amount) =
            nativeTokenGateway.withdrawNative(address(spoke1), _wethReserveId(spoke1), UINT256_MAX);
    ...
    function test_withdrawNative_revertsWith_ReentrancyGuardReentrantCall_spokeWithdraw() public {
        ...
        uint256 amount = 100e18;
        MockReentrantCaller reentrantCaller =
            new MockReentrantCaller(address(nativeTokenGateway), INativeTokenGateway.withdrawNative.selector);

        vm.mockFunction(address(spoke1), address(reentrantCaller), abi.encodeWithSelector(ISpoke.withdraw.selector));
        vm.expectRevert(ReentrancyGuardTransient.ReentrancyGuardReentrantCall.selector);
        vm.prank(bob);
        nativeTokenGateway.withdrawNative(address(spoke1), _wethReserveId(spoke1), amount);
    }
    ...
    function test_borrowNative_revertsWith_ReentrancyGuardReentrantCall_spokeBorrow() public {
        ...
        uint256 amount = 100e18;
        MockReentrantCaller reentrantCaller =
            new MockReentrantCaller(address(nativeTokenGateway), INativeTokenGateway.borrowNative.selector);

        vm.mockFunction(address(spoke1), address(reentrantCaller), abi.encodeWithSelector(ISpoke.borrow.selector));
        vm.expectRevert(ReentrancyGuardTransient.ReentrancyGuardReentrantCall.selector);
        vm.prank(bob);
        nativeTokenGateway.borrowNative(address(spoke1), _wethReserveId(spoke1), amount);
    }
    ...
    function test_repayNative_fuzz(uint256 repayAmount) public {
        ...
        (uint256 baseRestored,) =
            _calculateExactRestoreAmount(hub1, wethAssetId, userDrawnDebt, userPremiumDebt, repayAmount);
        IHubBase.PremiumDelta memory expectedPremiumDelta =
            _getExpectedPremiumDeltaForRestore(spoke1, bob, _wethReserveId(spoke1), repayAmount);
        ...
        vm.prank(bob);
        (returnValues.shares, returnValues.amount) =
            nativeTokenGateway.repayNative{value: repayAmount}(address(spoke1), _wethReserveId(spoke1), repayAmount);
    ...
    function test_repayNative_fuzz_withInterest(uint256 repayAmount, uint256 elapsedTime) public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: 100000e18, onBehalfOf: bob
        });
        SpokeActions.supply({
            spoke: spoke1, reserveId: _wethReserveId(spoke1), caller: alice, amount: 10e18, onBehalfOf: alice
        });
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _wethReserveId(spoke1), caller: bob, amount: borrowAmount, onBehalfOf: bob
        });
        ...
        assertApproxEqAbs(
            newUserDrawnDebt + newUserPremiumDebt, userDrawnDebt + userPremiumDebt - (baseRestored + premiumRestored), 2
        );
        assertApproxEqAbs(tokenList.weth.balanceOf(address(hub1)), prevHubBalance + (baseRestored + premiumRestored), 2);
    ...
    function test_repayNative_revertsWith_ReentrancyGuardReentrantCall_spokeRepay() public {
        ...
        uint256 amount = 100e18;
        MockReentrantCaller reentrantCaller =
            new MockReentrantCaller(address(nativeTokenGateway), INativeTokenGateway.repayNative.selector);

        vm.mockFunction(address(spoke1), address(reentrantCaller), abi.encodeWithSelector(ISpoke.repay.selector));
        vm.expectRevert(ReentrancyGuardTransient.ReentrancyGuardReentrantCall.selector);
        vm.prank(bob);
        nativeTokenGateway.repayNative{value: amount}(address(spoke1), _wethReserveId(spoke1), amount);
    }
    ...
    function test_repayNative_revertsWith_SpokeNotRegistered() public {
        ...
        vm.expectRevert(IPositionManagerBase.SpokeNotRegistered.selector);
        vm.prank(bob);
        nativeTokenGateway.repayNative{value: repayAmount}(address(spoke2), _wethReserveId(spoke1), repayAmount);

        vm.expectRevert(IPositionManagerBase.SpokeNotRegistered.selector);
        vm.prank(bob);
        nativeTokenGateway.repayNative{value: repayAmount}(address(0), _wethReserveId(spoke1), repayAmount);
    }
    ...
    function test_repayNative_revertsWith_NotNativeWrappedAsset() public {
        ...
        vm.expectRevert(INativeTokenGateway.NotNativeWrappedAsset.selector);
        vm.prank(bob);
        nativeTokenGateway.repayNative{value: repayAmount}(address(spoke1), _wethReserveId(spoke1) + 1, repayAmount);
    }
    ...
    function test_repayNative_revertsWith_NativeAmountMismatch() public {
        ...
        vm.expectRevert(INativeTokenGateway.NativeAmountMismatch.selector);
        vm.prank(bob);
        nativeTokenGateway.repayNative{value: repayAmount / 2}(address(spoke1), _wethReserveId(spoke1), repayAmount);
    }
