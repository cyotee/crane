...
contract LiquidationLogicDebtToLiquidateTest is LiquidationLogicBaseTest {
    ...
    function test_calculateDebtToLiquidate_fuzz(LiquidationLogic.CalculateDebtToLiquidateParams memory params) public {
        ...
        uint256 rawPremiumDebtRayToLiquidate = debtRayToTarget.fromRayUp().toRay().min(params.premiumDebtRay);
        ...
        uint256 rawDrawnSharesToLiquidate = drawnSharesToTarget.min(drawnSharesToCover).min(params.drawnShares);
        ...
        uint256 debtRayRemaining = (params.drawnShares - rawDrawnSharesToLiquidate) * params.drawnIndex
            + params.premiumDebtRay - rawPremiumDebtRayToLiquidate;

        bool leavesDebtDust = _convertAmountToValue(debtRayRemaining, params.debtAssetPrice, params.debtAssetUnit)
            < LiquidationLogic.DUST_LIQUIDATION_THRESHOLD.toRay();

        (uint256 drawnSharesToLiquidate, uint256 premiumDebtRayToLiquidate) =
            liquidationLogicWrapper.calculateDebtToLiquidate(params);
        if (leavesDebtDust) {
