...
contract SpokeAccrueInterestScenarioTest is Base {
    ...
    function test_accrueInterest_fuzz_RPBorrowAndSkipTime_twoActions(TestInputs memory amounts, uint40 skipTime)
        public
    {
        ...
        for (uint256 i = 0; i < 4; ++i) {
            ...
            if (testAmounts[i].supplyAmount < MAX_SUPPLY_AMOUNT) {
                _openSupplyPosition(spoke2, testAmounts[i].reserveId, MAX_SUPPLY_AMOUNT - testAmounts[i].supplyAmount);
            }
        }
        ...
        for (uint256 i = 0; i < 4; ++i) {
            uint256 drawnDebt =
                _calculateExpectedDrawnDebt(testAmounts[i].borrowAmount, values[i].baseDrawnRate, startTime);
        ...
        for (uint256 i = 0; i < 4; ++i) {
            uint256 drawnDebt =
                _calculateExpectedDrawnDebt(testAmounts[i].borrowAmount, values[i].baseDrawnRate, startTime);
            uint256 expectedPremiumDebt = _calculateExpectedPremiumDebt(testAmounts[i].borrowAmount, drawnDebt, bobRp);
            uint256 interest = (drawnDebt + expectedPremiumDebt) - testAmounts[i].borrowAmount
        ...
        if (_getUserHealthFactor(spoke2, bob) >= HEALTH_FACTOR_LIQUIDATION_THRESHOLD) {
            ...
            SpokeActions.borrow({
                spoke: spoke2, reserveId: _daiReserveId(spoke2), caller: bob, amount: daiBorrowAmount, onBehalfOf: bob
            });
            ...
            // Check bob's drawn debt, premium debt, and supplied amounts for all assets at user, reserve, spoke, and asset level
            for (uint256 i = 0; i < 4; ++i) {
                ISpoke.UserPosition memory bobPosition = spoke2.getUserPosition(testAmounts[i].reserveId, bob);
                uint256 drawnDebt = testAmounts[i].borrowAmount;
                uint256 expectedPremiumDebt = _calculatePremiumDebt(
                    hub1, testAmounts[i].assetId, bobPosition.premiumShares, bobPosition.premiumOffsetRay
                );
                uint256 interest = (drawnDebt + expectedPremiumDebt) - testAmounts[i].originalBorrowAmount
                    - _calculateBurntInterest(hub1, testAmounts[i].assetId);
    ...
    function _ensureSufficientCollateral(ISpoke spoke, TestInputs memory amounts)
        internal
        view
        returns (TestInputs memory)
    {
    ...
    function _assertProtocolSupplyAndDebt(
        ...
        _assertUserSupply(spoke2, reserveId, bob, expectedUserSupply, string.concat(reserveName, label));
        _assertReserveSupply(spoke2, reserveId, expectedReserveSupply, string.concat(reserveName, label));
        ...
        _assertOnlyOneUserDebt(
            spoke2, reserveId, bob, expectedDrawnDebt, expectedPremiumDebt, string.concat(reserveName, label)
        );
    }
}
