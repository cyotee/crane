...
contract SpokeRiskPremiumEdgeCasesTest is Base {
    ...
    function test_riskPremium_nonIncreasingAfterRepay(
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke2, reserveId: _usdzReserveId(spoke2), caller: bob, amount: MAX_SUPPLY_AMOUNT, onBehalfOf: bob
        });
        ...
        SpokeActions.repay({
            spoke: spoke2, reserveId: _usdzReserveId(spoke2), caller: bob, amount: repayAmount, onBehalfOf: bob
        });
        ...
        assertEq(
            _getUserRiskPremium(spoke2, bob), _calculateExpectedUserRP(spoke2, bob), "Bob user risk premium after repay"
        );
    }

    /// Supply two collaterals, borrow, then remove lower collateral-risk reserve and risk premium shouldn't decrease
    function test_riskPremium_nonDecreasesAfterCollateralRemoval(uint256 daiSupplyAmount, uint256 borrowAmount) public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke2, reserveId: _usdzReserveId(spoke2), caller: bob, amount: usdzSupplyAmount, onBehalfOf: bob
        });
        SpokeActions.supplyCollateral({
            spoke: spoke2, reserveId: _daiReserveId(spoke2), caller: bob, amount: daiSupplyAmount, onBehalfOf: bob
        });
    ...
    function test_riskPremium_increasesAfterWithdrawal(uint256 daiSupplyAmount, uint256 borrowAmount) public {
        daiSupplyAmount = bound(daiSupplyAmount, 1, MAX_SUPPLY_AMOUNT);
        borrowAmount = bound(borrowAmount, 1, MAX_SUPPLY_AMOUNT / 2);
        uint256 withdrawAmount = daiSupplyAmount;
        test_riskPremium_fuzz_nonDecreasingAfterWithdrawal(daiSupplyAmount, borrowAmount, withdrawAmount);
    }
    ...
    function test_riskPremium_fuzz_nonDecreasingAfterWithdrawal(
        ...
        SpokeActions.withdraw({
            spoke: spoke2, reserveId: _daiReserveId(spoke2), caller: bob, amount: withdrawAmount, onBehalfOf: bob
        });
    ...
    /// Debt is initially covered by 2 collaterals, then 1 collateral becomes enough to cover the debt due to interest accrual
    function test_riskPremium_fuzz_nonIncreasesAfterCollateralAccrual(uint256 daiSupplyAmount, uint40 skipTime) public {
        ...
        uint256 wethBorrowAmount =
            _convertAssetAmount(spoke2, _daiReserveId(spoke2), daiSupplyAmount, _wethReserveId(spoke2)) + 1; // Borrow more than dai supply value so 2 collaterals cover debt
        uint256 usdzSupplyAmount = MAX_SUPPLY_AMOUNT;
        ...
        // Bob borrows weth
        SpokeActions.borrow({
            spoke: spoke2, reserveId: _wethReserveId(spoke2), caller: bob, amount: wethBorrowAmount, onBehalfOf: bob
        });
        ...
        uint256 aliceCollateralAmount = _calcMinimumCollAmount(
            spoke2, _wbtcReserveId(spoke2), _daiReserveId(spoke2), daiSupplyAmount + usdzSupplyAmount
        );
        SpokeActions.supplyCollateral({
            spoke: spoke2,
        ...
        SpokeActions.borrow({
            spoke: spoke2, reserveId: _daiReserveId(spoke2), caller: alice, amount: daiSupplyAmount, onBehalfOf: alice
        });
        SpokeActions.borrow({
            spoke: spoke2, reserveId: _usdzReserveId(spoke2), caller: alice, amount: usdzSupplyAmount, onBehalfOf: alice
        });
    ...
    function test_riskPremium_increasesAfterDebtAccrual() public {
        ...
        uint256 daiBorrowAmount =
            _convertAssetAmount(spoke2, _wbtcReserveId(spoke2), wbtcSupplyAmount, _daiReserveId(spoke2)); // Dai debt to equal wbtc supply value
    ...
    function test_riskPremium_fuzz_increasesAfterDebtAccrual(uint256 borrowAmount, uint40 skipTime) public {
        // Find max supply amount of dai in terms of weth
        uint256 maxWethDebt =
            _convertAssetAmount(spoke2, _daiReserveId(spoke2), MAX_SUPPLY_AMOUNT, _wethReserveId(spoke2));
        assertLt(maxWethDebt, MAX_SUPPLY_AMOUNT / 2, "Max weth debt should be less than half max supply amount");
        ...
        uint256 daiSupplyAmount =
            _convertAssetAmount(spoke2, _wethReserveId(spoke2), borrowAmount, _daiReserveId(spoke2));
        uint256 usdzSupplyAmount = MAX_SUPPLY_AMOUNT;
        skipTime = bound(skipTime, 365 days, MAX_SKIP_TIME).toUint40(); // At least skip one year to ensure sufficient accrual
        ...
        // Bob borrows weth
        SpokeActions.borrow({
            spoke: spoke2, reserveId: _wethReserveId(spoke2), caller: bob, amount: borrowAmount, onBehalfOf: bob
        });
        ...
        assertGt(
            ...
            _convertAmountToValue(
                spoke2, _daiReserveId(spoke2), spoke2.getUserSuppliedAssets(_daiReserveId(spoke2), bob)
            ),
    ...
    /// Initially debt is covered by 1 collateral, both debt and collateral accrue at different rates, such that finally debt is covered by 2 collaterals
    function test_riskPremium_fuzz_changesAfterAccrual(uint256 wethBorrowAmount, uint40 skipTime) public {
        uint256 usdzSupplyAmount = MAX_SUPPLY_AMOUNT;
        // Find max supply amount of dai in terms of weth
        uint256 maxWethDebt =
            _convertAssetAmount(spoke2, _daiReserveId(spoke2), MAX_SUPPLY_AMOUNT, _wethReserveId(spoke2));
        assertLe(maxWethDebt, MAX_SUPPLY_AMOUNT / 2, "Max weth debt should be less than half max supply amount");
        wethBorrowAmount = bound(wethBorrowAmount, 1e18, maxWethDebt); // Allow room for dai supply to cover weth debt
        uint256 daiSupplyAmount =
            _convertAssetAmount(spoke2, _wethReserveId(spoke2), wethBorrowAmount, _daiReserveId(spoke2)); // Dai collateral will fully cover initial weth borrow
        skipTime = bound(skipTime, 365 days, MAX_SKIP_TIME).toUint40(); // At least skip one year to ensure sufficient accrual
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke2, reserveId: _wbtcReserveId(spoke2), caller: alice, amount: 1e8, onBehalfOf: alice
        });
        SpokeActions.borrow({
            spoke: spoke2, reserveId: _daiReserveId(spoke2), caller: alice, amount: 1e6, onBehalfOf: alice
        });
    ...
    function test_riskPremium_fuzz_borrowingMoreNonDecreasesRP(
        ...
        // Bob borrows dai
        SpokeActions.borrow({
            spoke: spoke2, reserveId: _daiReserveId(spoke2), caller: bob, amount: initialBorrowAmount, onBehalfOf: bob
        });
        ...
        assertLt(
            _convertAmountToValue(spoke2, _daiReserveId(spoke2), daiSupplyAmount),
            _convertAmountToValue(spoke2, _daiReserveId(spoke2), spoke2.getUserTotalDebt(_daiReserveId(spoke2), bob)),
    ...
    function test_riskPremium_supplyingLowerCRCollateral_decreasesRP() public {
        ...
        uint256 daiBorrowAmount =
            _convertAssetAmount(spoke1, _wethReserveId(spoke1), wethSupplyAmount / 2, _daiReserveId(spoke1)); // Half of the weth collateral value
        test_riskPremium_fuzz_supplyingLowerCRCollateral_nonIncreasesRP(wbtcSupplyAmount, daiBorrowAmount);
    }
