...
contract SpokeRepayTest is Base {
    ...
    function test_repay_revertsWith_ERC20InsufficientAllowance() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IERC20Errors.ERC20InsufficientAllowance.selector, address(spoke1), approvalAmount, daiRepayAmount
            )
        );
    ...
    function test_repay_fuzz_revertsWith_ERC20InsufficientBalance(uint256 daiRepayAmount) public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IERC20Errors.ERC20InsufficientBalance.selector, address(bob), 0, daiRepayAmount)
        );
        spoke1.repay(_daiReserveId(spoke1), daiRepayAmount, bob);
        vm.stopPrank();
    }
    ...
    function test_repay_revertsWith_ReentrancyGuardReentrantCall() public {
        ...
        MockReentrantCaller reentrantCaller = new MockReentrantCaller(address(spoke1), ISpoke.repay.selector);
        vm.mockFunction(
            address(_hub(spoke1, _daiReserveId(spoke1))),
            address(reentrantCaller),
            abi.encodeCall(
                IHubBase.restore,
                (daiAssetId, amount, _getExpectedPremiumDeltaForRestore(spoke1, bob, _daiReserveId(spoke1), amount))
            )
        );
    ...
    function test_repay() public {
        ...
        emit ISpoke.Repay(_daiReserveId(spoke1), bob, bob, expectedShares, daiRepayAmount, expectedPremiumDelta);
        ...
        assertApproxEqAbs(
            r.ownerAfter.totalDebt, r.ownerBefore.totalDebt - daiRepayAmount, 2, "bob dai debt final balance"
        );
    ...
    function test_repay_all_with_accruals() public {
        ...
        assertEq(spoke1.getUserTotalDebt(_daiReserveId(spoke1), bob), 0, "user total debt after full repay");
    ...
    function test_repay_same_block() public {
        ...
        (uint256 bobDaiDrawnDebtBefore, uint256 bobDaiPremiumDebtBefore) =
            spoke1.getUserDebt(_daiReserveId(spoke1), bob);
        assertEq(bobDaiDrawnDebtBefore + bobDaiPremiumDebtBefore, daiBorrowAmount, "bob dai debt before");

        IHubBase.PremiumDelta memory expectedPremiumDelta =
            _getExpectedPremiumDeltaForRestore(spoke1, bob, _daiReserveId(spoke1), daiRepayAmount);
        (uint256 baseRestored,) = _calculateExactRestoreAmount(
            hub1, daiAssetId, bobDaiDrawnDebtBefore, bobDaiPremiumDebtBefore, daiRepayAmount
        );
        ...
        assertEq(r.ownerAfter.suppliedShares, r.ownerBefore.suppliedShares);
        assertEq(r.ownerAfter.totalDebt, r.ownerBefore.totalDebt - daiRepayAmount, "bob dai debt final balance");
        assertEq(r.callerAfter.tokenBalance, r.callerBefore.tokenBalance - daiRepayAmount, "bob dai final balance");
    ...
    function test_repay_only_interest() public {
        ...
        assertApproxEqAbs(r.ownerAfter.drawnDebt, daiBorrowAmount, 1, "bob dai drawn debt final balance");
    ...
    function test_fuzz_repay_only_premium(uint256 daiBorrowAmount, uint40 skipTime) public {
        ...
        SpokeActions.supply({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: alice, amount: daiBorrowAmount, onBehalfOf: alice
        });
        ...
        assertEq(r.ownerAfter.suppliedShares, r.ownerBefore.suppliedShares);
        assertApproxEqAbs(
            r.ownerAfter.totalDebt, r.ownerBefore.totalDebt - daiRepayAmount, 1, "bob dai debt final balance"
        );
        assertApproxEqAbs(
    ...
    function test_repay_max() public {
        ...
        vm.expectEmit(address(spoke1));
        emit ISpoke.Repay(_daiReserveId(spoke1), bob, bob, expectedShares, fullDebt, expectedPremiumDelta);
        _assertRefreshPremiumNotCalled(hub1);
    ...
    function test_fuzz_repay_same_block_fuzz_amounts(uint256 daiBorrowAmount, uint256 daiRepayAmount) public {
        daiBorrowAmount = bound(daiBorrowAmount, 1, MAX_SUPPLY_AMOUNT / 2);
        daiRepayAmount = bound(daiRepayAmount, 1, daiBorrowAmount);
    ...
    function test_repay_fuzz_amountsAndWait(uint256 daiBorrowAmount, uint256 daiRepayAmount, uint40 skipTime) public {
        daiBorrowAmount = bound(daiBorrowAmount, 1, MAX_SUPPLY_AMOUNT / 2);
        daiRepayAmount = bound(daiRepayAmount, 1, daiBorrowAmount);
        ...
        if (baseRestored > 0) {
            assertApproxEqAbs(r.ownerAfter.premiumDebt, 0, 1, "bob dai premium debt final balance when base repaid");
        }
    ...
    function test_fuzz_repay_amounts_only_interest(uint256 daiBorrowAmount, uint256 daiRepayAmount, uint40 skipTime)
        public
    {
        daiBorrowAmount = bound(daiBorrowAmount, 1, MAX_SUPPLY_AMOUNT / 2);
        skipTime = bound(skipTime, 0, MAX_SKIP_TIME).toUint40();
    ...
    function test_fuzz_amounts_repay_only_premium(uint256 daiBorrowAmount, uint256 daiRepayAmount, uint40 skipTime)
        public
    {
        daiBorrowAmount = bound(daiBorrowAmount, 1, MAX_SUPPLY_AMOUNT / 2);
        skipTime = bound(skipTime, 0, MAX_SKIP_TIME).toUint40();
        ...
        assertApproxEqAbs(
            r.ownerAfter.totalDebt, r.ownerBefore.totalDebt - r.premiumRestored, 1, "bob dai debt final balance"
        );
    ...
    function test_repay_fuzz_amounts_base_debt(uint256 daiBorrowAmount, uint256 daiRepayAmount, uint40 skipTime)
        public
    {
        daiBorrowAmount = bound(daiBorrowAmount, 1, MAX_SUPPLY_AMOUNT / 2);
        skipTime = bound(skipTime, 0, MAX_SKIP_TIME).toUint40();
        ...
        assertApproxEqAbs(
            r.ownerAfter.drawnDebt, r.ownerBefore.drawnDebt - r.baseRestored, 1, "bob dai drawn debt final balance"
        );
    ...
    function test_repay_fuzz_amounts_base_debt_no_premium(
        ...
        uint256 baseRestored;
        {
            (baseRestored,) = _calculateExactRestoreAmount(hub1, daiAssetId, bobDaiDrawnDebt, 0, daiRepayAmount);
        }
        deal(address(tokenList.dai), bob, daiRepayAmount);
        ...
        // repays only drawn debt
        assertGe(r.ownerAfter.drawnDebt, r.ownerBefore.drawnDebt - daiRepayAmount, "bob dai drawn debt final balance");
    ...
    function test_repay_multiple_reserves_fuzz_amountsAndWait(
        ...
        {
            uint256 wethSupplyAmount =
                _calcMinimumCollAmount(spoke1, _wethReserveId(spoke1), _daiReserveId(spoke1), daiInfo.borrowAmount);
            uint256 wbtcSupplyAmount = _calcMinimumCollAmount(
                spoke1, _wbtcReserveId(spoke1), _wethReserveId(spoke1), wethInfo.borrowAmount
            ) + _calcMinimumCollAmount(spoke1, _wbtcReserveId(spoke1), _wbtcReserveId(spoke1), wbtcInfo.borrowAmount)
            + _calcMinimumCollAmount(spoke1, _wbtcReserveId(spoke1), _usdxReserveId(spoke1), usdxInfo.borrowAmount);
            ...
            SpokeActions.supplyCollateral({
                spoke: spoke1, reserveId: _wbtcReserveId(spoke1), caller: bob, amount: wbtcSupplyAmount, onBehalfOf: bob
            });
        }
        ...
        // Bob borrows
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: daiInfo.borrowAmount, onBehalfOf: bob
        });
        SpokeActions.borrow({
            spoke: spoke1,
            reserveId: _wethReserveId(spoke1),
        ...
        if (daiInfo.repayAmount > 0) {
            (daiInfo.baseRestored, daiInfo.premiumRestored) = _calculateExactRestoreAmount(
                hub1, daiAssetId, bobDaiBefore.drawnDebt, bobDaiBefore.premiumDebt, daiInfo.repayAmount
            );
        ...
        if (wethInfo.repayAmount > 0) {
            (wethInfo.baseRestored, wethInfo.premiumRestored) = _calculateExactRestoreAmount(
                hub1, wethAssetId, bobWethBefore.drawnDebt, bobWethBefore.premiumDebt, wethInfo.repayAmount
            );
        ...
        if (wbtcInfo.repayAmount > 0) {
            (wbtcInfo.baseRestored, wbtcInfo.premiumRestored) = _calculateExactRestoreAmount(
                hub1, wbtcAssetId, bobWbtcBefore.drawnDebt, bobWbtcBefore.premiumDebt, wbtcInfo.repayAmount
            );
        ...
        if (usdxInfo.repayAmount > 0) {
            (usdxInfo.baseRestored, usdxInfo.premiumRestored) = _calculateExactRestoreAmount(
                hub1, usdxAssetId, bobUsdxBefore.drawnDebt, bobUsdxBefore.premiumDebt, usdxInfo.repayAmount
            );
        ...
        if (usdxInfo.repayAmount > 0) {
            assertApproxEqAbs(
                ...
                usdxInfo.baseRestored >= bobUsdxBefore.drawnDebt ? 0 : bobUsdxBefore.drawnDebt - usdxInfo.baseRestored,
                1,
        ...
        if (wbtcInfo.repayAmount > 0) {
            assertApproxEqAbs(
                ...
                wbtcInfo.baseRestored >= bobWbtcBefore.drawnDebt ? 0 : bobWbtcBefore.drawnDebt - wbtcInfo.baseRestored,
                1,
    ...
    function test_fuzz_repay_x_y_shares(uint256 borrowAmount, uint40 skipTime) public {
        ...
        // calculate weth collateral
        uint256 wethSupplyAmount =
            _calcMinimumCollAmount(spoke1, _wethReserveId(spoke1), _daiReserveId(spoke1), borrowAmount);
        ...
        SpokeActions.supply({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: alice, amount: borrowAmount, onBehalfOf: alice
        });
        ...
        assertEq(tokenList.dai.balanceOf(bob), bobDaiBalanceBefore + borrowAmount, "bob dai balance after borrow");
        ...
        assertGe(spoke1.getUserTotalDebt(_daiReserveId(spoke1), bob), borrowAmount, "bob total debt after time passed");
