...
contract SpokeLiquidationCallDustTest is SpokeLiquidationCallBaseTest {
    ...
    function setUp() public virtual override {
        ...
        _spoke.updateLiquidationConfig(
            ISpoke.LiquidationConfig({
                targetHealthFactor: 1.000001e18, healthFactorForMaxBonus: 0.9e18, liquidationBonusFactor: 0
            })
        );
        ...
        SpokeActions.approve({spoke: _spoke, reserveId: _usdxReserveId(_spoke), owner: liquidator, amount: UINT256_MAX});
        SpokeActions.approve({spoke: _spoke, reserveId: _daiReserveId(_spoke), owner: liquidator, amount: UINT256_MAX});
        SpokeActions.approve({spoke: _spoke, reserveId: _usdyReserveId(_spoke), owner: liquidator, amount: UINT256_MAX});
    ...
    function test_collateralDust_min_debtToTarget() public {
        ...
        _updateCollateralFactorAndLiquidationBonus(_spoke, _daiReserveId(_spoke), collateralFactor, liquidationBonus);
        ...
        SpokeActions.borrow({
            spoke: _spoke, reserveId: _usdyReserveId(_spoke), caller: alice, amount: 9_000e18, onBehalfOf: alice
        });
        ...
        uint256 debtRayToTarget = liquidationLogicWrapper.calculateDebtToTargetHealthFactor(
            _getCalculateDebtToTargetHealthFactorParams(_spoke, _daiReserveId(_spoke), _usdxReserveId(_spoke), alice)
        );
        ...
        _spoke.liquidationCall(_daiReserveId(_spoke), _usdxReserveId(_spoke), alice, debtRayToTarget.fromRayUp(), false);

        // valid debtToCover succeeds
        _spoke.liquidationCall(_daiReserveId(_spoke), _usdxReserveId(_spoke), alice, UINT256_MAX, false);
        vm.stopPrank();
    ...
    function test_debtToCover_exceeds_collateralValue() public {
        ...
        uint256 theoreticalRemainingDebt = _spoke.getUserTotalDebt(_usdxReserveId(_spoke), alice) - debtToCover;
        assertLt(
        ...
        _spoke.liquidationCall(_daiReserveId(_spoke), _usdxReserveId(_spoke), alice, debtToCover, false);
        vm.stopPrank();
    ...
    function test_dustDebt_allowed() public {
        ...
        SpokeActions.borrow({
            spoke: _spoke, reserveId: _usdyReserveId(_spoke), caller: alice, amount: 9_500e18, onBehalfOf: alice
        });
        ...
        assertLt(
            _convertAmountToValue(
                _spoke, _usdxReserveId(_spoke), _spoke.getUserTotalDebt(_usdxReserveId(_spoke), alice)
            ),
            LiquidationLogic.DUST_LIQUIDATION_THRESHOLD
        );
    }
    ...
    function test_dustColl_allowed() public {
        ...
        SpokeActions.borrow({
            spoke: _spoke, reserveId: _usdyReserveId(_spoke), caller: alice, amount: 10_500e18, onBehalfOf: alice
        });
        _borrowToBeAtHf(_spoke, alice, _usdxReserveId(_spoke), 0.999e18);
        ...
        assertLt(_getCollateralValue(_spoke, _daiReserveId(_spoke), alice), LiquidationLogic.DUST_LIQUIDATION_THRESHOLD);
    }

    function _calculateDebtToTargetValue(ISpoke spoke, uint256 collateralReserveId, uint256 debtReserveId, address user)
        internal
        returns (uint256)
    {
        return _convertAmountToValue(
            ...
            liquidationLogicWrapper.calculateDebtToTargetHealthFactor(
                _getCalculateDebtToTargetHealthFactorParams(spoke, collateralReserveId, debtReserveId, user)
            )
        );
    }

    function _getCollateralValue(ISpoke spoke, uint256 collateralReserveId, address user)
        internal
        view
        returns (uint256)
    {
        return _convertAmountToValue(spoke, collateralReserveId, spoke.getUserSuppliedAssets(collateralReserveId, user));
    }
}
