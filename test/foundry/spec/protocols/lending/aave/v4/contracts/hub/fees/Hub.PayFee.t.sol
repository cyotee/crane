...
contract HubPayFeeTest is Base {
    ...
    function test_payFee_revertsWith_underflow_added_shares_exceeded() public {
        ...
        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: addAmount, user: alice});
    ...
    function test_payFee_fuzz_with_interest(uint256 addAmount, uint256 feeShares, uint256 skipTime) public {
        ...
        uint256 feeReceiverSharesBefore = hub1.getSpokeAddedShares(daiAssetId, _getFeeReceiver(hub1, daiAssetId));

        vm.expectEmit(address(hub1));
        emit IHubBase.TransferShares(daiAssetId, address(spoke1), _getFeeReceiver(hub1, daiAssetId), feeShares);
        ...
        uint256 feeReceiverSharesAfter = hub1.getSpokeAddedShares(daiAssetId, _getFeeReceiver(hub1, daiAssetId));

        assertEq(spokeSharesAfter, spokeSharesBefore - feeShares, "spoke supplied shares after");
        assertEq(feeReceiverSharesAfter, feeReceiverSharesBefore + feeShares, "fee receiver supplied shares after");
    }
}
