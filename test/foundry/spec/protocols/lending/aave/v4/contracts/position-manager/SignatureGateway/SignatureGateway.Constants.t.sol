...
contract SignatureGatewayConstantsTest is SignatureGatewayBaseTest {
    ...
    function test_eip712Domain() public {
        SignatureGateway instance = new SignatureGateway{salt: bytes32(vm.randomUint())}(vm.randomAddress());
        (
            bytes1 fields,
            string memory name,
    ...
    function test_supply_typeHash() public view {
        assertEq(gateway.SUPPLY_TYPEHASH(), vm.eip712HashType(EIP712Types.TYPE_Supply));
        assertEq(
    ...
    function test_withdraw_typeHash() public view {
        assertEq(gateway.WITHDRAW_TYPEHASH(), vm.eip712HashType(EIP712Types.TYPE_Withdraw));
        assertEq(
    ...
    function test_borrow_typeHash() public view {
        assertEq(gateway.BORROW_TYPEHASH(), vm.eip712HashType(EIP712Types.TYPE_Borrow));
        assertEq(
    ...
    function test_repay_typeHash() public view {
        assertEq(gateway.REPAY_TYPEHASH(), vm.eip712HashType(EIP712Types.TYPE_Repay));
        assertEq(
    ...
    function test_setUsingAsCollateral_typeHash() public view {
        assertEq(gateway.SET_USING_AS_COLLATERAL_TYPEHASH(), vm.eip712HashType(EIP712Types.TYPE_SetUsingAsCollateral));
        assertEq(
    ...
    function test_updateUserRiskPremium_typeHash() public view {
        assertEq(gateway.UPDATE_USER_RISK_PREMIUM_TYPEHASH(), vm.eip712HashType(EIP712Types.TYPE_UpdateUserRiskPremium));
        assertEq(
