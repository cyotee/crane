...
} from "@crane/test/foundry/spec/protocols/lending/aave/v4/helpers/mocks/ConfigPermissionsMapWrapper.sol";
...
contract ConfigPermissionsMapTest is Test {
    ...
    function test_getConfigPermissionValues(uint8 rawPermissions) public view {
        ConfigPermissions perms = _sanitizePermissions(rawPermissions);
        IConfigPositionManager.ConfigPermissionValues memory values = w.getConfigPermissionValues(perms);
    ...
    function _changeStatus(ConfigPermissions perms, uint8 mask, bool status) internal pure returns (uint8) {
        return
            status ? (uint8(ConfigPermissions.unwrap(perms)) | mask) : (uint8(ConfigPermissions.unwrap(perms)) & ~mask);
    }
}
