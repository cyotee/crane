...
contract HubReportDeficitTest is Base {
    ...
    function test_reportDeficit_revertsWith_SpokeNotActive(address caller) public {
        vm.assume(!hub1.getSpoke(usdxAssetId, caller).active && caller != _getProxyAdminAddress(address(hub1)));
    ...
    function test_reportDeficit_fuzz_revertsWith_SurplusDrawnDeficitReported(uint256 drawnAmount) public {
        ...
        (uint256 spokePremiumShares, int256 spokePremiumOffsetRay) =
            hub1.getSpokePremiumData(usdxAssetId, address(spoke1));
        uint256 spokePremiumRay = _calculatePremiumDebtRay(hub1, usdxAssetId, spokePremiumShares, spokePremiumOffsetRay);
    ...
    function test_reportDeficit_fuzz_revertsWith_SurplusPremiumRayDeficitReported(uint256 drawnAmount) public {
        drawnAmount = bound(drawnAmount, 1, MAX_SUPPLY_AMOUNT_USDX);
        ...
        uint256 spokePremiumRay =
            _calculatePremiumDebtRay(hub1, usdxAssetId, spokeData.premiumShares, spokeData.premiumOffsetRay);

        uint256 drawnDeficit = vm.randomUint(0, drawn);
        uint256 premiumDeficitRay = vm.randomUint(spokePremiumRay + 1, 2 ** 255 - 1);

        vm.expectRevert(abi.encodeWithSelector(IHub.SurplusPremiumRayDeficitReported.selector, spokePremiumRay));
        vm.prank(address(spoke1));
        hub1.reportDeficit(
            usdxAssetId,
            drawnDeficit,
            ...
            IHubBase.PremiumDelta({
                sharesDelta: 0, offsetRayDelta: premiumDeficitRay.toInt256(), restoredPremiumRay: premiumDeficitRay
            })
        );
    }
    ...
    function test_reportDeficit_halted() public {
        // draw usdx liquidity to be restored
        _drawLiquidity({
            hub: hub1, assetId: usdxAssetId, amount: 1, withPremium: true, skipTime: true, spoke: address(spoke1)
        });
    ...
    function test_reportDeficit_with_premium() public {
        ...
        test_reportDeficit_fuzz_with_premium({
            drawnAmount: drawnAmount, baseAmount: drawnAmount / 2, premiumAmountRay: 0, skipTime: 365 days
        });
    }
    ...
    function test_reportDeficit_fuzz_with_premium(
        ...
        params.totalDeficitRay = params.drawnShares * hub1.getAssetDrawnIndex(usdxAssetId) + premiumAmountRay;
        ...
        if (premiumDelta.restoredPremiumRay > params.premiumRay) {
        ...
        } else if (expectedNewPremiumShares > (params.drawnSharesBefore - params.drawnShares).percentMulUp(1000_00)) {
            ...
            emit IHubBase.ReportDeficit(
                usdxAssetId, address(spoke1), params.drawnShares, premiumDelta, params.totalDeficitRay
            );
            vm.prank(address(spoke1));
            (uint256 returnedDrawnShares, uint256 returnedDeficitAmount) =
                hub1.reportDeficit(usdxAssetId, baseAmount, premiumDelta);

            assertEq(returnedDrawnShares, params.drawnShares, "returned drawn shares");
            assertEq(returnedDeficitAmount, params.totalDeficitRay.fromRayUp(), "returned deficit amount");
            ...
            assertApproxEqAbs(params.premiumRayAfter, params.premiumRay - premiumAmountRay, 1, "premium debt");
            ...
            assertEq(params.deficitRayAfter, params.deficitRayBefore + params.totalDeficitRay, "deficit accounting");
            assertGe(
                params.supplyExchangeRateAfter, params.supplyExchangeRateBefore, "supply exchange rate should increase"
            );
