...
contract SpokeUpgradeableTest is Base {
    ...
    function test_proxy_reinitialization_fuzz(uint64 initialRevision) public {
        ...
        ITransparentUpgradeableProxy(address(spokeProxy))
            .upgradeToAndCall(address(spokeImpl2), _getInitializeCalldata(address(accessManager)));
    ...
    function test_proxy_constructor_revertsWith_InvalidInitialization_ZeroRevision() public {
        ...
        new TransparentUpgradeableProxy(
            address(spokeImpl), proxyAdminOwner, abi.encodeCall(ISpokeInstance.initialize, address(accessManager))
        );
    }

    function test_proxy_constructor_fuzz_revertsWith_InvalidInitialization(uint64 initialRevision) public {
        initialRevision = uint64(bound(initialRevision, 1, type(uint64).max));

        ISpokeInstance spokeImpl = _deployMockSpokeInstance(initialRevision);
        ITransparentUpgradeableProxy spokeProxy =
            ITransparentUpgradeableProxy(address(_deploySpokeProxy(address(spokeImpl))));
        ...
        vm.expectRevert(Initializable.InvalidInitialization.selector);
        vm.prank(ProxyHelper.getProxyAdmin(address(spokeProxy)));
        spokeProxy.upgradeToAndCall(address(spokeImpl2), _getInitializeCalldata(address(accessManager)));
    }

    function test_proxy_constructor_revertsWith_InvalidAddress() public {
        ISpokeInstance spokeImpl =
        ...
        new TransparentUpgradeableProxy(address(spokeImpl), proxyAdminOwner, _getInitializeCalldata(address(0)));
    }
    ...
    function _deploySpokeProxy(address spokeImpl) internal returns (ISpoke) {
        return ISpoke(
            address(
                new TransparentUpgradeableProxy(
                    spokeImpl, proxyAdminOwner, _getInitializeCalldata(address(accessManager))
                )
            )
        );
    }
    ...
    function _deployMockSpokeInstance(uint64 revision) internal returns (ISpokeInstance) {
        return ISpokeInstance(address(new MockSpokeInstance(revision, oracle, MAX_ALLOWED_USER_RESERVES_LIMIT)));
    }
}
