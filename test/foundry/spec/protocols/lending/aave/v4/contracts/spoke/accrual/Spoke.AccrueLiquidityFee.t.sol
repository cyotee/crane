...
contract SpokeAccrueLiquidityFeeTest is Base {
    ...
    function test_accrueLiquidityFee_fuzz_BorrowAmountAndSkipTime(uint256 borrowAmount, uint40 skipTime) public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: reserveId, caller: bob, amount: supplyAmount, onBehalfOf: bob
        });
        SpokeActions.borrow({spoke: spoke1, reserveId: reserveId, caller: bob, amount: borrowAmount, onBehalfOf: bob});
        ...
        ISpoke.UserPosition memory bobPosition = spoke1.getUserPosition(reserveId, bob);
        {
            ...
            uint256 expectedPremiumDebt =
                _calculatePremiumDebt(hub1, assetId, expectedPremiumShares, bobPosition.premiumOffsetRay);
            _assertOnlyOneUserDebt(spoke1, reserveId, bob, drawnDebt, expectedPremiumDebt, "after accrual");
        }
        ...
        assertApproxEqAbs(
            hub1.getSpokeAddedShares(assetId, address(treasurySpoke)), expectedFeeShares, 1, "treasury shares"
        );
    ...
    function test_accrueLiquidityFee_exact() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: reserveId, caller: alice, amount: supplyAmount, onBehalfOf: alice
        });
        SpokeActions.borrow({
            spoke: spoke1, reserveId: reserveId, caller: alice, amount: borrowAmount, onBehalfOf: alice
        });
        ...
        _assertSpokeDebt(
            spoke1, reserveId, expectedDrawnDebt, expectedPremiumDebt, "after base and premium debt accrual"
        );
        assertEq(
        ...
        _assertSpokeDebt(spoke1, reserveId, expectedDrawnDebt, expectedPremiumDebt, "after drawn debt accrual");
        assertEq(
            hub1.getSpokeAddedShares(assetId, address(treasurySpoke)),
            hub1.previewAddByAssets(assetId, expectedTreasuryFees),
    ...
    function test_accrueLiquidityFee() public {
        ...
        uint256 expectedTreasuryFees = (expectedDrawnDebtAccrual + expectedPremiumDebt).percentMulUp(liquidityFee);
    ...
    function test_accrueLiquidityFee_setUsingAsCollateral() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: reserveId2, caller: alice, amount: supplyAmount2, onBehalfOf: alice
        });
        SpokeActions.borrow({
            spoke: spoke1, reserveId: reserveId, caller: alice, amount: borrowAmount, onBehalfOf: alice
        });
    ...
    function test_accrueLiquidityFee_maxLiquidityFee() public {
        ...
        uint256 expectedDrawnDebtAccrual = borrowAmount.percentMulUp(rate);
        uint256 expectedDrawnDebt = borrowAmount + expectedDrawnDebtAccrual;
        uint256 expectedPremiumDebt = expectedDrawnDebtAccrual.percentMulUp(_getCollateralRisk(spoke1, reserveId));
        uint256 expectedTreasuryFees = (expectedDrawnDebtAccrual + expectedPremiumDebt).percentMulUp(liquidityFee);
        _mockDrawnRateBps({irStrategy: address(irStrategy), drawnRateBps: rate});
        ...
        assertEq(spoke1.getUserSuppliedAssets(reserveId, alice), supplyAmount, "alice does not earn anything");
        assertEq(
