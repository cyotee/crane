...
contract GiverPositionManagerTest is Base {
    ...
    function test_supplyOnBehalfOf_fuzz(uint256 amount) public {
        ...
        emit ISpoke.Supply(
            _daiReserveId(spoke1), address(positionManager), alice, hub1.previewAddByAssets(daiAssetId, amount), amount
        );
        vm.expectEmit(address(positionManager));
        emit IGiverPositionManager.SupplyOnBehalfOf(
            address(spoke1), bob, alice, _daiReserveId(spoke1), hub1.previewAddByAssets(daiAssetId, amount), amount
        );
        vm.prank(bob);
    ...
    function test_repayOnBehalfOf_fuzz(uint256 repayAmount) public {
        ...
        SpokeActions.supply({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: 150e18, onBehalfOf: bob
        });
        SpokeActions.borrow({
        ...
        (uint256 baseRestored,) =
            _calculateExactRestoreAmount(hub1, daiAssetId, userDrawnDebt, userPremiumDebt, repayAmount);

        {
            IHubBase.PremiumDelta memory expectedPremiumDelta =
                _getExpectedPremiumDeltaForRestore(spoke1, alice, _daiReserveId(spoke1), repayAmount);
    ...
    function test_repayOnBehalfOf_fuzz_withInterest(uint256 repayAmount, uint256 elapsedTime) public {
        ...
        (uint256 newUserDrawnDebt, uint256 newUserPremiumDebt) = spoke1.getUserDebt(_daiReserveId(spoke1), alice);

        assertApproxEqAbs(
            newUserDrawnDebt + newUserPremiumDebt, userDrawnDebt + userPremiumDebt - (baseRestored + premiumRestored), 2
        );
        assertApproxEqAbs(
            tokenList.dai.balanceOf(address(hub1)), hubBalanceBefore + (baseRestored + premiumRestored), 2
        );
        assertEq(tokenList.dai.balanceOf(alice), userBalanceBefore);
        assertApproxEqAbs(tokenList.dai.balanceOf(bob), callerBalanceBefore - (baseRestored + premiumRestored), 1);
        assertEq(tokenList.dai.balanceOf(address(positionManager)), 0);
        assertEq(tokenList.dai.allowance(address(positionManager), address(hub1)), 0);
    }
    ...
    function test_multicall() public {
        ...
        calls[0] = abi.encodeWithSignature(
            "supplyOnBehalfOf(address,uint256,uint256,address)", address(spoke1), _daiReserveId(spoke1), amount, alice
        );
        calls[1] = abi.encodeWithSignature(
            "supplyOnBehalfOf(address,uint256,uint256,address)", address(spoke1), _daiReserveId(spoke1), amount, carol
        );
        ...
        assertEq(spoke1.getUserSuppliedAssets(_daiReserveId(spoke1), alice), aliceSuppliedAmountBefore + amount);
        assertEq(spoke1.getUserSuppliedAssets(_daiReserveId(spoke1), carol), carolSuppliedAmountBefore + amount);
    }
}
