...
contract TreasurySpokeUpgradeableTest is Base {
    ...
    function test_proxy_constructor_fuzz(uint64 revision) public {
        ...
        ITreasurySpoke proxy = ITreasurySpoke(
            AaveV4TestOrchestration.proxify(
                address(impl), proxyAdminOwner, abi.encodeCall(MockTreasurySpokeInstance.initialize, (TREASURY_ADMIN))
            )
        );
    ...
    function test_proxy_reinitialization_fuzz(uint64 initialRevision) public {
        ...
        proxy.upgradeToAndCall(address(impl2), abi.encodeCall(MockTreasurySpokeInstance.initialize, (TREASURY_ADMIN)));
    ...
    function test_proxy_constructor_fuzz_revertsWith_InvalidInitialization(uint64 initialRevision) public {
        ...
        vm.expectRevert(Initializable.InvalidInitialization.selector);
        vm.prank(_getProxyAdminAddress(address(proxy)));
        proxy.upgradeToAndCall(address(impl), abi.encodeCall(MockTreasurySpokeInstance.initialize, (TREASURY_ADMIN)));
    ...
    function test_proxy_constructor_revertsWith_InvalidAddress() public {
        ...
        AaveV4TestOrchestration.proxify(
            address(impl), proxyAdminOwner, abi.encodeCall(TreasurySpokeInstance.initialize, (address(0)))
        );
    }
    ...
    function test_proxy_reinitialization_revertsWith_CallerNotProxyAdmin() public {
        ...
        ITransparentUpgradeableProxy proxy = ITransparentUpgradeableProxy(
            AaveV4TestOrchestration.proxify(
                address(impl), proxyAdminOwner, abi.encodeCall(TreasurySpokeInstance.initialize, (TREASURY_ADMIN))
            )
        );
        ...
        vm.expectRevert();
        vm.prank(_makeUser());
        proxy.upgradeToAndCall(address(impl2), abi.encodeCall(TreasurySpokeInstance.initialize, (TREASURY_ADMIN)));
    }

    function _deployMockTreasurySpokeInstance(uint64 revision) internal returns (MockTreasurySpokeInstance) {
