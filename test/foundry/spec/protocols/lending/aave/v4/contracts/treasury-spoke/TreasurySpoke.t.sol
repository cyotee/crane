...
contract TreasurySpokeTest is Base {
    ...
    function setUp() public virtual override {
        ...
        ISpoke.DynamicReserveConfig memory dynDaiHub2Config =
        ...
        hub2DaiReserveId = spoke1.addReserve(
            address(hub2), daiAssetId, _deployMockPriceFeed(spoke1, 1e8), daiHub2Config, dynDaiHub2Config
        );
    ...
    function test_deploy_reverts_on_invalid_params() public {
        ...
        AaveV4TestOrchestration.proxify(
            address(impl), ADMIN, abi.encodeCall(TreasurySpokeInstance.initialize, (address(0)))
        );
    }
    ...
    function test_withdraw_fuzz_amount_interestOnly(uint256 amount) public {
        ...
        uint256 suppliedSharesBefore = treasurySpoke.getSuppliedShares(address(hub1), address(tokenList.dai));
        uint256 suppliedAssetsBefore = treasurySpoke.getSuppliedAssets(address(hub1), address(tokenList.dai));
        ...
        assertEq(suppliedSharesBefore, treasurySpoke.getSuppliedShares(address(hub1), address(tokenList.dai)));
        uint256 interest = treasurySpoke.getSuppliedAssets(address(hub1), address(tokenList.dai)) - suppliedAssetsBefore;
    ...
    function test_withdraw_fuzz_amount_interestAndFees(uint256 amount) public {
        ...
        assertGe(treasurySpoke.getSuppliedShares(address(hub1), address(tokenList.dai)), suppliedSharesBefore);
        uint256 interestAndFees =
            treasurySpoke.getSuppliedAssets(address(hub1), address(tokenList.dai)) - suppliedAssetsBefore;
    ...
    function test_transfer_revertsWith_ERC20InsufficientBalance(uint256 amount) public {
        ...
        vm.prank(TREASURY_ADMIN);
        vm.expectRevert(
            abi.encodeWithSelector(IERC20Errors.ERC20InsufficientBalance.selector, address(treasurySpoke), 0, amount)
        );
    ...
    function test_transfer_fuzz(address recipient, uint256 amount, uint256 transferAmount) public {
        ...
        emit Transfer(address(treasurySpoke), recipient, transferAmount);
        vm.prank(TREASURY_ADMIN);
    ...
    function test_withdraw_multiHub_sameAsset(uint256 amount) public {
        ...
        assertEq(treasurySpoke.getSuppliedAssets(address(hub1), address(tokenList.dai)), amount - amount / 2);
        assertEq(treasurySpoke.getSuppliedAssets(address(hub2), address(tokenList.dai)), amount);
    ...
    function test_withdraw_fuzz_maxLiquidityFee(uint256 reserveId, uint256 amount, uint256 skipTime) public {
        ...
        // create debt
        address tempUser = _openDebtPosition({spoke: spoke1, reserveId: reserveId, amount: amount, withPremium: true});
        ...
        assertApproxEqAbs(
            fees, hub.getSpokeAddedAssets(assetId, address(treasurySpoke)), 3, "treasury spoke supplied amount on spoke"
        );
        ...
        if (fees > 0) {
            ...
            SpokeActions.repay({
                spoke: spoke1, reserveId: reserveId, caller: tempUser, amount: UINT256_MAX, onBehalfOf: tempUser
            });
            vm.prank(TREASURY_ADMIN);
            ...
            assertEq(
                0, hub.getSpokeAddedAssets(assetId, address(treasurySpoke)), "treasury spoke remaining supplied amount"
            );
        }
    }
    ...
    function test_getters() public {
        ...
        _openDebtPosition({spoke: spoke1, reserveId: hub2DaiReserveId, amount: amount, withPremium: true});
        ...
        assertApproxEqAbs(
            treasurySpoke.getSuppliedAssets(address(hub1), address(tokenList.dai)), fees, 1, "reserve supplied assets"
        );
        assertApproxEqAbs(
            treasurySpoke.getSuppliedShares(address(hub1), address(tokenList.dai)),
