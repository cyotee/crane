...
contract SpokeMulticall is Base {
    ...
    function test_multicall_supply_setCollateral() public {
        ...
        vm.expectEmit(address(spoke1));
        emit ISpoke.SetUsingAsCollateral({reserveId: daiReserveId, caller: bob, user: bob, usingAsCollateral: true});
    ...
    function test_multicall_supply_updateUserRp() public {
        ...
        SpokeActions.borrow({
            spoke: spoke2, reserveId: _usdzReserveId(spoke2), caller: bob, amount: 1000e18, onBehalfOf: bob
        });

        // Check bob's premium drawn shares as proxy for user rp
        uint256 bobpremiumSharesBefore = spoke2.getUserPosition(_usdzReserveId(spoke2), bob).premiumShares;
        ...
        emit ISpoke.SetUsingAsCollateral({
            reserveId: _daiReserveId(spoke2), caller: bob, user: bob, usingAsCollateral: true
        });
        vm.expectEmit(address(spoke2));
        ...
        uint256 bobpremiumSharesAfter = spoke2.getUserPosition(_usdzReserveId(spoke2), bob).premiumShares;

        assertLt(bobpremiumSharesAfter, bobpremiumSharesBefore, "Bob premium drawn shares should decrease");
    }
    ...
    function test_multicall_addMultipleReserves() public {
        ...
        ISpoke.DynamicReserveConfig memory usdzDynConfig =
            ISpoke.DynamicReserveConfig({collateralFactor: 88_00, maxLiquidationBonus: 100_00, liquidationFee: 0});
        ISpoke.ReserveConfig memory usdaConfig = _getDefaultReserveConfig(5_00);
        ISpoke.DynamicReserveConfig memory usdaDynConfig =
            ISpoke.DynamicReserveConfig({collateralFactor: 70_00, maxLiquidationBonus: 100_00, liquidationFee: 0});
