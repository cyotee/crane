...
contract KeyValueListTest is Test {
    ...
    function test_fuzz_sortByKey_with_collision(uint256[] memory seed) public pure {
        ...
        KeyValueList.List memory list = KeyValueList.init(seed.length);
        for (uint256 i; i < seed.length; ++i) {
            list.add(i, _truncateKey(collisionKeys[seed[i] % collisionKeys.length]), _truncateValue(seed[i]));
        }
        list.sortByKey();
        _assertSortedOrder(list);
    }
    ...
    function _generateRandomUint256Array(uint256 size, uint256 seed, uint256 lowerBound, uint256 upperBound)
        internal
        pure
        returns (uint256[] memory)
    {
        ...
        for (uint256 i; i < size; ++i) {
            result[i] = (uint256((keccak256(abi.encode(seed + i)))) % (upperBound - lowerBound + 1)) + lowerBound;
        }
        return result;
    }
}
...
contract KeyValueListWrapper {
    ...
    function add(KeyValueList.List memory list, uint256 idx, uint256 key, uint256 value)
        external
        pure
        returns (KeyValueList.List memory returnList)
    {
        returnList = list;
