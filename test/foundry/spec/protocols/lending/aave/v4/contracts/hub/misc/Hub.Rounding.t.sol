    ...
    function test_sharePriceWithMultipleDonations() public {
        ...
        for (uint256 i = 0; i < 1e4; ++i) {
            ...
            SpokeActions.withdraw({
                spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: alice, amount: 1, onBehalfOf: alice
            });

            assertLt(
                _getAddExRate(hub1, daiAssetId),
                initialSharePrice + initialSharePrice.mulDiv(i + 1, SharesMath.VIRTUAL_ASSETS, Math.Rounding.Ceil)
            );
        }
    }
}
