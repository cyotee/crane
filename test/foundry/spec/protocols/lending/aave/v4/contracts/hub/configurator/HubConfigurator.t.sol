...
contract HubConfiguratorTest is Base {
    ...
    function test_addAsset_fuzz_revertsWith_AccessManagedUnauthorized(address caller) public {
        ...
        _addAsset({
            ...
            decimals: vm.randomUint(MIN_ALLOWED_UNDERLYING_DECIMALS, MAX_ALLOWED_UNDERLYING_DECIMALS).toUint8(),
    ...
    function test_addAsset_fuzz_revertsWith_InvalidAssetDecimals(
        ...
        vm.prank(HUB_CONFIGURATOR_ADMIN);
        _addAsset(fetchErc20Decimals, underlying, decimals, feeReceiver, liquidityFee, irStrategy, _encodedIrData);
    }

    function test_addAsset_revertsWith_InvalidAddress_underlying() public {
        uint8 decimals = uint8(vm.randomUint(MIN_ALLOWED_UNDERLYING_DECIMALS, MAX_ALLOWED_UNDERLYING_DECIMALS));
    ...
    function test_addAsset_fuzz(
        ...
        decimals = bound(decimals, MIN_ALLOWED_UNDERLYING_DECIMALS, MAX_ALLOWED_UNDERLYING_DECIMALS).toUint8();
        ...
        IHub.SpokeConfig memory expectedSpokeConfig = IHub.SpokeConfig({
            active: true, halted: false, addCap: MAX_ALLOWED_SPOKE_CAP, drawCap: 0, riskPremiumThreshold: 0
        });
        ...
        vm.expectCall(
            address(hub1), abi.encodeCall(IHub.updateAssetConfig, (hub1.getAssetCount(), expectedConfig, new bytes(0)))
        );
    ...
    function test_updateLiquidityFee_revertsWith_InvalidLiquidityFee() public {
        ...
        uint16 liquidityFee = uint16(vm.randomUint(PercentageMath.PERCENTAGE_FACTOR + 1, type(uint16).max));
    ...
    function test_updateLiquidityFee_fuzz(uint256 assetId, uint16 liquidityFee) public {
        ...
        vm.expectCall(address(hub1), abi.encodeCall(IHub.updateAssetConfig, (_assetId, expectedConfig, new bytes(0))));
    ...
    function test_updateFeeReceiver_fuzz_revertsWith_AccessManagedUnauthorized(address caller) public {
        _assumeNonHubConfiguratorAdmin(caller);
        vm.expectRevert(abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, caller));
        vm.prank(caller);
    ...
    function test_updateFeeReceiver_fuzz(address feeReceiver) public {
        ...
        if (feeReceiver != oldConfig.feeReceiver) {
            if (!hub1.isSpokeListed(_assetId, feeReceiver)) {
                ...
                vm.expectCall(
                    address(hub1), abi.encodeCall(IHub.updateAssetConfig, (_assetId, expectedConfig, new bytes(0)))
                );
            } else {
    ...
    function test_updateFeeReceiver_WithdrawFromOldSpoke() public {
        ...
        _addAndDrawLiquidity(hub1, daiAssetId, bob, address(spoke1), 1000e18, bob, address(spoke1), 100e18, 365 days);

        assertGe(hub1.getSpokeAddedShares(daiAssetId, address(treasurySpoke)), 0);

        // Change the fee receiver
        address newTreasurySpoke =
            AaveV4TestOrchestration.deployTestTreasurySpoke({owner: HUB_ADMIN, salt: bytes32("newTreasurySpoke1")});
        vm.prank(HUB_CONFIGURATOR_ADMIN);
        hubConfigurator.updateFeeReceiver(address(hub1), daiAssetId, newTreasurySpoke);

        assertEq(hub1.getAssetConfig(daiAssetId).feeReceiver, newTreasurySpoke, "new fee receiver updated");
        assertTrue(hub1.getSpokeConfig(daiAssetId, address(treasurySpoke)).active, "old fee receiver is not active");
        ...
        assertEq(hub1.getSpokeAddedAssets(daiAssetId, address(treasurySpoke)), 0, "old treasury spoke should be empty");

        // Accrue more fees, this time to new fee receiver
        skip(365 days);
        HubActions.mintFeeShares({hub: hub1, assetId: daiAssetId, caller: ADMIN});

        assertGt(hub1.getSpokeAddedAssets(daiAssetId, newTreasurySpoke), 0, "new fee receiver should have accrued fees");
        assertEq(hub1.getSpokeAddedAssets(daiAssetId, address(treasurySpoke)), 0, "old fee receiver should be empty");
    }
    ...
    function test_updateFeeReceiver_correctAccruals() public {
        ...
        assertEq(hub1.getAssetConfig(daiAssetId).feeReceiver, address(treasurySpoke), "old fee receiver mismatch");
        ...
        // Change the fee receiver
        address newTreasurySpoke =
            AaveV4TestOrchestration.deployTestTreasurySpoke({owner: HUB_ADMIN, salt: bytes32("newTreasurySpoke2")});
        vm.prank(HUB_CONFIGURATOR_ADMIN);
        hubConfigurator.updateFeeReceiver(address(hub1), daiAssetId, newTreasurySpoke);

        // Ensure fee receiver was updated
        assertEq(hub1.getAssetConfig(daiAssetId).feeReceiver, newTreasurySpoke, "new fee receiver mismatch");
        ...
        vm.startPrank(address(treasurySpoke));
        hub1.remove(daiAssetId, hub1.getSpokeAddedAssets(daiAssetId, address(treasurySpoke)), TREASURY_ADMIN);
        vm.stopPrank();
        assertEq(hub1.getSpokeAddedShares(daiAssetId, address(treasurySpoke)), 0, "old fee receiver should be empty");
    }
    ...
    function test_updateFeeConfig_fuzz_revertsWith_AccessManagedUnauthorized(address caller) public {
        ...
        hubConfigurator.updateFeeConfig({
            hub: address(hub1), assetId: vm.randomUint(), liquidityFee: vm.randomUint(), feeReceiver: vm.randomAddress()
        });
    }
    ...
    function test_updateFeeConfig_fuzz(uint256 assetId_, uint16 liquidityFee, address feeReceiver) public {
        ...
        if (oldConfig.feeReceiver != feeReceiver) {
            if (!hub1.isSpokeListed(assetId_, feeReceiver)) {
                expectedConfig.feeReceiver = feeReceiver;
                vm.expectCall(
                    address(hub1), abi.encodeCall(IHub.updateAssetConfig, (assetId_, expectedConfig, new bytes(0)))
                );
            } else {
    ...
    function test_updateInterestRateStrategy_fuzz_revertsWith_AccessManagedUnauthorized(address caller) public {
        _assumeNonHubConfiguratorAdmin(caller);
        vm.expectRevert(abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, caller));
        vm.prank(caller);
        hubConfigurator.updateInterestRateStrategy(address(hub1), vm.randomUint(), vm.randomAddress(), _encodedIrData);
    }
    ...
    function test_updateInterestRateStrategy() public {
        ...
        vm.expectCall(address(hub1), abi.encodeCall(IHub.updateAssetConfig, (_assetId, expectedConfig, _encodedIrData)));
    ...
    function test_updateInterestRateStrategy_revertsWith_InvalidInterestRateStrategy() public {
        ...
        vm.prank(HUB_CONFIGURATOR_ADMIN);
        hubConfigurator.updateInterestRateStrategy(address(hub1), _assetId, address(irStrategy), _encodedIrData);
    }

    function test_updateReinvestmentController_fuzz_revertsWith_AccessManagedUnauthorized(address caller) public {
        _assumeNonHubConfiguratorAdmin(caller);
        vm.expectRevert(abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, caller));
        vm.prank(caller);
        hubConfigurator.updateReinvestmentController(address(hub1), vm.randomUint(), vm.randomAddress());
    }
    ...
    function test_resetAssetCaps() public {
        for (uint256 i; i < spokeAddresses.length; i++) {
            ...
            vm.expectCall(
                address(hub1), abi.encodeCall(IHub.updateSpokeConfig, (_assetId, spokeAddresses[i], spokeConfig))
            );
    ...
    function test_addSpoke() public {
        ...
        IHub.SpokeConfig memory daiSpokeConfig =
            IHub.SpokeConfig({active: true, halted: false, addCap: 1, drawCap: 2, riskPremiumThreshold: 22});
    ...
    function test_addSpokeToAssets_revertsWith_AccessManagedUnauthorized() public {
        vm.expectRevert(abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, alice));
        vm.prank(alice);
        hubConfigurator.addSpokeToAssets(address(hub1), vm.randomAddress(), new uint256[](0), new IHub.SpokeConfig[](0));
    }
    ...
    function test_addSpokeToAssets_revertsWith_MismatchedConfigs() public {
        ...
        spokeConfigs[0] =
            IHub.SpokeConfig({addCap: 1, drawCap: 2, active: true, halted: false, riskPremiumThreshold: 0});
        spokeConfigs[1] =
            IHub.SpokeConfig({addCap: 3, drawCap: 4, active: true, halted: false, riskPremiumThreshold: 0});
        spokeConfigs[2] =
            IHub.SpokeConfig({addCap: 5, drawCap: 6, active: true, halted: false, riskPremiumThreshold: 0});
    ...
    function test_addSpokeToAssets() public {
        ...
        IHub.SpokeConfig memory daiSpokeConfig =
            IHub.SpokeConfig({active: true, halted: false, addCap: 1, drawCap: 2, riskPremiumThreshold: 0});
        IHub.SpokeConfig memory wethSpokeConfig =
            IHub.SpokeConfig({active: true, halted: false, addCap: 3, drawCap: 4, riskPremiumThreshold: 0});
    ...
    function test_updateSpokeHalted() public {
        ...
        for (uint256 i = 0; i < 2; ++i) {
            ...
            vm.expectCall(address(hub1), abi.encodeCall(IHub.updateSpokeConfig, (_assetId, spoke, expectedSpokeConfig)));
            vm.prank(HUB_CONFIGURATOR_ADMIN);
    ...
    function test_updateSpokeRiskPremiumThreshold_revertsWith_AccessManagedUnauthorized() public {
        vm.expectRevert(abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, alice));
        vm.prank(alice);
        hubConfigurator.updateSpokeRiskPremiumThreshold(address(hub1), _assetId, spokeAddresses[0], 100);
    }
    ...
    function test_updateSpokeRiskPremiumThreshold() public {
        ...
        vm.expectCall(address(hub1), abi.encodeCall(IHub.updateSpokeConfig, (_assetId, spoke, expectedSpokeConfig)));
        vm.prank(HUB_CONFIGURATOR_ADMIN);
        hubConfigurator.updateSpokeRiskPremiumThreshold(address(hub1), _assetId, spoke, newRiskPremiumThreshold);
        assertEq(hub1.getSpokeConfig(_assetId, spoke), expectedSpokeConfig);
    }
    ...
    function test_deactivateSpoke() public {
        ...
        for (uint256 assetId = 0; assetId < 4; ++assetId) {
            ...
            vm.expectCall(
                address(hub1), abi.encodeCall(IHub.updateSpokeConfig, (assetId, address(spoke3), expectedSpokeConfig))
            );
        }
    ...
    function test_updateInterestRateData() public {
        ...
        vm.expectCall(address(hub1), abi.encodeCall(IHub.setInterestRateData, (_assetId, abi.encode(newIrData))));
        vm.prank(HUB_CONFIGURATOR_ADMIN);
        ...
        if (fetchErc20Decimals) {
            ...
            return
                hubConfigurator.addAsset(
                    address(hub1), underlying, feeReceiver, liquidityFee, irStrategy, encodedIrData
                );
        } else {
            return hubConfigurator.addAssetWithDecimals(
                address(hub1), underlying, decimals, feeReceiver, liquidityFee, irStrategy, encodedIrData
            );
        }
    }
    ...
    function _assumeNonHubConfiguratorAdmin(address caller) internal view {
        vm.assume(
            caller != HUB_CONFIGURATOR_ADMIN && caller != address(accessManager) && caller != ADMIN
                && caller != HUB_ADMIN
        );
    }
}
