...
contract HubRestoreTest is Base {
    ...
    function test_restore_revertsWith_SurplusDrawnRestored() public {
        ...
        // spoke1 add weth
        HubActions.add({hub: hub1, assetId: wethAssetId, caller: address(spoke1), amount: wethAmount, user: alice});

        // spoke2 add dai
        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke2), amount: daiAmount, user: bob});

        // spoke1 draw liquidity
        HubActions.draw({hub: hub1, assetId: daiAssetId, to: alice, caller: address(spoke1), amount: drawAmount});

        (uint256 drawn, uint256 premium) = hub1.getSpokeOwed(daiAssetId, address(spoke1));

        IHubBase.PremiumDelta memory premiumDelta =
            _getExpectedPremiumDelta(spoke1, alice, _daiReserveId(spoke1), premium);
    ...
    function test_restore_revertsWith_SurplusPremiumRayRestored() public {
        ...
        _addLiquidity(hub1, daiAssetId, drawAmount);
        _drawLiquidity({
            hub: hub1,
            assetId: daiAssetId,
            amount: drawAmount,
        ...
        uint256 spokePremiumRay =
            _calculatePremiumDebtRay(hub1, daiAssetId, spokeData.premiumShares, spokeData.premiumOffsetRay);
        ...
        IHubBase.PremiumDelta memory premiumDelta = IHubBase.PremiumDelta({
            sharesDelta: 0, offsetRayDelta: premiumRestoredRay.toInt256(), restoredPremiumRay: premiumRestoredRay
        });

        vm.expectRevert(abi.encodeWithSelector(IHub.SurplusPremiumRayRestored.selector, spokePremiumRay));
        vm.prank(address(spoke1));
        hub1.restore(daiAssetId, drawnRestored, premiumDelta);
    }

    function test_restore_revertsWith_InvalidAmount_zero() public {
        IHubBase.PremiumDelta memory premiumDelta = _getExpectedPremiumDelta(spoke1, alice, _daiReserveId(spoke1), 0);
    ...
    function test_restore_revertsWith_SpokeHalted() public {
        ...
        IHubBase.PremiumDelta memory premiumDelta = _getExpectedPremiumDelta(spoke1, alice, daiAssetId, 0);
    ...
    function test_restore_revertsWith_InsufficientTransferred() public {
        ...
        IHubBase.PremiumDelta memory premiumDelta = _getExpectedPremiumDelta(spoke1, alice, daiAssetId, premium);

        vm.startPrank(address(spoke1));
        tokenList.dai.transferFrom(alice, address(hub1), restoreDrawnAmount / 2);

        vm.expectRevert(abi.encodeWithSelector(IHub.InsufficientTransferred.selector, restoreDrawnAmount / 2));
    ...
    function test_restore_when_asset_caps_reset() public {
        ...
        // spoke2 add dai
        HubActions.add({hub: hub1, assetId: daiAssetId, amount: daiAmount, user: bob, caller: address(spoke2)});
        ...
        assertEq(
            daiData.addedShares, hub1.previewAddByAssets(daiAssetId, daiAmount), "hub dai total shares post-restore"
        );
        assertEq(daiData.liquidity, daiAmount - drawAmount + restoreAmount, "hub dai liquidity post-restore");
        ...
        assertEq(daiData.lastUpdateTimestamp, vm.getBlockTimestamp(), "hub dai lastUpdateTimestamp post-restore");
        _assertHubLiquidity(hub1, daiAssetId, "hub1.restore");
        // spoke1 dai data
        assertEq(hub1.getSpokeAddedShares(daiAssetId, address(spoke1)), 0, "spoke1 total dai shares post-restore");
        (uint256 spoke1DaiDrawn, uint256 spoke1DaiPremium) = hub1.getSpokeOwed(daiAssetId, address(spoke1));
        ...
        assertEq(
            tokenList.dai.balanceOf(alice), drawAmount - restoreAmount + MAX_SUPPLY_AMOUNT, "alice dai final balance"
        );
    ...
    function test_restore_revertsWith_SurplusDrawnRestored_with_interest() public {
        ...
        test_restore_fuzz_revertsWith_SurplusDrawnRestored_with_interest(daiAmount, drawAmount, skipTime);
    }
    ...
    function test_restore_revertsWith_SurplusDrawnRestored_with_interest_and_premium() public {
        ...
        test_restore_fuzz_revertsWith_SurplusDrawnRestored_with_interest_and_premium(
            daiAmount, drawAmount, skipTime, premiumRestored
        );
    }
    ...
    function test_restore_fuzz_revertsWith_SurplusDrawnRestored_with_interest_and_premium(
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _wethReserveId(spoke1), caller: alice, amount: wethAmount, onBehalfOf: alice
        });

        // spoke2 add dai
        SpokeActions.supplyCollateral({
            spoke: spoke2, reserveId: _daiReserveId(spoke2), caller: bob, amount: daiAmount, onBehalfOf: bob
        });

        // spoke1 draw half of dai reserve liquidity
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), onBehalfOf: alice, amount: drawAmount, caller: alice
        });
        ...
        IHubBase.PremiumDelta memory premiumDelta =
            _getExpectedPremiumDelta(spoke1, alice, _daiReserveId(spoke1), premiumRestored);
    ...
    function test_restore_tooMuchDrawn_revertsWith_SurplusDrawnRestored() public {
        ...
        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: drawAmount * 2, user: alice});

        HubActions.draw({
            hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: drawAmount, to: address(spoke1)
        });
        ...
        assertEq(hub1.previewRestoreByAssets(daiAssetId, drawn), hub1.previewRestoreByAssets(daiAssetId, drawn + 1));
    ...
    function test_restore_partial_drawn() public {
        ...
        assertApproxEqAbs(
            ...
            hub1.getSpokeAddedAssets(daiAssetId, address(spoke2)) + _calculateBurntInterest(hub1, daiAssetId),
            1,
    ...
    function test_restore_partial_same_block() public {
        ...
        assertEq(dai.balanceOf(alice), drawAmount - restoreAmount + MAX_SUPPLY_AMOUNT, "alice dai final balance");
    ...
    function test_restore_fuzz_full_amount_with_interest(uint256 daiAmount, uint256 drawAmount, uint256 skipTime)
        public
    {
        daiAmount = bound(daiAmount, 1, 1000e18); // max 1000 DAI
        ...
        // spoke2 add dai
        SpokeActions.supplyCollateral({
            spoke: spoke2, reserveId: _daiReserveId(spoke2), amount: daiAmount, caller: bob, onBehalfOf: bob
        });

        // spoke1 draw liquidity
        HubActions.draw({
            hub: hub1, assetId: daiAssetId, to: address(spoke1), caller: address(spoke1), amount: drawAmount
        });
        ...
        // spoke
        assertApproxEqAbs(
            daiData.addedAmount, hub1.getSpokeAddedAssets(daiAssetId, address(spoke2)), 1, "spoke addedAmount"
        );
        assertApproxEqAbs(
            daiData.addedShares, hub1.getSpokeAddedShares(daiAssetId, address(spoke2)), 1, "spoke addedShares"
        );
        (uint256 spoke1DaiDrawn, uint256 spoke1DaiPremium) = hub1.getSpokeOwed(daiAssetId, address(spoke1));
    ...
    function test_restore_full_amount_with_interest_and_premium() public {
        ...
        test_restore_fuzz_full_amount_with_interest_and_premium(daiAmount, drawAmount, skipTime, premiumRestored);
    }
    ...
    function test_restore_fuzz_full_amount_with_interest_and_premium(
        ...
        // spoke1 draw liquidity
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: alice, amount: drawAmount, onBehalfOf: alice
        });
