...
contract SpokeMultipleHubIsolationModeTest is SpokeMultipleHubBase {
    ...
    function setUpIsolationMode() internal {
        ...
        newHub.addAsset(
            address(assetA), assetA.decimals(), address(treasurySpoke), address(newIrStrategy), encodedIrData
        );
        isolationVars.assetAId = newHub.getAssetCount() - 1;
        newHub.addAsset(
            address(assetB), assetB.decimals(), address(treasurySpoke), address(newIrStrategy), encodedIrData
        );
    ...
    function test_isolation_mode() public {
        ...
        SpokeActions.borrow({
            spoke: newSpoke, reserveId: isolationVars.reserveBId, caller: bob, amount: 1, onBehalfOf: bob
        });
        ...
        hub1.addSpoke(
            isolationVars.assetBIdMainHub,
            address(newSpoke),
            IHub.SpokeConfig({active: true, halted: false, addCap: 0, drawCap: 100_000, riskPremiumThreshold: 1000_00})
        );
        vm.stopPrank();
        ...
        SpokeActions.borrow({
            spoke: newSpoke, reserveId: isolationVars.reserveBIdMainHub, caller: bob, amount: 1, onBehalfOf: bob
        });
        ...
        SpokeActions.supply({
            spoke: newSpoke, reserveId: isolationVars.reserveBIdMainHub, caller: bob, amount: 1e18, onBehalfOf: bob
        });
        ...
        SpokeActions.borrow({
            spoke: newSpoke, reserveId: isolationVars.reserveBId, caller: bob, amount: 100_000e18, onBehalfOf: bob
        });
        ...
        hub1.updateSpokeConfig(
            isolationVars.assetBIdMainHub,
            address(newSpoke),
            IHub.SpokeConfig({active: true, halted: false, addCap: 0, drawCap: 0, riskPremiumThreshold: 1000_00})
        );
