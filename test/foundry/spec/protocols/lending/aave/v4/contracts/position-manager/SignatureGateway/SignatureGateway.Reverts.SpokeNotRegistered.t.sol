...
contract SignatureGateway_SpokeNotRegistered_Test is SignatureGatewayBaseTest {
    ...
    function test_supplyWithSig_revertsWith_SpokeNotRegistered(ISignatureGateway.Supply memory p) public {
    ...
    function test_withdrawWithSig_revertsWith_SpokeNotRegistered(ISignatureGateway.Withdraw memory p) public {
        bytes memory signature = vm.randomBytes(32);
    ...
    function test_borrowWithSig_revertsWith_SpokeNotRegistered(ISignatureGateway.Borrow memory p) public {
        bytes memory signature = vm.randomBytes(32);
    ...
    function test_repayWithSig_revertsWith_SpokeNotRegistered(ISignatureGateway.Repay memory p) public {
        bytes memory signature = vm.randomBytes(32);
    ...
    function test_updateUserRiskPremiumWithSig_revertsWith_SpokeNotRegistered(
        ...
        vm.expectRevert(abi.encodeWithSelector(IPositionManagerBase.SpokeNotRegistered.selector, address(gateway)));
        vm.prank(vm.randomAddress());
