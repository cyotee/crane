...
contract SpokeBorrowValidationTest is Base {
    ...
    function test_borrow_fuzz_revertsWith_ReserveNotBorrowable(uint256 reserveId, uint256 amount) public {
    ...
    function test_borrow_fuzz_revertsWith_InsufficientLiquidity(uint256 daiAmount, uint256 wethAmount) public {
        uint256 daiReserveId = _daiReserveId(spoke1);
        ...
        // Bob supply weth
        SpokeActions.supply({spoke: spoke1, reserveId: wethReserveId, caller: bob, amount: wethAmount, onBehalfOf: bob});

        // Alice supply dai
        SpokeActions.supply({
            spoke: spoke1, reserveId: daiReserveId, caller: alice, amount: daiAmount, onBehalfOf: alice
        });
    ...
    function test_borrow_fuzz_revertsWith_DrawCapExceeded_due_to_interest(uint256 skipTime) public {
        ...
        // Bob draw dai
        SpokeActions.borrow({spoke: spoke1, reserveId: daiReserveId, caller: bob, amount: drawAmount, onBehalfOf: bob});

        skip(skipTime);
        vm.assume(spoke1.getReserveTotalDebt(daiReserveId) > drawCap);

        // Additional supply to accrue interest
        SpokeActions.supply({spoke: spoke1, reserveId: daiReserveId, caller: bob, amount: 1e18, onBehalfOf: bob});

        // Bob should be able to borrow 1 dai
        assertGt(_getUserHealthFactor(spoke1, bob), HEALTH_FACTOR_LIQUIDATION_THRESHOLD);

        vm.expectRevert(abi.encodeWithSelector(IHub.DrawCapExceeded.selector, drawCap));
        SpokeActions.borrow({spoke: spoke1, reserveId: daiReserveId, caller: bob, amount: 1, onBehalfOf: bob});
    }
    ...
    function test_borrow_revertsWith_MaximumUserReservesExceeded() public {
        ...
        for (uint256 i = 0; i < maxUserReservesLimit; ++i) {
            SpokeActions.supplyCollateral({
                spoke: spoke1, reserveId: i, caller: bob, amount: MAX_SUPPLY_AMOUNT, onBehalfOf: bob
            });
            SpokeActions.borrow({spoke: spoke1, reserveId: i, caller: bob, amount: 1e18, onBehalfOf: bob});
        }
        ...
        SpokeActions.supply({
            spoke: spoke1, reserveId: maxUserReservesLimit, caller: bob, amount: MAX_SUPPLY_AMOUNT, onBehalfOf: bob
        });
    ...
    function test_borrow_to_limit_repay_borrow_again() public {
        ...
        for (uint256 i = 0; i < maxUserReservesLimit; ++i) {
            ...
            SpokeActions.borrow({spoke: spoke1, reserveId: i, caller: bob, amount: borrowAmount, onBehalfOf: bob});
        }

        ISpoke.UserAccountData memory accountData = spoke1.getUserAccountData(bob);
        assertEq(accountData.borrowCount, maxUserReservesLimit, "Bob has reached the borrow limit");

        SpokeActions.repay({spoke: spoke1, reserveId: 0, caller: bob, amount: UINT256_MAX, onBehalfOf: bob});
        ...
        SpokeActions.borrow({
            spoke: spoke1, reserveId: maxUserReservesLimit, caller: bob, amount: borrowAmount, onBehalfOf: bob
        });
