...
contract HubMintFeeSharesTest is Base {
    ...
    function test_mintFeeShares() public {
        ...
        emit IHub.MintFeeShares({
            assetId: daiAssetId, feeReceiver: feeReceiver, shares: expectedMintedShares, assets: expectedMintedAssets
        });
        vm.expectEmit(address(hub1));
        emit IHub.UpdateAsset({
            assetId: daiAssetId, drawnIndex: hub1.getAssetDrawnIndex(daiAssetId), drawnRate: mockRate, accruedFees: 0
        });
        ...
        uint256 mintedShares = HubActions.mintFeeShares({hub: hub1, assetId: daiAssetId, caller: ADMIN});
        ...
        assertEq(hub1.getSpokeAddedShares(daiAssetId, feeReceiver), expectedMintedShares, "added shares");
    ...
    function test_mintFeeShares_noFees() public {
        ...
        vm.expectEmit(address(hub1));
        emit IHub.UpdateAsset({
            assetId: daiAssetId, drawnIndex: asset.drawnIndex, drawnRate: asset.drawnRate, accruedFees: 0
        });
