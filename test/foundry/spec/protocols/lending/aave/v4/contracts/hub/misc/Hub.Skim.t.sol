...
contract HubSkimTest is Base {
    ...
    function test_skimAdd_fuzz_donationAfterAdd(uint256 assetId, uint256 amount, uint256 donationAmount) public {
        ...
        assertEq(
            hub1.getSpokeAddedAssets(assetId, address(skimSpoke)), donationAmount, "hub skimSpoke addedAmount after"
        );
        assertEq(hub1.getSpokeAddedShares(assetId, address(skimSpoke)), skimShares, "hub skimSpoke addedShares after");
        assertEq(
            hub1.getAsset(assetId).liquidity, liquidityBefore + amount + donationAmount, "hub available liquidity after"
        );
        _assertDrawnRateSynced(hub1, assetId, "hub1.skimAdd");
        _assertHubLiquidity(hub1, assetId, "hub1.skimAdd");
        // token balance
        assertEq(underlying.balanceOf(address(hub1)), amount + donationAmount, "hub token balance post-add");
    }

    function test_skimAdd_fuzz_donationBeforeAdd(uint256 assetId, uint256 amount, uint256 donationAmount) public {
        assetId = bound(assetId, 0, hub1.getAssetCount() - 3); // Exclude usdy & usdz
        amount = bound(amount, 1, MAX_SUPPLY_AMOUNT / 10);
        donationAmount = bound(donationAmount, 1, MAX_SUPPLY_AMOUNT / 10);
    ...
    function test_skimAdd_fuzz_wrongSpokeTransfer(uint256 assetId, uint256 amount, uint256 donationAmount) public {
        assetId = bound(assetId, 0, hub1.getAssetCount() - 3); // Exclude usdy & usdz
        ...
        assertEq(hub1.getAddedShares(assetId), shares + skimShares, "hub asset addedShares after");
        assertEq(hub1.getSpokeAddedAssets(assetId, address(spoke1)), addAmount, "hub spoke addedAmount after");
        assertEq(hub1.getSpokeAddedShares(assetId, address(spoke1)), shares, "hub spoke addedShares after");
        assertEq(
            hub1.getSpokeAddedAssets(assetId, address(skimSpoke)), donationAmount, "hub skimSpoke addedAmount after"
        );
        assertEq(hub1.getSpokeAddedShares(assetId, address(skimSpoke)), skimShares, "hub skimSpoke addedShares after");
        assertEq(hub1.getAsset(assetId).liquidity, liquidityBefore + amount, "hub available liquidity after");
        _assertDrawnRateSynced(hub1, assetId, "hub1.skimAdd");
        _assertHubLiquidity(hub1, assetId, "hub1.skimAdd");
        // token balance
    ...
    function test_skimRestore_fuzz_liquidityDonation(uint256 assetId, uint256 drawAmount, uint256 donationAmount)
        public
    {
        assetId = bound(assetId, 0, hub1.getAssetCount() - 3); // Exclude usdy & usdz
        ...
        vm.expectEmit(address(hub1));
        emit IHubBase.Restore(assetId, address(skimSpoke), skimShares, ZERO_PREMIUM_DELTA, donationAmount, 0);
        ...
        assertEq(hub1.getAssetLiquidity(assetId), liquidityBefore + donationAmount, "hub available liquidity after");
        _assertDrawnRateSynced(hub1, assetId, "hub1.skimRestore");
        _assertHubLiquidity(hub1, assetId, "hub1.skimRestore");
        // token balance
        assertEq(underlying.balanceOf(address(hub1)), hubBalanceBefore + donationAmount, "hub token balance post-add");
    }
}
