...
contract LiquidationLogicExecuteLiquidationTest is LiquidationLogicBaseTest {
    ...
    function setUp() public override {
        ...
        params = LiquidationLogic.ExecuteLiquidationParams({
            ...
            collateralDynConfig: ISpoke.DynamicReserveConfig({
                maxLiquidationBonus: 120_00, collateralFactor: 50_00, liquidationFee: 10_00
            }),
    ...
    function test_executeLiquidation() public {
        uint256 initialCollateralReserveBalance = tokenList.usdx.balanceOf(address(params.collateralHub));
        ...
        vm.expectCall(
            address(params.collateralHub), abi.encodeCall(IHubBase.previewRemoveByShares, (usdxAssetId, 4800e6)), 1
        );

        vm.expectCall(
            address(params.collateralHub), abi.encodeCall(IHubBase.previewRemoveByShares, (usdxAssetId, 4720e6)), 1
        );

        vm.expectCall(
            address(params.collateralHub), abi.encodeCall(IHubBase.remove, (usdxAssetId, 5900e6, params.liquidator)), 1
        );

        vm.expectCall(address(params.collateralHub), abi.encodeCall(IHubBase.payFeeShares, (usdxAssetId, 80e6)), 1);
        ...
        assertEq(tokenList.usdx.balanceOf(address(params.collateralHub)), initialCollateralReserveBalance - 5900e6);
        assertEq(tokenList.usdx.balanceOf(address(params.liquidator)), 5900e6);
        assertApproxEqAbs(params.collateralHub.getSpokeAddedShares(usdxAssetId, address(treasurySpoke)), 80e6, 1);
