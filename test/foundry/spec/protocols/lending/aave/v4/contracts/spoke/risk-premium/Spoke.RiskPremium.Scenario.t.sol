...
contract SpokeRiskPremiumScenarioTest is Base {
    ...
    function test_riskPremiumPropagatesCorrectly_singleBorrow() public {
        ...
        SpokeActions.supply({
            ...
            caller: bob,
            amount: vars.daiBorrowAmount.percentDivDown(45_00),
            onBehalfOf: bob
        });

        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: reservesIds.usdx, caller: alice, amount: vars.usdxSupplyAmount, onBehalfOf: alice
        });

        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: reservesIds.weth, caller: alice, amount: vars.wethSupplyAmount, onBehalfOf: alice
        });

        SpokeActions.borrow({
            spoke: spoke1, reserveId: reservesIds.dai, caller: alice, amount: vars.daiBorrowAmount, onBehalfOf: alice
        });
        ...
        assertLt(
            wethCollateralRisk, usdxCollateralRisk, "weth collateral risk should be less than usdx collateral risk"
        );
        ...
        assertEq(alicePosition.premiumShares, vars.expectedPremiumShares, "premium drawn shares match expected");
        ...
        assertEq(spoke1.getUserSuppliedAssets(reservesIds.usdx, alice), vars.usdxSupplyAmount, "supplied usdx");
        assertEq(spoke1.getUserSuppliedAssets(reservesIds.weth, alice), vars.wethSupplyAmount, "supplied weth");

        uint256 accruedDaiDebt = vars.daiBorrowAmount
            .rayMulUp(
                MathUtils.calculateLinearInterest(
                        hub1.getAsset(daiAssetId).drawnRate.toUint96(), vars.lastUpdateTimestamp
                    ) - WadRayMath.RAY
            );
        ...
        uint256 daiDebtValue = _convertAmountToValue(spoke1, reservesIds.dai, accruedDaiDebt + daiPremiumDebt);
        uint256 usdxSupplyValue = _convertAmountToValue(spoke1, reservesIds.usdx, vars.usdxSupplyAmount);
        assertLt(daiDebtValue, usdxSupplyValue);

        vars.expectedUserRiskPremium = _calculateExpectedUserRP(spoke1, alice);

        assertEq(_getUserRiskPremium(spoke1, alice), vars.expectedUserRiskPremium, "user risk premium after accrual");

        // Alice supplies more usdx
        SpokeActions.supply({
            spoke: spoke1, reserveId: reservesIds.usdx, caller: alice, amount: 500e6, onBehalfOf: alice
        });

        assertEq(_getUserRiskPremium(spoke1, alice), vars.expectedUserRiskPremium, "user risk premium after supply");
    ...
    function test_getUserRiskPremium_applyInterest_two_users_two_reserves_borrowed() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: daiInfo.reserveId, caller: bob, amount: bobDaiInfo.supplyAmount, onBehalfOf: bob
        });

        // Bob supply usdx into spoke1
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: usdxInfo.reserveId, caller: bob, amount: bobUsdxInfo.supplyAmount, onBehalfOf: bob
        });

        // Alice supply dai into spoke1
        SpokeActions.supplyCollateral({
            spoke: spoke1,
            reserveId: daiInfo.reserveId,
            caller: alice,
            amount: aliceDaiInfo.supplyAmount,
            onBehalfOf: alice
        });
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1,
            reserveId: usdxInfo.reserveId,
            caller: alice,
            amount: aliceUsdxInfo.supplyAmount,
            onBehalfOf: alice
        });

        // Bob draw dai
        SpokeActions.borrow({
            spoke: spoke1, reserveId: daiInfo.reserveId, caller: bob, amount: bobDaiInfo.borrowAmount, onBehalfOf: bob
        });

        // Bob draw usdx
        SpokeActions.borrow({
            spoke: spoke1, reserveId: usdxInfo.reserveId, caller: bob, amount: bobUsdxInfo.borrowAmount, onBehalfOf: bob
        });
        ...
        SpokeActions.borrow({
            ...
            reserveId: daiInfo.reserveId,
            caller: alice,
            amount: aliceDaiInfo.borrowAmount,
            onBehalfOf: alice
        });
        ...
        SpokeActions.borrow({
            ...
            reserveId: usdxInfo.reserveId,
            caller: alice,
            amount: aliceUsdxInfo.borrowAmount,
            onBehalfOf: alice
        });
        ...
        assertEq(_getUserRiskPremium(spoke1, alice), expectedUserRp.aliceRiskPremium, "alice risk premium");
        ...
        (debtChecks.actualDrawnDebt, debtChecks.actualPremium) = spoke1.getUserDebt(daiInfo.reserveId, bob);
        ...
        (debtChecks.actualDrawnDebt, debtChecks.actualPremium) = spoke1.getUserDebt(usdxInfo.reserveId, bob);
        ...
        (debtChecks.actualDrawnDebt, debtChecks.actualPremium) = spoke1.getUserDebt(daiInfo.reserveId, alice);
        ...
        (debtChecks.actualDrawnDebt, debtChecks.actualPremium) = spoke1.getUserDebt(usdxInfo.reserveId, alice);
        ...
        assertEq(
            _getUserRiskPremium(spoke1, bob), _calculateExpectedUserRP(spoke1, bob), "bob risk premium after time skip"
        );
        assertEq(
            _getUserRiskPremium(spoke1, alice),
            _calculateExpectedUserRP(spoke1, alice),
        ...
        bobDaiInfo.drawnDebt =
            MathUtils.calculateLinearInterest(rates.baseRateDai, startTime).rayMulUp(bobDaiInfo.borrowAmount);
        (debtChecks.actualDrawnDebt, debtChecks.actualPremium) = spoke1.getUserDebt(daiInfo.reserveId, bob);
        ...
        bobDaiInfo.premiumDebt =
            (bobDaiInfo.drawnDebt - bobDaiInfo.borrowAmount).percentMulUp(expectedUserRp.bobRiskPremium);
        ...
        bobUsdxInfo.drawnDebt =
            MathUtils.calculateLinearInterest(rates.baseRateUsdx, startTime).rayMulUp(bobUsdxInfo.borrowAmount);
        (debtChecks.actualDrawnDebt, debtChecks.actualPremium) = spoke1.getUserDebt(usdxInfo.reserveId, bob);
        ...
        bobUsdxInfo.premiumDebt =
            (bobUsdxInfo.drawnDebt - bobUsdxInfo.borrowAmount).percentMulUp(expectedUserRp.bobRiskPremium);
        ...
        aliceDaiInfo.drawnDebt =
            MathUtils.calculateLinearInterest(rates.baseRateDai, startTime).rayMulUp(aliceDaiInfo.borrowAmount);
        (debtChecks.actualDrawnDebt, debtChecks.actualPremium) = spoke1.getUserDebt(daiInfo.reserveId, alice);
        ...
        aliceDaiInfo.premiumDebt =
            (aliceDaiInfo.drawnDebt - aliceDaiInfo.borrowAmount).percentMulUp(expectedUserRp.aliceRiskPremium);
        assertEq(aliceDaiInfo.premiumDebt, debtChecks.actualPremium, "alice premium debt after accrual");

        // See if Alice's drawn debt of usdx changes appropriately
        aliceUsdxInfo.drawnDebt =
            MathUtils.calculateLinearInterest(rates.baseRateUsdx, startTime).rayMulUp(aliceUsdxInfo.borrowAmount);
        (debtChecks.actualDrawnDebt, debtChecks.actualPremium) = spoke1.getUserDebt(usdxInfo.reserveId, alice);
        ...
        aliceUsdxInfo.premiumDebt =
            (aliceUsdxInfo.drawnDebt - aliceUsdxInfo.borrowAmount).percentMulUp(expectedUserRp.aliceRiskPremium);
        assertEq(aliceUsdxInfo.premiumDebt, debtChecks.actualPremium, "alice premium debt after accrual");

        _verifyProtocolDebtAmounts(bobDaiInfo, aliceDaiInfo, bobUsdxInfo, aliceUsdxInfo, "after accrual");

        RestoredAmountsLocal memory restored;
        (restored.baseRestored, restored.premiumRestored) = _calculateExactRestoreAmount(
            hub1, daiAssetId, aliceDaiInfo.drawnDebt, aliceDaiInfo.premiumDebt, aliceDaiInfo.borrowAmount / 2
        );
        ...
        SpokeActions.repay({
            ...
            reserveId: daiInfo.reserveId,
            caller: alice,
            amount: aliceDaiInfo.borrowAmount / 2,
            onBehalfOf: alice
        });
        ...
        expectedUserRp.aliceRiskPremium = _calculateExpectedUserRP(spoke1, alice);
        assertEq(
            _getUserRiskPremium(spoke1, alice), expectedUserRp.aliceRiskPremium, "alice risk premium after repay (usdx)"
        );
        ...
        assertApproxEqAbs(debtChecks.actualDrawnDebt, aliceDaiInfo.drawnDebt, 1, "alice drawn debt after repay");
        assertApproxEqAbs(debtChecks.actualPremium, aliceDaiInfo.premiumDebt, 1, "alice premium debt after repay");
        ...
        assertApproxEqAbs(debtChecks.actualPremium, aliceUsdxInfo.premiumDebt, 1, "alice usdx premium debt after");
        ...
        _verifyProtocolDebtAmounts(bobDaiInfo, aliceDaiInfo, bobUsdxInfo, aliceUsdxInfo, "after alice repay");
    }
    ...
    function test_getUserRiskPremium_fuzz_two_users_two_reserves_borrowed(
        ...
        assertEq(_getCollateralRisk(spoke1, _daiReserveId(spoke1)), daiCollateralRisk, "dai collateral risk");
        assertEq(_getCollateralRisk(spoke1, _usdxReserveId(spoke1)), usdxCollateralRisk, "usdx collateral risk");
        ...
        SpokeActions.supplyCollateral({
            ...
            caller: bob,
            amount: bobDaiInfo.supplyAmount,
            onBehalfOf: bob
        });
        ...
        SpokeActions.supplyCollateral({
            ...
            caller: bob,
            amount: bobUsdxInfo.supplyAmount,
            onBehalfOf: bob
        });
        ...
        if (bobDaiInfo.borrowAmount > 0) {
            SpokeActions.borrow({
                ...
                reserveId: _daiReserveId(spoke1),
                caller: bob,
                amount: bobDaiInfo.borrowAmount,
                onBehalfOf: bob
            });
        }
        ...
        if (bobUsdxInfo.borrowAmount > 0) {
            SpokeActions.borrow({
                ...
                reserveId: _usdxReserveId(spoke1),
                caller: bob,
                amount: bobUsdxInfo.borrowAmount,
                onBehalfOf: bob
            });
        }
        ...
        assertEq(_getUserRiskPremium(spoke1, alice), aliceExpectedRiskPremium, "alice initial risk premium");
        ...
        (debtChecks.actualDrawnDebt, debtChecks.actualPremium) = spoke1.getUserDebt(_daiReserveId(spoke1), bob);
        ...
        (debtChecks.actualDrawnDebt, debtChecks.actualPremium) = spoke1.getUserDebt(_usdxReserveId(spoke1), bob);
        assertEq(bobUsdxInfo.borrowAmount, debtChecks.actualDrawnDebt, "Bob usdx debt before");
        assertEq(debtChecks.actualPremium, 0, "Bob usdx premium before");

        // Alice's initial dai debt
        (debtChecks.actualDrawnDebt, debtChecks.actualPremium) = spoke1.getUserDebt(_daiReserveId(spoke1), alice);
        assertEq(aliceDaiInfo.borrowAmount, debtChecks.actualDrawnDebt, "Alice dai debt before");
        assertEq(debtChecks.actualPremium, 0, "Alice dai premium before");

        // Alice's initial usdx debt
        (debtChecks.actualDrawnDebt, debtChecks.actualPremium) = spoke1.getUserDebt(_usdxReserveId(spoke1), alice);
        assertEq(aliceUsdxInfo.borrowAmount, debtChecks.actualDrawnDebt, "Alice usdx debt before");
        assertEq(debtChecks.actualPremium, 0, "Alice usdx premium before");
        ...
        assertEq(expectedpremiumShares, bobPosition.premiumShares, "bob dai premium drawn shares after time skip");
        ...
        assertEq(expectedpremiumShares, bobPosition.premiumShares, "bob usdx premium drawn shares after time skip");
        ...
        assertEq(expectedpremiumShares, alicePosition.premiumShares, "alice dai premium drawn shares after time skip");
        aliceDaiInfo.premiumShares = expectedpremiumShares;

        alicePosition = spoke1.getUserPosition(_usdxReserveId(spoke1), alice);
        expectedpremiumShares = alicePosition.drawnShares.percentMulUp(aliceExpectedRiskPremium);
        assertEq(expectedpremiumShares, alicePosition.premiumShares, "alice usdx premium drawn shares after time skip");
        ...
        if (bobDaiInfo.borrowAmount > 0) {
            bobDaiInfo.drawnDebt = MathUtils.calculateLinearInterest(
                    hub1.getAsset(daiAssetId).drawnRate.toUint96(), startTime
        ...
        if (bobUsdxInfo.borrowAmount > 0) {
            bobUsdxInfo.drawnDebt = MathUtils.calculateLinearInterest(
                    hub1.getAsset(usdxAssetId).drawnRate.toUint96(), startTime
        ...
        if (aliceDaiInfo.borrowAmount > 0) {
            aliceDaiInfo.drawnDebt = MathUtils.calculateLinearInterest(
                    hub1.getAsset(daiAssetId).drawnRate.toUint96(), startTime
        ...
        if (aliceUsdxInfo.borrowAmount > 0) {
            aliceUsdxInfo.drawnDebt = MathUtils.calculateLinearInterest(
                    hub1.getAsset(usdxAssetId).drawnRate.toUint96(), startTime
        ...
        _verifyProtocolDebtShares(bobDaiInfo, aliceDaiInfo, bobUsdxInfo, aliceUsdxInfo, "after accrual");
        ...
        if (bobDaiInfo.borrowAmount > 2) {
            ...
            SpokeActions.repay({
                spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: repayAmount, onBehalfOf: bob
            });
            ...
            assertEq(_getUserRiskPremium(spoke1, bob), bobExpectedRiskPremium, "bob risk premium after repay");
        ...
        if (
            ...
            assertEq(_getUserRiskPremium(spoke1, alice), aliceExpectedRiskPremium, "alice risk premium after borrow");

            // Verify Bob's risk premium remains the same by checking premium drawn shares
            bobPosition = spoke1.getUserPosition(_usdxReserveId(spoke1), bob);
            assertEq(
                bobUsdxInfo.premiumShares, bobPosition.premiumShares, "bob dai premium drawn shares after alice borrow"
            );
            bobPosition = spoke1.getUserPosition(_daiReserveId(spoke1), bob);
            assertEq(
                bobDaiInfo.premiumShares, bobPosition.premiumShares, "bob usdx premium drawn shares after alice borrow"
            );
        }
        ...
        aliceUsdxInfo.premiumShares = spoke1.getUserPosition(_usdxReserveId(spoke1), alice).premiumShares;
    ...
    function test_getUserRiskPremium_fuzz_inflight_calcs(
        ...
        if (daiAmounts.supplyAmount > 0) {
            SpokeActions.supplyCollateral({
                ...
                reserveId: _daiReserveId(spoke1),
                caller: bob,
                amount: daiAmounts.supplyAmount,
                onBehalfOf: bob
            });
        }
        if (wethAmounts.supplyAmount > 0) {
            SpokeActions.supplyCollateral({
                ...
                caller: bob,
                amount: wethAmounts.supplyAmount,
                onBehalfOf: bob
            });
        }
        if (usdxAmounts.supplyAmount > 0) {
            SpokeActions.supplyCollateral({
                ...
                reserveId: _usdxReserveId(spoke1),
                caller: bob,
                amount: usdxAmounts.supplyAmount,
                onBehalfOf: bob
            });
        }
        if (wbtcAmounts.supplyAmount > 0) {
            SpokeActions.supplyCollateral({
                ...
                caller: bob,
                amount: wbtcAmounts.supplyAmount,
                onBehalfOf: bob
            });
        }
        ...
        if (daiAmounts.borrowAmount > 0) {
            SpokeActions.borrow({
                ...
                reserveId: _daiReserveId(spoke1),
                caller: bob,
                amount: daiAmounts.borrowAmount,
                onBehalfOf: bob
            });
        }
        if (wethAmounts.borrowAmount > 0) {
            SpokeActions.borrow({
                ...
                reserveId: _wethReserveId(spoke1),
                caller: bob,
                amount: wethAmounts.borrowAmount,
                onBehalfOf: bob
            });
        }
        if (usdxAmounts.borrowAmount > 0) {
            SpokeActions.borrow({
                ...
                reserveId: _usdxReserveId(spoke1),
                caller: bob,
                amount: usdxAmounts.borrowAmount,
                onBehalfOf: bob
            });
        }
        if (wbtcAmounts.borrowAmount > 0) {
            SpokeActions.borrow({
                ...
                reserveId: _wbtcReserveId(spoke1),
                caller: bob,
                amount: wbtcAmounts.borrowAmount,
                onBehalfOf: bob
            });
        }

        // Check bob's user risk premium
        assertEq(_getUserRiskPremium(spoke1, bob), _calculateExpectedUserRP(spoke1, bob), "user risk premium");
        ...
        assertEq(
            _getUserRiskPremium(spoke1, bob), _calculateExpectedUserRP(spoke1, bob), "user risk premium after time skip"
        );
    }

    function _boundSupplyBorrowAmounts(SupplyBorrowAmounts memory action, uint256 maxSupplyAmount)
        internal
        pure
        returns (SupplyBorrowAmounts memory)
    {
    ...
    function _verifyProtocolDebtAmounts(
        ...
        (debtChecks.reserveDebt, debtChecks.reservePremium) = spoke1.getReserveDebt(_daiReserveId(spoke1));
        ...
        (debtChecks.reserveDebt, debtChecks.reservePremium) = spoke1.getReserveDebt(_usdxReserveId(spoke1));
        ...
        (debtChecks.spokeOwed, debtChecks.spokePremium) = hub1.getSpokeOwed(daiAssetId, address(spoke1));
        ...
        (debtChecks.spokeOwed, debtChecks.spokePremium) = hub1.getSpokeOwed(usdxAssetId, address(spoke1));
    ...
    function _verifyProtocolDebtShares(
        ...
        SpokePosition memory reserve = _getSpokePosition(spoke1, _daiReserveId);
        ...
        reserve = _getSpokePosition(spoke1, _usdxReserveId);
