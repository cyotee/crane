...
contract HubRescueTest is Base {
    ...
    function test_rescue_scenario_fuzz(uint256 lostAmount) public {
        ...
        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: 10e20, user: alice});
        // spoke2, bob add dai
        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke2), amount: 7.5e22, user: bob});

        uint256 prevHubBalance = underlying.balanceOf(address(hub1));
        uint256 prevRescueBalance = underlying.balanceOf(_rescueSpoke);

        (uint256 rescueAmount, uint256 rescueAddedShares, uint256 rescueWithdrawnShares) =
            _rescue(hub1, _rescueSpoke, daiAssetId, underlying);
        ...
        HubActions.remove({hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: 5e20, to: alice});
        // spoke2, bob add dai
        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke2), amount: 2.5e22, user: bob});
        ...
        HubActions.remove({hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: 5e20, to: alice});
        HubActions.remove({hub: hub1, assetId: daiAssetId, caller: address(spoke2), amount: 10e22, to: bob});
    ...
    function test_rescue_fuzz_with_interest(uint256 lostAmount, uint256 skipTime) public {
        ...
        // spoke2, bob add dai
        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke2), amount: 7.5e22, user: bob});
        HubActions.draw({hub: hub1, assetId: daiAssetId, caller: address(spoke1), to: alice, amount: 10e20});
        ...
        // check amounts & balances
        assertApproxEqAbs(rescueAmount, lostAmount, hub1.previewAddByShares(daiAssetId, 1), "rescue amount"); // can differ by up to 1 share worth of assets due to remove donation rounding
        assertEq(rescueAddedShares, rescueWithdrawnShares, "rescue shares");
    ...
    function _rescue(IHub hub, address rescueSpoke, uint256 assetId, IERC20 underlying)
        internal
