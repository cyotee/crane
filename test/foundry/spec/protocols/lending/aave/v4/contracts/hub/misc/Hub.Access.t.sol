...
contract HubAccessTest is Base {
    ...
    function test_hub_admin_access() public {
        ...
        IHub.SpokeConfig memory spokeConfig =
            IHub.SpokeConfig({active: true, halted: false, addCap: 1000, drawCap: 1000, riskPremiumThreshold: 1000_00});
    ...
    function test_change_role_responsibility() public {
        ...
        vm.prank(ADMIN);
        accessManager.setTargetFunctionRole(address(hub1), hubSelectors, Roles.ACCESS_MANAGER_ADMIN_ROLE);

        // The old role (HUB_ADMIN) should no longer have access
        vm.expectRevert(abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, HUB_ADMIN));
        vm.prank(HUB_ADMIN);
        hub1.setInterestRateData(daiAssetId, encodedIrData);
    ...
    function test_migrate_role_responsibility() public {
        ...
        vm.prank(bob);
        hub1.setInterestRateData(daiAssetId, encodedIrData);
        vm.expectRevert(abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, carol));
        vm.prank(carol);
        hub1.setInterestRateData(daiAssetId, encodedIrData);
    ...
    function test_change_authority() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IAccessManager.AccessManagerUnauthorizedAccount.selector, address(this), Roles.ACCESS_MANAGER_ADMIN_ROLE
            )
        );
