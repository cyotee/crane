...
contract SpokeSetUserPositionManagersWithSigTest is Base {
    ...
    function test_setUserPositionManager_typeHash() public view {
        ...
        assertEq(EIP712Hash.SET_USER_POSITION_MANAGERS_TYPEHASH, spoke1.SET_USER_POSITION_MANAGERS_TYPEHASH());
    }
    ...
    function test_setUserPositionManagersWithSig_revertsWith_InvalidSignature_dueTo_ExpiredDeadline() public {
    ...
    function test_setUserPositionManagersWithSig_revertsWith_InvalidSignature_dueTo_InvalidSigner() public {
    ...
    function test_setUserPositionManagersWithSig_revertsWith_InvalidAccountNonce(bytes32) public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(INoncesKeyed.InvalidAccountNonce.selector, params.onBehalfOf, currentNonce)
        );
        vm.prank(vm.randomAddress());
        spoke1.setUserPositionManagersWithSig(params, signature);
    }
    ...
    function test_setUserPositionManagersWithSig() public {
        ...
        emit ISpoke.SetUserPositionManager(
            params.onBehalfOf, params.updates[0].positionManager, params.updates[0].approve
        );
        ...
        assertEq(
            spoke1.isPositionManager(params.onBehalfOf, params.updates[0].positionManager), params.updates[0].approve
        );
    }
    ...
    function test_setUserPositionManagersWithSig_multiple_updates(ISpoke.PositionManagerUpdate[] memory updates)
        public
    {
    ...
    function test_setUserPositionManagersWithSig_ERC1271_revertsWith_InvalidSignature_dueTo_ExpiredDeadline() public {
        MockERC1271Wallet smartWallet = new MockERC1271Wallet(alice);
        ISpoke.SetUserPositionManagers memory params =
            _setUserPositionManagerData(address(smartWallet), _warpAfterRandomDeadline(MAX_SKIP_TIME));
        bytes32 digest = _getTypedDataHash(spoke1, params);
    ...
    function test_setUserPositionManagersWithSig_ERC1271_revertsWith_InvalidSignature_dueTo_InvalidHash() public {
        ...
        ISpoke.SetUserPositionManagers memory params = _setUserPositionManagerData(address(smartWallet), deadline);
        bytes32 digest = _getTypedDataHash(spoke1, params);

        ISpoke.SetUserPositionManagers memory invalidParams =
            _setUserPositionManagerData(address(smartWallet), deadline);
    ...
    function test_setUserPositionManagersWithSig_ERC1271_revertsWith_InvalidAccountNonce(bytes32) public {
        MockERC1271Wallet smartWallet = new MockERC1271Wallet(alice);
        uint256 deadline = _warpBeforeRandomDeadline(MAX_SKIP_TIME);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(INoncesKeyed.InvalidAccountNonce.selector, address(smartWallet), currentNonce)
        );
        vm.prank(vm.randomAddress());
        spoke1.setUserPositionManagersWithSig(params, signature);
    }
    ...
    function _setUserPositionManagerData(address user, uint256 deadline)
        internal
        view
        returns (ISpoke.SetUserPositionManagers memory)
    {
        ...
        ISpoke.SetUserPositionManagers memory params = ISpoke.SetUserPositionManagers({
            onBehalfOf: user, updates: updates, nonce: spoke1.nonces(user, _randomNonceKey()), deadline: deadline
        });
        return params;
    }
}
