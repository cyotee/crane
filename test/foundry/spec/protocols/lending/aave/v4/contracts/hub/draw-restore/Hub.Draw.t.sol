...
contract HubDrawTest is Base {
    ...
    function test_draw_fuzz_amounts_same_block(uint256 assetId, uint256 amount) public {
        ...
        // spoke2, bob add dai
        HubActions.add({hub: hub1, assetId: assetId, caller: address(spoke2), amount: amount, user: bob});
        ...
        assertEq(hub1.getAsset(assetId).lastUpdateTimestamp, vm.getBlockTimestamp(), "asset lastUpdateTimestamp after");
        assertEq(hub1.getAsset(assetId).liquidity, assetBefore.liquidity - amount, "available liquidity after draw");
        assertEq(hub1.getAsset(assetId).drawnShares, assetBefore.drawnShares + shares, "drawnShares after draw");
        ...
        assertEq(hub1.previewDrawByAssets(assetId, amount), hub1.previewRestoreByShares(assetId, amount));
    }
    ...
    function test_draw_fuzz_revertsWith_InsufficientLiquidity(uint256 assetId, uint256 drawAmount) public {
        assetId = bound(assetId, 0, hub1.getAssetCount() - 3); // Exclude usdy & usdz
    ...
    function test_draw_revertsWith_InsufficientLiquidity_due_to_remove() public {
        ...
        HubActions.remove({hub: hub1, assetId: daiAssetId, caller: address(spoke2), amount: daiAmount, to: bob});
    ...
    function test_draw_fuzz_revertsWith_InsufficientLiquidity_due_to_remove(uint256 daiAmount) public {
    ...
    function test_draw_revertsWith_InsufficientLiquidity_due_to_draw() public {
        ...
        HubActions.draw({hub: hub1, assetId: daiAssetId, caller: address(spoke2), amount: daiAmount, to: bob});
    ...
    function test_draw_fuzz_revertsWith_DrawCapExceeded_due_to_interest(uint40 drawCap, uint256 rate, uint256 skipTime)
        public
    {
        ...
        hub1.restore({assetId: daiAssetId, drawnAmount: singleShareInAssets, premiumDelta: ZERO_PREMIUM_DELTA});
    ...
    function test_draw_revertsWith_DrawCapExceeded_due_to_deficit() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IHub.DrawCapExceeded.selector, drawCap));
        HubActions.draw({hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: 1, to: address(spoke1)});
    }
