...
contract HubConfigTest is Base {
    ...
    function test_addSpoke_fuzz_revertsWith_AssetNotListed(uint256 assetId, IHub.SpokeConfig calldata spokeConfig)
        public
    {
        assetId = bound(assetId, hub1.getAssetCount(), UINT256_MAX);
        vm.expectRevert(IHub.AssetNotListed.selector, address(hub1));
        HubActions.addSpoke({
            hub: hub1, hubAdmin: ADMIN, assetId: assetId, spoke: address(spoke1), spokeConfig: spokeConfig
        });
    }

    function test_addSpoke_fuzz_revertsWith_InvalidAddress_spoke(uint256 assetId, IHub.SpokeConfig calldata spokeConfig)
        public
    {
        assetId = bound(assetId, 0, hub1.getAssetCount() - 1);

        vm.expectRevert(IHub.InvalidAddress.selector, address(hub1));
        HubActions.addSpoke({hub: hub1, hubAdmin: ADMIN, assetId: assetId, spoke: address(0), spokeConfig: spokeConfig});
    }
    ...
    function test_addSpoke_revertsWith_SpokeAlreadyListed() public {
        ...
        HubActions.addSpoke({
            hub: hub1, hubAdmin: ADMIN, assetId: daiAssetId, spoke: address(spoke1), spokeConfig: spokeConfig
        });
    }
    ...
    function test_addSpoke_fuzz(uint256 assetId, IHub.SpokeConfig calldata spokeConfig) public {
        ...
        HubActions.addSpoke({hub: hub1, hubAdmin: ADMIN, assetId: assetId, spoke: newSpoke, spokeConfig: spokeConfig});
    ...
    function test_updateSpokeConfig_revertsWith_AssetNotListed() public {
        ...
        HubActions.updateSpokeConfig({
            hub: hub1, hubAdmin: ADMIN, assetId: assetId, spoke: spoke, spokeConfig: spokeConfig
        });
    }
    ...
    function test_updateSpokeConfig_fuzz(uint256 assetId, IHub.SpokeConfig calldata spokeConfig) public {
    ...
    function test_addAsset_revertsWith_DrawnRateDowncastOverflow() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(SafeCast.SafeCastOverflowedUintDowncast.selector, 96, drawnRateRay), address(hub1)
        );
        HubActions.addAsset({
            hub: hub1,
            hubAdmin: ADMIN,
    ...
    function test_addAsset_revertsWith_BlockTimestampDowncastOverflow() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(SafeCast.SafeCastOverflowedUintDowncast.selector, 40, blockTimestamp), address(hub1)
        );
        HubActions.addAsset({
            hub: hub1,
            hubAdmin: ADMIN,
    ...
    function test_addAsset_fuzz(address underlying, uint8 decimals, address feeReceiver) public {
        ...
        decimals = bound(decimals, MAX_ALLOWED_UNDERLYING_DECIMALS, MAX_ALLOWED_UNDERLYING_DECIMALS).toUint8();
    ...
    function test_updateAssetConfig_fuzz_revertsWith_calculateInterestRateReverts(
        ...
        vm.mockCallRevert(
            newConfig.irStrategy, IBasicInterestRateStrategy.calculateInterestRate.selector, "custom revert"
        );
    ...
    function test_updateAssetConfig_fuzz(uint256 assetId, IHub.AssetConfig memory newConfig) public {
        ...
        if (isNewFeeReceiver && !hub1.isSpokeListed(assetId, newConfig.feeReceiver)) {
            if (_calcUnrealizedFees(hub1, assetId) > 0) {
                ...
                emit IHub.MintFeeShares(
                    assetId, _getFeeReceiver(hub1, assetId), hub1.previewAddByAssets(assetId, accruedFees), accruedFees
                );
            }
            vm.expectEmit(address(hub1));
        ...
        emit IHub.UpdateAsset(
            ...
            IBasicInterestRateStrategy(irStrategy)
                .calculateInterestRate({assetId: assetId, liquidity: liquidity, drawn: drawn, deficit: 0, swept: 0}),
        ...
        HubActions.updateAssetConfig({
            ...
            assetId: assetId,
            config: newConfig,
            encodedIrData: isNewIrStrategy ? encodedIrData : new bytes(0)
        });
    ...
    function test_updateAssetConfig_fuzz_NewFeeReceiver(uint256 assetId) public {
        ...
        assertApproxEqAbs(hub1.getSpokeAddedAssets(assetId, oldFeeReceiver), expectedFeeReceiverAddedAssets, 2);
    ...
    function _test_updateAssetConfig_oldFeeReceiver_flags(bool active, bool halted) internal {
        ...
        HubActions.updateSpokeConfig({
            hub: hub1, hubAdmin: ADMIN, assetId: assetId, spoke: oldFeeReceiver, spokeConfig: oldFeeReceiverConfig
        });
        ...
        assertEq(hub1.getSpokeConfig(assetId, oldFeeReceiver).active, active, "old fee receiver active");
        assertEq(hub1.getSpokeConfig(assetId, oldFeeReceiver).halted, halted, "old fee receiver halted");
    }
    ...
    function test_updateAssetConfig_NewFeeReceiver_revertsWith_SpokeNotActive_noFees() public {
        ...
        HubActions.updateAssetConfig({
            hub: hub1, hubAdmin: ADMIN, assetId: assetId, config: config, encodedIrData: new bytes(0)
        });
    }
    ...
    function test_updateAssetConfig_fuzz_ReuseFeeReceiver_revertsWith_SpokeAlreadyListed(uint256 assetId) public {
        assetId = bound(assetId, 0, hub1.getAssetCount() - 1);
    ...
    function test_updateAssetConfig_UseExistingSpokeAndListedAsFeeReceiver_revertsWith_SpokeAlreadyListed() public {
        uint256 assetId = 3;
    ...
    function test_updateAssetConfig_fuzz_revertsWith_InvalidInterestRateStrategy(uint256 assetId) public {
        assetId = bound(assetId, 0, hub1.getAssetCount() - 1);
        IHub.AssetConfig memory config = hub1.getAssetConfig(assetId);
        vm.expectRevert(IHub.InvalidInterestRateStrategy.selector, address(hub1));
        HubActions.updateAssetConfig({
            hub: hub1, hubAdmin: ADMIN, assetId: assetId, config: config, encodedIrData: encodedIrData
        });
    }
    ...
    function test_updateAssetConfig_fuzz_FromZeroLiquidityFee(uint256 assetId, uint16 liquidityFee) public {
        assetId = bound(assetId, 0, hub1.getAssetCount() - 1);
        liquidityFee = bound(liquidityFee, 1, PercentageMath.PERCENTAGE_FACTOR).toUint16();
    ...
    function test_updateAssetConfig_fuzz_NewDrawnRateStrategy(uint256 assetId) public {
        ...
        _mockDrawnRateRay({irStrategy: address(newIrStrategy), drawnRateRay: hub1.getAssetDrawnRate(assetId) * 10});
        IHub.AssetConfig memory config = hub1.getAssetConfig(assetId);
