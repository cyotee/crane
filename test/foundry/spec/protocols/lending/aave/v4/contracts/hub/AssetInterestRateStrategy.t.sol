...
contract AssetInterestRateStrategyTest is Base {
    ...
    function test_maxDrawnRate() public view {
        assertEq(rateStrategy.MAX_ALLOWED_DRAWN_RATE(), MAX_ALLOWED_DRAWN_RATE);
    }
    ...
    function test_setInterestRateData_revertsWith_InvalidMaxDrawnRate() public {
        rateData.baseDrawnRate = rateData.rateGrowthBeforeOptimal = rateData.rateGrowthAfterOptimal = rateStrategy.MAX_ALLOWED_DRAWN_RATE()
                .toUint32() / 3 + 1;
        encodedRateData = abi.encode(rateData);
    ...
    function test_calculateInterestRate_revertsWith_InterestRateDataNotSet() public {
        ...
        rateStrategy.calculateInterestRate({assetId: mockAssetId2, liquidity: 0, drawn: 0, deficit: 0, swept: 0});
    }
    ...
    function test_calculateInterestRate_fuzz_ZeroDebt(uint256 liquidity) public view {
        ...
        uint256 variableDrawnRate = rateStrategy.calculateInterestRate({
            assetId: mockAssetId, liquidity: liquidity, drawn: 0, deficit: 0, swept: 0
        });
    ...
    function test_calculateInterestRate_LeftToOptimalPoint(uint256 utilizationRatio) public view {
        ...
        (uint256 liquidity, uint256 drawn, uint256 deficit, uint256 swept) =
            _generateCalculateDrawnRateParams(utilizationRatioRay);

        uint256 variableDrawnRate = rateStrategy.calculateInterestRate({
            assetId: mockAssetId, liquidity: liquidity, drawn: drawn, deficit: deficit, swept: swept
        });

        uint256 expectedVariableRate = rateData.baseDrawnRate.bpsToRay()
            + rateData.rateGrowthBeforeOptimal.bpsToRay().rayMulUp(utilizationRatioRay)
                .rayDivUp(rateData.optimalUsageRatio.bpsToRay());
    ...
    function test_calculateInterestRate_RightToOptimalPoint(uint256 utilizationRatio) public view {
        ...
        uint256 expectedVariableRate = rateData.baseDrawnRate.bpsToRay() + rateData.rateGrowthBeforeOptimal.bpsToRay()
            + rateData.rateGrowthAfterOptimal.bpsToRay()
    ...
    function _generateCalculateDrawnRateParams(uint256 targetUtilizationRatioRay)
        internal
        view
        returns (uint256 liquidity, uint256 drawn, uint256 deficit, uint256 swept)
    {
        ...
        liquidity = drawn.rayMulUp(WadRayMath.RAY - targetUtilizationRatioRay).rayDivUp(targetUtilizationRatioRay);
