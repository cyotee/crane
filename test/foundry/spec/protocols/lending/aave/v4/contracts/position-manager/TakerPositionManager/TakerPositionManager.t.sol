...
contract TakerPositionManagerTest is TakerPositionManagerBaseTest {
    ...
    function test_renounceWithdrawAllowance_fuzz(uint256 initialAllowance) public {
        ...
        initialAllowance = bound(initialAllowance, 1, MAX_SUPPLY_AMOUNT_DAI);
    ...
    function test_withdrawOnBehalfOf_fuzz(uint256 amount) public {
        ...
        uint256 expectedSupplyShares = hub1.previewAddByAssets(daiAssetId, MAX_SUPPLY_AMOUNT_DAI);
        ...
        emit ISpoke.Withdraw(_daiReserveId(spoke1), address(positionManager), alice, expectedShares, amount);
        vm.expectEmit(address(positionManager));
        emit ITakerPositionManager.WithdrawApproval(
            address(spoke1), alice, bob, _daiReserveId(spoke1), expectedAllowance
        );
        vm.expectEmit(address(positionManager));
        emit ITakerPositionManager.WithdrawOnBehalfOf(
            address(spoke1), bob, alice, _daiReserveId(spoke1), expectedShares, amount
        );
        vm.prank(bob);
        (returnValues.shares, returnValues.amount) =
        ...
        assertEq(spoke1.getUserSuppliedAssets(_daiReserveId(spoke1), alice), userSuppliedAmountBefore - amount);
        ...
        assertEq(
            positionManager.withdrawAllowance(address(spoke1), _daiReserveId(spoke1), alice, bob), expectedAllowance
        );
    }
    ...
    function test_withdrawOnBehalfOf_fuzz_allBalance(uint256 supplyAmount) public {
        ...
        SpokeActions.supply({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: alice, amount: supplyAmount, onBehalfOf: alice
        });
        ...
        uint256 callerBalanceBefore = tokenList.dai.balanceOf(bob);
        uint256 hubBalanceBefore = tokenList.dai.balanceOf(address(hub1));
        uint256 allowanceBefore = positionManager.withdrawAllowance(address(spoke1), _daiReserveId(spoke1), alice, bob);
        ...
        vm.expectEmit(address(spoke1));
        emit ISpoke.Withdraw(_daiReserveId(spoke1), address(positionManager), alice, expectedSupplyShares, supplyAmount);
        vm.expectEmit(address(positionManager));
        emit ITakerPositionManager.WithdrawApproval(
            address(spoke1), alice, bob, _daiReserveId(spoke1), expectedAllowance
        );
        vm.expectEmit(address(positionManager));
        emit ITakerPositionManager.WithdrawOnBehalfOf(
            address(spoke1), bob, alice, _daiReserveId(spoke1), expectedSupplyShares, supplyAmount
        );
        vm.prank(bob);
        (returnValues.shares, returnValues.amount) =
            positionManager.withdrawOnBehalfOf(address(spoke1), _daiReserveId(spoke1), supplyAmount * 2, alice);
    ...
    function test_withdrawOnBehalfOf_fuzz_allBalance_noAllowanceDecreased(uint256 supplyAmount) public {
        supplyAmount = bound(supplyAmount, 1, MAX_SUPPLY_AMOUNT_DAI);
        ...
        assertEq(tokenList.dai.balanceOf(address(positionManager)), 0);
        assertEq(tokenList.dai.allowance(address(positionManager), address(hub1)), 0);
        assertEq(positionManager.withdrawAllowance(address(spoke1), _daiReserveId(spoke1), alice, bob), UINT256_MAX);
    }

    function test_withdrawOnBehalfOf_fuzz_allBalanceWithInterest(uint256 supplyAmount, uint256 borrowAmount) public {
        supplyAmount = bound(supplyAmount, 2, MAX_SUPPLY_AMOUNT_DAI / 2);
        ...
        emit ISpoke.Withdraw(
            _daiReserveId(spoke1), address(positionManager), alice, expectedSupplyShares, expectedWithdrawAmount
        );
        vm.expectEmit(address(positionManager));
        emit ITakerPositionManager.WithdrawOnBehalfOf(
            address(spoke1), bob, alice, _daiReserveId(spoke1), expectedSupplyShares, expectedWithdrawAmount
        );
        vm.recordLogs();
        vm.prank(bob);
        (returnValues.shares, returnValues.amount) =
            positionManager.withdrawOnBehalfOf(address(spoke1), _daiReserveId(spoke1), supplyAmount * 10, alice);

        assertEq(returnValues.amount, expectedWithdrawAmount);
        assertEq(returnValues.shares, expectedSupplyShares);

        uint256 correctedWithdrawAmount =
            suppliedAssetsBefore - spoke1.getUserSuppliedAssets(_daiReserveId(spoke1), alice);
        uint256 expectedAllowance = supplyAmount * 10 - correctedWithdrawAmount;

        _assertWithdrawApprovalEmitted(address(spoke1), _daiReserveId(spoke1), alice, bob, expectedAllowance);
    ...
    function test_withdrawOnBehalfOf_revertsWith_InsufficientWithdrawAllowance(uint256 approvalAmount) public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(ITakerPositionManager.InsufficientWithdrawAllowance.selector, approvalAmount, amount)
        );
        vm.prank(bob);
        positionManager.withdrawOnBehalfOf(address(spoke1), _daiReserveId(spoke1), amount, alice);
    }
    ...
    function test_borrowOnBehalfOf_fuzz(uint256 borrowAmount, uint256 approveBorrowAmount) public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: alice, amount: aliceSupplyAmount, onBehalfOf: alice
        });
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: bobSupplyAmount, onBehalfOf: bob
        });
        ...
        vm.expectEmit(address(spoke1));
        emit ISpoke.Borrow(_daiReserveId(spoke1), address(positionManager), alice, expectedBorrowShares, borrowAmount);
        vm.expectEmit(address(positionManager));
        emit ITakerPositionManager.BorrowOnBehalfOf(
            address(spoke1), bob, alice, _daiReserveId(spoke1), expectedBorrowShares, borrowAmount
        );
        vm.recordLogs();
        vm.prank(bob);
        (returnValues.shares, returnValues.amount) =
        ...
        _assertBorrowApprovalEmitted(address(spoke1), _daiReserveId(spoke1), alice, bob, expectedBorrowAllowance);
        ...
        assertEq(tokenList.dai.allowance(address(positionManager), address(hub1)), 0);
        assertEq(
            positionManager.borrowAllowance(address(spoke1), _daiReserveId(spoke1), alice, bob), expectedBorrowAllowance
        );
    }
    ...
    function test_borrowOnBehalfOf_fuzz_withInterest(
        ...
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: initialBorrowAmount, onBehalfOf: bob
        });
    ...
    function test_borrowOnBehalfOf_fuzz_noAllowanceDecrease(uint256 borrowAmount) public {
        ...
        (uint256 userDrawnDebt, uint256 userPremiumDebt) = spoke1.getUserDebt(_daiReserveId(spoke1), alice);
        ...
        assertEq(tokenList.dai.balanceOf(address(bob)), callerBalanceBefore + borrowAmount);
        assertEq(tokenList.dai.allowance(address(positionManager), address(hub1)), 0);
        assertEq(positionManager.borrowAllowance(address(spoke1), _daiReserveId(spoke1), alice, bob), UINT256_MAX);
    }

    function test_borrowOnBehalfOf_revertsWith_InsufficientBorrowAllowance(uint256 approveBorrowAmount) public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ITakerPositionManager.InsufficientBorrowAllowance.selector, approveBorrowAmount, borrowAmount
            )
        );
        vm.prank(bob);
        positionManager.borrowOnBehalfOf(address(spoke1), _daiReserveId(spoke1), borrowAmount, alice);
    ...
    function _assertWithdrawApprovalEmitted(
        ...
        for (uint256 i; i < entries.length; ++i) {
            if (
                entries[i].topics[0] == sig && entries[i].topics[1] == bytes32(uint256(uint160(spoke)))
    ...
    function test_multicall() public {
        ...
        calls[0] = abi.encodeWithSignature(
            "approveWithdraw(address,uint256,address,uint256)", address(spoke1), _daiReserveId(spoke1), bob, amount
        );
        calls[1] = abi.encodeWithSignature(
            "approveBorrow(address,uint256,address,uint256)", address(spoke1), _daiReserveId(spoke1), bob, amount
        );
        ...
        assertEq(positionManager.withdrawAllowance(address(spoke1), _daiReserveId(spoke1), alice, bob), amount);
        assertEq(positionManager.borrowAllowance(address(spoke1), _daiReserveId(spoke1), alice, bob), amount);
    }
}
