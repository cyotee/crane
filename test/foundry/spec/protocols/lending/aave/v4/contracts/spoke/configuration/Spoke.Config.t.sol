...
contract SpokeConfigTest is Base {
    ...
    function test_spoke_deploy() public {
        ...
        ISpoke instance =
            ISpoke(address(AaveV4TestOrchestration.deploySpokeImplementation(oracle, MAX_ALLOWED_USER_RESERVES_LIMIT)));
    ...
    function test_updateReservePriceSource_revertsWith_AccessManagedUnauthorized(address caller) public {
        vm.assume(
            caller != SPOKE_ADMIN && caller != ADMIN && caller != SPOKE_CONFIGURATOR_ADMIN
                && caller != address(spokeConfigurator) && caller != ProxyHelper.getProxyAdmin(address(spoke1))
        );
    ...
    function test_updateReservePriceSource() public {
        ...
        vm.expectCall(address(oracle1), abi.encodeCall(IAaveOracle.setReserveSource, (reserveId, reserveSource)));
        vm.prank(SPOKE_ADMIN);
    ...
    function test_updateReserveConfig_fuzz(ISpoke.ReserveConfig memory newReserveConfig) public {
        newReserveConfig.collateralRisk =
            bound(newReserveConfig.collateralRisk, 0, MAX_ALLOWED_COLLATERAL_RISK).toUint24();
    ...
    function test_updateReserveConfig_revertsWith_InvalidCollateralRisk() public {
        ...
        config.collateralRisk = vm.randomUint(PercentageMath.PERCENTAGE_FACTOR * 10 + 1, type(uint24).max).toUint24();
    ...
    function test_addReserve() public {
        ...
        ISpoke.DynamicReserveConfig memory newDynReserveConfig =
            ISpoke.DynamicReserveConfig({collateralFactor: 10_00, maxLiquidationBonus: 110_00, liquidationFee: 10_00});
        ...
        vm.expectEmit(address(spoke1));
        emit ISpoke.AddDynamicReserveConfig({reserveId: reserveId, dynamicConfigKey: 0, config: newDynReserveConfig});

        vm.prank(SPOKE_ADMIN);
        spoke1.addReserve(address(hub1), usdzAssetId, reserveSource, newReserveConfig, newDynReserveConfig);
    ...
    function test_addReserve_fuzz_revertsWith_AssetNotListed() public {
        ...
        ISpoke.ReserveConfig memory newReserveConfig = _getDefaultReserveConfig(10_00);
        ISpoke.DynamicReserveConfig memory newDynReserveConfig =
            ISpoke.DynamicReserveConfig({collateralFactor: 10_00, maxLiquidationBonus: 110_00, liquidationFee: 0});
    ...
    function test_addReserve_revertsWith_InvalidAddress_hub() public {
        ...
        vm.prank(ADMIN);
        newSpoke.addReserve(address(0), vm.randomUint(), vm.randomAddress(), newReserveConfig, newDynReserveConfig);
    }
    ...
    function test_addReserve_revertsWith_InvalidAddress_oracle() public {
        ...
        vm.expectRevert(ISpoke.InvalidAddress.selector, address(newSpoke));
        vm.prank(ADMIN);
        newSpoke.addReserve(address(hub1), wethAssetId, address(0), newReserveConfig, newDynReserveConfig);
    }
    ...
    function test_updateLiquidationConfig_fuzz_targetHealthFactor(uint128 newTargetHealthFactor) public {
        newTargetHealthFactor =
            bound(newTargetHealthFactor, HEALTH_FACTOR_LIQUIDATION_THRESHOLD, type(uint128).max).toUint128();
        ...
        assertEq(spoke1.getLiquidationConfig().targetHealthFactor, newTargetHealthFactor, "wrong target health factor");
    }
    ...
    function test_updateLiquidationConfig_fuzz_liqBonusConfig(ISpoke.LiquidationConfig memory liquidationConfig)
        public
    {
        liquidationConfig.healthFactorForMaxBonus = bound(
                liquidationConfig.healthFactorForMaxBonus, 0, HEALTH_FACTOR_LIQUIDATION_THRESHOLD - 1
            ).toUint64();
        liquidationConfig.liquidationBonusFactor =
            bound(liquidationConfig.liquidationBonusFactor, 0, MAX_LIQUIDATION_BONUS_FACTOR).toUint16();
        liquidationConfig.targetHealthFactor = bound(
                liquidationConfig.targetHealthFactor, HEALTH_FACTOR_LIQUIDATION_THRESHOLD, type(uint128).max
            ).toUint128();
    ...
    function test_updateLiquidationConfig_revertsWith_InvalidLiquidationConfig_healthFactorForMaxBonus() public {
        ISpoke.LiquidationConfig memory liquidationConfig = ISpoke.LiquidationConfig({
            targetHealthFactor: HEALTH_FACTOR_LIQUIDATION_THRESHOLD,
    ...
    function test_updateLiquidationConfig_fuzz_revertsWith_InvalidLiquidationConfig_healthFactorForMaxBonus(
        ...
        liquidationConfig.healthFactorForMaxBonus = bound(
                liquidationConfig.healthFactorForMaxBonus, HEALTH_FACTOR_LIQUIDATION_THRESHOLD, type(uint64).max
            ).toUint64();
        liquidationConfig.liquidationBonusFactor =
            bound(liquidationConfig.liquidationBonusFactor, 0, MAX_LIQUIDATION_BONUS_FACTOR).toUint16();
        liquidationConfig.targetHealthFactor = bound(
    ...
    function test_updateLiquidationConfig_revertsWith_InvalidLiquidationConfig_liquidationBonusFactor() public {
        ISpoke.LiquidationConfig memory liquidationConfig = ISpoke.LiquidationConfig({
            targetHealthFactor: HEALTH_FACTOR_LIQUIDATION_THRESHOLD,
            healthFactorForMaxBonus: 0.9e18,
        ...
        test_updateLiquidationConfig_fuzz_revertsWith_InvalidLiquidationConfig_liquidationBonusFactor(liquidationConfig);
    }
    ...
    function test_updateLiquidationConfig_fuzz_revertsWith_InvalidLiquidationConfig_liquidationBonusFactor(
        ...
        liquidationConfig.healthFactorForMaxBonus =
            bound(liquidationConfig.healthFactorForMaxBonus, 0, HEALTH_FACTOR_LIQUIDATION_THRESHOLD).toUint64();
        liquidationConfig.liquidationBonusFactor = bound(
                liquidationConfig.liquidationBonusFactor, MAX_LIQUIDATION_BONUS_FACTOR + 1, type(uint16).max
            ).toUint16();
        liquidationConfig.targetHealthFactor = bound(
                liquidationConfig.targetHealthFactor, HEALTH_FACTOR_LIQUIDATION_THRESHOLD, type(uint128).max
            ).toUint128(); // valid values
