...
contract HubRefreshPremiumTest is Base {
    using SafeCast for *;
    ...
    function _createDrawnSharesAndPremiumData() internal {
        ...
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: amount1, onBehalfOf: bob
        });
        skip(322 days);
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: amount2, onBehalfOf: bob
        });
        skip(322 days);
    }
    ...
    function test_refreshPremium_riskPremiumThreshold() public {
        ...
        riskPremiumThreshold = (vm.randomUint(0, MAX_RISK_PREMIUM_THRESHOLD - 1)).toUint24();
        _updateSpokeRiskPremiumThreshold(hub1, daiAssetId, address(spoke1), riskPremiumThreshold);
    ...
    function test_refreshPremium_maxRiskPremiumThreshold() public {
        ...
        _updateSpokeRiskPremiumThreshold(hub1, daiAssetId, address(spoke1), MAX_RISK_PREMIUM_THRESHOLD);

        assertEq(hub1.getSpokeConfig(daiAssetId, address(spoke1)).riskPremiumThreshold, MAX_RISK_PREMIUM_THRESHOLD);
        ...
        IHubBase.PremiumDelta memory premiumDelta = _getExpectedPremiumDelta({
            ...
            riskPremium: MAX_ALLOWED_COLLATERAL_RISK + 1,
            restoredPremiumRay: 0
        });
        ...
        assertFalse(
            premiumData.premiumShares + premiumDelta.sharesDelta.toUint256()
                <= spokeData.drawnShares.percentMulUp(MAX_ALLOWED_COLLATERAL_RISK)
        );
    ...
    function test_refreshPremium_emitsEvent() public {
        ...
        assertEq(_loadAssetPremiumData(hub1, daiAssetId), _applyPremiumDelta(premiumDataBefore, premiumDelta));
    ...
    function test_refreshPremium_fuzz_positiveDeltas(uint256 borrowAmount, int256 sharesDelta, int256 offsetRayDelta)
        public
    {
        ...
        if (borrowAmount > 0) {
            ...
            _updateSpokeRiskPremiumThreshold(hub1, daiAssetId, address(spoke1), MAX_RISK_PREMIUM_THRESHOLD);
            SpokeActions.supplyCollateral({
                spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: borrowAmount * 2, onBehalfOf: bob
            });
            SpokeActions.borrow({
        ...
        if (expectedOffsetRay > _calculatePremiumAssetsRay(hub1, daiAssetId, expectedPremiumShares).toInt256()) {
            reverting = true;
            vm.expectRevert(
                abi.encodeWithSelector(
                    SafeCast.SafeCastOverflowedIntToUint.selector,
                    _calculatePremiumAssetsRay(hub1, daiAssetId, expectedPremiumShares).toInt256() - expectedOffsetRay
                )
            );
        } else if (
            riskPremiumThreshold != MAX_RISK_PREMIUM_THRESHOLD
                && asset.drawnShares.percentMulUp(riskPremiumThreshold) < asset.premiumShares + sharesDelta.toUint256()
        ) {
            reverting = true;
            vm.expectRevert(IHub.InvalidPremiumChange.selector);
        } else if (_calculatePremiumAssetsRay(hub1, daiAssetId, sharesDelta.toUint256()).toInt256() != offsetRayDelta) {
            reverting = true;
            vm.expectRevert(IHub.InvalidPremiumChange.selector);
        }
        vm.prank(address(spoke1));
    ...
    function test_refreshPremium_negativeDeltas(uint256 sharesDeltaPos) public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: 10000e18, onBehalfOf: bob
        });
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: 5000e18, onBehalfOf: bob
        });
        ...
        assertEq(_loadAssetPremiumData(hub1, assetId), _applyPremiumDelta(premiumDataBefore, premiumDelta));
        assertEq(premiumAfter, premiumBefore, "premium should not change");
        _assertDrawnRateSynced(hub1, daiAssetId, "after refreshPremium");
    }
    ...
    function test_refreshPremium_negativeDeltas_withAccrual(uint256 sharesDeltaPos) public {
        ...
        skip(322 days);
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: 1e18, onBehalfOf: bob
        });
    ...
    function test_refreshPremium_fuzz_withAccrual(
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: MAX_SUPPLY_AMOUNT, onBehalfOf: bob
        });
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: borrowAmount, onBehalfOf: bob
        });
        ...
        userAccruedPremiumRay = bound(
            ...
            0,
            _calculatePremiumDebtRay(hub1, assetId, asset.premiumShares, asset.premiumOffsetRay)
                .min(_calculatePremiumAssetsRay(hub1, assetId, userPremiumShares))
        );
        uint256 userPremiumOffsetRay =
            _calculatePremiumAssetsRay(hub1, assetId, userPremiumShares) - userAccruedPremiumRay;

        // New user position
        userPremiumSharesNew =
            bound(userPremiumSharesNew, 0, hub1.previewRestoreByAssets(assetId, MAX_SUPPLY_AMOUNT / 2));
        ...
        if (asset.drawnShares.percentMulUp(1000_00) < expectedPremiumShares) {
            reverting = true;
            vm.expectRevert(IHub.InvalidPremiumChange.selector);
        } else if (premiumDelta.sharesDelta < 0 && -premiumDelta.sharesDelta > asset.premiumShares.toInt256()) {
            reverting = true;
    ...
    function test_refreshPremium_spokePremiumUpdateIsContained() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke2, reserveId: _daiReserveId(spoke2), caller: alice, amount: 10000e18, onBehalfOf: alice
        });
        SpokeActions.borrow({
            spoke: spoke2, reserveId: _daiReserveId(spoke2), caller: alice, amount: 5000e18, onBehalfOf: alice
        });

        skip(322 days);

        (uint256 spoke1PremiumShares, int256 spoke1PremiumOffsetRay) =
            hub1.getSpokePremiumData(assetId, address(spoke1));
        (uint256 spoke2PremiumShares, int256 spoke2PremiumOffsetRay) =
            hub1.getSpokePremiumData(assetId, address(spoke2));
        uint256 spoke1PremiumDebtRay =
            _calculatePremiumDebtRay(hub1, assetId, spoke1PremiumShares, spoke1PremiumOffsetRay);
        uint256 spoke2PremiumDebtRay =
            _calculatePremiumDebtRay(hub1, assetId, spoke2PremiumShares, spoke2PremiumOffsetRay);
        ...
        hub1.refreshPremium(
            ...
            IHubBase.PremiumDelta({
                ...
                offsetRayDelta: spoke1PremiumAssetsRay.toInt256()
                    - (spoke1PremiumDebtRay + spoke2PremiumDebtRay).toInt256() - spoke1PremiumOffsetRay,
                restoredPremiumRay: 0
            })
        );
    }

    function _loadAssetPremiumData(IHub hub, uint256 assetId) internal view returns (PremiumDataLocal memory) {
        IHub.Asset memory asset = hub.getAsset(assetId);
        return PremiumDataLocal(asset.premiumShares, asset.premiumOffsetRay);
    }

    function _applyPremiumDelta(PremiumDataLocal memory premiumData, IHubBase.PremiumDelta memory premiumDelta)
        internal
        pure
        returns (PremiumDataLocal memory)
    {
