...
contract SpokeLiquidationCallBaseTest is LiquidationLogicBaseTest {
    ...
    function _bound(ISpoke spoke, uint256 collateralReserveId, uint256 debtReserveId)
        internal
        view
        virtual
    ...
    function _boundDebtToCoverNoDustRevert(
        ...
        debtToCover = bound(debtToCover, _convertValueToAmount(spoke, debtReserveId, 1e26), MAX_SUPPLY_AMOUNT);

        LiquidationLogic.CalculateLiquidationAmountsParams memory params =
            _getCalculateLiquidationAmountsParams(spoke, collateralReserveId, debtReserveId, user, debtToCover);
        try liquidationLogicWrapper.calculateLiquidationAmounts(params) returns (
            LiquidationLogic.LiquidationAmounts memory
        ) {}
        catch {
            uint256 liquidationBonus =
                spoke.getLiquidationBonus(collateralReserveId, user, spoke.getUserAccountData(user).healthFactor);
            uint256 debtReserveBalance =
                params.drawnShares.rayMulUp(params.drawnIndex) + params.premiumDebtRay.fromRayUp();
            uint256 collateralReserveBalance = params.collateralReserveHub
            .previewRemoveByShares(params.collateralReserveAssetId, params.suppliedShares);
            debtToCover = bound(
                debtToCover,
        ...
        SpokeActions.approve({
            spoke: spoke, reserveId: debtReserveId, owner: liquidator, amount: debtToCover.percentMulUp(101_00)
        });
    ...
    function _getCalculateDebtToTargetHealthFactorParams(
        ...
        return LiquidationLogic.CalculateDebtToTargetHealthFactorParams({
            ...
            collateralFactor: spoke.getDynamicReserveConfig(
                collateralReserveId, spoke.getUserPosition(collateralReserveId, user).dynamicConfigKey
            )
            .collateralFactor,
            liquidationBonus: spoke.getLiquidationBonus(collateralReserveId, user, userAccountData.healthFactor),
    ...
    function _getCalculateLiquidationAmountsParams(
        ...
        return LiquidationLogic.CalculateLiquidationAmountsParams({
            ...
            maxLiquidationBonus: spoke.getDynamicReserveConfig(
                collateralReserveId, spoke.getUserPosition(collateralReserveId, user).dynamicConfigKey
            )
            .maxLiquidationBonus,
            targetHealthFactor: spoke.getLiquidationConfig().targetHealthFactor,
            healthFactor: userAccountData.healthFactor,
            liquidationFee: spoke.getDynamicReserveConfig(
                collateralReserveId, spoke.getUserPosition(collateralReserveId, user).dynamicConfigKey
            )
            .liquidationFee
        });
    ...
    function _makeUserLiquidatable(ISpoke spoke, address user, uint256 debtReserveId, uint256 newHealthFactor)
        internal
        virtual
    {
        // add liquidity
        _openSupplyPosition(
            spoke, debtReserveId, _getRequiredDebtAmountForHf(spoke, user, debtReserveId, newHealthFactor)
        );
    ...
    function _calculateExpectedUserAccountData(
        ...
        for (uint256 reserveId = 0; reserveId < params.spoke.getReserveCount(); reserveId++) {
            ...
            uint256 userSuppliedShares = params.spoke.getUserPosition(reserveId, params.user).suppliedShares;
            ...
            if (params.debtReserveId == reserveId) {
                ...
                uint256 premiumDebtRay =
                    _calculatePremiumDebtRay(asset.premiumShares, asset.premiumOffsetRay, drawnIndex);
                totalAddedAssets += liquidationMetadata.debtAssetsToLiquidate;
                uint256 aggregatedOwedRayBefore = asset.drawnShares * drawnIndex + premiumDebtRay + asset.deficitRay;
            ...
            uint256 userSuppliedAssets =
                userSuppliedShares.mulDivDown(totalAddedAssets + VIRTUAL_ASSETS, totalAddedShares + VIRTUAL_SHARES);
            uint256 userSuppliedValue = _convertAmountToValue(params.spoke, reserveId, userSuppliedAssets);
            list.add(
            ...
            expectedUserAccountData.avgCollateralFactor += _getCollateralFactor(params.spoke, reserveId, params.user)
            * userSuppliedValue;
        }
        ...
        if (expectedUserAccountData.totalCollateralValue != 0) {
            expectedUserAccountData.avgCollateralFactor = expectedUserAccountData.avgCollateralFactor
                .mulDivDown(
                    WadRayMath.WAD / PercentageMath.PERCENTAGE_FACTOR, expectedUserAccountData.totalCollateralValue
                );
        }
        list.sortByKey();
        ...
        for (uint256 i = 0; i < list.length() && remainingDebtToCover > 0; i++) {
            ...
            expectedUserAccountData.riskPremium += collateralRisk * _min(collateralValue, remainingDebtToCover);
        ...
        expectedUserAccountData.riskPremium = _divUp(
            ...
            _max(
                1,
                _min(
                    expectedUserAccountData.totalDebtValueRay.fromRayUp(), expectedUserAccountData.totalCollateralValue
                )
            )
        );
    ...
    function _expectEventsAndCalls(
        ...
        (vars.userDrawnDebt, vars.userPremiumDebt) = params.spoke.getUserDebt(params.debtReserveId, params.user);

        vars.drawnAmountToRestore =
            vars.debtHub.previewRestoreByShares(vars.debtAssetId, liquidationMetadata.drawnSharesToLiquidate);
        uint256 amountToRestore = vars.drawnAmountToRestore + liquidationMetadata.premiumDebtRayToLiquidate.fromRayUp();
        vars.premiumDelta =
            _getExpectedPremiumDeltaForRestore(params.spoke, params.user, params.debtReserveId, amountToRestore);

        if (liquidationMetadata.collateralSharesToLiquidate > liquidationMetadata.collateralSharesToLiquidator) {
        ...
        emit ISpoke.LiquidationCall({
            ...
            collateralAmountRemoved: vars.collateralHub
                .previewRemoveByShares(vars.collateralAssetId, liquidationMetadata.collateralSharesToLiquidate),
        ...
        vm.expectCall(
            ...
            abi.encodeCall(
                IHubBase.remove,
                (vars.collateralAssetId, liquidationMetadata.collateralAssetsToLiquidator, params.liquidator)
            ),
        ...
        vm.expectCall(
            ...
            abi.encodeCall(IHubBase.restore, (vars.debtAssetId, vars.drawnAmountToRestore, vars.premiumDelta)),
            1
        );
        ...
        vm.expectCall(
            ...
            abi.encodeCall(
                ...
                (
                    ...
                    liquidationMetadata.collateralSharesToLiquidate - liquidationMetadata.collateralSharesToLiquidator
                )
            ),
            liquidationMetadata.collateralSharesToLiquidate > liquidationMetadata.collateralSharesToLiquidator ? 1 : 0
        );

        bool riskPremiumOptimisation =
            accountsInfoBefore.userLastRiskPremium == 0 && expectedUserAccountData.riskPremium == 0;

        {
            for (uint256 i = params.spoke.getReserveCount(); i != 0;) {
                ...
                if (_isBorrowing(params.spoke, reserveId, params.user)) {
                    ...
                    if (reserveId == params.debtReserveId) {
                        vars.userReservePosition.drawnShares -= liquidationMetadata.drawnSharesToLiquidate.toUint120();
                        ...
                        vars.userReservePosition.premiumShares = uint256(vars.userReservePosition.premiumShares)
                            .add(vars.premiumDelta.sharesDelta).toUint120();
                        vars.userReservePosition.premiumOffsetRay =
                            (vars.userReservePosition.premiumOffsetRay + vars.premiumDelta.offsetRayDelta).toInt200();
                    }

                    IHub targetHub = _hub(params.spoke, reserveId);
                    uint256 userReserveDrawnDebt =
                        targetHub.previewRestoreByShares(assetId, vars.userReservePosition.drawnShares);
                    ...
                    if (liquidationMetadata.hasDeficit) {
                        ...
                        vm.expectCall(
                            address(targetHub), abi.encodeWithSelector(IHubBase.reportDeficit.selector, assetId), 0
                        );
                        ...
                        if (!riskPremiumOptimisation) {
                            ...
                            vm.expectCall(
                                address(targetHub), abi.encodeCall(IHubBase.refreshPremium, (assetId, premiumDelta)), 1
                            );
                            vm.expectEmit(address(params.spoke));
                            emit ISpoke.RefreshPremiumDebt({
                                reserveId: reserveId, user: params.user, premiumDelta: premiumDelta
                            });
                        } else {
                            vm.expectCall(
                                address(targetHub), abi.encodeWithSelector(IHubBase.refreshPremium.selector, assetId), 0
                            );
                        }
                    }
                }
            ...
            if (!liquidationMetadata.hasDeficit && !riskPremiumOptimisation) {
                vm.expectEmit(address(params.spoke));
                emit ISpoke.UpdateUserRiskPremium({user: params.user, riskPremium: expectedUserAccountData.riskPremium});
            }
        }
    }

    function _getBalanceInfo(ISpoke spoke, address addr, uint256 collateralReserveId, uint256 debtReserveId)
        internal
        virtual
        returns (LiquidationBalanceSnapshot memory)
    {
        return LiquidationBalanceSnapshot({
            collateralErc20Balance: _getAssetUnderlyingByReserveId(spoke, collateralReserveId).balanceOf(addr),
            suppliedInSpoke: spoke.getUserSuppliedAssets(collateralReserveId, addr),
            addedInHub: _hub(spoke, collateralReserveId)
                .getSpokeAddedAssets(_reserveAssetId(spoke, collateralReserveId), addr),
            ...
            drawnFromHub: _hub(spoke, debtReserveId).getSpokeTotalOwed(_reserveAssetId(spoke, debtReserveId), addr)
        });
    }

    function _getAccountsInfo(CheckedLiquidationCallParams memory params)
        internal
        virtual
        returns (LiquidationAccountsSnapshot memory)
    {
        return LiquidationAccountsSnapshot({
            userAccountData: params.spoke.getUserAccountData(params.user),
            userBalanceInfo: _getBalanceInfo(
                params.spoke, params.user, params.collateralReserveId, params.debtReserveId
            ),
            ...
            liquidatorBalanceInfo: _getBalanceInfo(
                params.spoke, params.liquidator, params.collateralReserveId, params.debtReserveId
            ),
            ...
            spokeBalanceInfo: _getBalanceInfo(
                params.spoke, address(params.spoke), params.collateralReserveId, params.debtReserveId
            ),
    ...
    function _getLiquidationMetadata(
        ...
        uint256 debtRayToTarget = liquidationLogicWrapper.calculateDebtToTargetHealthFactor(
            _getCalculateDebtToTargetHealthFactorParams(
                params.spoke, params.collateralReserveId, params.debtReserveId, params.user
            )
        );
        ...
        LiquidationLogic.LiquidationAmounts memory liquidationAmounts =
            liquidationLogicWrapper.calculateLiquidationAmounts(
                _getCalculateLiquidationAmountsParams(
                    params.spoke, params.collateralReserveId, params.debtReserveId, params.user, params.debtToCover
                )
            );

        uint256 liquidationBonus = params.spoke
        .getLiquidationBonus(params.collateralReserveId, params.user, userAccountDataBefore.healthFactor);
        ...
        uint256 drawnIndex = _hub(params.spoke, params.debtReserveId)
            .getAssetDrawnIndex(_reserveAssetId(params.spoke, params.debtReserveId));
        ...
        return LiquidationMetadata({
            ...
            debtAssetsToRestore: _calculateDebtAssetsToRestore(
                liquidationAmounts.drawnSharesToLiquidate, liquidationAmounts.premiumDebtRayToLiquidate, drawnIndex
            ),
    ...
    function _isCollateralAffectingUserHf(
        ...
        uint256 collateralValueRemoved =
            userAccountDataBefore.totalCollateralValue - userAccountDataAfter.totalCollateralValue;
        ...
        uint256 debtValueRayRepaid = _convertAmountToValue(
            params.spoke,
            params.debtReserveId,
            liquidationAmounts.drawnSharesToLiquidate * drawnIndex + liquidationAmounts.premiumDebtRayToLiquidate
        );
        ...
        uint256 effectiveLiquidationBonusWad = Math.mulDiv(
            collateralValueRemoved, WadRayMath.RAY * WadRayMath.WAD, debtValueRayRepaid, Math.Rounding.Ceil
        );

        // health factor is decreasing due to liquidation bonus / collateral factor if:
        //   lb * cf > hf_beforeLiq
        return effectiveLiquidationBonusWad
    ...
    function _checkErc20Balances(
        ...
        if (params.receiveShares) {
            _checkErc20BalancesReceiveShares(params, accountsInfoBefore, accountsInfoAfter, liquidationMetadata);
        } else {
            _checkErc20BalancesReceiveAssets(params, accountsInfoBefore, accountsInfoAfter, liquidationMetadata);
        }
    ...
    function _checkErc20BalancesReceiveShares(
        ...
        if (collateralHub == debtHub && params.collateralReserveId == params.debtReserveId) {
            ...
            assertEq(
                ...
                accountsInfoBefore.debtHubBalanceInfo.debtErc20Balance + liquidationMetadata.debtAssetsToLiquidate,
        ...
        if (
            ...
            assertEq(
                ...
                accountsInfoBefore.liquidatorBalanceInfo.debtErc20Balance - liquidationMetadata.debtAssetsToLiquidate,
    ...
    function _checkErc20BalancesReceiveAssets(
        ...
        if (collateralHub == debtHub && params.collateralReserveId == params.debtReserveId) {
            assertEq(
                accountsInfoAfter.collateralHubBalanceInfo.collateralErc20Balance,
                accountsInfoBefore.collateralHubBalanceInfo.collateralErc20Balance
                    - liquidationMetadata.collateralAssetsToLiquidator + liquidationMetadata.debtAssetsToLiquidate,
                "collateral hub: collateral erc20 balance"
            );
        } else {
            assertEq(
    ...
    function _checkSpokeBalances(
        ...
        assertApproxEqAbs(
            ...
            accountsInfoBefore.userBalanceInfo.suppliedInSpoke - liquidationMetadata.collateralAssetsToLiquidate,
            2,
        ...
        assertApproxEqAbs(
            ...
            (liquidationMetadata.hasDeficit)
                ? 0
                : accountsInfoBefore.userBalanceInfo.borrowedFromSpoke - liquidationMetadata.debtAssetsToLiquidate,
            2,
            "user: debt borrowed"
        );
    ...
    function _checkHubBalances(
        ...
        // User
        assertEq(
            accountsInfoAfter.userBalanceInfo.addedInHub, accountsInfoBefore.userBalanceInfo.addedInHub, "user: added"
        );
        assertEq(
        ...
        assertApproxEqAbs(
            ...
            accountsInfoBefore.collateralFeeReceiverBalanceInfo.addedInHub
                + liquidationMetadata.collateralAssetsToLiquidate - liquidationMetadata.collateralAssetsToLiquidator,
            2,
        ...
        assertApproxEqAbs(
            ...
            (liquidationMetadata.hasDeficit)
                ...
                : accountsInfoBefore.spokeBalanceInfo.drawnFromHub - liquidationMetadata.debtAssetsToLiquidate,
            2,
            "spoke: drawn"
        );
    }
    ...
    function _checkUserAccountData(
        ...
        for (uint256 reserveId = 0; reserveId < params.spoke.getReserveCount(); reserveId++) {
            if (_isBorrowing(params.spoke, reserveId, params.user)) {
                ISpoke.UserPosition memory userPosition = params.spoke.getUserPosition(reserveId, params.user);
    ...
    function _checkedLiquidationCall(CheckedLiquidationCallParams memory params) internal virtual {
        ...
        LiquidationMetadata memory liquidationMetadata =
            _getLiquidationMetadata(params, accountsInfoBefore.userAccountData);
        ISpoke.UserAccountData memory expectedUserAccountData =
            _calculateExpectedUserAccountData(params, liquidationMetadata);
        ...
        params.spoke
            .liquidationCall(
                params.collateralReserveId, params.debtReserveId, params.user, params.debtToCover, params.receiveShares
            );
        ...
        _assertHubLiquidity(
            _hub(params.spoke, params.collateralReserveId), params.collateralReserveId, "spoke1.liquidationCall"
        );
        _assertHubLiquidity(_hub(params.spoke, params.debtReserveId), params.debtReserveId, "spoke1.liquidationCall");
    }
}
