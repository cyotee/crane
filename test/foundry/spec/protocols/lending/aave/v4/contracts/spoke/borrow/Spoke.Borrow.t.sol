...
contract SpokeBorrowTest is Base {
    function test_borrow_revertsWith_ReentrancyGuardReentrantCall_hubDraw() public {
        ...
        MockReentrantCaller reentrantCaller = new MockReentrantCaller(address(spoke1), ISpoke.borrow.selector);
    ...
    function test_borrow() public {
        ...
        state.wethAlice.premiumDebtRayBefore = _calculatePremiumDebtRay(spoke1, state.wethReserveId, alice);
        ...
        vm.prank(bob);
        (returnValues.shares, returnValues.amount) = spoke1.borrow(state.daiReserveId, state.daiBob.borrowAmount, bob);
    ...
    function test_borrow_fuzz_amounts(uint256 wethSupplyAmount, uint256 daiBorrowAmount) public {
        ...
        assertEq(state.wethBob.userBalanceAfter, state.wethBob.userBalanceBefore, "bob weth balance after");
        assertEq(state.daiAlice.userBalanceAfter, state.daiAlice.userBalanceBefore, "alice dai balance after");
        assertEq(state.wethAlice.userBalanceAfter, state.wethAlice.userBalanceBefore, "alice weth balance after");
