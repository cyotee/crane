...
contract SpokeMultipleHubTest is Base {
    ...
    function setUp() public virtual override {
        ...
        ISpoke.DynamicReserveConfig memory dynDaiHub3Config =
            ISpoke.DynamicReserveConfig({collateralFactor: 78_00, maxLiquidationBonus: 100_00, liquidationFee: 0});
        daiHub3ReserveId = spoke1.addReserve(
            address(hub3), hub3DaiAssetId, _deployMockPriceFeed(spoke1, 1e8), daiHub3Config, dynDaiHub3Config
        );
    ...
    function test_borrow_secondHub() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: hub1SupplyAmount, onBehalfOf: bob
        });
        ...
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: hub1BorrowAmount, onBehalfOf: bob
        });
        ...
        SpokeActions.supply({
            spoke: spoke1, reserveId: daiHub2ReserveId, caller: alice, amount: MAX_SUPPLY_AMOUNT, onBehalfOf: alice
        });

        // Bob can also borrow dai from hub 2 via spoke 1
        SpokeActions.borrow({
            spoke: spoke1, reserveId: daiHub2ReserveId, caller: bob, amount: hub2BorrowAmount, onBehalfOf: bob
        });
        ...
        assertEq(address(_getAssetUnderlyingByReserveId(spoke1, _daiReserveId(spoke1))), address(tokenList.dai));
        assertEq(address(_getAssetUnderlyingByReserveId(spoke1, daiHub2ReserveId)), address(tokenList.dai));

        // Bob can partially repay both debt positions on hub 1 and hub 2
        SpokeActions.repay({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: hub1RepayAmount, onBehalfOf: bob
        });
        assertEq(spoke1.getUserTotalDebt(_daiReserveId(spoke1), bob), hub1BorrowAmount - hub1RepayAmount);
        assertEq(hub1.getAssetTotalOwed(daiAssetId), hub1BorrowAmount - hub1RepayAmount);

        SpokeActions.repay({
            spoke: spoke1, reserveId: daiHub2ReserveId, caller: bob, amount: hub2RepayAmount, onBehalfOf: bob
        });
    ...
    function test_borrow_thirdHub() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: bob, amount: daiSupplyAmount, onBehalfOf: bob
        });
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: daiHub3ReserveId, caller: bob, amount: daiSupplyAmount, onBehalfOf: bob
        });
