...
contract SpokeGettersTest is Base {
    ...
    function setUp() public virtual override {
        ...
        spokeInfo[spoke].wbtc.dynReserveConfig =
            ISpoke.DynamicReserveConfig({collateralFactor: 75_00, maxLiquidationBonus: 103_00, liquidationFee: 15_00});
        spokeInfo[spoke].dai.reserveConfig = _getDefaultReserveConfig(20_00);
        spokeInfo[spoke].dai.dynReserveConfig =
            ISpoke.DynamicReserveConfig({collateralFactor: 78_00, maxLiquidationBonus: 102_00, liquidationFee: 10_00});
        spokeInfo[spoke].usdx.reserveConfig = _getDefaultReserveConfig(50_00);
        spokeInfo[spoke].usdx.dynReserveConfig =
            ISpoke.DynamicReserveConfig({collateralFactor: 78_00, maxLiquidationBonus: 101_00, liquidationFee: 12_00});
        spokeInfo[spoke].usdy.reserveConfig = _getDefaultReserveConfig(20_00);
        spokeInfo[spoke].usdy.dynReserveConfig =
            ISpoke.DynamicReserveConfig({collateralFactor: 78_00, maxLiquidationBonus: 101_50, liquidationFee: 15_00});
    ...
    function test_getLiquidationBonus_fuzz_notConfigured(uint256 reserveId, uint256 healthFactor) public {
        ...
        assertEq(
            ...
            ISpoke.LiquidationConfig({
                targetHealthFactor: WadRayMath.WAD.toUint128(), healthFactorForMaxBonus: 0, liquidationBonusFactor: 0
            })
        );
    ...
    function test_getLiquidationBonus_fuzz_configured(
        ...
        reserveId = bound(reserveId, 0, spoke.getReserveCount() - 1);
        healthFactor = bound(healthFactor, 0, HEALTH_FACTOR_LIQUIDATION_THRESHOLD);
        liquidationBonusFactor = bound(liquidationBonusFactor, 0, PercentageMath.PERCENTAGE_FACTOR).toUint16();
        healthFactorForMaxBonus = bound(healthFactorForMaxBonus, 0, HEALTH_FACTOR_LIQUIDATION_THRESHOLD - 1).toUint64();
    ...
    function test_protocol_getters() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke, reserveId: reserveId, caller: alice, amount: supplyAmount, onBehalfOf: alice
        });
        ...
        assertEq(spoke.getUserSuppliedShares(reserveId, alice), hub1.previewAddByAssets(assetId, supplyAmount));

        // Reserve supply
        assertEq(spoke.getReserveSuppliedAssets(reserveId), supplyAmount);
        assertEq(spoke.getReserveSuppliedShares(reserveId), hub1.previewAddByAssets(assetId, supplyAmount));
        ...
        (uint256 premiumShares, int256 premiumOffset) = hub1.getSpokePremiumData(assetId, address(spoke));
        ...
        assertEq(hub1.getSpokeAddedShares(assetId, address(spoke)), hub1.previewAddByAssets(assetId, supplyAmount));
    ...
    function test_premiumRayGetters() public {
        ...
        SpokeActions.approve({spoke: spoke, reserveId: _daiReserveId(spoke), owner: alice, amount: 9_000e18});
        SpokeActions.supplyCollateral({
            spoke: spoke, reserveId: _daiReserveId(spoke), caller: alice, amount: 9_000e18, onBehalfOf: alice
        }); // CR 20%
        SpokeActions.approve({spoke: spoke, reserveId: _usdxReserveId(spoke), owner: bob, amount: 18_000e18});
        SpokeActions.supplyCollateral({
            spoke: spoke, reserveId: _usdxReserveId(spoke), caller: bob, amount: 18_000e18, onBehalfOf: bob
        }); // CR 50%
        _openSupplyPosition(spoke, _wethReserveId(spoke), 5e18); // liquidity provision
        SpokeActions.borrow({
            spoke: spoke, reserveId: _wethReserveId(spoke), caller: alice, amount: 1e18, onBehalfOf: alice
        });
        SpokeActions.borrow({
            spoke: spoke, reserveId: _wethReserveId(spoke), caller: bob, amount: 2e18, onBehalfOf: bob
        });
        skip(365 days);
        ...
        assertEq(hub1.getSpokePremiumRay(wethAssetId, address(spoke)), alicePremiumDebtRay + bobPremiumDebtRay);
        assertEq(hub1.getAssetPremiumRay(wethAssetId), alicePremiumDebtRay + bobPremiumDebtRay);

        // introduce another spoke
        SpokeActions.approve({spoke: spoke, reserveId: _daiReserveId(spoke), owner: carol, amount: 1_000e18});
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: carol, amount: 1_000e18, onBehalfOf: carol
        });
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _wethReserveId(spoke), caller: carol, amount: 0.1e18, onBehalfOf: carol
        });
