...
contract SpokeDynamicConfigTriggersTest is Base {
    ...
    function test_supply_does_not_trigger_dynamicConfigUpdate() public {
        ...
        uint256 maxLiquidationBonus = _getUserDynConfig(spoke1, alice, _daiReserveId(spoke1)).maxLiquidationBonus;
        uint256 supplyAmount = 1000e6;
        uint256 collateralReserveId = _usdxReserveId(spoke1);
        uint256 debtReserveId = _daiReserveId(spoke1);
        uint256 collateralFactor = vm.randomUint(0, _collateralFactorUpperBound(maxLiquidationBonus)).toUint16();

        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: collateralReserveId, caller: alice, amount: supplyAmount, onBehalfOf: alice
        });
        ...
        uint256 borrowAmount = collateralFactor.percentMulDown(
            _convertValueToAmount(
                spoke1, debtReserveId, _convertAmountToValue(spoke1, collateralReserveId, supplyAmount)
            )
        );
        _openSupplyPosition(spoke1, debtReserveId, borrowAmount);
        SpokeActions.borrow({
            spoke: spoke1, reserveId: debtReserveId, caller: alice, amount: borrowAmount, onBehalfOf: alice
        });
        configs = _getUserDynConfigKeys(spoke1, alice);
        _updateCollateralFactor(
            spoke1, collateralReserveId, vm.randomUint(0, _collateralFactorUpperBound(maxLiquidationBonus)).toUint16()
        );
    ...
    function test_repay_does_not_trigger_dynamicConfigUpdate() public {
        DynamicConfigEntry[] memory configs = _getUserDynConfigKeys(spoke1, alice);
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _usdxReserveId(spoke1), caller: alice, amount: 1000e6, onBehalfOf: alice
        });
    ...
    function test_liquidate_does_not_trigger_dynamicConfigUpdate() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _usdxReserveId(spoke1), caller: alice, amount: 1_000_000e6, onBehalfOf: alice
        });
        _openSupplyPosition(spoke1, _daiReserveId(spoke1), 500_000e18);
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: alice, amount: 500_000e18, onBehalfOf: alice
        });
        configs = _getUserDynConfigKeys(spoke1, alice);
        skip(322 days);
        ...
        vm.prank(bob);
        spoke1.liquidationCall(_usdxReserveId(spoke1), _daiReserveId(spoke1), alice, UINT256_MAX, false);
    ...
    function test_withdraw_triggers_dynamicConfigUpdate() public {
        ...
        _updateCollateralFactor(spoke1, _usdxReserveId(spoke1), _randomCollateralFactor(spoke1, _usdxReserveId(spoke1)));
        configs = _getUserDynConfigKeys(spoke1, alice);
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _wethReserveId(spoke1), caller: alice, amount: 1e18, onBehalfOf: alice
        });
        ...
        SpokeActions.withdraw({
            spoke: spoke1, reserveId: _usdxReserveId(spoke1), caller: alice, amount: 500e6, onBehalfOf: alice
        });
    ...
    function test_updateUserDynamicConfig_reverts_when_not_authorized(address caller) public {
        vm.assume(
            caller != alice && caller != ADMIN && caller != POSITION_MANAGER && caller != SPOKE_ADMIN
        ...
        vm.expectRevert(abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, POSITION_MANAGER));
    ...
    function test_updateUserDynamicConfig_updatesRP() public {
        ...
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: alice, amount: 2000e18, onBehalfOf: alice
        });
    ...
    function _updateUserDynamicConfig(address caller, DynamicConfigEntry[] memory existingConfigs) internal {
