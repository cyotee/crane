...
} from "@crane/test/foundry/spec/protocols/lending/aave/v4/helpers/position-manager/taker-position-manager/TakerPositionManagerHelpers.sol";
...
contract TakerPositionManagerBaseTest is Base, TakerPositionManagerHelpers {
    ...
    function _withdrawPermitData(address spender, address onBehalfOf, uint256 deadline)
        internal
        view
        returns (ITakerPositionManager.WithdrawPermit memory)
    {
        return _withdrawPermitData(positionManager, spoke1, spender, onBehalfOf, deadline);
    }

    function _approveBorrowData(address spender, address onBehalfOf, uint256 deadline)
        internal
        view
        returns (ITakerPositionManager.BorrowPermit memory)
    {
