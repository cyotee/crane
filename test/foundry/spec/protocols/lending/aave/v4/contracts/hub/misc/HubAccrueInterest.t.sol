...
contract HubAccrueInterestTest is Base {
    ...
    function test_accrueInterest_NoInterest_OnlyAdd(uint40 elapsed) public {
        ...
        HubActions.add({
            hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: addAmount, user: address(spoke1)
        });
        ...
        HubActions.add({
            hub: hub1, assetId: daiAssetId, caller: address(spoke2), amount: addAmount, user: address(spoke2)
        });
    ...
    function test_accrueInterest_NoInterest_NoDebt(uint40 elapsed) public {
        ...
        HubActions.draw({
            hub: hub1, assetId: daiAssetId, caller: address(spoke1), to: address(spoke1), amount: borrowAmount
        });
        ...
        HubActions.add({
            hub: hub1, assetId: daiAssetId, caller: address(spoke2), amount: addAmount2, user: address(spoke2)
        });

        IHub.Asset memory daiInfo = hub1.getAsset(daiAssetId);

        (uint256 expectedDrawnIndex1, uint256 expectedDrawnDebt1) =
            _calculateExpectedDebt(daiInfo.drawnShares, WadRayMath.RAY, drawnRate, startTime);
        ...
        assertEq(_getAddedAssetsWithFees(hub1, daiAssetId), addAmount + addAmount2 + interest, "addAmount");
        ...
        (uint256 expectedDrawnIndex2, uint256 expectedDrawnDebt2) =
            _calculateExpectedDebt(daiInfo.drawnShares, expectedDrawnIndex1, drawnRate, startTime);
        ...
        assertEq(daiInfo.lastUpdateTimestamp, vm.getBlockTimestamp(), "lastUpdateTimestamp");
        assertEq(daiInfo.drawnIndex, expectedDrawnIndex2, "drawnIndex2");
        assertEq(_getAddedAssetsWithFees(hub1, daiAssetId), addAmount + addAmount2 * 2 + interest, "addAmount");
        assertEq(_getAssetDrawnDebt(hub1, daiAssetId), 0, "drawn");
    }
    ...
    function test_accrueInterest_fuzz_BorrowAndWait(uint40 elapsed) public {
        ...
        (uint256 expectedDrawnIndex, uint256 expectedDrawnDebt) =
            _calculateExpectedDebt(daiInfo.drawnShares, initialDrawnIndex, drawnRate, startTime);
    ...
    function test_accrueInterest_fuzz_BorrowAmountAndElapsed(uint256 borrowAmount, uint40 elapsed) public {
    ...
    function test_accrueInterest_fuzz_BorrowAmountRateAndElapsed(
        ...
        HubActions.add({
            hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: spoke1Amounts.add0, user: address(spoke1)
        });
        HubActions.draw({
            hub: hub1, assetId: daiAssetId, caller: address(spoke1), to: address(spoke1), amount: borrowAmount
        });
        ...
        (uint256 expectedDrawnIndex, uint256 expectedDrawnDebt1) =
            _calculateExpectedDebt(assetData.t0.drawnShares, initialDrawnIndex, assetData.t0.drawnRate, timestamps.t0);
        ...
        assertEq(_getAddedAssetsWithFees(hub1, daiAssetId), spoke1Amounts.add0 + addAmount2 + interest1, "addAmount");
        assertEq(_getAssetDrawnDebt(hub1, daiAssetId), expectedDrawnDebt1, "drawn");
        ...
        (expectedDrawnIndex, expectedDrawnDebt2) =
            _calculateExpectedDebt(assetData.t0.drawnShares, cumulated.t1, assetData.t2.drawnRate, timestamps.t1);
    ...
    function test_getAssetDrawnRate_fuzz_DiffersAfterTimePasses(uint40 elapsed) public {
        ...
        uint256 expectedRate = IBasicInterestRateStrategy(asset.irStrategy)
