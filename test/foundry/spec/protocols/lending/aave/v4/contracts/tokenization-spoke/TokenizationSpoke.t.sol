...
contract TokenizationSpokeTest is TokenizationSpokeBaseTest {
    ...
    function test_deposit() public {
        ...
        emit Transfer(address(0), depositor, expectedShares);
        vm.expectEmit(address(vault));
        emit IERC4626.Deposit({sender: depositor, owner: depositor, assets: assets, shares: expectedShares});
    ...
    function test_deposit_receiverDifferentFromCaller() public {
        ...
        vm.expectEmit(address(vault));
        emit IERC4626.Deposit({sender: depositor, owner: receiver, assets: assets, shares: expectedShares});
    ...
    function test_mint() public {
        ...
        vm.expectEmit(address(vault));
        emit Transfer(address(0), depositor, shares);
        vm.expectEmit(address(vault));
        emit IERC4626.Deposit({sender: depositor, owner: depositor, assets: expectedAssets, shares: shares});
    ...
    function test_mint_receiverDifferentFromCaller() public {
        ...
        vm.expectEmit(address(vault));
        emit IERC4626.Deposit({sender: depositor, owner: receiver, assets: expectedAssets, shares: shares});
    ...
    function test_withdraw() public {
        ...
        uint256 expectedShares = IHub(vault.hub()).previewRemoveByAssets(vault.assetId(), withdrawAssets);

        vm.expectCall(vault.hub(), abi.encodeCall(IHubBase.remove, (vault.assetId(), withdrawAssets, owner)));
        vm.expectEmit(address(vault));
        emit Transfer(owner, address(0), expectedShares);
        vm.expectEmit(address(vault));
        emit IERC4626.Withdraw({
            sender: owner, receiver: owner, owner: owner, assets: withdrawAssets, shares: expectedShares
        });
    ...
    function test_withdraw_ownerDifferentFromCaller() public {
        ...
        vm.expectCall(vault.hub(), abi.encodeCall(IHubBase.remove, (vault.assetId(), withdrawAssets, receiver)));
        vm.expectEmit(address(vault));
        emit Transfer(owner, address(0), expectedShares);
        vm.expectEmit(address(vault));
        emit IERC4626.Withdraw({
            sender: caller, receiver: receiver, owner: owner, assets: withdrawAssets, shares: expectedShares
        });
    ...
    function test_redeem() public {
        ...
        vm.expectCall(vault.hub(), abi.encodeCall(IHubBase.remove, (vault.assetId(), expectedAssets, owner)));
        vm.expectEmit(address(vault));
        emit Transfer(owner, address(0), redeemShares);
        vm.expectEmit(address(vault));
        emit IERC4626.Withdraw({
            sender: owner, receiver: owner, owner: owner, assets: expectedAssets, shares: redeemShares
        });
    ...
    function test_redeem_ownerDifferentFromCaller() public {
        ...
        vm.expectCall(vault.hub(), abi.encodeCall(IHubBase.remove, (vault.assetId(), expectedAssets, receiver)));
        vm.expectEmit(address(vault));
        emit Transfer(owner, address(0), redeemShares);
        vm.expectEmit(address(vault));
        emit IERC4626.Withdraw({
            sender: caller, receiver: receiver, owner: owner, assets: expectedAssets, shares: redeemShares
        });
