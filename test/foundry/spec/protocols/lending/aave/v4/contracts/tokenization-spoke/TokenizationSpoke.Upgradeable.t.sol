...
import {
    MockTokenizationSpokeInstance
} from "@crane/test/foundry/spec/protocols/lending/aave/v4/helpers/mocks/MockTokenizationSpokeInstance.sol";
...
contract TokenizationSpokeUpgradeableTest is TokenizationSpokeBaseTest {
    ...
    function test_proxy_constructor_fuzz_revertsWith_InvalidInitialization(uint64 initialRevision) public {
        ...
        ITransparentUpgradeableProxy vaultProxy = ITransparentUpgradeableProxy(
            address(
                new TransparentUpgradeableProxy(
                    address(vaultImpl), proxyAdminOwner, _getInitializeCalldata(SHARE_NAME, SHARE_SYMBOL)
                )
            )
        );
        ...
        vm.expectRevert(Initializable.InvalidInitialization.selector);
        vm.prank(ProxyHelper.getProxyAdmin(address(vaultProxy)));
        vaultProxy.upgradeToAndCall(address(vaultImpl), _getInitializeCalldata(SHARE_NAME, SHARE_SYMBOL));

        uint64 secondRevision = uint64(vm.randomUint(0, initialRevision - 1));
        TokenizationSpokeInstance vaultImpl2 = _deployMockTokenizationSpokeInstance(secondRevision);
        vm.expectRevert(Initializable.InvalidInitialization.selector);
        vm.prank(ProxyHelper.getProxyAdmin(address(vaultProxy)));
        vaultProxy.upgradeToAndCall(address(vaultImpl2), _getInitializeCalldata(SHARE_NAME, SHARE_SYMBOL));
    }
    ...
    function _getInitializeCalldata(string memory shareName, string memory shareSymbol)
        internal
        pure
    ...
    function _deployMockTokenizationSpokeInstance(uint64 revision) internal returns (TokenizationSpokeInstance) {
        return TokenizationSpokeInstance(
