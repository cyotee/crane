...
abstract contract SpokeLiquidationCallHelperTest is SpokeLiquidationCallBaseTest {
        ...
        virtual
    {
        ...
        _updateTargetHealthFactor(spoke, vm.randomUint(MIN_TARGET_HEALTH_FACTOR, MAX_TARGET_HEALTH_FACTOR).toUint128());
        _updateLiquidationConfig(
            spoke,
            ISpoke.LiquidationConfig({
                targetHealthFactor: vm.randomUint(MIN_TARGET_HEALTH_FACTOR, MAX_TARGET_HEALTH_FACTOR).toUint128(),
                healthFactorForMaxBonus: vm.randomUint(0, HEALTH_FACTOR_LIQUIDATION_THRESHOLD - 1).toUint64(),
        ...
        for (uint256 i = 0; i < spoke.getReserveCount(); i++) {
            ...
            _updateLiquidationFee(spoke, i, vm.randomUint(MIN_LIQUIDATION_FEE, MAX_LIQUIDATION_FEE).toUint16());
            _updateCollateralFactor(spoke, i, _randomCollateralFactor(spoke, i));
            _updateCollateralRisk(spoke, i, vm.randomUint(MIN_COLLATERAL_RISK_BPS, MAX_COLLATERAL_RISK_BPS).toUint24());
        ...
        for (uint256 reserveId = 0; reserveId < spoke.getReserveCount(); reserveId++) {
            if (vm.randomBool()) {
                SpokeActions.setUsingAsCollateral({
                    spoke: spoke, reserveId: reserveId, caller: user, usingAsCollateral: true, onBehalfOf: user
                });
            }
        }
    }
    ...
    function _testLiquidationCall(
        ...
        debtToCover =
            _boundDebtToCoverNoDustRevert(spoke, collateralReserveId, debtReserveId, user, debtToCover, liquidator);
    ...
    function test_liquidationCall_fuzz_OneCollateral_OneDebt_UserSolvent(
        ...
        _increaseCollateralSupply(
            spoke, collateralReserveId, _convertValueToAmount(spoke, collateralReserveId, baseAmountValue), user
        );
    ...
    function _calculateMaxHealthyBorrowValue(address addr) internal returns (uint256) {
        uint256 maxBorrowValue = _getRequiredDebtValueForHf(spoke, addr, HEALTH_FACTOR_LIQUIDATION_THRESHOLD);
        ...
        maxBorrowValue = maxBorrowValue.percentDivDown(
            PercentageMath.PERCENTAGE_FACTOR + (_spokeMaxCollateralRisk(spoke) + PercentageMath.PERCENTAGE_FACTOR)
        );
    ...
    function _processAdditionalCollateralReserves(uint256 debtReserveId) internal {
        ...
        uint256 suppliableValue = (_convertAmountToValue(
                spoke, debtReserveId, _calculateMaxSupplyAmount(spoke, debtReserveId)
            ))
        .percentDivDown(10 * PercentageMath.PERCENTAGE_FACTOR + (_spokeMaxDrawnRate(spoke) * skipTime) / 365 days)
        - baseAmountValue;

        uint256 count = vm.randomUint(1, spoke.getReserveCount() * 2);
        for (uint256 i = 0; i < count; i++) {
            uint256 reserveId = vm.randomUint(0, spoke.getReserveCount() - 1);
            uint256 minAmount = _hub(spoke, reserveId).previewAddByShares(_reserveAssetId(spoke, reserveId), 1);
    ...
    function _processAdditionalDebtReserves() internal {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            uint256 maxBorrowAmount = _min(
                _convertValueToAmount(spoke, reserveId, borrowableValue), _calculateMaxSupplyAmount(spoke, reserveId)
            );
...
contract SpokeLiquidationCallTest_SmallPosition is SpokeLiquidationCallHelperTest {
    function _processAdditionalSetup(uint256 collateralReserveId, uint256 debtReserveId) internal virtual override {
...
contract SpokeLiquidationCallTest_NoLiquidationBonus is SpokeLiquidationCallHelperTest {
    ...
    function _assertBeforeLiquidation(
        ...
        /* params */
        LiquidationAccountsSnapshot memory,
        /* accountsInfoBefore */
...
contract SpokeLiquidationCallTest_SmallLiquidationBonus is SpokeLiquidationCallHelperTest {
    ...
    function _processAdditionalSetup(uint256 collateralReserveId, uint256 debtReserveId) internal virtual override {
        ...
        _updateCollateralFactor(spoke, collateralReserveId, _randomCollateralFactor(spoke, collateralReserveId));
    }
    ...
    function _assertBeforeLiquidation(
        ...
        assertLe(liquidationMetadata.liquidationBonus, MAX_LIQUIDATION_BONUS.percentMulUp(102_00), "Liquidation bonus");
    }
}
...
contract SpokeLiquidationCallTest_LargeLiquidationBonus is SpokeLiquidationCallHelperTest {
    ...
    function _assertBeforeLiquidation(
        ...
        assertGe(liquidationMetadata.liquidationBonus, MAX_LIQUIDATION_BONUS.percentMulDown(97_00), "Liquidation bonus");
    }
}
...
contract SpokeLiquidationCallTest_LiquidationFeeZero is SpokeLiquidationCallHelperTest {
    ...
    function _assertBeforeLiquidation(
        ...
        assertEq(_getLiquidationFee(params.spoke, params.collateralReserveId, params.user), 0, "Liquidation fee");
    }
}
...
contract SpokeLiquidationCallTest_Premium is SpokeLiquidationCallHelperTest {
    ...
    function _processAdditionalSetup(uint256 collateralReserveId, uint256 debtReserveId) internal virtual override {
        ...
        _updateCollateralRisk(spoke, collateralReserveId, vm.randomUint(1, MAX_COLLATERAL_RISK_BPS).toUint24());
        _setConstantDrawnRateBps(
...
contract SpokeLiquidationCallTest_LiquidatorHistory is SpokeLiquidationCallHelperTest {
    function _processAdditionalSetup(uint256 collateralReserveId, uint256 debtReserveId) internal virtual override {
        ...
        for (uint256 i = 0; i < count; ++i) {
            uint256 reserveId = vm.randomUint(0, spoke.getReserveCount() - 1);
            _increaseCollateralSupply(spoke, reserveId, _convertValueToAmount(spoke, reserveId, 100e26), liquidator);
            liquidatorAccountData = spoke.getUserAccountData(liquidator);
            uint256 maxBorrowAmount = _convertValueToAmount(
                spoke,
                reserveId,
                liquidatorAccountData.healthFactor <= 1.5e18 ? 0 : _getRequiredDebtValueForHf(spoke, liquidator, 1.5e18)
            );
            if (maxBorrowAmount == 0) {
                break;
            }
