...
import "@crane/test/foundry/spec/protocols/lending/aave/v4/contracts/spoke/liquidation/Spoke.LiquidationCall.Base.t.sol";
...
contract SpokeLiquidationCallScenariosTest is SpokeLiquidationCallBaseTest {
    ...
    function setUp() public virtual override {
        ...
        for (uint256 reserveId = 0; reserveId < spoke.getReserveCount(); reserveId++) {
            ...
            SpokeActions.approve({spoke: spoke, reserveId: reserveId, owner: liquidator, amount: MAX_SUPPLY_AMOUNT});
        }
    }
    ...
    function test_liquidationCall_revertsWith_ReentrancyGuardReentrantCall_hubRemove() public {
        ...
        MockReentrantCaller reentrantCaller = new MockReentrantCaller(address(spoke), ISpoke.liquidationCall.selector);
    ...
    function test_liquidationCall_revertsWith_ReentrancyGuardReentrantCall_hubRefreshPremium() public {
        uint256 collateralReserveId = _daiReserveId(spoke);
        uint256 debtReserveId = _wethReserveId(spoke);
        _increaseCollateralSupply(spoke, collateralReserveId, 100000e18, user);
        _makeUserLiquidatable(spoke, user, debtReserveId, 0.999e18);
    ...
    function test_liquidationCall_scenario1() public {
        ...
        irStrategy.setInterestRateData(
            ...
            abi.encode(
                IAssetInterestRateStrategy.InterestRateData({
                    optimalUsageRatio: 90_00, baseDrawnRate: 3_00, rateGrowthBeforeOptimal: 0, rateGrowthAfterOptimal: 0
                })
            )
        );
        ...
        assertApproxEqAbs(userAccountData.healthFactor, 1.0061e18, 0.0001e18, "pre liquidation: health factor");
        // Risk Premium: 5%
        ...
        assertApproxEqAbs(
            userAccountData.healthFactor, 0.975e18, 0.001e18, "pre liquidation: health factor after 1 year"
        );
        ...
        assertApproxEqAbs(
            _getUserDebt(spoke, user, _daiReserveId(spoke)).premiumDebt, 0, 2, "post liquidation: premium debt left"
        );
        // Health Factor after liquidation: ($500 * 0.7 + $100 * 0.72) / ($3713.4 - $3225.8) = ~0.8654
        userAccountData = spoke.getUserAccountData(user);
        assertApproxEqAbs(userAccountData.healthFactor, 0.8654e18, 0.0001e18, "post liquidation: health factor");
    ...
    function test_liquidationCall_scenario2() public {
        ...
        assertApproxEqAbs(userAccountData.healthFactor, 1.0063e18, 0.0001e18, "pre liquidation: health factor");
        ...
        assertApproxEqAbs(
            userAccountData.healthFactor, 0.99e18, 0.001e18, "pre liquidation: health factor after half of year"
        );
        ...
        userAccountData = spoke.getUserAccountData(user);
        assertApproxEqAbs(userAccountData.healthFactor, 0.9311e18, 0.0001e18, "post liquidation: health factor");
    ...
    function test_liquidationCall_scenario3() public {
        ...
        assertEq(
            _hub(spoke, _daiReserveId(spoke)).getAssetDeficitRay(_reserveAssetId(spoke, _daiReserveId(spoke))),
            0,
            "Deficit should be 0"
        );
    }
    ...
    function test_liquidationCall_scenario5() public {
        ...
        ISpoke.UserPosition memory userCollateralPositionBefore = spoke.getUserPosition(_wethReserveId(spoke), user);
        assertEq(userCollateralPositionBefore.suppliedShares, 2, "User should have 2 shares of WETH");
        ISpoke.UserPosition memory userDebtPositionBefore = spoke.getUserPosition(_daiReserveId(spoke), user);
        assertEq(userDebtPositionBefore.drawnShares, 1, "User should have 1 drawn share of DAI");
        assertEq(
            userDebtPositionBefore.premiumShares * 1.1e27 - userDebtPositionBefore.premiumOffsetRay.toUint256(),
            0.1e27,
        ...
        ISpoke.UserPosition memory userCollateralPositionAfter = spoke.getUserPosition(_wethReserveId(spoke), user);
        assertEq(userCollateralPositionAfter.suppliedShares, 1, "User should have 1 share of WETH after liquidation");
        ISpoke.UserPosition memory userDebtPositionAfter = spoke.getUserPosition(_daiReserveId(spoke), user);
        assertEq(userDebtPositionAfter.drawnShares, 0, "User should have 0 drawn share of DAI after liquidation");
        assertEq(userDebtPositionAfter.premiumShares, 0, "User should have 0 premium share of DAI after liquidation");
        assertEq(userDebtPositionAfter.premiumOffsetRay, 0, "User should have 0 premium offset after liquidation");
    }
    ...
    function test_liquidationCall_scenario6() public {
        ...
        ISpoke.UserPosition memory usdxUserPosition = spoke.getUserPosition(_usdxReserveId(spoke), user);
        assertEq(usdxUserPosition.suppliedShares, 20, "User should have 20 supplied shares of USDX before liquidation");
        ISpoke.UserPosition memory usdyUserPosition = spoke.getUserPosition(_usdyReserveId(spoke), user);
        assertEq(usdyUserPosition.drawnShares, 2, "User should have 2 drawn shares of USDY before liquidation");
        ...
        assertEq(usdxUserPosition.suppliedShares, 5, "User should have 5 supplied shares of USDX after liquidation");
        ...
        assertEq(usdyUserPosition.drawnShares, 1, "User should have 1 drawn shares of USDY after liquidation");
    ...
    function test_liquidationCall_scenario7() public {
        ...
        ISpoke.UserPosition memory usdxUserPosition = spoke.getUserPosition(_usdxReserveId(spoke), user);
        assertEq(usdxUserPosition.suppliedShares, 40, "User should have 40 supplied shares of USDX before liquidation");
        ISpoke.UserPosition memory usdyUserPosition = spoke.getUserPosition(_usdyReserveId(spoke), user);
        assertEq(usdyUserPosition.drawnShares, 2, "User should have 2 drawn shares of USDY before liquidation");
        ...
        usdxUserPosition = spoke.getUserPosition(_usdxReserveId(spoke), user);
        assertEq(usdxUserPosition.suppliedShares, 6, "User should have 6 supplied shares of USDX after liquidation");
        usdyUserPosition = spoke.getUserPosition(_usdyReserveId(spoke), user);
        assertEq(usdyUserPosition.drawnShares, 1, "User should have 1 drawn shares of USDY after liquidation");
    ...
    function test_liquidationCall_scenario8() public {
        ...
        assertEq(hub1.getSpokeAddedShares(wethAssetId, feeReceiver), 0, "Option 1: Fee receiver WETH balance before");
        ...
        assertEq(spoke.getUserSuppliedAssets(_wethReserveId(spoke), user), 58, "Option 1: User collateral after");
        ...
        assertEq(hub1.getSpokeAddedAssets(wethAssetId, feeReceiver), 1, "Option 1: Fee receiver WETH balance after");
        ...
        assertEq(spoke.getUserSuppliedAssets(_wethReserveId(spoke), user), 58, "Option 2: User collateral after");
        ...
        assertEq(hub1.getSpokeAddedAssets(wethAssetId, feeReceiver), 2, "Option 2: Fee receiver WETH balance after");
        vm.revertToState(snapshot);
    }
    ...
    function test_scenario_halted_asset() public {
        ...
        SpokeActions.borrow({
            spoke: spoke, reserveId: _usdxReserveId(spoke), caller: user, amount: 100e6, onBehalfOf: user
        });
        _makeUserLiquidatable(spoke, user, debtReserveId, 0.95e18);
        ...
        vm.expectCall(address(hub), abi.encodeWithSelector(IHubBase.refreshPremium.selector, usdxAssetId));
