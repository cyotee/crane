...
contract TokenizationSpokeInsufficientAllowanceTest is TokenizationSpokeBaseTest {
    ...
    function test_deposit_revertsWith_ERC20InsufficientAllowance() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IERC20Errors.ERC20InsufficientAllowance.selector, address(vault), allowance, amount)
        );
        vm.prank(alice);
    ...
    function test_depositWithSig_revertsWith_ERC20InsufficientAllowance() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IERC20Errors.ERC20InsufficientAllowance.selector, address(vault), allowance, p.assets
            )
        );
    ...
    function test_mintWithSig_revertsWith_ERC20InsufficientAllowance() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IERC20Errors.ERC20InsufficientAllowance.selector, address(vault), allowance, neededAssets
            )
        );
        vm.prank(vm.randomAddress());
