...
contract LiquidationLogicLiquidateUserTest is LiquidationLogicBaseTest {
    ...
    function setUp() public override {
        ...
        _mockDrawnRateBps({irStrategy: debtReserveHub.getAsset(wethAssetId).irStrategy, drawnRateBps: 5_00});
        ...
        params = LiquidationLogic.LiquidateUserParams({
            ...
            liquidationConfig: ISpoke.LiquidationConfig({
                targetHealthFactor: 1e18, healthFactorForMaxBonus: 0.8e18, liquidationBonusFactor: 50_00
            }),
            debtToCover: 3e18,
        ...
        liquidationLogicWrapper.setCollateralReserveFlags(ReserveFlagsMap.create(false, false, false, true));
        liquidationLogicWrapper.setDynamicCollateralConfig(
            ISpoke.DynamicReserveConfig({maxLiquidationBonus: 120_00, collateralFactor: 50_00, liquidationFee: 10_00})
        );
        ...
        vm.stopPrank();
        skip(365 days);
        (uint256 spokeDrawnOwed, uint256 spokePremiumOwed) =
    ...
    function test_liquidateUser() public {
        uint256 initialCollateralReserveBalance = tokenList.usdx.balanceOf(address(collateralReserveHub));
        ...
        vm.expectCall(
            address(collateralReserveHub), abi.encodeCall(IHubBase.previewRemoveByShares, (usdxAssetId, 4800e6)), 1
        );

        vm.expectCall(
            address(collateralReserveHub), abi.encodeCall(IHubBase.previewRemoveByShares, (usdxAssetId, 4720e6)), 1
        );

        vm.expectCall(
            address(collateralReserveHub), abi.encodeCall(IHubBase.remove, (usdxAssetId, 5900e6, params.liquidator)), 1
        );

        vm.expectCall(address(collateralReserveHub), abi.encodeCall(IHubBase.payFeeShares, (usdxAssetId, 80e6)), 1);
        ...
        assertEq(tokenList.usdx.balanceOf(address(collateralReserveHub)), initialCollateralReserveBalance - 5900e6);
        assertEq(tokenList.usdx.balanceOf(address(params.liquidator)), 5900e6);
        assertApproxEqAbs(collateralReserveHub.getSpokeAddedShares(usdxAssetId, address(treasurySpoke)), 80e6, 1);

        assertEq(tokenList.weth.balanceOf(address(debtReserveHub)), initialDebtReserveBalance + 2.5e18);
        assertEq(tokenList.weth.balanceOf(address(params.liquidator)), initialLiquidatorWethBalance - 2.5e18);
    }
