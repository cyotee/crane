...
contract HubAddTest is Base {
    ...
    function setUp() public override {
        ...
        minDecimalAssetId = hub1.addAsset(
            address(usda), MIN_ALLOWED_UNDERLYING_DECIMALS, address(treasurySpoke), address(irStrategy), encodedIrData
        );
    ...
    function test_add_revertsWith_InsufficientTransferred() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IHub.InsufficientTransferred.selector, amount - transferAmount));
    ...
    function test_add_revertsWith_SharesDowncastOverflow() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(SafeCast.SafeCastOverflowedUintDowncast.selector, 120, shares));
        hub1.add(daiAssetId, amount);
        vm.stopPrank();
    }
    ...
    function test_add_revertsWith_AmountDowncastOverflow() public {
        ...
        vm.startPrank(address(spoke1));
        tokenList.dai.transferFrom(alice, address(hub1), amount);
        vm.expectRevert(abi.encodeWithSelector(SafeCast.SafeCastOverflowedUintDowncast.selector, 120, amount));
        hub1.add(daiAssetId, amount);
        vm.stopPrank();
    }
    ...
    function test_add_AddCapReachedButNotExceeded_rounding() public {
        ...
        assertEq(
            uint256(1).toAssetsDown(totalAddedAssets, totalAddedShares)
                .toSharesDown(totalAddedAssets, totalAddedShares),
            0,
        ...
        uint256 spokeAddedAssetsRoundedUp = spokeAddedShares.toAssetsUp(totalAddedAssets, totalAddedShares);
        ...
        HubActions.add({
            hub: hub1, assetId: minDecimalAssetId, caller: address(spoke1), amount: addedAmount, user: alice
        });
    }
    ...
    function test_add_fuzz_single_asset(uint256 assetId, address user, uint256 amount) public {
        ...
        assertEq(hub1.getSpokeAddedAssets(assetId, address(spoke1)), amount, "hub spoke addedAmount after");
        assertEq(hub1.getSpokeAddedShares(assetId, address(spoke1)), shares, "hub spoke addedShares after");
        assertEq(hub1.getAsset(assetId).lastUpdateTimestamp, vm.getBlockTimestamp());
        assertEq(hub1.getAsset(assetId).liquidity, hubBefore.liquidity + amount, "hub available liquidity after");
    ...
    function test_add_fuzz_multi_asset_multi_spoke(uint256 assetId, uint256 amount, uint256 amount2) public {
        ...
        // asset1
        assertEq(hub1.getAddedShares(assetId), hub1.previewAddByAssets(assetId, amount), "asset addedShares after");
        ...
        assertEq(hub1.getAsset(assetId).lastUpdateTimestamp, timestamp, "asset lastUpdateTimestamp after");
        assertEq(
            hub1.getSpokeAddedShares(assetId, address(spoke1)),
            hub1.previewAddByAssets(assetId, amount),
            "spoke1 addedShares after"
        );
        assertEq(hub1.getSpokeAddedAssets(assetId, address(spoke1)), amount, "spoke1 addedAmount after");
        ...
        _assertHubLiquidity(hub1, assetId, "hub1.add");
        // asset2
        assertEq(hub1.getAddedShares(assetId2), hub1.previewAddByAssets(assetId2, amount2), "asset2 addedShares after");
        assertEq(hub1.getAssetLiquidity(assetId2), amount2, "asset2 liquidity after");
        assertEq(hub1.getAsset(assetId2).lastUpdateTimestamp, timestamp, "asset2 lastUpdateTimestamp after");
        assertEq(
            hub1.getSpokeAddedShares(assetId2, address(spoke2)),
            hub1.previewAddByAssets(assetId2, amount2),
            "spoke2 addedShares after"
        );
        assertEq(hub1.getSpokeAddedAssets(assetId2, address(spoke2)), amount2, "spoke2 addedAmount after");
    ...
    function test_add_with_increased_index() public {
        ...
        assertGe(hub1.getAddedAssets(daiAssetId), spokeAddedAssetsBefore + addAmount, "hub addedAssets after");
        assertGe(hub1.getAddedShares(daiAssetId), hubBefore.addedShares + shares, "hub addedShares after");
        assertEq(hub1.getAsset(daiAssetId).liquidity, hubBefore.liquidity + addAmount, "hub available liquidity after");
    ...
    function test_add_with_increased_index_with_premium() public {
        ...
        uint256 spokeAddedAmount =
            _calculateEffectiveAddedAssets(addAmount, hub1.getAddedAssets(daiAssetId), hub1.getAddedShares(daiAssetId));

        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke2), amount: addAmount, user: bob});
        ...
        // Hub and Spoke accounting do not match because of liquidity fees
        assertGe(hub1.getAddedAssets(daiAssetId), addedAssetsBefore + spokeAddedAmount, "hub addedAssets after");
        assertGe(hub1.getAddedShares(daiAssetId), addedSharesBefore + expectedAddedShares, "hub addedShares after");
        _assertHubLiquidity(hub1, daiAssetId, "hub1.add");
    }
    ...
    function test_add_multi_add_minimal_shares() public {
        ...
        HubActions.add({hub: hub1, assetId: daiAssetId, caller: address(spoke1), amount: addAmount, user: bob});
        ...
        assertGe(hub1.getAddedShares(daiAssetId), addedSharesBefore1 + addShares, "hub addedShares after");
        assertEq(hub1.getAssetLiquidity(daiAssetId), amount + addAmount - drawnAmount, "asset liquidity after");
        assertEq(
            hub1.getAsset(daiAssetId).lastUpdateTimestamp, vm.getBlockTimestamp(), "asset lastUpdateTimestamp after"
        );
        // spoke1
        assertEq(hub1.getSpokeAddedAssets(daiAssetId, address(spoke1)), spokeAddedAmount, "spoke1 addedAssets after");
        assertEq(hub1.getSpokeAddedShares(daiAssetId, address(spoke1)), addShares, "spoke1 addedShares after");
        // spoke2
        assertGe(hub1.getSpokeAddedAssets(daiAssetId, address(spoke2)), addedAssetsBefore2, "spoke2 addedAmount after");
        assertEq(hub1.getSpokeAddedShares(daiAssetId, address(spoke2)), addedSharesBefore2, "spoke2 addedShares after");
        // token balance
        assertEq(tokenList.dai.balanceOf(address(hub1)), addAmount + amount - drawnAmount, "hub token balance after");
        assertEq(tokenList.dai.balanceOf(alice), MAX_SUPPLY_AMOUNT + drawnAmount, "alice token balance after");
        assertEq(tokenList.dai.balanceOf(bob), MAX_SUPPLY_AMOUNT - amount - addAmount, "bob token balance after");
        _assertHubLiquidity(hub1, daiAssetId, "hub1.add");
    }
    ...
    function test_add_fuzz_single_spoke_multi_add(uint256 amount, uint256 skipTime) public {
        ...
        for (uint256 i = 0; i < numAdds; i++) {
            ...
            HubActions.add({hub: hub1, assetId: assetId, caller: address(spoke1), amount: addAmount, user: bob});
            ...
            assertEq(
                hub1.getAsset(assetId).lastUpdateTimestamp, vm.getBlockTimestamp(), "asset lastUpdateTimestamp after"
            );
            _assertHubLiquidity(hub1, assetId, "hub1.add");
            // spoke1
            assertEq(
            ...
            assertEq(
                hub1.getSpokeAddedShares(assetId, address(spoke1)), params.spoke1AddedShares, "spoke1 addedShares after"
            );
            // spoke2
            assertEq(
            ...
            assertEq(
                hub1.getSpokeAddedShares(assetId, address(spoke2)), params.spoke2AddedShares, "spoke2 addedShares after"
            );
            // token balance
            assertEq(tokenList.dai.balanceOf(address(hub1)), params.availableLiq, "hub token balance after");
