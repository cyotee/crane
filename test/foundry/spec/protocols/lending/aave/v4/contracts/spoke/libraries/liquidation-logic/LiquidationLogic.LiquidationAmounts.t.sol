...
contract LiquidationLogicLiquidationAmountsTest is LiquidationLogicBaseTest {
    ...
    function test_calculateLiquidationAmounts_fuzz_EnoughCollateral_NoCollateralDust(
        ...
        LiquidationLogic.LiquidationAmounts memory expectedLiquidationAmounts = _calculateRawLiquidationAmounts(params);

        uint256 dustSharesBufferLowerBound = params.collateralReserveHub
        ...
        params.suppliedShares = bound(
            ...
            expectedLiquidationAmounts.collateralSharesToLiquidate + dustSharesBufferLowerBound + 1 + MAX_SUPPLY_AMOUNT
        );
    ...
    function test_calculateLiquidationAmounts_fuzz_InsufficientCollateral(
        ...
        params = _bound(params);
        LiquidationLogic.LiquidationAmounts memory rawLiquidationAmounts = _calculateRawLiquidationAmounts(params);
        vm.assume(rawLiquidationAmounts.collateralSharesToLiquidate > 0);
        params.suppliedShares = bound(params.suppliedShares, 0, rawLiquidationAmounts.collateralSharesToLiquidate - 1);

        LiquidationLogic.LiquidationAmounts memory expectedLiquidationAmounts =
    ...
    function test_calculateLiquidationAmounts_fuzz_revertsWith_MustNotLeaveDust_Debt(
        ...
        uint256 debtAssetsToRestore =
            _calculateDebtAssetsToRestore(params.drawnShares, params.premiumDebtRay, params.drawnIndex);
    ...
    function test_calculateLiquidationAmounts_fuzz_revertsWith_MustNotLeaveDust_Collateral(
        ...
        params.debtToCover = _calculateDebtAssetsToRestore(
            expectedLiquidationAmounts.drawnSharesToLiquidate,
            expectedLiquidationAmounts.premiumDebtRayToLiquidate,
            params.drawnIndex
        ) - 1;
    ...
    function _calculateRawLiquidationAmounts(LiquidationLogic.CalculateLiquidationAmountsParams memory params)
        internal
        view
        returns (LiquidationLogic.LiquidationAmounts memory)
    {
        ...
        (uint256 drawnSharesToLiquidate, uint256 premiumDebtRayToLiquidate) =
            liquidationLogicWrapper.calculateDebtToLiquidate(_getCalculateDebtToLiquidateParams(params));
        uint256 debtRayToLiquidate = drawnSharesToLiquidate * params.drawnIndex + premiumDebtRayToLiquidate;
        uint256 collateralToLiquidate = Math.mulDiv(
            debtRayToLiquidate,
            params.debtAssetPrice * (10 ** params.collateralAssetDecimals) * liquidationBonus,
            (10 ** params.debtAssetDecimals) * params.collateralAssetPrice * PercentageMath.PERCENTAGE_FACTOR
                * WadRayMath.RAY,
            Math.Rounding.Floor
        );
        ...
        uint256 collateralSharesToLiquidator = _calculateCollateralSharesToLiquidator(
            collateralSharesToLiquidate, liquidationBonus, params.liquidationFee
        );

        return LiquidationLogic.LiquidationAmounts({
    ...
    function _calculateAdjustedLiquidationAmounts(LiquidationLogic.CalculateLiquidationAmountsParams memory params)
        internal
        view
        returns (LiquidationLogic.LiquidationAmounts memory)
    {
        ...
        uint256 debtRayToLiquidate = Math.mulDiv(
            ...
            params.collateralAssetPrice * (10 ** params.debtAssetDecimals) * PercentageMath.PERCENTAGE_FACTOR
                * WadRayMath.RAY,
            (10 ** params.collateralAssetDecimals) * params.debtAssetPrice * liquidationBonus,
            Math.Rounding.Ceil
        );

        uint256 premiumDebtRayToLiquidate = debtRayToLiquidate.fromRayUp().toRay().min(params.premiumDebtRay);
    ...
    function _boundWithCollateralDustAdjustment(LiquidationLogic.CalculateLiquidationAmountsParams memory params)
        internal
        virtual
        returns (LiquidationLogic.CalculateLiquidationAmountsParams memory)
    {
        ...
        params.collateralAssetPrice = bound(
            ...
            params.collateralReserveHub
                .previewAddByAssets(
                    params.collateralReserveAssetId,
                    _convertDecimals(
                        LiquidationLogic.DUST_LIQUIDATION_THRESHOLD, 18, params.collateralAssetDecimals, false
                    )
                )
        );
        ...
        uint256 dustSharesBufferUpperBound = params.collateralReserveHub
            .previewAddByAssets(
                params.collateralReserveAssetId,
                _convertValueToAmount(
    ...
    function assertApproxEqAbs(
        ...
        assertEq(a.collateralSharesToLiquidate, b.collateralSharesToLiquidate, "collateralSharesToLiquidate");
        assertApproxEqAbs(
            a.collateralSharesToLiquidator, b.collateralSharesToLiquidator, 1, "collateralSharesToLiquidator"
        );
