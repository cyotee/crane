...
contract LiquidationLogicCollateralToLiquidateTest is LiquidationLogicBaseTest {
    ...
    function test_calculateCollateralToLiquidate_fuzz(
        ...
        uint256 expectedCollateralSharesToLiquidate =
            _calculateCollateralSharesToLiquidate(params, collateralAmountToLiquidate);

        vm.expectCall(
            address(params.collateralReserveHub),
            abi.encodeWithSelector(
                IHubBase.previewAddByAssets.selector, params.collateralReserveAssetId, collateralAmountToLiquidate
            ),
            1
        );

        uint256 collateralSharesToLiquidate = liquidationLogicWrapper.calculateCollateralToLiquidate(params);
    ...
    function test_calculateCollateralAmountToLiquidate() public {
        ...
        vm.expectCall(
            address(hub1), abi.encodeWithSelector(IHubBase.previewAddByAssets.selector, usdxAssetId, 5250e6), 1
        );
    ...
    function _calculateCollateralAmountToLiquidate(LiquidationLogic.CalculateCollateralToLiquidateParams memory params)
        internal
        pure
        returns (uint256)
    {
    ...
    function _calculateCollateralSharesToLiquidate(
        ...
        return params.collateralReserveHub
            .previewAddByAssets(params.collateralReserveAssetId, collateralAmountToLiquidate);
    }
}
