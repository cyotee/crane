...
import {
    UserPositionUtilsWrapper
} from "@crane/test/foundry/spec/protocols/lending/aave/v4/helpers/mocks/UserPositionUtilsWrapper.sol";
...
contract UserPositionUtilsTest is Base {
    ...
    UserPositionUtilsWrapper internal u;
    ...
    function test_fuzz_applyPremiumDelta(IHubBase.PremiumDelta memory premiumDelta) public {
        ...
        assertEq(u.getUserPosition().premiumOffsetRay, PREMIUM_OFFSET_RAY + premiumDelta.offsetRayDelta);
    }

    function test_applyPremiumDelta() public {
        u.applyPremiumDelta(
            IHubBase.PremiumDelta({
                sharesDelta: -10e18, offsetRayDelta: 10e18 * 1e27, restoredPremiumRay: vm.randomUint()
            })
        );
    ...
    function test_fuzz_calculatePremiumDelta(BoundParams memory params) public {
        ...
        assertEq(
            u.calculatePremiumDelta(
                params.drawnSharesTaken, params.drawnIndex, params.riskPremium, params.restoredPremiumRay
            ),
    ...
    function test_fuzz_calculatePremiumRay(uint256 premiumShares, int256 premiumOffsetRay, uint256 drawnIndex) public {
        (premiumShares, premiumOffsetRay, drawnIndex) =
            _bound({premiumShares: premiumShares, premiumOffsetRay: premiumOffsetRay, drawnIndex: drawnIndex});
        _mockUserPremiumData(premiumShares, premiumOffsetRay);
        assertEq(
            u.calculatePremiumRay(drawnIndex), _calculatePremiumDebtRay(premiumShares, premiumOffsetRay, drawnIndex)
        );
    }
    ...
    function test_fuzz_getUserDebt_HubAndAssetId(
        ...
        uint256 expectedPremiumDebtRay = _calculatePremiumDebtRay(premiumShares, premiumOffsetRay, drawnIndex);
    ...
    function test_fuzz_calculateRestoreAmount(
        ...
        (uint256 restoredDrawnDebt, uint256 restoredPremiumDebtRay) = u.calculateRestoreAmount(drawnIndex, amount);
    ...
    function test_calculateRestoreAmount() public {
        (uint256 restoredDrawnDebt, uint256 restoredPremiumDebtRay) = u.calculateRestoreAmount(DRAWN_INDEX, 400e18);
    ...
    function _mockHubDrawnIndex(uint256 drawnIndex) internal {
        vm.mockCall(address(hub), abi.encodeCall(IHubBase.getAssetDrawnIndex, (assetId)), abi.encode(drawnIndex));
    }

    function _bound(IHubBase.PremiumDelta memory premiumDelta) internal pure returns (IHubBase.PremiumDelta memory) {
    ...
    function _bound(uint256 premiumShares, int256 premiumOffsetRay, uint256 drawnIndex)
        internal
        pure
    ...
    function _bound(uint256 drawnShares, uint256 premiumShares, int256 premiumOffsetRay, uint256 drawnIndex)
        internal
        pure
    ...
    function _bound(BoundParams memory params) internal pure returns (BoundParams memory) {
        (params.drawnShares, params.premiumShares, params.premiumOffsetRay, params.drawnIndex) =
            _bound(params.drawnShares, params.premiumShares, params.premiumOffsetRay, params.drawnIndex);
