...
contract SpokeDynamicConfigTest is Base {
    ...
    function setUp() public override {
        ...
        address mockSpokeImpl = address(new MockSpoke(address(spoke.ORACLE()), MAX_ALLOWED_USER_RESERVES_LIMIT));
    ...
    function test_addDynamicReserveConfig_revertsWith_InvalidLiquidationFee() public {
        uint256 reserveId = _randomReserveId(spoke1);
        ISpoke.DynamicReserveConfig memory config = _getLatestDynamicReserveConfig(spoke1, reserveId);
        config.liquidationFee = vm.randomUint(PercentageMath.PERCENTAGE_FACTOR + 1, type(uint16).max).toUint16();
    ...
    function test_addDynamicReserveConfig_revertsWith_InvalidCollateralFactorAndMaxLiquidationBonus_collateralFactor()
        public
    {
        uint16 collateralFactor = vm.randomUint(PercentageMath.PERCENTAGE_FACTOR, type(uint16).max).toUint16();
    ...
    function test_addDynamicReserveConfig_revertsWith_MaximumDynamicConfigKeyReached() public {
        ...
        MockSpoke(address(spoke1)).setReserveDynamicConfigKey(reserveId, uint32(MAX_ALLOWED_DYNAMIC_CONFIG_KEY));
    ...
    function test_addDynamicReserveConfig_revertsWith_AccessManagedUnauthorized(address caller) public {
        vm.assume(
            caller != SPOKE_ADMIN && caller != ADMIN && caller != SPOKE_CONFIGURATOR_ADMIN
                && caller != address(spokeConfigurator) && caller != address(spoke1).getProxyAdmin()
        );
        uint256 reserveId = _randomReserveId(spoke1);
    ...
    function test_updateDynamicReserveConfig_revertsWith_InvalidCollateralFactorAndMaxLiquidationBonus_collateralFactor()
        public
    {
        uint16 collateralFactor = vm.randomUint(PercentageMath.PERCENTAGE_FACTOR + 1, type(uint16).max).toUint16();
    ...
    function test_updateDynamicReserveConfig_revertsWith_AccessManagedUnauthorized(address caller) public {
        vm.assume(
            caller != SPOKE_ADMIN && caller != ADMIN && caller != SPOKE_CONFIGURATOR_ADMIN
                && caller != address(spokeConfigurator) && caller != address(spoke1).getProxyAdmin()
        );
    ...
    function test_fuzz_addDynamicReserveConfig_trailing_order(bytes32) public {
        ...
        while (--runs != 0) {
            ...
            dynConf.maxLiquidationBonus = vm.randomUint(
                    MIN_LIQUIDATION_BONUS, _maxLiquidationBonusUpperBound(dynConf.collateralFactor)
                ).toUint32();
    ...
    function test_fuzz_addDynamicReserveConfig_spaced_dup_updates(bytes32) public {
        ...
        while (--runs != 0) {
            ...
            dynConf.collateralFactor = vm.randomUint() % 2 == 0
                ? spoke1.getDynamicReserveConfig(reserveId, vm.randomUint(0, dynamicConfigKey - 1).toUint32())
                .collateralFactor
    ...
    function test_offboardReserve_existing_borrows_remain_unaffected() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _usdxReserveId(spoke1), caller: alice, amount: 2600e6, onBehalfOf: alice
        });
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _usdxReserveId(spoke1), caller: bob, amount: 2600e6, onBehalfOf: bob
        });
        SpokeActions.borrow({
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _usdxReserveId(spoke1), caller: carol, amount: 2600e6, onBehalfOf: carol
        });
        vm.expectRevert(ISpoke.HealthFactorBelowThreshold.selector);
        SpokeActions.borrow({
        ...
        vm.expectRevert(ISpoke.HealthFactorBelowThreshold.selector);
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _wethReserveId(spoke1), caller: alice, amount: 1, onBehalfOf: alice
        });
    }
}
