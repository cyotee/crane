...
contract SpokeAccrueInterestTest is Base {
    ...
    function test_accrueInterest_NoInterest_OnlySupply(uint40 skipTime) public {
        ...
        _assertOnlyOneUserSupply(spoke1, daiReserveId, bob, amount, "after supply, no interest accrued");
    }
    ...
    function test_accrueInterest_NoInterest_NoDebt(uint40 elapsed) public {
        ...
        _assertOnlyOneUserDebt(spoke1, daiReserveId, bob, drawnDebt, expectedPremiumDebt, "after accrual");
        ...
        SpokeActions.repay({spoke: spoke1, reserveId: daiReserveId, caller: bob, amount: UINT256_MAX, onBehalfOf: bob});

        _assertOnlyOneUserDebt(spoke1, daiReserveId, bob, 0, 0, "after repay, no debt");
        _assertOnlyOneUserSupply(
            spoke1, daiReserveId, bob, supplyAmount + interest, "after repay, no additional supply"
        );
        ...
        _assertOnlyOneUserSupply(
            spoke1, daiReserveId, bob, supplyAmount + interest, "after repay and time skip, no additional supply"
        );
    }

    function test_accrueInterest_fuzz_BorrowAmountAndSkipTime(uint256 borrowAmount, uint40 skipTime) public {
    ...
    function test_accrueInterest_TenPercentRp(uint256 borrowAmount, uint40 skipTime) public {
        ...
        // Bob supply usdx
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: usdxReserveId, caller: bob, amount: supplyAmount, onBehalfOf: bob
        });

        // Bob borrows usdx
        SpokeActions.borrow({
            spoke: spoke1, reserveId: usdxReserveId, caller: bob, amount: borrowAmount, onBehalfOf: bob
        });
        ...
        uint256 expectedPremiumDebt = _calculateExpectedPremiumDebt(borrowAmount, expectedDrawnDebt, riskPremium);
        uint256 interest =
            (expectedDrawnDebt + expectedPremiumDebt) - borrowAmount - _calculateBurntInterest(hub1, usdxAssetId);

        _assertOnlyOneUserDebt(spoke1, usdxReserveId, bob, expectedDrawnDebt, expectedPremiumDebt, "after accrual");
    ...
    function test_accrueInterest_fuzz_RPBorrowAndSkipTime(TestAmounts memory amounts, uint40 skipTime) public {
        ...
        if (amounts.daiSupplyAmount < MAX_SUPPLY_AMOUNT) {
            _openSupplyPosition(spoke1, _daiReserveId(spoke1), MAX_SUPPLY_AMOUNT - amounts.daiSupplyAmount);
        }
        if (amounts.wethSupplyAmount < MAX_SUPPLY_AMOUNT) {
            _openSupplyPosition(spoke1, _wethReserveId(spoke1), MAX_SUPPLY_AMOUNT - amounts.wethSupplyAmount);
        }
        if (amounts.usdxSupplyAmount < MAX_SUPPLY_AMOUNT) {
            _openSupplyPosition(spoke1, _usdxReserveId(spoke1), MAX_SUPPLY_AMOUNT - amounts.usdxSupplyAmount);
        }
        if (amounts.wbtcSupplyAmount < MAX_SUPPLY_AMOUNT) {
            _openSupplyPosition(spoke1, _wbtcReserveId(spoke1), MAX_SUPPLY_AMOUNT - amounts.wbtcSupplyAmount);
        }
        ...
        Rates memory rates;
        rates.daiBaseDrawnRate = hub1.getAssetDrawnRate(daiAssetId).toUint96();
        rates.wethBaseDrawnRate = hub1.getAssetDrawnRate(wethAssetId).toUint96();
        rates.usdxBaseDrawnRate = hub1.getAssetDrawnRate(usdxAssetId).toUint96();
        rates.wbtcBaseDrawnRate = hub1.getAssetDrawnRate(wbtcAssetId).toUint96();

        // Check bob's drawn debt, premium debt, and supplied amounts for all assets at user, reserve, spoke, and asset level
        uint256 drawnDebt = _calculateExpectedDrawnDebt(amounts.daiBorrowAmount, rates.daiBaseDrawnRate, startTime);
        _assertOnlyOneUserDebt(spoke1, _daiReserveId(spoke1), bob, drawnDebt, 0, "dai before accrual");
        _assertUserSupply(spoke1, _daiReserveId(spoke1), bob, amounts.daiSupplyAmount, "dai before accrual");
        ...
        drawnDebt = _calculateExpectedDrawnDebt(amounts.wethBorrowAmount, rates.wethBaseDrawnRate, startTime);
        _assertOnlyOneUserDebt(spoke1, _wethReserveId(spoke1), bob, drawnDebt, 0, "weth before accrual");
        _assertUserSupply(spoke1, _wethReserveId(spoke1), bob, amounts.wethSupplyAmount, "weth before accrual");
        ...
        drawnDebt = _calculateExpectedDrawnDebt(amounts.usdxBorrowAmount, rates.usdxBaseDrawnRate, startTime);
        _assertOnlyOneUserDebt(spoke1, _usdxReserveId(spoke1), bob, drawnDebt, 0, "usdx before accrual");
        _assertUserSupply(spoke1, _usdxReserveId(spoke1), bob, amounts.usdxSupplyAmount, "usdx before accrual");
        ...
        drawnDebt = _calculateExpectedDrawnDebt(amounts.wbtcBorrowAmount, rates.wbtcBaseDrawnRate, startTime);
        _assertOnlyOneUserDebt(spoke1, _wbtcReserveId(spoke1), bob, drawnDebt, 0, "wbtc before accrual");
        _assertUserSupply(spoke1, _wbtcReserveId(spoke1), bob, amounts.wbtcSupplyAmount, "wbtc before accrual");
        ...
        // Check bob's drawn debt, premium debt, and supplied amounts for all assets at user, reserve, spoke, and asset level
        drawnDebt = _calculateExpectedDrawnDebt(amounts.daiBorrowAmount, rates.daiBaseDrawnRate, startTime);
        uint256 expectedPremiumDebt = _calculateExpectedPremiumDebt(amounts.daiBorrowAmount, drawnDebt, bobRp);
        uint256 interest =
            (drawnDebt + expectedPremiumDebt) - amounts.daiBorrowAmount - _calculateBurntInterest(hub1, daiAssetId);
        _assertOnlyOneUserDebt(spoke1, _daiReserveId(spoke1), bob, drawnDebt, expectedPremiumDebt, "dai after accrual");
        _assertUserSupply(
            spoke1,
        ...
        _assertReserveSupply(spoke1, _daiReserveId(spoke1), MAX_SUPPLY_AMOUNT + interest, "dai after accrual");
        _assertSpokeSupply(spoke1, _daiReserveId(spoke1), MAX_SUPPLY_AMOUNT + interest, "dai after accrual");
        _assertAssetSupply(spoke1, _daiReserveId(spoke1), MAX_SUPPLY_AMOUNT + interest, "dai after accrual");

        drawnDebt = _calculateExpectedDrawnDebt(amounts.wethBorrowAmount, rates.wethBaseDrawnRate, startTime);
        expectedPremiumDebt = _calculateExpectedPremiumDebt(amounts.wethBorrowAmount, drawnDebt, bobRp);
        interest =
            (drawnDebt + expectedPremiumDebt) - amounts.wethBorrowAmount - _calculateBurntInterest(hub1, wethAssetId);
        _assertOnlyOneUserDebt(
            spoke1, _wethReserveId(spoke1), bob, drawnDebt, expectedPremiumDebt, "weth after accrual"
        );
        _assertUserSupply(
            spoke1,
        ...
        _assertReserveSupply(spoke1, _wethReserveId(spoke1), MAX_SUPPLY_AMOUNT + interest, "weth after accrual");
        _assertSpokeSupply(spoke1, _wethReserveId(spoke1), MAX_SUPPLY_AMOUNT + interest, "weth after accrual");
        _assertAssetSupply(spoke1, _wethReserveId(spoke1), MAX_SUPPLY_AMOUNT + interest, "weth after accrual");

        drawnDebt = _calculateExpectedDrawnDebt(amounts.usdxBorrowAmount, rates.usdxBaseDrawnRate, startTime);
        expectedPremiumDebt = _calculateExpectedPremiumDebt(amounts.usdxBorrowAmount, drawnDebt, bobRp);
        interest =
            (drawnDebt + expectedPremiumDebt) - amounts.usdxBorrowAmount - _calculateBurntInterest(hub1, usdxAssetId);
        _assertOnlyOneUserDebt(
            spoke1, _usdxReserveId(spoke1), bob, drawnDebt, expectedPremiumDebt, "usdx after accrual"
        );
        _assertUserSupply(
            spoke1,
        ...
        _assertReserveSupply(spoke1, _usdxReserveId(spoke1), MAX_SUPPLY_AMOUNT + interest, "usdx after accrual");
        _assertSpokeSupply(spoke1, _usdxReserveId(spoke1), MAX_SUPPLY_AMOUNT + interest, "usdx after accrual");
        _assertAssetSupply(spoke1, _usdxReserveId(spoke1), MAX_SUPPLY_AMOUNT + interest, "usdx after accrual");

        drawnDebt = _calculateExpectedDrawnDebt(amounts.wbtcBorrowAmount, rates.wbtcBaseDrawnRate, startTime);
        expectedPremiumDebt = _calculateExpectedPremiumDebt(amounts.wbtcBorrowAmount, drawnDebt, bobRp);
        interest =
            (drawnDebt + expectedPremiumDebt) - amounts.wbtcBorrowAmount - _calculateBurntInterest(hub1, wbtcAssetId);
        _assertOnlyOneUserDebt(
            spoke1, _wbtcReserveId(spoke1), bob, drawnDebt, expectedPremiumDebt, "wbtc after accrual"
        );
        _assertUserSupply(
            spoke1,
        ...
        _assertReserveSupply(spoke1, _wbtcReserveId(spoke1), MAX_SUPPLY_AMOUNT + interest, "wbtc after accrual");
        _assertSpokeSupply(spoke1, _wbtcReserveId(spoke1), MAX_SUPPLY_AMOUNT + interest, "wbtc after accrual");
        _assertAssetSupply(spoke1, _wbtcReserveId(spoke1), MAX_SUPPLY_AMOUNT + interest, "wbtc after accrual");
    }
    ...
    function test_accrueInterest_fuzz_RatesRPBorrowAndSkipTime(
        ...
        if (amounts.daiBorrowAmount > 0) {
            ...
            _mockDrawnRateRay({
                ...
                drawnRateRay: rates.daiBaseDrawnRate,
                assetId: daiAssetId,
        ...
        if (amounts.wethBorrowAmount > 0) {
            ...
            _mockDrawnRateRay({
                irStrategy: address(irStrategy),
                drawnRateRay: rates.wethBaseDrawnRate,
                assetId: wethAssetId,
        ...
        if (amounts.usdxBorrowAmount > 0) {
            ...
            _mockDrawnRateRay({
                irStrategy: address(irStrategy),
                drawnRateRay: rates.usdxBaseDrawnRate,
                assetId: usdxAssetId,
        ...
        if (amounts.wbtcBorrowAmount > 0) {
            ...
            _mockDrawnRateRay({
                irStrategy: address(irStrategy),
                drawnRateRay: rates.wbtcBaseDrawnRate,
                assetId: wbtcAssetId,
        ...
        uint256 expectedPremiumDebt =
            _calculatePremiumDebt(hub1, daiAssetId, expectedpremiumShares, bobPosition.premiumOffsetRay);
        uint256 interest =
            (drawnDebt + expectedPremiumDebt) - amounts.daiBorrowAmount - _calculateBurntInterest(hub1, daiAssetId);
        _assertOnlyOneUserDebt(spoke1, _daiReserveId(spoke1), bob, drawnDebt, expectedPremiumDebt, "dai after accrual");
        _assertUserSupply(
        ...
        expectedPremiumDebt =
            _calculatePremiumDebt(hub1, wethAssetId, expectedpremiumShares, bobPosition.premiumOffsetRay);
        interest =
            (drawnDebt + expectedPremiumDebt) - amounts.wethBorrowAmount - _calculateBurntInterest(hub1, wethAssetId);
        _assertOnlyOneUserDebt(
            spoke1, _wethReserveId(spoke1), bob, drawnDebt, expectedPremiumDebt, "weth after accrual"
        ...
        expectedpremiumShares = bobPosition.drawnShares.percentMulUp(bobRp);
        expectedPremiumDebt =
            _calculatePremiumDebt(hub1, usdxAssetId, expectedpremiumShares, bobPosition.premiumOffsetRay);
        interest =
            (drawnDebt + expectedPremiumDebt) - amounts.usdxBorrowAmount - _calculateBurntInterest(hub1, usdxAssetId);
        _assertOnlyOneUserDebt(
            spoke1, _usdxReserveId(spoke1), bob, drawnDebt, expectedPremiumDebt, "usdx after accrual"
        ...
        expectedpremiumShares = bobPosition.drawnShares.percentMulUp(bobRp);
        expectedPremiumDebt =
            _calculatePremiumDebt(hub1, wbtcAssetId, expectedpremiumShares, bobPosition.premiumOffsetRay);
        interest =
            (drawnDebt + expectedPremiumDebt) - amounts.wbtcBorrowAmount - _calculateBurntInterest(hub1, wbtcAssetId);
        _assertOnlyOneUserDebt(
            spoke1, _wbtcReserveId(spoke1), bob, drawnDebt, expectedPremiumDebt, "wbtc after accrual"
    ...
    function _bound(Rates memory rates) internal pure returns (Rates memory) {
        rates.daiBaseDrawnRate = _bpsToRay(bound(rates.daiBaseDrawnRate, 1, MAX_ALLOWED_DRAWN_RATE)).toUint96();
        rates.wethBaseDrawnRate = _bpsToRay(bound(rates.wethBaseDrawnRate, 1, MAX_ALLOWED_DRAWN_RATE)).toUint96();
        rates.usdxBaseDrawnRate = _bpsToRay(bound(rates.usdxBaseDrawnRate, 1, MAX_ALLOWED_DRAWN_RATE)).toUint96();
        rates.wbtcBaseDrawnRate = _bpsToRay(bound(rates.wbtcBaseDrawnRate, 1, MAX_ALLOWED_DRAWN_RATE)).toUint96();

        return rates;
    }

    function _ensureSufficientCollateral(ISpoke spoke, TestAmounts memory amounts)
        internal
        view
        returns (TestAmounts memory)
    {
        uint256 remainingCollateralValue = _convertAmountToValue(spoke, _daiReserveId(spoke), amounts.daiSupplyAmount)
        ...
        remainingCollateralValue -= _convertAmountToValue(spoke, _daiReserveId(spoke), amounts.daiBorrowAmount) * 2;
        ...
        remainingCollateralValue -= _convertAmountToValue(spoke, _wethReserveId(spoke), amounts.wethBorrowAmount) * 2;
        ...
        remainingCollateralValue -= _convertAmountToValue(spoke, _usdxReserveId(spoke), amounts.usdxBorrowAmount) * 2;
