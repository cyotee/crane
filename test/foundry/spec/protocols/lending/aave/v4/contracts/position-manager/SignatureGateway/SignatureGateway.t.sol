...
contract SignatureGatewayTest is SignatureGatewayBaseTest {
    ...
    function test_supplyWithSig() public {
        ...
        SpokeActions.approve({
            spoke: spoke1, reserveId: p.reserveId, owner: alice, spender: address(gateway), amount: p.amount
        });

        uint256 shares = _hub(spoke1, p.reserveId).previewAddByAssets(_reserveAssetId(spoke1, p.reserveId), p.amount);
    ...
    function test_withdrawWithSig() public {
        ...
        SpokeActions.supply({
            spoke: spoke1, reserveId: p.reserveId, caller: alice, amount: p.amount + 1, onBehalfOf: alice
        });

        uint256 shares = _hub(spoke1, p.reserveId).previewRemoveByAssets(_reserveAssetId(spoke1, p.reserveId), p.amount);
        SharesAndAmount memory returnValues;
        vm.expectEmit(address(spoke1));
    ...
    function test_borrowWithSig() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: p.reserveId, caller: alice, amount: p.amount * 2, onBehalfOf: alice
        });
        bytes memory signature = _sign(alicePk, _getTypedDataHash(gateway, p));

        uint256 shares = _hub(spoke1, p.reserveId).previewDrawByAssets(_reserveAssetId(spoke1, p.reserveId), p.amount);
        SharesAndAmount memory returnValues;
        vm.expectEmit(address(spoke1));
    ...
    function test_repayWithSig() public {
        ...
        SpokeActions.borrow({spoke: spoke1, reserveId: p.reserveId, caller: alice, amount: p.amount, onBehalfOf: alice});
        SpokeActions.approve({
            spoke: spoke1, reserveId: p.reserveId, owner: alice, spender: address(gateway), amount: p.amount
        });
        bytes memory signature = _sign(alicePk, _getTypedDataHash(gateway, p));
        ...
        (uint256 baseRestored, uint256 premiumRestored) =
            _calculateExactRestoreAmount(spoke1, p.reserveId, alice, p.amount);
        uint256 shares =
            _hub(spoke1, p.reserveId).previewRestoreByAssets(_reserveAssetId(spoke1, p.reserveId), baseRestored);
        SharesAndAmount memory returnValues;
        vm.expectEmit(address(spoke1));
        emit ISpoke.Repay(
            p.reserveId,
    ...
    function test_setUsingAsCollateralWithSig() public {
        ...
        p.reserveId = _daiReserveId(spoke1);
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: p.reserveId, caller: alice, amount: 1e18, onBehalfOf: alice
        });
        bytes memory signature = _sign(alicePk, _getTypedDataHash(gateway, p));
    ...
    function test_updateUserRiskPremiumWithSig() public {
        ...
        SpokeActions.supplyCollateral({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: alice, amount: 10e18, onBehalfOf: alice
        });
        SpokeActions.borrow({
            spoke: spoke1, reserveId: _daiReserveId(spoke1), caller: alice, amount: 7e18, onBehalfOf: alice
        });
    ...
    function test_multicall() public {
        ...
        uint256 expectedShares =
            _hub(spoke1, reserveId).previewAddByAssets(_reserveAssetId(spoke1, reserveId), p.amount);

        ISignatureGateway.SetUsingAsCollateral memory p2 = _setAsCollateralData(spoke1, alice, deadline);
    ...
    function test_multicall_atomicity_on_revert() public {
        ...
        SpokeActions.approve({
            spoke: spoke1, reserveId: p1.reserveId, owner: alice, spender: address(gateway), amount: p1.amount
        });
        ...
        vm.expectRevert(abi.encodeWithSelector(INoncesKeyed.InvalidAccountNonce.selector, alice, p2.nonce));
