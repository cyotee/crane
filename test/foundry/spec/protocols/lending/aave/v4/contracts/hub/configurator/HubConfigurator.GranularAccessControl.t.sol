...
contract HubConfiguratorGranularAccessControlTest is Base {
    ...
    function _buildAssetManagerCalldata() internal {
        ...
        assetManagerCalldata.push(abi.encodeCall(IHubConfigurator.updateLiquidityFee, (address(hub1), assetId, 10_00)));
        assetManagerCalldata.push(
            abi.encodeCall(IHubConfigurator.updateFeeReceiver, (address(hub1), assetId, newFeeReceiver))
        );
        assetManagerCalldata.push(
            abi.encodeCall(IHubConfigurator.updateFeeConfig, (address(hub1), assetId, 5_00, newFeeReceiver))
        );
        assetManagerCalldata.push(
            abi.encodeCall(
                IHubConfigurator.updateInterestRateStrategy, (address(hub1), assetId, newIrStrategy, encodedIrData)
            )
        );
        assetManagerCalldata.push(
            abi.encodeCall(IHubConfigurator.updateReinvestmentController, (address(hub1), assetId, newController))
        );
        assetManagerCalldata.push(
            abi.encodeCall(IHubConfigurator.updateInterestRateData, (address(hub1), assetId, encodedIrData))
        );
        assetManagerCalldata.push(abi.encodeCall(IHubConfigurator.resetAssetCaps, (address(hub1), assetId)));
        assetManagerCalldata.push(abi.encodeCall(IHubConfigurator.deactivateAsset, (address(hub1), assetId)));
    ...
    function _buildSpokeManagerCalldata() internal {
        ...
        IHub.SpokeConfig memory config =
            IHub.SpokeConfig({active: true, halted: false, addCap: 1000, drawCap: 500, riskPremiumThreshold: 0});
        ...
        spokeManagerCalldata.push(abi.encodeCall(IHubConfigurator.addSpoke, (address(hub1), newSpoke, assetId, config)));
        spokeManagerCalldata.push(
            abi.encodeCall(
                IHubConfigurator.addSpokeToAssets, (address(hub1), makeAddr("NEW_SPOKE_2"), assetIds, configs)
            )
        );
        spokeManagerCalldata.push(
        ...
        spokeManagerCalldata.push(
            abi.encodeCall(IHubConfigurator.updateSpokeRiskPremiumThreshold, (address(hub1), assetId, spokeAddr, 500))
        );
        spokeManagerCalldata.push(
            abi.encodeCall(IHubConfigurator.updateSpokeCaps, (address(hub1), assetId, spokeAddr, 100, 50))
        );
        spokeManagerCalldata.push(abi.encodeCall(IHubConfigurator.deactivateSpoke, (address(hub1), spokeAddr)));
        spokeManagerCalldata.push(abi.encodeCall(IHubConfigurator.haltSpoke, (address(hub1), spokeAddr)));
        spokeManagerCalldata.push(abi.encodeCall(IHubConfigurator.resetSpokeCaps, (address(hub1), spokeAddr)));
    }
    ...
    function test_assetManager_cannotCall_anySpokeManagerMethod() public {
        for (uint256 i = 0; i < spokeManagerCalldata.length; ++i) {
            ...
            (bool ok, bytes memory ret) = address(hubConfigurator).call(spokeManagerCalldata[i]);
            assertFalse(ok);
            assertEq(ret, abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, ASSET_MANAGER));
        }
    }
    ...
    function test_spokeManager_cannotCall_anyAssetManagerMethod() public {
        for (uint256 i = 0; i < assetManagerCalldata.length; ++i) {
            ...
            (bool ok, bytes memory ret) = address(hubConfigurator).call(assetManagerCalldata[i]);
            assertFalse(ok);
            assertEq(ret, abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, SPOKE_MANAGER));
        }
    }
