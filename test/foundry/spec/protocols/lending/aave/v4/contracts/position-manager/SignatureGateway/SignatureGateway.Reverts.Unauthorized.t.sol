...
contract SignatureGateway_Unauthorized_PositionManagerNotActive_Test is SignatureGatewayBaseTest {
    ...
    function test_updateUserRiskPremiumWithSig_revertsWith_Unauthorized() public {
        ISignatureGateway.UpdateUserRiskPremium memory p =
            _updateRiskPremiumData(spoke1, alice, _warpBeforeRandomDeadline(MAX_SKIP_TIME));
        bytes memory signature = _sign(alicePk, _getTypedDataHash(gateway, p));

        vm.expectRevert(abi.encodeWithSelector(IAccessManaged.AccessManagedUnauthorized.selector, address(gateway)));
        vm.prank(vm.randomAddress());
