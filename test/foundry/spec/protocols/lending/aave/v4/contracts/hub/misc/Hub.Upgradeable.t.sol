...
contract HubUpgradeableTest is Base {
    ...
    function test_proxy_reinitialization_fuzz(uint64 initialRevision) public {
        ...
        ITransparentUpgradeableProxy(address(hubProxy))
            .upgradeToAndCall(address(hubImpl2), _getInitializeCalldata(address(accessManager)));
    ...
    function test_proxy_constructor_revertsWith_InvalidInitialization_ZeroRevision() public {
        ...
        new TransparentUpgradeableProxy(
            address(hubImpl), proxyAdminOwner, _getInitializeCalldata(address(accessManager))
        );
    }
    ...
    function test_proxy_constructor_fuzz_revertsWith_InvalidInitialization(uint64 initialRevision) public {
        ...
        MockHubInstance hubImpl = new MockHubInstance(initialRevision);
        ITransparentUpgradeableProxy hubProxy = ITransparentUpgradeableProxy(address(_deployHubProxy(address(hubImpl))));
    ...
    function test_proxy_constructor_revertsWith_InvalidAddress() public {
        ...
        new TransparentUpgradeableProxy(address(hubImpl), proxyAdminOwner, _getInitializeCalldata(address(0)));
    }
    ...
    function _deployHubProxy(address hubImpl) internal returns (IHub) {
        return IHub(
            address(
                new TransparentUpgradeableProxy(
                    hubImpl, proxyAdminOwner, _getInitializeCalldata(address(accessManager))
                )
            )
        );
    }
    ...
    function _addAsset(IHub hub) internal returns (uint256 assetId, address underlying) {
        ...
        IAssetInterestRateStrategy.InterestRateData memory irDataLocal = IAssetInterestRateStrategy.InterestRateData({
