...
contract HubEliminateDeficitTest is Base {
    ...
    function test_eliminateDeficit_fuzz_revertsWith_ArithmeticUnderflow_CallerSpokeNoFunds(uint256) public {
        _createDeficit(_assetId, _coveredSpoke, _deficitAmountRay);
    ...
    function test_eliminateDeficit_fuzz_revertsWith_AccessManagedUnauthorized(address caller) public {
        ...
        (bool immediate, uint32 delay) =
            IAccessManager(hub1.authority()).canCall(caller, address(hub1), IHub.eliminateDeficit.selector);
    ...
    function test_eliminateDeficit(uint256) public {
        ...
        HubActions.add({
            ...
            assetId: _assetId,
            caller: _callerSpoke,
            amount: hub1.previewAddByShares(_assetId, hub1.previewRemoveByAssets(_assetId, clearedDeficit)),
            user: alice
        });
        ...
        uint256 addExRate = _getAddExRate(hub1, _assetId);

        vm.expectEmit(address(hub1));
        emit IHub.EliminateDeficit(_assetId, _callerSpoke, _coveredSpoke, expectedRemoveShares, clearedDeficitRay);
        vm.prank(_callerSpoke);
        (uint256 removedShares, uint256 deficitEliminated) =
            hub1.eliminateDeficit(_assetId, eliminateDeficit, _coveredSpoke);

        assertEq(removedShares, expectedRemoveShares);
        assertEq(deficitEliminated, clearedDeficit);
        assertEq(hub1.getAssetDeficitRay(_assetId), deficitAmountRay2 + _deficitAmountRay - clearedDeficitRay);
        assertEq(hub1.getAddedShares(_assetId), assetSuppliedShares - expectedRemoveShares);
        assertEq(hub1.getSpokeAddedShares(_assetId, _callerSpoke), spokeAddedShares - expectedRemoveShares);
        assertEq(hub1.getSpokeDeficitRay(_assetId, _coveredSpoke), _deficitAmountRay - clearedDeficitRay);
    ...
    function _createDeficit(uint256 assetId, address spoke, uint256 amountRay) internal {
        ...
        _drawLiquidity({hub: hub1, assetId: assetId, amount: amount, withPremium: true, skipTime: true, spoke: spoke});

        (uint256 spokePremiumShares, int256 spokePremiumOffsetRay) = hub1.getSpokePremiumData(assetId, spoke);
