...
contract SpokeOperations_Gas_Tests is Base {
    ...
    function test_multicall_ops() public {
        ...
        calls[0] =
            abi.encodeCall(ISpoke.permitReserve, (reserveId.dai, permit.owner, permit.value, permit.deadline, v, r, s));
        ...
        calls[0] = abi.encodeCall(
            ISpoke.permitReserve, (reserveId.usdx, permit.owner, permit.value, permit.deadline, v, r, s)
        );
        ...
        calls[0] = abi.encodeCall(
            ISpoke.permitReserve, (reserveId.wbtc, permit.owner, permit.value, permit.deadline, v, r, s)
        );
    ...
    function test_setUserPositionManagersWithSig() public {
        ...
        ISpoke.SetUserPositionManagers memory p = ISpoke.SetUserPositionManagers({
            onBehalfOf: user, updates: updates, nonce: spoke.nonces(user, nonceKey), deadline: vm.getBlockTimestamp()
        });
    ...
    function _liquidationSetup(uint256 pricePercentage) internal {
        ...
        ISpoke.UserAccountData memory userAccountData = _borrowToBeLiquidatableWithPriceChange(
            spoke, alice, reserveId.dai, reserveId.usdx, 1.05e18, pricePercentage
        );
