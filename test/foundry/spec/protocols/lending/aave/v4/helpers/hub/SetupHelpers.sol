...
abstract contract SetupHelpers is MathHelpers {
    ...
    function _addAndDrawLiquidity(
        ...
        addedShares = HubActions.add({hub: hub, assetId: assetId, caller: addSpoke, amount: addAmount, user: addUser});

        drawnShares = HubActions.draw({hub: hub, assetId: assetId, to: drawUser, caller: drawSpoke, amount: drawAmount});
    ...
    function _drawLiquidity(IHub hub, uint256 assetId, uint256 amount, bool withPremium, bool skipTime, address spoke)
        internal
    {
    ...
    function _drawLiquidity(
        ...
        HubActions.draw({hub: hub, assetId: assetId, caller: spoke, to: vm.randomAddress(), amount: amount});
    ...
    function _addLiquidity(IHub hub, uint256 assetId, uint256 amount, address admin, uint24 riskPremiumThreshold)
        public
    {
        address tempSpoke = vm.randomAddress();
    ...
    function _getAssetPosition(IHub hub, uint256 assetId) internal view returns (AssetPosition memory) {
