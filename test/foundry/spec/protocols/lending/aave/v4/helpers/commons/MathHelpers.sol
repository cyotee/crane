...
abstract contract MathHelpers {
    ...
    function _convertAmountToValue(uint256 amount, uint256 assetPrice, uint256 assetUnit)
        internal
        pure
        returns (uint256)
    {
    ...
    function _convertValueToAmount(uint256 valueAmount, uint256 assetPrice, uint256 assetUnit)
        internal
        pure
        returns (uint256)
    {
    ...
    function _convertDecimals(uint256 amount, uint256 fromDecimals, uint256 toDecimals, bool roundUp)
        internal
        pure
        returns (uint256)
    {
        return Math.mulDiv(
            amount, 10 ** toDecimals, 10 ** fromDecimals, (roundUp) ? Math.Rounding.Ceil : Math.Rounding.Floor
        );
    }
