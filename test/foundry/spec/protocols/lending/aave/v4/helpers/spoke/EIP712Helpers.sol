...
abstract contract EIP712Helpers is Test {
    ...
    function _getTypedDataHash(ISpoke spoke, ISpoke.SetUserPositionManagers memory setUserPositionManagers)
        internal
        view
        returns (bytes32)
    {
        return keccak256(
            abi.encodePacked(
                ...
                vm.eip712HashStruct(EIP712Types.TYPE_SetUserPositionManagers, abi.encode(setUserPositionManagers))
            )
        );
    }
    ...
    function _burnRandomNoncesAtKey(INoncesKeyed verifier, address user, uint192 key) internal returns (uint256) {
    ...
    function _getRandomInvalidNonceAtKey(INoncesKeyed verifier, address user, uint192 key)
        internal
        view
        returns (uint256)
    {
    ...
    function _assertNonceIncrement(INoncesKeyed verifier, address who, uint256 prevKeyNonce) internal view {
