...
abstract contract Assertions is QueryHelpers {
    ...
    function _assertHubLiquidity(IHub targetHub, uint256 assetId, string memory label) internal view {
        ...
        assertEq(targetHub.getAssetLiquidity(assetId), currentHubBalance, string.concat("hub liquidity ", label));
    }

    function _assertDrawnRateSynced(IHub targetHub, uint256 assetId, string memory operation) internal view {
        IHub.Asset memory asset = targetHub.getAsset(assetId);
        (uint256 drawn,) = targetHub.getAssetOwed(assetId);

        vm.assertEq(
            asset.drawnRate,
            IBasicInterestRateStrategy(asset.irStrategy)
                .calculateInterestRate(assetId, asset.liquidity, drawn, asset.deficitRay, asset.swept),
