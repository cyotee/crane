...
abstract contract MockHelpers is CommonHelpers, Constants {
    ...
    function _mockDrawnRateBps(address irStrategy, uint256 drawnRateBps) internal {
        vm.mockCall(
            irStrategy, IBasicInterestRateStrategy.calculateInterestRate.selector, abi.encode(_bpsToRay(drawnRateBps))
        );
    }
    ...
    function _mockDrawnRateBps(
        ...
        vm.mockCall(
            irStrategy,
            abi.encodeCall(
                IBasicInterestRateStrategy.calculateInterestRate, (assetId, liquidity, drawn, deficit, swept)
            ),
    ...
    function _mockDrawnRateRay(address irStrategy, uint256 drawnRateRay) internal {
        vm.mockCall(irStrategy, IBasicInterestRateStrategy.calculateInterestRate.selector, abi.encode(drawnRateRay));
    }
    ...
    function _mockSupplySharePrice(
        ...
        HubActions.add({hub: hub, assetId: assetId, caller: spoke, amount: totalAddedAssets, user: makeAddr("alice")});
        ...
        vm.store(
            address(hub),
            bytes32(
                uint256(SlotDerivation.deriveMapping({slot: bytes32(_assetsSlot), key: assetId})) + _addedSharesOffset
            ),
            bytes32(addedShares)
        );
    ...
    function _setConstantDrawnRateBps(IHub hub, uint256 assetId, uint32 drawnRateBps, address hubAdmin) internal {
        vm.prank(hubAdmin);
