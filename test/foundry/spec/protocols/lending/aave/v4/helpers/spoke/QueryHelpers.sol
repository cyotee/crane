...
abstract contract QueryHelpers is HubHelpers, Constants, Types {
    ...
    function _getReserveIdByAssetId(ISpoke spoke, IHub hub, uint256 assetId) internal view returns (uint256) {
    ...
    function _getAssetByReserveId(ISpoke spoke, uint256 reserveId) internal view returns (uint256, IERC20) {
        ISpoke.Reserve memory reserve = spoke.getReserve(reserveId);
        (address underlying,) = reserve.hub.getAssetUnderlyingAndDecimals(reserve.assetId);
        return (reserve.assetId, IERC20(underlying));
    }

    function _getAssetUnderlyingByReserveId(ISpoke spoke, uint256 reserveId) internal view returns (IERC20) {
        ISpoke.Reserve memory reserve = spoke.getReserve(reserveId);
        (address underlying,) = reserve.hub.getAssetUnderlyingAndDecimals(reserve.assetId);
        return IERC20(underlying);
    }

    function _getLatestDynamicReserveConfig(ISpoke spoke, uint256 reserveId)
        internal
        view
    ...
    function _getCollateralFactor(ISpoke spoke, uint256 reserveId, address user) internal view returns (uint16) {
    ...
    function _getCollateralFactor(ISpoke spoke, function(ISpoke) internal view returns (uint256) reserveId)
        internal
        view
        returns (uint16)
    {
    ...
    function _getLiquidationFee(ISpoke spoke, uint256 reserveId, address user) internal view returns (uint16) {
        uint32 dynamicConfigKey = spoke.getUserPosition(reserveId, user).dynamicConfigKey;
        return spoke.getDynamicReserveConfig(reserveId, dynamicConfigKey).liquidationFee;
    }

    function _getTokenBalances(IERC20 token, address spoke, address hub) internal view returns (TokenBalances memory) {
        return TokenBalances({spokeBalance: token.balanceOf(spoke), hubBalance: token.balanceOf(hub)});
    }

    function _hasRole(IAccessManager authority, uint64 role, address account) internal view returns (bool) {
    ...
    function _getSpokeDynConfigKeys(ISpoke spoke) internal view returns (DynamicConfigEntry[] memory) {
        ...
        for (uint256 reserveId; reserveId < reserveCount; ++reserveId) {
            configs[reserveId] = DynamicConfigEntry({key: spoke.getReserve(reserveId).dynamicConfigKey, enabled: true});
        }
        return configs;
    }

    // returns reserveId => User(DynamicConfigKey, usingAsCollateral) map.
    function _getUserDynConfigKeys(ISpoke spoke, address user) internal view returns (DynamicConfigEntry[] memory) {
        uint256 reserveCount = spoke.getReserveCount();
        DynamicConfigEntry[] memory configs = new DynamicConfigEntry[](reserveCount);
        for (uint256 reserveId; reserveId < reserveCount; ++reserveId) {
    ...
    function _getUserDynConfig(ISpoke spoke, address user, uint256 reserveId)
        internal
        view
        returns (ISpoke.DynamicReserveConfig memory)
    {
        return spoke.getDynamicReserveConfig(reserveId, spoke.getUserPosition(reserveId, user).dynamicConfigKey);
    }

    // deref and return current UserDynamicReserveConfig for a specific reserveId on user position.
    function _getUserDynConfigKeys(ISpoke spoke, address user, uint256 reserveId)
        internal
        view
        returns (DynamicConfigEntry memory)
    {
    ...
    function _getUserInfo(ISpoke spoke, address user, uint256 reserveId)
        internal
        view
        returns (ISpoke.UserPosition memory)
    {
        return spoke.getUserPosition(reserveId, user);
    }

    function _getUserDebt(ISpoke spoke, address user, uint256 reserveId) internal view returns (DebtData memory data) {
    ...
    function _isUsingAsCollateral(ISpoke spoke, uint256 reserveId, address user) internal view returns (bool) {
        (bool res,) = spoke.getUserReserveStatus(reserveId, user);
        return res;
    }

    function _isBorrowing(ISpoke spoke, uint256 reserveId, address user) internal view returns (bool) {
        (, bool res) = spoke.getUserReserveStatus(reserveId, user);
        return res;
    }

    function _getTotalWithdrawable(ISpoke spoke, uint256 reserveId, address user) internal view returns (uint256) {
    ...
    function _getUserDrawnShares(ISpoke spoke, uint256 reserveId, address user) internal view returns (uint256) {
    ...
    function _getSpokePosition(ISpoke spoke, function(ISpoke) internal view returns (uint256) reserveIdFn)
        internal
        view
        returns (SpokePosition memory)
    {
        return _getSpokePosition(spoke, reserveIdFn(spoke));
    }

    function _getSpokePosition(ISpoke spoke, uint256 reserveId) internal view returns (SpokePosition memory) {
    ...
    function _snapshotUser(ISpoke spoke, uint256 reserveId, address user)
        internal
        view
        returns (UserSnapshot memory snap)
    {
    ...
    function _snapshotReserve(ISpoke spoke, uint256 reserveId) internal view returns (ReserveSnapshot memory snap) {
