...
abstract contract MathHelpers is QueryHelpers {
    ...
    function _getExpectedPremiumDelta(
        ...
        uint256 premiumDebtRay = _calculatePremiumDebtRay(oldPremiumShares, oldPremiumOffsetRay, drawnIndex);

        uint256 newPremiumShares = drawnShares.percentMulUp(riskPremium);
        int256 newPremiumOffsetRay =
            _calculatePremiumAssetsRay(newPremiumShares, drawnIndex).signedSub(premiumDebtRay - restoredPremiumRay);
    ...
    function _calculatePremiumDebtRay(uint256 premiumShares, int256 premiumOffsetRay, uint256 drawnIndex)
        internal
        pure
        returns (uint256)
    {
    ...
    function _calculatePremiumAssetsRay(uint256 premiumShares, uint256 drawnIndex) internal pure returns (uint256) {
        return premiumShares * drawnIndex;
    }

    function _calculatePremiumDebtRay(IHub hub, uint256 assetId, uint256 premiumShares, int256 premiumOffsetRay)
        internal
        view
        returns (uint256)
    {
    ...
    function _calculatePremiumDebt(IHub hub, uint256 assetId, uint256 premiumShares, int256 premiumOffsetRay)
        internal
        view
        returns (uint256)
    {
    ...
    function _calculatePremiumAssetsRay(IHub hub, uint256 assetId, uint256 premiumShares)
        internal
        view
        returns (uint256)
    {
    ...
    function _calculateExpectedDrawnIndex(uint256 initialDrawnIndex, uint96 drawnRate, uint40 startTime)
        internal
        view
        returns (uint256)
    {
    ...
    function _calculateExpectedDrawnDebt(uint256 initialDebt, uint96 drawnRate, uint40 startTime)
        internal
        view
        returns (uint256)
    {
    ...
    function _calculateExactRestoreAmount(
        ...
        drawnRestored = hub.previewRestoreByShares(assetId, hub.previewRestoreByAssets(assetId, drawnRestored));
    ...
    function _calculateExpectedFeesAmount(
        ...
        return indexDelta.rayMulUp(initialDrawnShares + initialPremiumShares).percentMulDown(liquidityFee);
    }

    function _calculateEffectiveAddedAssets(uint256 assetsAmount, uint256 totalAddedAssets, uint256 totalAddedShares)
        internal
        pure
        returns (uint256)
    {
        uint256 sharesAmount = assetsAmount.toSharesDown(totalAddedAssets, totalAddedShares);
        return sharesAmount.toAssetsDown(totalAddedAssets + assetsAmount, totalAddedShares + sharesAmount);
    }
    ...
    function _calcUnrealizedFees(IHub hub, uint256 assetId) internal view returns (uint256) {
        ...
        uint256 drawnIndex = asset.drawnIndex
            .rayMulUp(MathUtils.calculateLinearInterest(asset.drawnRate, uint40(asset.lastUpdateTimestamp)));

        uint256 aggregatedOwedRayAfter = (((uint256(asset.drawnShares) + asset.premiumShares) * drawnIndex).toInt256()
            - asset.premiumOffsetRay)
        .toUint256() + asset.deficitRay;
        uint256 aggregatedOwedRayBefore = (((uint256(asset.drawnShares) + asset.premiumShares) * previousIndex)
            .toInt256()
            - asset.premiumOffsetRay)
        .toUint256() + asset.deficitRay;

        return
            (aggregatedOwedRayAfter.fromRayUp() - aggregatedOwedRayBefore.fromRayUp())
            .percentMulDown(asset.liquidityFee);
    }

    function _getExpectedFeeReceiverAddedAssets(IHub hub, uint256 assetId) internal view returns (uint256) {
    ...
    function _getAddedAssetsWithFees(IHub hub, uint256 assetId) internal view returns (uint256) {
        return hub.getAddedAssets(assetId) + hub.getAsset(assetId).realizedFees + _calcUnrealizedFees(hub, assetId);
    }

    function _calculateBurntInterest(IHub hub, uint256 assetId) internal view returns (uint256) {
        return hub.getAddedAssets(assetId) - hub.previewRemoveByShares(assetId, hub.getAddedShares(assetId));
    }
}
