...
import {CheckedActions} from "@crane/test/foundry/spec/protocols/lending/aave/v4/helpers/spoke/CheckedActions.sol";
import {ConfigHelpers} from "@crane/test/foundry/spec/protocols/lending/aave/v4/helpers/spoke/ConfigHelpers.sol";
import {MockHelpers} from "@crane/test/foundry/spec/protocols/lending/aave/v4/helpers/spoke/MockHelpers.sol";
...
abstract contract SetupHelpers is CheckedActions, ConfigHelpers, MockHelpers {
    ...
    function _increaseCollateralSupply(ISpoke spoke, uint256 reserveId, uint256 amount, address user) public {
        ...
        _checkedSupplyCollateral(
            CheckedSupplyCollateralParams({
                spoke: spoke, reserveId: reserveId, user: user, amount: amount, onBehalfOf: user
            })
        );
    ...
    function _increaseReserveDebt(ISpoke spoke, uint256 reserveId, uint256 amount, address user) internal {
        _openSupplyPosition(
            spoke,
            reserveId,
            _max(_hub(spoke, reserveId).previewAddByShares(_reserveAssetId(spoke, reserveId), 1), amount)
        );
        SpokeActions.borrow({spoke: spoke, reserveId: reserveId, caller: user, amount: amount, onBehalfOf: user});
    }

    /// @dev Opens a debt position for a random user, using same asset as collateral and borrow
    function _openDebtPosition(ISpoke spoke, uint256 reserveId, uint256 amount, bool withPremium, address spokeAdmin)
        internal
        returns (address)
    {
        ...
        uint256 supplyAmount = _calcMinimumCollAmount({
            spoke: spoke, collReserveId: reserveId, debtReserveId: reserveId, debtAmount: amount
        });

        _deal(spoke, reserveId, tempUser, supplyAmount);
        SpokeActions.approve({spoke: spoke, reserveId: reserveId, owner: tempUser, amount: UINT256_MAX});

        SpokeActions.supplyCollateral({
            spoke: spoke, reserveId: reserveId, caller: tempUser, amount: supplyAmount, onBehalfOf: tempUser
        });
        ...
        SpokeActions.borrow({
            spoke: spoke, reserveId: reserveId, caller: tempUser, amount: amount, onBehalfOf: tempUser
        });
        skip(365 days);
    ...
    function _backedBorrow(
        ...
        uint256 supplyAmount = _calcMinimumCollAmount({
            spoke: spoke, collReserveId: collateralReserveId, debtReserveId: debtReserveId, debtAmount: borrowAmount
        }) * 5;
        _deal(spoke, collateralReserveId, user, supplyAmount);
        SpokeActions.approve({spoke: spoke, reserveId: collateralReserveId, owner: user, amount: UINT256_MAX});
        SpokeActions.supplyCollateral({
            spoke: spoke, reserveId: collateralReserveId, caller: user, amount: supplyAmount, onBehalfOf: user
        });
        SpokeActions.borrow({
            spoke: spoke, reserveId: debtReserveId, caller: user, amount: borrowAmount, onBehalfOf: user
        });
    }

    /// @dev Borrow to be at a certain health factor
    function _borrowToBeAtHf(ISpoke spoke, address user, uint256 reserveId, uint256 desiredHf)
        internal
        returns (uint256, uint256)
    {
        uint256 requiredDebtAmount = _getRequiredDebtAmountForHf(spoke, user, reserveId, desiredHf);
        require(0 < requiredDebtAmount && requiredDebtAmount <= MAX_SUPPLY_AMOUNT, "required debt amount 0 or too high");
    ...
    function _borrowToBeLiquidatableWithPriceChange(
        ...
        SpokeActions.borrow({
            spoke: spoke, reserveId: reserveId, caller: user, amount: requiredDebtAmount, onBehalfOf: user
        });
    ...
    function _borrowWithoutHfCheck(ISpoke spoke, address user, uint256 reserveId, uint256 debtAmount) internal {
    ...
    function _executeSpokeSupplyAndBorrow(
        ...
        assertEq(borrowResult.ownerAfter.drawnDebt - borrowResult.ownerBefore.drawnDebt, borrow.borrowAmount);
        assertEq(
            borrowResult.reserveAfter.totalDrawnDebt - borrowResult.reserveBefore.totalDrawnDebt, borrow.borrowAmount
        );
    ...
    function _repayAll(ISpoke spoke, uint256 reserveId, address[] memory users) internal {
        ...
        uint256 assetOwedWithoutSpoke = hub.getAssetTotalOwed(assetId) - hub.getSpokeTotalOwed(assetId, address(spoke));
        ...
        assertEq(hub.getAssetTotalOwed(assetId), assetOwedWithoutSpoke, "hub asset total debt not settled");
    }
    ...
    function _increaseReserveIndex(
        ...
        assertGt(borrow.supplyAmount, hub.previewAddByAssets(state.borrowReserveAssetId, borrow.supplyAmount));
    ...
    function _updateMaxUserReservesLimit(ISpoke spoke, uint16 newLimit) internal {
        ...
        ISpokeInstance newImpl = AaveV4TestOrchestration.deploySpokeImplementation(spoke.ORACLE(), newLimit);
    ...
    function _deploySpokeWithOracle(address proxyAdminOwner, address _accessManager)
        internal
        pausePrank
    ...
    function _deploySpokeWithOracle(address proxyAdminOwner, address _accessManager, uint16 maxUserReservesLimit)
        internal
        pausePrank
        returns (ISpoke, IAaveOracle)
    {
        ...
        ISpoke spoke = ISpoke(
            AaveV4TestOrchestration.proxify(
                address(AaveV4TestOrchestration.deploySpokeImplementation(address(oracle), maxUserReservesLimit)),
                proxyAdminOwner,
    ...
    function _randomUninitializedConfigKey(ISpoke spoke, uint256 reserveId) internal view returns (uint32) {
    ...
    function _randomMaxLiquidationBonus(ISpoke spoke, uint256 reserveId) internal view returns (uint32) {
        return vm.randomUint(MIN_LIQUIDATION_BONUS, _maxLiquidationBonusUpperBound(spoke, reserveId)).toUint32();
    }
