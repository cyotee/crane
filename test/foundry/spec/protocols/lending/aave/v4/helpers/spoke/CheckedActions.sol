...
abstract contract CheckedActions is MathHelpers {
    ...
    function _checkedSupply(CheckedSupplyParams memory params) internal returns (CheckedSupplyResult memory result) {
        ...
        (result.shares, result.amount) = params.spoke.supply(params.reserveId, params.amount, params.onBehalfOf);
        ...
        // Basic invariants
        assertGe(
            result.ownerAfter.suppliedShares, result.ownerBefore.suppliedShares, "checkedSupply: shares should increase"
        );
        assertGe(
    ...
    function _checkedWithdraw(CheckedWithdrawParams memory params)
        internal
        returns (CheckedWithdrawResult memory result)
    {
        result.callerBefore = _snapshotUser(params.spoke, params.reserveId, params.user);
        result.ownerBefore = _snapshotUser(params.spoke, params.reserveId, params.onBehalfOf);
        result.reserveBefore = _snapshotReserve(params.spoke, params.reserveId);
        ...
        vm.prank(params.user);
        (result.shares, result.amount) = params.spoke.withdraw(params.reserveId, params.amount, params.onBehalfOf);
    ...
    function _checkedBorrow(CheckedBorrowParams memory params) internal returns (CheckedBorrowResult memory result) {
        result.callerBefore = _snapshotUser(params.spoke, params.reserveId, params.user);
        result.ownerBefore = _snapshotUser(params.spoke, params.reserveId, params.onBehalfOf);
        result.reserveBefore = _snapshotReserve(params.spoke, params.reserveId);

        vm.prank(params.user);
        (result.shares, result.amount) = params.spoke.borrow(params.reserveId, params.amount, params.onBehalfOf);
        ...
        // Basic invariants
        assertGe(result.ownerAfter.totalDebt, result.ownerBefore.totalDebt, "checkedBorrow: user debt should increase");
        assertGe(
            result.reserveAfter.totalDebt, result.reserveBefore.totalDebt, "checkedBorrow: reserve debt should increase"
        );
    }

    function _checkedRepay(CheckedRepayParams memory params) internal returns (CheckedRepayResult memory result) {
        result.callerBefore = _snapshotUser(params.spoke, params.reserveId, params.user);
        result.ownerBefore = _snapshotUser(params.spoke, params.reserveId, params.onBehalfOf);
        result.reserveBefore = _snapshotReserve(params.spoke, params.reserveId);

        (result.baseRestored, result.premiumRestored) =
            _calculateExactRestoreAmount(params.spoke, params.reserveId, params.onBehalfOf, params.amount);

        vm.prank(params.user);
        (result.shares, result.amount) = params.spoke.repay(params.reserveId, params.amount, params.onBehalfOf);
        ...
        // Basic invariants
        assertLe(result.ownerAfter.totalDebt, result.ownerBefore.totalDebt, "checkedRepay: user debt should decrease");
        assertLe(
            result.reserveAfter.totalDebt, result.reserveBefore.totalDebt, "checkedRepay: reserve debt should decrease"
        );
    }

    function _checkedSupplyCollateral(CheckedSupplyCollateralParams memory params)
        internal
        returns (CheckedSupplyResult memory result)
    {
