...
abstract contract MathHelpers is QueryHelpers {
    ...
    function _convertAmountToValue(ISpoke spoke, uint256 reserveId, uint256 amount) internal view returns (uint256) {
        return _convertAmountToValue(
            amount,
    ...
    function _convertValueToAmount(ISpoke spoke, uint256 reserveId, uint256 valueAmount)
        internal
        view
        returns (uint256)
    {
        return _convertValueToAmount(
            valueAmount,
            IPriceOracle(spoke.ORACLE()).getReservePrice(reserveId),
            10 ** _underlying(spoke, reserveId).decimals()
        );
    ...
    function _convertAssetAmount(ISpoke spoke, uint256 reserveId, uint256 amount, uint256 toReserveId)
        internal
        view
        returns (uint256)
    {
        return _convertValueToAmount(spoke, toReserveId, _convertAmountToValue(spoke, reserveId, amount));
    }

    function _calculatePremiumDebtRay(ISpoke spoke, uint256 reserveId, uint256 premiumShares, int256 premiumOffsetRay)
        internal
        view
        returns (uint256)
    {
    ...
    function _calculatePremiumDebtRay(ISpoke spoke, uint256 reserveId, address user) internal view returns (uint256) {
        ISpoke.UserPosition memory userPosition = spoke.getUserPosition(reserveId, user);
        return _calculatePremiumDebtRay(spoke, reserveId, userPosition.premiumShares, userPosition.premiumOffsetRay);
    }

    function _calculateMaxSupplyAmount(ISpoke spoke, uint256 reserveId) internal view returns (uint256) {
        return MAX_SUPPLY_ASSET_UNITS * 10 ** spoke.getReserve(reserveId).decimals;
    }

    function _calculateRestoreAmounts(uint256 restoreAmount, uint256 drawn, uint256 premiumRay)
        internal
        pure
        returns (uint256 drawnAmountToRestore, uint256 premiumRayToRestore)
    {
    ...
    function _calculateExpectedPremiumDebt(uint256 initialDrawnDebt, uint256 currentDrawnDebt, uint256 userRiskPremium)
        internal
        pure
        returns (uint256)
    {
    ...
    function _calculateExactRestoreAmount(ISpoke spoke, uint256 reserveId, address user, uint256 repayAmount)
        internal
        view
        returns (uint256 baseRestored, uint256 premiumRestored)
    {
        (uint256 drawn, uint256 premium) = spoke.getUserDebt(reserveId, user);
        return _calculateExactRestoreAmount(
            _hub(spoke, reserveId), _reserveAssetId(spoke, reserveId), drawn, premium, repayAmount
        );
    }
    ...
    function _getUserAccountData(ISpoke spoke, address user, bool refreshConfig)
        internal
        returns (ISpoke.UserAccountData memory)
    {
        ...
        ISpoke.UserAccountData memory userAccountData =
            MockSpoke(address(spoke)).calculateUserAccountData(user, refreshConfig);
    ...
    function _getExpectedPremiumDelta(ISpoke spoke, address user, uint256 reserveId, uint256 repayAmount)
        internal
        view
        virtual
        returns (IHubBase.PremiumDelta memory)
    {
        ...
        (, uint256 premiumRayToRestore) =
            _calculateRestoreAmounts(repayAmount, userDebt.drawnDebt, userDebt.premiumDebtRay);

        ISpoke.UserPosition memory userPosition = spoke.getUserPosition(reserveId, user);
        uint256 assetId = spoke.getReserve(reserveId).assetId;
        IHub hub = _hub(spoke, reserveId);

        return _getExpectedPremiumDelta({
            hub: hub,
            assetId: assetId,
    ...
    function _getExpectedPremiumDeltaForRestore(ISpoke spoke, address user, uint256 reserveId, uint256 repayAmount)
        internal
        view
        virtual
        returns (IHubBase.PremiumDelta memory)
    {
        DebtData memory userDebt = _getUserDebt(spoke, user, reserveId);
        (uint256 drawnDebtToRestore, uint256 premiumRayToRestore) =
            _calculateRestoreAmounts(repayAmount, userDebt.drawnDebt, userDebt.premiumDebtRay);
    ...
    function _calcMinimumCollAmount(ISpoke spoke, uint256 collReserveId, uint256 debtReserveId, uint256 debtAmount)
        internal
        view
        returns (uint256)
    {
        ...
        ISpoke.DynamicReserveConfig memory collDynConf = _getLatestDynamicReserveConfig(spoke, collReserveId);
        ...
        return normalizedDebtAmount.wadDivUp(normalizedCollPrice.toWad().percentMulDown(collDynConf.collateralFactor));
    }

    function _calcMaxDebtAmount(ISpoke spoke, uint256 collReserveId, uint256 debtReserveId, uint256 collAmount)
        internal
        view
        returns (uint256)
    {
        ...
        {
            ISpoke.Reserve memory collData = spoke.getReserve(collReserveId);
            uint256 collAssetUnits = 10 ** IHub(address(collData.hub)).getAsset(collData.assetId).decimals;
            normalizedCollPrice = (collAmount * oracle.getReservePrice(collReserveId)).wadDivDown(collAssetUnits);
        }

        uint256 normalizedDebtAmount;
        {
            ISpoke.Reserve memory debtData = spoke.getReserve(debtReserveId);
            uint256 debtAssetUnits = 10 ** IHub(address(debtData.hub)).getAsset(debtData.assetId).decimals;
            normalizedDebtAmount = oracle.getReservePrice(debtReserveId).wadDivDown(debtAssetUnits);
        }

        uint256 maxDebt = normalizedCollPrice.toWad().percentMulDown(collFactor) / normalizedDebtAmount.toWad();

        return maxDebt > 1 ? maxDebt - 1 : maxDebt;
    }

    function _getRequiredDebtAmountForHf(ISpoke spoke, address user, uint256 reserveId, uint256 desiredHf)
        internal
    ...
    function _getRequiredDebtValueForHf(ISpoke spoke, address user, uint256 desiredHf)
        internal
        returns (uint256 requiredDebtValue)
    {
        ISpoke.UserAccountData memory userAccountData =
            _getUserAccountData({spoke: spoke, user: user, refreshConfig: true});
        uint256 totalAdjustedCollateralValue =
            userAccountData.totalCollateralValue.wadMulDown(userAccountData.avgCollateralFactor);
        uint256 targetTotalDebtValue = totalAdjustedCollateralValue.wadDivUp(desiredHf);
        assertLe(userAccountData.totalDebtValueRay / WadRayMath.RAY, targetTotalDebtValue, "User has enough debt");
    ...
    function _calculateExpectedUserRP(ISpoke spoke, address user, bool refreshDynamicConfig)
        internal
        view
        returns (uint256)
    {
        ...
        for (uint256 reserveId; reserveId < vars.reserveCount; ++reserveId) {
            ...
            if (_isUsingAsCollateral(spoke, reserveId, user)) {
                ...
                vars.collateralFactor = spoke.getDynamicReserveConfig(reserveId, vars.dynamicConfigKey).collateralFactor;

                vars.collateralValue =
                    _convertAmountToValue(spoke, reserveId, spoke.getUserSuppliedAssets(reserveId, user));
        ...
        while (vars.totalDebtValue > 0 && vars.idx < reserveCollateralRisk.length()) {
            ...
            vars.collateralValue = _convertAmountToValue(spoke, reserveId, spoke.getUserSuppliedAssets(reserveId, user));
    ...
    function _spokeMaxDrawnRate(ISpoke spoke) internal view returns (uint32) {
        ...
        for (uint256 reserveId; reserveId < spoke.getReserveCount(); ++reserveId) {
            uint32 drawnRate = (_hub(spoke, reserveId).getAssetDrawnRate(_reserveAssetId(spoke, reserveId))
                    .mulDivUp(PercentageMath.PERCENTAGE_FACTOR, WadRayMath.RAY))
            .toUint32();
    ...
    function _maxLiquidationBonusUpperBound(ISpoke spoke, uint256 reserveId) internal view returns (uint32) {
        return
            _maxLiquidationBonusUpperBound(_getLatestDynamicReserveConfig(spoke, reserveId).collateralFactor).toUint32();
    }

    function _maxLiquidationBonusUpperBound(uint256 collateralFactor) internal pure returns (uint256) {
        return collateralFactor == 0
            ? MIN_LIQUIDATION_BONUS
            : (PercentageMath.PERCENTAGE_FACTOR - 1).percentDivDown(collateralFactor).toUint32();
    }

    function _collateralFactorUpperBound(ISpoke spoke, uint256 reserveId) internal view returns (uint16) {
        return _collateralFactorUpperBound(_getLatestDynamicReserveConfig(spoke, reserveId).maxLiquidationBonus);
    }
