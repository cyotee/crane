...
abstract contract MockHelpers is CommonHelpers {
    ...
    function _mockReservePrice(ISpoke spoke, uint256 reserveId, uint256 price, address admin) internal {
    ...
    function _mockReservePriceByPercent(ISpoke spoke, uint256 reserveId, uint256 percentage, address admin) internal {
    ...
    function _mockDecimals(address underlying, uint8 decimals) internal {
        vm.mockCall(underlying, abi.encodeWithSelector(IERC20Metadata.decimals.selector), abi.encode(decimals));
    }
}
