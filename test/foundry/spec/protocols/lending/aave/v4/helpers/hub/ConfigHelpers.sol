...
import {Assertions} from "@crane/test/foundry/spec/protocols/lending/aave/v4/helpers/hub/Assertions.sol";
...
abstract contract ConfigHelpers is Assertions {
    ...
    function _updateLiquidityFee(IHub hub, uint256 assetId, uint256 liquidityFee, address hubAdmin)
        internal
        pausePrank
    {
        IHub.AssetConfig memory config = hub.getAssetConfig(assetId);
    ...
    function _updateSpokeHalted(IHub hub, uint256 assetId, address spoke, bool halted, address hubAdmin)
        internal
        pausePrank
    {
    ...
    function _updateSpokeActive(IHub hub, uint256 assetId, address spoke, bool newActive, address hubAdmin)
        internal
        pausePrank
    {
        IHub.SpokeConfig memory spokeConfig = hub.getSpokeConfig(assetId, spoke);
    ...
    function _updateAddCap(IHub hub, uint256 assetId, address spoke, uint40 newAddCap, address hubAdmin)
        internal
        pausePrank
    {
        IHub.SpokeConfig memory spokeConfig = hub.getSpokeConfig(assetId, spoke);
    ...
    function _updateDrawCap(IHub hub, uint256 assetId, address spoke, uint40 newDrawCap, address hubAdmin)
        internal
        pausePrank
    {
        IHub.SpokeConfig memory spokeConfig = hub.getSpokeConfig(assetId, spoke);
    ...
    function _grantDeficitEliminatorRole(IHub hub, address target, address admin) internal pausePrank {
