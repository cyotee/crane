...
} from "@crane/contracts/protocols/lending/aave/v4/deployments/orchestration/AaveV4DeployOrchestration.sol";
import {
    WETHDeployProcedure
} from "@crane/test/foundry/spec/protocols/lending/aave/v4/deployments/procedures/WETHDeployProcedure.sol";
import {
...
import {
    BytecodeHelper
} from "@crane/contracts/protocols/lending/aave/v4/deployments/utils/libraries/BytecodeHelper.sol";
...
import {
    INativeTokenGateway
} from "@crane/contracts/protocols/lending/aave/v4/position-manager/interfaces/INativeTokenGateway.sol";
...
contract BatchTestProcedures is Test, Create2TestHelper, WETHDeployProcedure {
    ...
    function checkedV4Deployment() public {
        ...
        OrchestrationReports.FullDeploymentReport memory report =
            AaveV4DeployOrchestration.deployAaveV4(_logger, _deployer, _inputs, hubBytecode, spokeBytecode);
        vm.stopPrank();
    ...
    function _checkConfiguratorBatchDeployments(OrchestrationReports.FullDeploymentReport memory report) internal view {
        assertEq(
    ...
    function _checkRoles(
        ...
        IAccessManagerEnumerable accessManager = IAccessManagerEnumerable(report.authorityBatchReport.accessManager);
    ...
    function _sanitizeInputs(InputUtils.FullDeployInputs memory inputs)
        internal
        view
        returns (InputUtils.FullDeployInputs memory)
    {
        inputs.accessManagerAdmin = inputs.accessManagerAdmin != address(0) ? inputs.accessManagerAdmin : _deployer;
        inputs.hubAdmin = inputs.hubAdmin != address(0) ? inputs.hubAdmin : _deployer;
        inputs.hubConfiguratorAdmin =
            inputs.hubConfiguratorAdmin != address(0) ? inputs.hubConfiguratorAdmin : _deployer;
        inputs.treasurySpokeOwner = inputs.treasurySpokeOwner != address(0) ? inputs.treasurySpokeOwner : _deployer;
        inputs.spokeAdmin = inputs.spokeAdmin != address(0) ? inputs.spokeAdmin : _deployer;
        inputs.proxyAdminOwner = inputs.proxyAdminOwner != address(0) ? inputs.proxyAdminOwner : _deployer;
        inputs.spokeConfiguratorAdmin =
            inputs.spokeConfiguratorAdmin != address(0) ? inputs.spokeConfiguratorAdmin : _deployer;
        inputs.gatewayOwner = inputs.gatewayOwner != address(0) ? inputs.gatewayOwner : _deployer;
        inputs.positionManagerOwner =
            inputs.positionManagerOwner != address(0) ? inputs.positionManagerOwner : _deployer;
    ...
    function _defaultSpokeMaxReservesLimits(uint256 count) internal pure returns (uint16[] memory limits) {
    ...
    function _checkFullReport(
        ...
        if (inputs.deployPositionManagers) {
            assertNotEq(report.positionManagerBatchReport.giverPositionManager, address(0), "GiverPositionManager");
            assertNotEq(report.positionManagerBatchReport.takerPositionManager, address(0), "TakerPositionManager");
            assertNotEq(report.positionManagerBatchReport.configPositionManager, address(0), "ConfigPositionManager");
        } else {
            assertEq(report.positionManagerBatchReport.giverPositionManager, address(0), "Zero GiverPositionManager");
            assertEq(report.positionManagerBatchReport.takerPositionManager, address(0), "Zero TakerPositionManager");
            assertEq(report.positionManagerBatchReport.configPositionManager, address(0), "Zero ConfigPositionManager");
        }
        ...
        for (uint256 i = 0; i < report.hubInstanceBatchReports.length; i++) {
            ...
            assertNotEq(report.hubInstanceBatchReports[i].report.hubImplementation, address(0), "HubImplementation");
        ...
        for (uint256 i = 0; i < report.spokeInstanceBatchReports.length; i++) {
            ...
            assertNotEq(
                report.spokeInstanceBatchReports[i].report.spokeImplementation, address(0), "SpokeImplementation"
            );
            assertNotEq(report.spokeInstanceBatchReports[i].report.aaveOracle, address(0), "AaveOracle");
        }
        assertEq(report.hubInstanceBatchReports.length, inputs.hubLabels.length, "HubBatchReportsLength");
        assertEq(report.spokeInstanceBatchReports.length, inputs.spokeLabels.length, "SpokeInstanceBatchReportsLength");
    }
    ...
    function _checkSpokeBatchDeployments(
        ...
        for (uint256 i = 0; i < inputs.spokeLabels.length; i++) {
            ...
            OrchestrationReports.SpokeDeploymentReport memory spokeReport = report.spokeInstanceBatchReports[i];
    ...
    function _checkSpokeDeployment(
        ...
        assertEq(
            ProxyHelper.getProxyInitializedVersion(ProxyHelper.getImplementation(report.report.spokeProxy)),
            type(uint64).max,
        ...
        _assertBytecodeMatchExcludingImmutables(
            ...
            vm.getDeployedCode("contracts/protocols/lending/aave/v4/spoke/instances/SpokeInstance.sol:SpokeInstance"),
            string.concat(label, " spoke implementation bytecode")
        );
    }

    function _checkOracleDeployment(OrchestrationReports.SpokeDeploymentReport memory report, string memory label)
        internal
        view
    {
        assertEq(
    ...
    function _checkHubDeployment(
        ...
        assertEq(
            IAccessManaged(report.report.hubProxy).authority(), accessManager, string.concat(label, " hub authority")
        );
        assertEq(
        ...
        assertEq(
            ...
            keccak256(
                vm.getDeployedCode("contracts/protocols/lending/aave/v4/hub/instances/HubInstance.sol:HubInstance")
            ),
    ...
    function _checkTreasurySpokeDeployment(OrchestrationReports.FullDeploymentReport memory report) internal pure {
        assertNotEq(report.treasurySpokeBatchReport.treasurySpoke, address(0), "treasury spoke deployed");
    }

    function _checkAccessManagerRoles(IAccessManagerEnumerable accessManager, InputUtils.FullDeployInputs memory inputs)
        internal
        view
    {
        address expectedAdmin =
            (inputs.grantRoles && inputs.accessManagerAdmin != address(0)) ? inputs.accessManagerAdmin : _deployer;
        assertEq(
            accessManager.getRoleMember(Roles.ACCESS_MANAGER_ADMIN_ROLE, 0), expectedAdmin, "DefaultAdminRoleMember"
        );
        assertEq(accessManager.getRoleMemberCount(Roles.ACCESS_MANAGER_ADMIN_ROLE), 1, "DefaultAdminRoleCount");
    ...
    function _checkRoleLabels(IAccessManagerEnumerable accessManager) internal view {
        ...
        // Hub roles
        assertTrue(accessManager.isRoleLabeled(Roles.HUB_DOMAIN_ADMIN_ROLE), "HUB_DOMAIN_ADMIN labeled");
        ...
        assertEq(accessManager.getLabelOfRole(Roles.HUB_DEFICIT_ELIMINATOR_ROLE), "HUB_DEFICIT_ELIMINATOR_ROLE");

        // HubConfigurator roles
        assertEq(
            accessManager.getLabelOfRole(Roles.HUB_CONFIGURATOR_DOMAIN_ADMIN_ROLE), "HUB_CONFIGURATOR_DOMAIN_ADMIN_ROLE"
        );

        // Spoke roles
        assertEq(accessManager.getLabelOfRole(Roles.SPOKE_DOMAIN_ADMIN_ROLE), "SPOKE_DOMAIN_ADMIN_ROLE");
        assertEq(accessManager.getLabelOfRole(Roles.SPOKE_CONFIGURATOR_ROLE), "SPOKE_CONFIGURATOR_ROLE");
        assertEq(
            accessManager.getLabelOfRole(Roles.SPOKE_USER_POSITION_UPDATER_ROLE), "SPOKE_USER_POSITION_UPDATER_ROLE"
        );
    ...
    function _checkSpokeConfiguratorRoles(
        ...
        if (inputs.spokeLabels.length > 0 && inputs.grantRoles) {
            assertEq(
                accessManager.getRoleMemberCount(Roles.SPOKE_CONFIGURATOR_ROLE), 2, "SpokeConfiguratorRole member count"
            );
            assertEq(
            ...
            assertEq(
                accessManager.getRoleMemberCount(Roles.SPOKE_CONFIGURATOR_ROLE), 0, "SpokeConfiguratorRole member count"
            );
        }
            ...
            for (uint256 j = 0; j < _spokeConfiguratorRoleSelectors.length; j++) {
                assertEq(
                    accessManager.getTargetFunctionRole(
                        report.spokeInstanceBatchReports[i].report.spokeProxy, _spokeConfiguratorRoleSelectors[j]
                    ),
                ...
                assertEq(allowed, inputs.grantRoles ? true : false, "SpokeConfiguratorRole allowed - configurator");
                ...
                assertEq(allowed, inputs.grantRoles ? true : false, "SpokeConfiguratorRole allowed - spoke admin");
    ...
    function _checkSpokeAdminRoles(
        ...
        for (uint256 i = 0; i < inputs.spokeLabels.length; i++) {
            address proxyAdminOwner =
                Ownable(ProxyHelper.getProxyAdmin(report.spokeInstanceBatchReports[i].report.spokeProxy)).owner();
            assertEq(
                proxyAdminOwner, inputs.proxyAdminOwner, string.concat(inputs.spokeLabels[i], " proxy admin owner")
            );
            ...
            for (uint256 j = 0; j < _spokePositionUpdaterRoleSelectors.length; j++) {
                ...
                assertEq(
                    accessManager.getTargetFunctionRole(
                        report.spokeInstanceBatchReports[i].report.spokeProxy, _spokePositionUpdaterRoleSelectors[j]
                    ),
    ...
    function _checkHubBatchRoles(
        ...
        if (inputs.hubLabels.length > 0 && inputs.grantRoles) {
            assertEq(accessManager.getRoleMemberCount(Roles.HUB_FEE_MINTER_ROLE), 1, "HubFeeMinterRoleCount");
            assertEq(
            ...
            assertEq(accessManager.getRoleMemberCount(Roles.HUB_FEE_MINTER_ROLE), 0, "HubFeeMinterRoleCount");
        }
        ...
        for (uint256 i = 0; i < inputs.hubLabels.length; i++) {
            for (uint256 j = 0; j < _hubFeeMinterRoleSelectors.length; j++) {
                assertEq(
                    accessManager.getTargetFunctionRole(
                        report.hubInstanceBatchReports[i].report.hubProxy, _hubFeeMinterRoleSelectors[j]
                    ),
                ...
                (bool allowed, uint32 delay) = accessManager.canCall(
                    inputs.hubAdmin, report.hubInstanceBatchReports[i].report.hubProxy, _hubFeeMinterRoleSelectors[j]
                );
    ...
    function _checkTreasurySpokeRoles(address treasurySpoke, InputUtils.FullDeployInputs memory inputs) internal view {
    ...
    function _checkHubSelectorRoles(
        ...
        if (inputs.hubLabels.length > 0 && inputs.grantRoles) {
            assertEq(
                accessManager.getRoleMemberCount(Roles.HUB_CONFIGURATOR_ROLE), 2, "HubConfiguratorRole member count"
            );
            assertEq(
            ...
            assertEq(
                accessManager.getRoleMemberCount(Roles.HUB_CONFIGURATOR_ROLE), 0, "HubConfiguratorRole member count"
            );
        }
        for (uint256 i = 0; i < inputs.hubLabels.length; i++) {
            for (uint256 j = 0; j < _hubConfiguratorRoleSelectors.length; j++) {
                assertEq(
                    accessManager.getTargetFunctionRole(
                        report.hubInstanceBatchReports[i].report.hubProxy, _hubConfiguratorRoleSelectors[j]
                    ),
                ...
                assertEq(allowed, inputs.grantRoles ? true : false, "HubConfiguratorRole allowed - configurator");
                assertEq(delay, 0, "HubConfiguratorRole delay - configurator");

                (allowed, delay) = accessManager.canCall(
                    inputs.hubAdmin, report.hubInstanceBatchReports[i].report.hubProxy, _hubConfiguratorRoleSelectors[j]
                );
    ...
    function _checkHubConfiguratorBatchRoles(
        ...
        if (inputs.grantRoles && inputs.hubLabels.length > 0) {
            for (uint256 i; i < selectors.length; i++) {
                (bool allowed,) = accessManager.canCall(inputs.hubConfiguratorAdmin, hubConfigurator, selectors[i]);
    ...
    function _checkSpokeConfiguratorBatchRoles(
        ...
        if (inputs.grantRoles && inputs.spokeLabels.length > 0) {
            for (uint256 i; i < selectors.length; i++) {
                (bool allowed,) = accessManager.canCall(inputs.spokeConfiguratorAdmin, spokeConfigurator, selectors[i]);
    ...
    function _checkGatewayRoles(
        ...
        if (inputs.deployNativeTokenGateway) {
            assertEq(
                Ownable(report.gatewaysBatchReport.nativeGateway).owner(), inputs.gatewayOwner, "NativeGateway owner"
            );
        }
        if (inputs.deploySignatureGateway) {
            assertEq(
    ...
    function _checkAddressesHaveCode(OrchestrationReports.FullDeploymentReport memory report) internal view {
        ...
        for (uint256 i; i < report.hubInstanceBatchReports.length; i++) {
            ...
            _assertHasCode(report.hubInstanceBatchReports[i].report.hubProxy, string.concat("hub proxy: ", label));
            _assertHasCode(
                report.hubInstanceBatchReports[i].report.hubImplementation, string.concat("hub impl: ", label)
            );
            _assertHasCode(report.hubInstanceBatchReports[i].report.irStrategy, string.concat("irStrategy: ", label));
        }

        for (uint256 i; i < report.spokeInstanceBatchReports.length; i++) {
            string memory label = report.spokeInstanceBatchReports[i].label;
            _assertHasCode(report.spokeInstanceBatchReports[i].report.spokeProxy, string.concat("spoke proxy: ", label));
            _assertHasCode(
                report.spokeInstanceBatchReports[i].report.spokeImplementation, string.concat("spoke impl: ", label)
            );
            _assertHasCode(report.spokeInstanceBatchReports[i].report.aaveOracle, string.concat("oracle: ", label));
        }
        ...
        if (report.positionManagerBatchReport.giverPositionManager != address(0)) {
            _assertHasCode(report.positionManagerBatchReport.giverPositionManager, "giverPositionManager");
        }
        if (report.positionManagerBatchReport.takerPositionManager != address(0)) {
            _assertHasCode(report.positionManagerBatchReport.takerPositionManager, "takerPositionManager");
        }
        if (report.positionManagerBatchReport.configPositionManager != address(0)) {
            _assertHasCode(report.positionManagerBatchReport.configPositionManager, "configPositionManager");
        }
    }
    ...
    function _assertBytecodeMatchExcludingImmutables(bytes memory actual, bytes memory artifact, string memory label)
        internal
        pure
    {
