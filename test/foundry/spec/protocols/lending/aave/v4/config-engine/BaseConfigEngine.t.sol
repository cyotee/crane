...
import {ISpokeConfigurator} from "@crane/contracts/protocols/lending/aave/v4/spoke/interfaces/ISpokeConfigurator.sol";
...
import {AaveV4Payload} from "@crane/contracts/protocols/lending/aave/v4/config-engine/AaveV4Payload.sol";
import {AaveV4ConfigEngine} from "@crane/contracts/protocols/lending/aave/v4/config-engine/AaveV4ConfigEngine.sol";
import {
    IAaveV4ConfigEngine
} from "@crane/contracts/protocols/lending/aave/v4/config-engine/interfaces/IAaveV4ConfigEngine.sol";
import {EngineFlags} from "@crane/contracts/protocols/lending/aave/v4/config-engine/libraries/EngineFlags.sol";
import {
    AccessManagerEngine
} from "@crane/contracts/protocols/lending/aave/v4/config-engine/libraries/AccessManagerEngine.sol";
import {HubEngine} from "@crane/contracts/protocols/lending/aave/v4/config-engine/libraries/HubEngine.sol";
import {SpokeEngine} from "@crane/contracts/protocols/lending/aave/v4/config-engine/libraries/SpokeEngine.sol";
import {
    PositionManagerEngine
} from "@crane/contracts/protocols/lending/aave/v4/config-engine/libraries/PositionManagerEngine.sol";
import {
    TokenizationSpokeDeployer
} from "@crane/contracts/protocols/lending/aave/v4/config-engine/libraries/TokenizationSpokeDeployer.sol";
...
} from "@crane/test/foundry/spec/protocols/lending/aave/v4/helpers/mocks/config-engine/AaveV4PayloadWrapper.sol";
...
abstract contract BaseConfigEngineTest is Test, Create2TestHelper {
    ...
    IAssetInterestRateStrategy.InterestRateData internal IR_DATA = IAssetInterestRateStrategy.InterestRateData({
        optimalUsageRatio: 80_00, baseDrawnRate: 1_00, rateGrowthBeforeOptimal: 4_00, rateGrowthAfterOptimal: 60_00
    });
    ...
    function setUp() public virtual {
        vm.stopPrank(); // ensure no pending prank from previous tests or setup
        // consume any latent one-shot so we can safely start pranks below (target has-code contract)
        address(0x0000000000000000000000000000000000000004).call("");
        _etchCreate2Factory();
        ...
        address(0x0000000000000000000000000000000000000004).call(""); // ensure no pending before startPrank
        vm.startPrank(ADMIN);
    ...
    function _setupRoles(TestTypes.TestEnvReport memory report) internal {
        ...
        AaveV4HubConfiguratorRolesProcedure.grantHubConfiguratorAllRoles(address(accessManager), address(engine));
        AaveV4SpokeConfiguratorRolesProcedure.grantSpokeConfiguratorAllRoles(address(accessManager), address(engine));
    ...
    function _seedAsset(IHub hub, AssetInterestRateStrategy strategy, address token, uint8 decimals)
        internal
        returns (uint256 assetId)
    {
        vm.stopPrank();
        address(0x0000000000000000000000000000000000000004).call(""); // consume any pending
        vm.startPrank(ADMIN);
    ...
    function _seedReserve(ISpoke spoke, IHub hub, uint256 assetId, address priceSource)
        internal
        returns (uint256 reserveId)
    {
        ...
        ISpoke.DynamicReserveConfig memory dynConfig =
            ISpoke.DynamicReserveConfig({collateralFactor: 80_00, maxLiquidationBonus: 105_00, liquidationFee: 10_00});
        vm.stopPrank();
        address(0x0000000000000000000000000000000000000004).call(""); // consume any pending
        vm.startPrank(ADMIN);
    ...
    function _seedFullEnvironment() internal {
        for (uint256 h; h < NUM_HUBS; ++h) {
            for (uint256 t; t < NUM_TOKENS; ++t) {
                assetIds[h][t] = _seedAsset(hubs[h], irStrategies[h], tokenList[t].token, tokenList[t].decimals);
            }
        }
        ...
        for (uint256 s; s < NUM_SPOKES; ++s) {
            ...
            spokes[s].updateLiquidationConfig(
                ISpoke.LiquidationConfig({
                    targetHealthFactor: 1.05e18, healthFactorForMaxBonus: 0.95e18, liquidationBonusFactor: 100_00
                })
            );
            vm.stopPrank();
        }
    ...
    function _defaultAssetListing() internal view returns (IAaveV4ConfigEngine.AssetListing memory) {
        return IAaveV4ConfigEngine.AssetListing({
    ...
    function _defaultAssetConfigUpdate() internal view returns (IAaveV4ConfigEngine.AssetConfigUpdate memory) {
        return IAaveV4ConfigEngine.AssetConfigUpdate({
            hubConfigurator: hubConfigurator,
            hub: address(hub1()),
            underlying: address(weth),
            liquidityFee: LIQUIDITY_FEE,
    ...
    function _defaultSpokeConfigUpdate() internal view returns (IAaveV4ConfigEngine.SpokeConfigUpdate memory) {
        return IAaveV4ConfigEngine.SpokeConfigUpdate({
            hubConfigurator: hubConfigurator,
            hub: address(hub1()),
            underlying: address(weth),
    ...
    function _defaultReserveConfigUpdate() internal view returns (IAaveV4ConfigEngine.ReserveConfigUpdate memory) {
        return IAaveV4ConfigEngine.ReserveConfigUpdate({
    ...
    {
        return IAaveV4ConfigEngine.LiquidationConfigUpdate({
            spokeConfigurator: spokeConfigurator,
            spoke: address(spoke1()),
    ...
    {
        return IAaveV4ConfigEngine.DynamicReserveConfigUpdate({
            spokeConfigurator: spokeConfigurator,
            spoke: address(spoke1()),
            hub: address(hub1()),
            underlying: address(weth),
    ...
    function _defaultReserveConfig() internal pure returns (ISpoke.ReserveConfig memory) {
        return ISpoke.ReserveConfig({
            collateralRisk: 50_00, paused: false, frozen: false, borrowable: true, receiveSharesEnabled: true
        });
    }

    function _defaultDynamicReserveConfig() internal pure returns (ISpoke.DynamicReserveConfig memory) {
        return
            ISpoke.DynamicReserveConfig({collateralFactor: 80_00, maxLiquidationBonus: 105_00, liquidationFee: 10_00});
    }

    function _defaultReserveListing() internal view returns (IAaveV4ConfigEngine.ReserveListing memory) {
        return IAaveV4ConfigEngine.ReserveListing({
            spokeConfigurator: spokeConfigurator,
            spoke: address(spoke1()),
            hub: address(hub1()),
            underlying: address(weth),
    ...
    {
        return IAaveV4ConfigEngine.DynamicReserveConfigAddition({
            spokeConfigurator: spokeConfigurator,
            spoke: address(spoke1()),
            hub: address(hub1()),
            underlying: address(weth),
    ...
    function _defaultPositionManagerUpdate() internal view returns (IAaveV4ConfigEngine.PositionManagerUpdate memory) {
        return IAaveV4ConfigEngine.PositionManagerUpdate({
            spokeConfigurator: spokeConfigurator,
            spoke: address(spoke1()),
    ...
    function _assertSpokeConfig(IHub hub, uint256 assetId, address spoke, IHub.SpokeConfig memory expected)
        internal
        view
    {
    ...
    function _toAssetConfigUpdateArray(IAaveV4ConfigEngine.AssetConfigUpdate memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.AssetConfigUpdate[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.AssetConfigUpdate[](1);
        arr[0] = item;
    }

    function _toSpokeConfigUpdateArray(IAaveV4ConfigEngine.SpokeConfigUpdate memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.SpokeConfigUpdate[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.SpokeConfigUpdate[](1);
        arr[0] = item;
    }

    function _toSpokeToAssetsAdditionArray(IAaveV4ConfigEngine.SpokeToAssetsAddition memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.SpokeToAssetsAddition[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.SpokeToAssetsAddition[](1);
        arr[0] = item;
    }

    function _toAssetHaltArray(IAaveV4ConfigEngine.AssetHalt memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.AssetHalt[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.AssetHalt[](1);
        arr[0] = item;
    }

    function _toAssetDeactivationArray(IAaveV4ConfigEngine.AssetDeactivation memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.AssetDeactivation[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.AssetDeactivation[](1);
        arr[0] = item;
    }

    function _toAssetCapsResetArray(IAaveV4ConfigEngine.AssetCapsReset memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.AssetCapsReset[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.AssetCapsReset[](1);
        arr[0] = item;
    }

    function _toSpokeDeactivationArray(IAaveV4ConfigEngine.SpokeDeactivation memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.SpokeDeactivation[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.SpokeDeactivation[](1);
        arr[0] = item;
    }

    function _toSpokeCapsResetArray(IAaveV4ConfigEngine.SpokeCapsReset memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.SpokeCapsReset[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.SpokeCapsReset[](1);
        arr[0] = item;
    }

    function _toAssetListingArray(IAaveV4ConfigEngine.AssetListing memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.AssetListing[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.AssetListing[](1);
        arr[0] = item;
    }

    function _toReserveConfigUpdateArray(IAaveV4ConfigEngine.ReserveConfigUpdate memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.ReserveConfigUpdate[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.ReserveConfigUpdate[](1);
        arr[0] = item;
    }

    function _toLiquidationConfigUpdateArray(IAaveV4ConfigEngine.LiquidationConfigUpdate memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.LiquidationConfigUpdate[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.LiquidationConfigUpdate[](1);
        arr[0] = item;
    }

    function _toDynamicReserveConfigUpdateArray(IAaveV4ConfigEngine.DynamicReserveConfigUpdate memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.DynamicReserveConfigUpdate[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.DynamicReserveConfigUpdate[](1);
        arr[0] = item;
    }

    function _toRoleMembershipArray(IAaveV4ConfigEngine.RoleMembership memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.RoleMembership[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.RoleMembership[](1);
        arr[0] = item;
    }

    function _toRoleUpdateArray(IAaveV4ConfigEngine.RoleUpdate memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.RoleUpdate[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.RoleUpdate[](1);
        arr[0] = item;
    }

    function _toTargetFunctionRoleUpdateArray(IAaveV4ConfigEngine.TargetFunctionRoleUpdate memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.TargetFunctionRoleUpdate[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.TargetFunctionRoleUpdate[](1);
        arr[0] = item;
    }

    function _toTargetAdminDelayUpdateArray(IAaveV4ConfigEngine.TargetAdminDelayUpdate memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.TargetAdminDelayUpdate[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.TargetAdminDelayUpdate[](1);
        arr[0] = item;
    }

    function _toReserveListingArray(IAaveV4ConfigEngine.ReserveListing memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.ReserveListing[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.ReserveListing[](1);
        arr[0] = item;
    }

    function _toDynamicReserveConfigAdditionArray(IAaveV4ConfigEngine.DynamicReserveConfigAddition memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.DynamicReserveConfigAddition[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.DynamicReserveConfigAddition[](1);
        arr[0] = item;
    }

    function _toPositionManagerUpdateArray(IAaveV4ConfigEngine.PositionManagerUpdate memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.PositionManagerUpdate[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.PositionManagerUpdate[](1);
        arr[0] = item;
    }

    function _toSpokeRegistrationArray(IAaveV4ConfigEngine.SpokeRegistration memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.SpokeRegistration[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.SpokeRegistration[](1);
        arr[0] = item;
    }

    function _toPositionManagerRoleRenouncementArray(IAaveV4ConfigEngine.PositionManagerRoleRenouncement memory item)
        internal
        pure
        returns (IAaveV4ConfigEngine.PositionManagerRoleRenouncement[] memory arr)
    {
        arr = new IAaveV4ConfigEngine.PositionManagerRoleRenouncement[](1);
        arr[0] = item;
    }

    function _keepCurrentIrData() internal pure returns (IAssetInterestRateStrategy.InterestRateData memory) {
