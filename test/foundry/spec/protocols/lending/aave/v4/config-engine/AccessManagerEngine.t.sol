...
contract AccessManagerEngineTest is BaseConfigEngineTest {
    ...
    function _assertRoleConfig(uint64 roleId, uint64 expectedAdmin, uint64 expectedGuardian) internal view {
    ...
    function test_fuzz_executeRoleMemberships_grant(uint64 roleId, address account, uint32 executionDelay) public {
    ...
    function test_executeRoleMemberships_revoke() public {
        ...
        vm.expectCall(address(accessManager), abi.encodeCall(IAccessManager.revokeRole, (TEST_ROLE_ID, ACCOUNT)));
    ...
    function test_executeRoleMemberships_grant_revert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IAccessManager.AccessManagerUnauthorizedAccount.selector, address(engine), DEFAULT_ADMIN_ROLE
            )
        );
        engine.executeRoleMemberships(
            _toRoleMembershipArray(
    ...
    function test_executeRoleUpdates_adminOnly() public {
        ...
        vm.expectCall(
            address(accessManager), abi.encodeCall(IAccessManager.setRoleAdmin, (TEST_ROLE_ID, TEST_ADMIN_ROLE_ID))
        );
        engine.executeRoleUpdates(
            _toRoleUpdateArray(
                IAaveV4ConfigEngine.RoleUpdate({
    ...
    function test_executeRoleUpdates_grantDelayOnly() public {
        ...
        vm.expectCall(
            address(accessManager), abi.encodeCall(IAccessManager.setGrantDelay, (TEST_ROLE_ID, TEST_GRANT_DELAY))
        );
        engine.executeRoleUpdates(
            _toRoleUpdateArray(
                IAaveV4ConfigEngine.RoleUpdate({
    ...
    function test_fuzz_executeRoleUpdates_allFields(uint64 roleId, uint64 admin, uint64 guardian, uint32 grantDelay)
        public
    {
        ...
        engine.executeTargetFunctionRoleUpdates(
            _toTargetFunctionRoleUpdateArray(
                IAaveV4ConfigEngine.TargetFunctionRoleUpdate({
                    authority: address(accessManager), target: TARGET, selectors: selectors, roleId: TEST_ROLE_ID
                })
            )
        );
    ...
    function test_fuzz_executeTargetFunctionRoleUpdates(address target, bytes4 selector1, uint64 roleId) public {
        ...
        engine.executeTargetFunctionRoleUpdates(
            _toTargetFunctionRoleUpdateArray(
                IAaveV4ConfigEngine.TargetFunctionRoleUpdate({
                    authority: address(accessManager), target: target, selectors: selectors, roleId: roleId
                })
            )
        );
    ...
    function test_executeTargetAdminDelayUpdates() public {
        vm.expectCall(
            address(accessManager), abi.encodeCall(IAccessManager.setTargetAdminDelay, (TARGET, TEST_ADMIN_DELAY))
        );
        ...
        engine.executeTargetAdminDelayUpdates(
            _toTargetAdminDelayUpdateArray(
                IAaveV4ConfigEngine.TargetAdminDelayUpdate({
                    authority: address(accessManager), target: TARGET, newDelay: TEST_ADMIN_DELAY
                })
            )
        );
    ...
    function test_fuzz_executeTargetAdminDelayUpdates(address target, uint32 newDelay) public {
        engine.executeTargetAdminDelayUpdates(
            _toTargetAdminDelayUpdateArray(
                IAaveV4ConfigEngine.TargetAdminDelayUpdate({
                    authority: address(accessManager), target: target, newDelay: newDelay
                })
            )
        );
    ...
    function test_executeRoleMemberships_multipleMemberships() public {
        IAaveV4ConfigEngine.RoleMembership[] memory memberships = new IAaveV4ConfigEngine.RoleMembership[](2);
