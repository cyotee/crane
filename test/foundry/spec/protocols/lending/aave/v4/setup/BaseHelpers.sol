...
abstract contract BaseHelpers is BaseState {
    ...
    function _checkSupplyRateIncreasing(uint256 oldRate, uint256 newRate, string memory label) internal pure {
        assertGe(newRate, oldRate, string.concat("supply rate monotonically increasing ", label));
    }

    function _checkDebtRateConstant(uint256 oldRate, uint256 newRate, string memory label) internal pure {
    ...
    function _withdrawLiquidityFees(IHub hub, uint256 assetId, uint256 amount, address admin) internal {
        ...
        address(0).call(""); // clear any pending one-shot
    ...
    function _drawLiquidityViaTempSpoke(IHub hub, uint256 assetId, uint256 amount, bool withPremium, bool skipTime)
        internal
    {
        _drawLiquidityViaTempSpoke(hub, assetId, amount, withPremium, skipTime, HUB_ADMIN, MAX_ALLOWED_COLLATERAL_RISK);
    }
    ...
    function _mockSupplySharePrice(
        ...
        _mockSupplySharePrice(
            hub, assetId, totalAddedAssets, addedShares, spoke, HUB_ADMIN, MAX_ALLOWED_COLLATERAL_RISK
        );
    }
    ...
    function _updateAssetReinvestmentController(IHub hub, uint256 assetId, address newReinvestmentController) internal {
        _updateAssetReinvestmentController(hub, assetId, newReinvestmentController, HUB_ADMIN);
    }

    function _updateSpokeRiskPremiumThreshold(IHub hub, uint256 assetId, address spoke, uint24 newRiskPremiumThreshold)
        internal
    {
    ...
    function _openDebtPosition(ISpoke spoke, uint256 reserveId, uint256 amount, bool withPremium)
        internal
        returns (address)
    {
    ...
    function _borrowToBeLiquidatableWithPriceChange(
        ...
        return _borrowToBeLiquidatableWithPriceChange(
            spoke, user, reserveId, collateralReserveId, desiredHf, pricePercentage, SPOKE_ADMIN
        );
    }
    ...
    function _updateReserveBorrowableFlag(ISpoke spoke, uint256 reserveId, bool newBorrowable) internal {
        _updateReserveBorrowableFlag(spoke, reserveId, newBorrowable, SPOKE_ADMIN);
    }

    function _updateCollateralRisk(ISpoke spoke, uint256 reserveId, uint24 newCollateralRisk) internal {
    ...
    function _updateMaxLiquidationBonus(ISpoke spoke, uint256 reserveId, uint32 newMaxLiquidationBonus)
        internal
        returns (uint32)
    {
    ...
    function _updateLiquidationFee(ISpoke spoke, uint256 reserveId, uint16 newLiquidationFee)
        internal
        returns (uint32)
    {
    ...
    function _updateCollateralFactorAndLiquidationBonus(
        ...
        return _updateCollateralFactorAndLiquidationBonus(
            spoke, reserveId, newCollateralFactor, newLiquidationBonus, SPOKE_ADMIN
        );
    }

    function _updateCollateralFactor(ISpoke spoke, uint256 reserveId, uint256 newCollateralFactor)
        internal
        returns (uint32)
    {
    ...
    function _addDynamicReserveConfig(ISpoke spoke, uint256 reserveId, ISpoke.DynamicReserveConfig memory config)
        internal
        returns (uint32)
    {
