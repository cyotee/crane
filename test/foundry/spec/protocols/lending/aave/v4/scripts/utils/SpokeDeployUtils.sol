...
library SpokeDeployUtils {
    ...
    function deployLiquidationLogic(bytes32 salt) internal returns (address) {
        bytes memory bytecode =
            vm.getCode("contracts/protocols/lending/aave/v4/spoke/libraries/LiquidationLogic.sol:LiquidationLogic");
    ...
    function getLibraryString(address liquidationLogic) internal pure returns (string memory) {
        return string(
            abi.encodePacked(
                "src/spoke/libraries/LiquidationLogic.sol:LiquidationLogic:", vm.toString(liquidationLogic)
            )
        );
    }
    ...
    function _deployAndWriteLibrariesConfig(bytes32 salt) internal {
        ...
        string memory sedCommand = string(abi.encodePacked("echo FOUNDRY_LIBRARIES=", librariesSolcString, " >> .env"));
    ...
    function _librariesPathExists() internal returns (bool) {
        ...
        command[1] = "-c";
        command[2] =
            string(abi.encodePacked('response="$(', checkCommand, ')"; cast abi-encode "response(bool)" $response;'));
