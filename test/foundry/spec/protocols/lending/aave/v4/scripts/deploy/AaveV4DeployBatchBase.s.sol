...
abstract contract AaveV4DeployBatchBaseScript is Script {
    ...
    function run() external virtual {
        ...
        OrchestrationReports.FullDeploymentReport memory report = AaveV4DeployOrchestration.deployAaveV4(
            logger, deployer, inputs, BytecodeHelper.getHubBytecode(), BytecodeHelper.getSpokeBytecode()
        );
        vm.stopBroadcast();
    ...
    function _loadWarningsAndSanitizeInputs(InputUtils.FullDeployInputs memory inputs, address deployer)
        internal
        virtual
    ...
    function _executeUserPrompt() internal virtual {
        if (_promptLines.s.length > 0) {
            string memory ack = vm.prompt(string.concat(_joinLines(_promptLines), "\nenter 'y' to continue"));
