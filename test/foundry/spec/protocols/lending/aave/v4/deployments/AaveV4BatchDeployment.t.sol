...
contract AaveV4BatchDeploymentTest is BatchTestProcedures {
    ...
    function testAaveV4BatchDeployment_withZeroPositionManagerOwner_withPositionManagers_reverts() public {
    ...
    function testAaveV4BatchDeployment_accessManagerAdminTransfer() public {
        ...
        assertEq(
            accessManager.getRoleMember(Roles.ACCESS_MANAGER_ADMIN_ROLE, 0), newAdmin, "sole admin should be newAdmin"
        );
    }
    ...
    function testAaveV4BatchDeployment_fuzz_sanitizedInputs(InputUtils.FullDeployInputs memory deployInputs) public {
    ...
    function _getExpectedError() internal view returns (bool isExpectedError, bytes memory errorMessage) {
        ...
        if ((_inputs.hubLabels.length > 0 || _inputs.spokeLabels.length > 0) && _inputs.proxyAdminOwner == address(0)) {
        ...
        if ((_inputs.deployNativeTokenGateway || _inputs.deploySignatureGateway) && _inputs.gatewayOwner == address(0))
        {
            return (true, bytes("invalid owner"));
        }
        ...
        if (_inputs.grantRoles) {
            ...
            if (
                (hasHubs && (_inputs.hubAdmin == address(0) || _inputs.hubConfiguratorAdmin == address(0)))
                    || (hasSpokes && (_inputs.spokeAdmin == address(0) || _inputs.spokeConfiguratorAdmin == address(0)))
            ) {
    ...
    function _assumeDeploymentMatchesExpectation(bool isExpectedError, bytes memory errorMessage) internal {
    ...
    function _matchesRevertString(bytes memory actualReason, bytes memory expectedMessage)
        internal
        pure
        returns (bool)
    {
        return keccak256(actualReason) == keccak256(abi.encodeWithSignature("Error(string)", string(expectedMessage)));
    }
}
