...
contract AaveV4HubRolesProcedureTest is ProceduresBase {
    ...
    function test_grantHubAllRoles_reverts() public {
        ...
        aaveV4HubRolesProcedureWrapper.grantHubAllRoles({accessManager: accessManager, admin: address(0)});
    }

    function test_grantHubRole_reverts() public {
        vm.expectRevert("invalid access manager");
        aaveV4HubRolesProcedureWrapper.grantHubRole({
            accessManager: address(0), role: Roles.HUB_FEE_MINTER_ROLE, admin: admin
        });

        vm.expectRevert("invalid admin");
        aaveV4HubRolesProcedureWrapper.grantHubRole({
            accessManager: accessManager, role: Roles.HUB_FEE_MINTER_ROLE, admin: address(0)
        });
    }
    ...
    function test_setupHubFeeMinterRole_reverts() public {
        ...
        vm.expectRevert("invalid hub");
        aaveV4HubRolesProcedureWrapper.setupHubFeeMinterRole({accessManager: accessManager, hub: address(0)});
    }
    ...
    function test_setupHubConfiguratorRole_reverts() public {
        ...
        vm.expectRevert("invalid hub");
        aaveV4HubRolesProcedureWrapper.setupHubConfiguratorRole({accessManager: accessManager, hub: address(0)});
    }
    ...
    function test_grantHubAllRoles() public {
        ...
        (bool hasConfigurator,) = IAccessManager(accessManager).hasRole(Roles.HUB_CONFIGURATOR_ROLE, admin);
        ...
        (bool hasDeficitEliminator,) = IAccessManager(accessManager).hasRole(Roles.HUB_DEFICIT_ELIMINATOR_ROLE, admin);
    ...
    function test_canCall_hubFeeMinterRole() public {
        _grantAdminToWrapper(address(aaveV4HubRolesProcedureWrapper));
        aaveV4HubRolesProcedureWrapper.grantHubRole({
            accessManager: accessManager, role: Roles.HUB_FEE_MINTER_ROLE, admin: admin
        });
    ...
    function test_canCall_hubConfiguratorRole() public {
        _grantAdminToWrapper(address(aaveV4HubRolesProcedureWrapper));
        aaveV4HubRolesProcedureWrapper.grantHubRole({
            accessManager: accessManager, role: Roles.HUB_CONFIGURATOR_ROLE, admin: admin
        });
        aaveV4HubRolesProcedureWrapper.setupHubConfiguratorRole({accessManager: accessManager, hub: hub});
    ...
    function test_setupHubDeficitEliminatorRole_reverts() public {
        vm.expectRevert("invalid access manager");
        aaveV4HubRolesProcedureWrapper.setupHubDeficitEliminatorRole({accessManager: address(0), hub: hub});

        vm.expectRevert("invalid hub");
        aaveV4HubRolesProcedureWrapper.setupHubDeficitEliminatorRole({accessManager: accessManager, hub: address(0)});
    }

    function test_getHubDeficitEliminatorRoleSelectors() public view {
        bytes4[] memory selectors = aaveV4HubRolesProcedureWrapper.getHubDeficitEliminatorRoleSelectors();
        assertEq(selectors.length, 1);
    ...
    function test_canCall_hubDeficitEliminatorRole() public {
        _grantAdminToWrapper(address(aaveV4HubRolesProcedureWrapper));
        aaveV4HubRolesProcedureWrapper.grantHubRole({
            accessManager: accessManager, role: Roles.HUB_DEFICIT_ELIMINATOR_ROLE, admin: admin
        });
        aaveV4HubRolesProcedureWrapper.setupHubDeficitEliminatorRole({accessManager: accessManager, hub: hub});
