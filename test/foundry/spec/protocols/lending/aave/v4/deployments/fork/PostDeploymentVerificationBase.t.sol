...
import {BatchTestProcedures} from "@crane/test/foundry/spec/protocols/lending/aave/v4/utils/BatchTestProcedures.sol";
...
abstract contract PostDeploymentVerificationBase is BatchTestProcedures {
    ...
    function _parseReportFromJson(string memory json)
        internal
        view
        returns (OrchestrationReports.FullDeploymentReport memory report)
    {
        ...
        report.configuratorBatchReport.spokeConfigurator = vm.parseJsonAddress(json, "$.spokeConfigurator");
        ...
        if (vm.keyExistsJson(json, "$.giverPositionManager")) {
            report.positionManagerBatchReport.giverPositionManager = vm.parseJsonAddress(json, "$.giverPositionManager");
        }
        if (vm.keyExistsJson(json, "$.takerPositionManager")) {
            report.positionManagerBatchReport.takerPositionManager = vm.parseJsonAddress(json, "$.takerPositionManager");
        }
        ...
        for (uint256 i; i < hubCount; i++) {
            ...
            report.hubInstanceBatchReports[i].report.hubProxy =
                vm.parseJsonAddress(json, string.concat("$.hub.", label));
            report.hubInstanceBatchReports[i].report.hubImplementation =
                vm.parseJsonAddress(json, string.concat("$.hubImplementation.", label));
            report.hubInstanceBatchReports[i].report.irStrategy =
                vm.parseJsonAddress(json, string.concat("$.irStrategy.", label));
        }
        ...
        for (uint256 i; i < spokeCount; i++) {
            ...
            report.spokeInstanceBatchReports[i].report.spokeProxy =
                vm.parseJsonAddress(json, string.concat("$.spoke.", label));
            report.spokeInstanceBatchReports[i].report.spokeImplementation =
                vm.parseJsonAddress(json, string.concat("$.spokeImplementation.", label));
            report.spokeInstanceBatchReports[i].report.aaveOracle =
                vm.parseJsonAddress(json, string.concat("$.oracle.", label));
        }
    }

    /// @notice Deploys all contracts, serializes the JSON report in memory, parses it back, and verifies.
    function _deployWriteReportAndVerify(InputUtils.FullDeployInputs memory sanitizedInputs) internal {
        MetadataLogger logger = new MetadataLogger("");

        vm.startPrank(_deployer);
        OrchestrationReports.FullDeploymentReport memory report = AaveV4DeployOrchestration.deployAaveV4({
            logger: logger,
            deployer: _deployer,
        ...
        _verifyPostDeployment({report: _parseReportFromJson(logger.getJson()), inputs: sanitizedInputs});
    }
    ...
    function _deployWriteReportAndVerify(
        ...
        _inputs = sanitizedInputs;
        _verifyPostDeployment({report: _parseReportFromJson(vm.readFile(_reportFile)), inputs: sanitizedInputs});
    }
