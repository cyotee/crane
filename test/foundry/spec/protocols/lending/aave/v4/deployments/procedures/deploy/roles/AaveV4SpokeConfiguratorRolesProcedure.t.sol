...
contract AaveV4SpokeConfiguratorRolesProcedureTest is ProceduresBase {
    ...
    function test_grantSpokeConfiguratorRole_reverts() public {
        ...
        wrapper.grantSpokeConfiguratorRole({
            accessManager: address(0), role: Roles.SPOKE_CONFIGURATOR_DOMAIN_ADMIN_ROLE, admin: admin
        });

        vm.expectRevert("invalid admin");
        wrapper.grantSpokeConfiguratorRole({
            accessManager: accessManager, role: Roles.SPOKE_CONFIGURATOR_DOMAIN_ADMIN_ROLE, admin: address(0)
        });
    }

    function test_setupSpokeConfiguratorRoles_reverts() public {
        vm.expectRevert("invalid access manager");
        wrapper.setupSpokeConfiguratorRoles({accessManager: address(0), spokeConfigurator: spokeConfigurator});

        vm.expectRevert("invalid spoke configurator");
        wrapper.setupSpokeConfiguratorRoles({accessManager: accessManager, spokeConfigurator: address(0)});
    }
    ...
    function test_grantSpokeConfiguratorAllRoles() public {
        ...
        (bool hasRole,) = IAccessManager(accessManager).hasRole(Roles.SPOKE_CONFIGURATOR_DOMAIN_ADMIN_ROLE, admin);
        assertTrue(hasRole);
    }

    function test_setupSpokeConfiguratorRoles() public {
        _grantAdminToWrapper(address(wrapper));
        wrapper.setupSpokeConfiguratorRoles({accessManager: accessManager, spokeConfigurator: spokeConfigurator});
