...
} from "@crane/test/foundry/spec/protocols/lending/aave/v4/deployments/fork/PostDeploymentVerificationBase.t.sol";
import {
    AaveV4DeployAnvil
} from "@crane/test/foundry/spec/protocols/lending/aave/v4/scripts/deploy/examples/AaveV4DeployAnvil.s.sol";
...
contract PostDeploymentVerificationTest is PostDeploymentVerificationBase, AaveV4DeployAnvil {
    ...
    function test_jsonReportRoundTrip() public {
        InputUtils.FullDeployInputs memory sanitizedInputs = _loadWarningsAndSanitizeInputs(_defaultInputs(), _deployer);
    ...
    function testFuzz_postDeploymentCheck(FuzzParams memory params) public {
        ...
        InputUtils.FullDeployInputs memory inputs = InputUtils.FullDeployInputs({
            ...
            nativeWrapper: (params.deployNativeTokenGateway && params.useValidNativeWrapper) ? weth : address(0),
    ...
    function _shouldExpectRevert(InputUtils.FullDeployInputs memory inputs) internal pure returns (bool) {
    ...
    function _sanitizeAndDeploy(InputUtils.FullDeployInputs memory rawInputs) internal {
        InputUtils.FullDeployInputs memory sanitizedInputs = _loadWarningsAndSanitizeInputs(rawInputs, _deployer);
