...
import "@crane/test/foundry/spec/protocols/lending/aave/v4/deployments/procedures/ProceduresBase.t.sol";
...
contract AaveV4SpokeRolesProcedureTest is ProceduresBase {
    ...
    function test_grantSpokeAllRoles_reverts() public {
        ...
        aaveV4SpokeRolesProcedureWrapper.grantSpokeAllRoles({accessManager: accessManager, admin: address(0)});
    }

    function test_grantSpokeRole_reverts() public {
        vm.expectRevert("invalid access manager");
        aaveV4SpokeRolesProcedureWrapper.grantSpokeRole({
            accessManager: address(0), role: Roles.SPOKE_USER_POSITION_UPDATER_ROLE, admin: admin
        });

        vm.expectRevert("invalid admin");
        aaveV4SpokeRolesProcedureWrapper.grantSpokeRole({
            accessManager: accessManager, role: Roles.SPOKE_USER_POSITION_UPDATER_ROLE, admin: address(0)
        });
    }
    ...
    function test_setupSpokeRoles_reverts() public {
        ...
        aaveV4SpokeRolesProcedureWrapper.setupSpokeRoles({accessManager: accessManager, spoke: address(0)});
    }

    function test_setupSpokePositionUpdaterRole_reverts() public {
        vm.expectRevert("invalid access manager");
        aaveV4SpokeRolesProcedureWrapper.setupSpokePositionUpdaterRole({accessManager: address(0), spoke: spoke});

        vm.expectRevert("invalid spoke");
        aaveV4SpokeRolesProcedureWrapper.setupSpokePositionUpdaterRole({
            accessManager: accessManager, spoke: address(0)
        });
    }

    function test_setupSpokeConfiguratorRole_reverts() public {
        vm.expectRevert("invalid access manager");
        aaveV4SpokeRolesProcedureWrapper.setupSpokeConfiguratorRole({accessManager: address(0), spoke: spoke});

        vm.expectRevert("invalid spoke");
        aaveV4SpokeRolesProcedureWrapper.setupSpokeConfiguratorRole({accessManager: accessManager, spoke: address(0)});
    }

    function test_grantSpokeAllRoles() public {
        _grantAdminToWrapper(address(aaveV4SpokeRolesProcedureWrapper));
        aaveV4SpokeRolesProcedureWrapper.grantSpokeAllRoles({accessManager: accessManager, admin: admin});

        (bool hasPositionUpdater,) =
            IAccessManager(accessManager).hasRole(Roles.SPOKE_USER_POSITION_UPDATER_ROLE, admin);
        assertTrue(hasPositionUpdater);

        (bool hasConfigurator,) = IAccessManager(accessManager).hasRole(Roles.SPOKE_CONFIGURATOR_ROLE, admin);
    ...
    function test_setupSpokeRoles() public {
        ...
        assertEq(
            IAccessManager(accessManager).getTargetFunctionRole(spoke, ISpoke.updateUserDynamicConfig.selector),
    ...
    function test_getSpokePositionUpdaterRoleSelectors() public view {
        bytes4[] memory selectors = aaveV4SpokeRolesProcedureWrapper.getSpokePositionUpdaterRoleSelectors();
    ...
    function test_getSpokeConfiguratorRoleSelectors() public view {
        bytes4[] memory selectors = aaveV4SpokeRolesProcedureWrapper.getSpokeConfiguratorRoleSelectors();
    ...
    function test_canCall_spokePositionUpdaterRole() public {
        _grantAdminToWrapper(address(aaveV4SpokeRolesProcedureWrapper));
        aaveV4SpokeRolesProcedureWrapper.grantSpokeRole({
            accessManager: accessManager, role: Roles.SPOKE_USER_POSITION_UPDATER_ROLE, admin: admin
        });
        aaveV4SpokeRolesProcedureWrapper.setupSpokePositionUpdaterRole({accessManager: accessManager, spoke: spoke});

        bytes4[] memory selectors = aaveV4SpokeRolesProcedureWrapper.getSpokePositionUpdaterRoleSelectors();
        for (uint256 i = 0; i < selectors.length; i++) {
            (bool allowed, uint32 delay) = IAccessManager(accessManager).canCall(admin, spoke, selectors[i]);
            assertTrue(allowed);
            assertEq(delay, 0);
        }
    ...
    function test_canCall_spokeConfiguratorRole() public {
        _grantAdminToWrapper(address(aaveV4SpokeRolesProcedureWrapper));
        aaveV4SpokeRolesProcedureWrapper.grantSpokeRole({
            accessManager: accessManager, role: Roles.SPOKE_CONFIGURATOR_ROLE, admin: admin
        });
        aaveV4SpokeRolesProcedureWrapper.setupSpokeConfiguratorRole({accessManager: accessManager, spoke: spoke});
    ...
    function test_canCall_spokeAllRoles() public {
        ...
        bytes4[] memory positionUpdaterSelectors =
            aaveV4SpokeRolesProcedureWrapper.getSpokePositionUpdaterRoleSelectors();
        for (uint256 i = 0; i < positionUpdaterSelectors.length; i++) {
            (bool allowed, uint32 delay) =
                IAccessManager(accessManager).canCall(admin, spoke, positionUpdaterSelectors[i]);
            assertTrue(allowed);
            assertEq(delay, 0);
        }

        bytes4[] memory configuratorSelectors = aaveV4SpokeRolesProcedureWrapper.getSpokeConfiguratorRoleSelectors();
        for (uint256 i = 0; i < configuratorSelectors.length; i++) {
            (bool allowed, uint32 delay) = IAccessManager(accessManager).canCall(admin, spoke, configuratorSelectors[i]);
            assertTrue(allowed);
            assertEq(delay, 0);
        }
    }
