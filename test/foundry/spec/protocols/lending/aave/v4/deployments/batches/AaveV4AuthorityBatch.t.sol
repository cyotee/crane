...
contract AaveV4AuthorityBatchTest is BatchBaseTest {
    ...
    function test_getReport() public view {
        ...
        (bool hasRole, uint32 executionDelay) =
            IAccessManagerEnumerable(report.accessManager).hasRole(Roles.ACCESS_MANAGER_ADMIN_ROLE, admin);
        assertTrue(hasRole);
    ...
    function test_adminRoleMemberTracking() public view {
        IAccessManagerEnumerable am = IAccessManagerEnumerable(aaveV4AuthorityBatch.getReport().accessManager);
    ...
    function test_differentSaltProducesDifferentAddress() public {
        AaveV4AuthorityBatch newBatch = new AaveV4AuthorityBatch({admin_: admin, salt_: keccak256("differentSalt")});
