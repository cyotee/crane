...
contract AaveV4AccessManagerRolesProcedureTest is ProceduresBase {
    ...
    function test_replaceDefaultAdminRole() public {
        ...
        (bool hasRole, uint32 executionDelay) =
            IAccessManagerEnumerable(accessManager).hasRole(Roles.ACCESS_MANAGER_ADMIN_ROLE, newAdmin);
        assertTrue(hasRole);
    ...
    function test_replaceDefaultAdminRole_reverts() public {
        ...
        aaveV4AccessManagerRolesProcedureWrapper.replaceDefaultAdminRole({
            accessManager: address(0), adminToAdd: newAdmin, adminToRemove: accessManagerAdmin
        });

        vm.expectRevert("invalid admin");
        aaveV4AccessManagerRolesProcedureWrapper.replaceDefaultAdminRole({
            accessManager: accessManager, adminToAdd: address(0), adminToRemove: newAdmin
        });

        vm.prank(accessManagerAdmin);
        IAccessManager(accessManager)
            .grantRole(Roles.ACCESS_MANAGER_ADMIN_ROLE, address(aaveV4AccessManagerRolesProcedureWrapper), 0);
        vm.expectRevert("invalid admin");
        aaveV4AccessManagerRolesProcedureWrapper.replaceDefaultAdminRole({
            accessManager: accessManager, adminToAdd: newAdmin, adminToRemove: address(0)
        });
    }
    ...
    function test_grantAccessManagerAdminRole() public {
        ...
        aaveV4AccessManagerRolesProcedureWrapper.grantAccessManagerAdminRole({
            accessManager: accessManager, admin: newAdmin
        });
    ...
    function test_grantAccessManagerAdminRole_reverts() public {
        vm.expectRevert("invalid access manager");
        aaveV4AccessManagerRolesProcedureWrapper.grantAccessManagerAdminRole({accessManager: address(0), admin: admin});

        vm.expectRevert("invalid admin");
        aaveV4AccessManagerRolesProcedureWrapper.grantAccessManagerAdminRole({
            accessManager: accessManager, admin: address(0)
        });
    }
    ...
    function test_labelAllRoles() public {
        ...
        assertTrue(accessManager.isRoleLabeled(Roles.HUB_CONFIGURATOR_ROLE), "HUB_CONFIGURATOR labeled");
        ...
        assertTrue(accessManager.isRoleLabeled(Roles.HUB_DEFICIT_ELIMINATOR_ROLE), "HUB_DEFICIT_ELIMINATOR labeled");
        assertEq(accessManager.getLabelOfRole(Roles.HUB_DEFICIT_ELIMINATOR_ROLE), "HUB_DEFICIT_ELIMINATOR_ROLE");
        assertEq(accessManager.getRoleOfLabel("HUB_DEFICIT_ELIMINATOR_ROLE"), Roles.HUB_DEFICIT_ELIMINATOR_ROLE);
        ...
        // Spoke roles
        assertTrue(accessManager.isRoleLabeled(Roles.SPOKE_DOMAIN_ADMIN_ROLE), "SPOKE_DOMAIN_ADMIN labeled");
        assertEq(accessManager.getLabelOfRole(Roles.SPOKE_DOMAIN_ADMIN_ROLE), "SPOKE_DOMAIN_ADMIN_ROLE");
        assertEq(accessManager.getRoleOfLabel("SPOKE_DOMAIN_ADMIN_ROLE"), Roles.SPOKE_DOMAIN_ADMIN_ROLE);

        assertTrue(accessManager.isRoleLabeled(Roles.SPOKE_CONFIGURATOR_ROLE), "SPOKE_CONFIGURATOR labeled");
        assertEq(accessManager.getLabelOfRole(Roles.SPOKE_CONFIGURATOR_ROLE), "SPOKE_CONFIGURATOR_ROLE");
        assertEq(accessManager.getRoleOfLabel("SPOKE_CONFIGURATOR_ROLE"), Roles.SPOKE_CONFIGURATOR_ROLE);

        assertTrue(
            accessManager.isRoleLabeled(Roles.SPOKE_USER_POSITION_UPDATER_ROLE), "SPOKE_USER_POSITION_UPDATER labeled"
        );
        assertEq(
            accessManager.getLabelOfRole(Roles.SPOKE_USER_POSITION_UPDATER_ROLE), "SPOKE_USER_POSITION_UPDATER_ROLE"
        );
        assertEq(
            accessManager.getRoleOfLabel("SPOKE_USER_POSITION_UPDATER_ROLE"), Roles.SPOKE_USER_POSITION_UPDATER_ROLE
        );
