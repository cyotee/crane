...
import "@crane/test/foundry/spec/protocols/lending/aave/v4/deployments/batches/BatchBase.t.sol";
...
contract AaveV4TokenizationSpokeBatchTest is BatchBaseTest {
    ...
    function setUp() public override {
        ...
        // Use a dedicated proxy admin owner distinct from 'admin' (the business caller with configurator role)
        // to avoid "admin cannot fallback to proxy target" when calling impl functions via the Transparent proxy.
        address hubProxyAdminOwner = makeAddr("hubProxyAdminOwner");
        AaveV4HubInstanceBatch hubInstanceBatch = new AaveV4HubInstanceBatch({
            proxyAdminOwner_: hubProxyAdminOwner, authority_: accessManager, hubBytecode_: hubBytecode, salt_: salt
        });
        ...
        assetId = IHub(hub)
            .addAsset({
                underlying: underlying, decimals: 18, feeReceiver: feeReceiver, irStrategy: irStrategy, irData: irData
            });
        vm.stopPrank();

        // Deploy the TokenizationSpoke batch
        tokenizationSpokeBatch = new AaveV4TokenizationSpokeBatch(hub, underlying, admin, shareName, shareSymbol, salt);
    ...
    function test_revert_zeroProxyAdminOwner() public {
        ...
        new AaveV4TokenizationSpokeBatch(
            hub, underlying, address(0), shareName, shareSymbol, keccak256("zeroAdminSalt")
        );
    }

    function test_revert_emptyShareName() public {
        vm.expectRevert("invalid share name");
        new AaveV4TokenizationSpokeBatch(hub, underlying, admin, "", shareSymbol, keccak256("emptyNameSalt"));
    }

    function test_revert_emptyShareSymbol() public {
        vm.expectRevert("invalid share symbol");
        new AaveV4TokenizationSpokeBatch(hub, underlying, admin, shareName, "", keccak256("emptySymbolSalt"));
    }

    function test_revert_invalidUnderlying() public {
        vm.expectRevert();
        new AaveV4TokenizationSpokeBatch(
            hub, makeAddr("nonExistentUnderlying"), admin, shareName, shareSymbol, keccak256("invalidAssetSalt")
        );
    }
    ...
    function test_differentSaltProducesDifferentAddress() public {
        AaveV4TokenizationSpokeBatch newBatch = new AaveV4TokenizationSpokeBatch(
            hub, underlying, admin, shareName, shareSymbol, keccak256("differentSalt")
        );
