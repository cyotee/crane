...
contract AaveV4HubConfiguratorRolesProcedureTest is ProceduresBase {
    ...
    function test_grantHubConfiguratorRole_reverts() public {
        ...
        wrapper.grantHubConfiguratorRole({
            accessManager: address(0), role: Roles.HUB_CONFIGURATOR_DOMAIN_ADMIN_ROLE, admin: admin
        });

        vm.expectRevert("invalid admin");
        wrapper.grantHubConfiguratorRole({
            accessManager: accessManager, role: Roles.HUB_CONFIGURATOR_DOMAIN_ADMIN_ROLE, admin: address(0)
        });
    }

    function test_setupHubConfiguratorAllRoles_reverts() public {
        vm.expectRevert("invalid access manager");
        wrapper.setupHubConfiguratorAllRoles({accessManager: address(0), hubConfigurator: hubConfigurator});

        vm.expectRevert("invalid hub configurator");
        wrapper.setupHubConfiguratorAllRoles({accessManager: accessManager, hubConfigurator: address(0)});
    }
    ...
    function test_grantHubConfiguratorAllRoles() public {
        ...
        (bool hasRole,) = IAccessManager(accessManager).hasRole(Roles.HUB_CONFIGURATOR_DOMAIN_ADMIN_ROLE, admin);
        assertTrue(hasRole);
    }

    function test_setupHubConfiguratorAllRoles() public {
        _grantAdminToWrapper(address(wrapper));
        wrapper.setupHubConfiguratorAllRoles({accessManager: accessManager, hubConfigurator: hubConfigurator});
