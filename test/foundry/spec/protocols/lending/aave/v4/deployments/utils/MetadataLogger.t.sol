...
import {MetadataLogger} from "@crane/contracts/protocols/lending/aave/v4/deployments/utils/MetadataLogger.sol";
import {
...
contract MetadataLoggerTest is Test {
    ...
    function _fullReport() internal returns (OrchestrationReports.FullDeploymentReport memory report) {
    ...
    function test_writeJsonReportMarket_fullReport() public {
        ...
        assertEq(vm.parseJsonAddress(json, "$.accessManager"), report.authorityBatchReport.accessManager);
        assertEq(vm.parseJsonAddress(json, "$.hubConfigurator"), report.configuratorBatchReport.hubConfigurator);
        assertEq(vm.parseJsonAddress(json, "$.spokeConfigurator"), report.configuratorBatchReport.spokeConfigurator);
        assertEq(vm.parseJsonAddress(json, "$.treasurySpoke"), report.treasurySpokeBatchReport.treasurySpoke);

        // Hubs
        assertEq(vm.parseJsonAddress(json, "$.hub.core"), report.hubInstanceBatchReports[0].report.hubProxy);
        assertEq(
            vm.parseJsonAddress(json, "$.hubImplementation.core"),
            report.hubInstanceBatchReports[0].report.hubImplementation
        );
        assertEq(vm.parseJsonAddress(json, "$.irStrategy.core"), report.hubInstanceBatchReports[0].report.irStrategy);
        assertEq(vm.parseJsonAddress(json, "$.hub.prime"), report.hubInstanceBatchReports[1].report.hubProxy);

        // Spokes
        assertEq(vm.parseJsonAddress(json, "$.spoke.mainnet"), report.spokeInstanceBatchReports[0].report.spokeProxy);
        assertEq(
            vm.parseJsonAddress(json, "$.spokeImplementation.mainnet"),
            report.spokeInstanceBatchReports[0].report.spokeImplementation
        );
        assertEq(vm.parseJsonAddress(json, "$.oracle.mainnet"), report.spokeInstanceBatchReports[0].report.aaveOracle);
        assertEq(vm.parseJsonAddress(json, "$.spoke.lrt"), report.spokeInstanceBatchReports[1].report.spokeProxy);

        // Gateways
        assertEq(vm.parseJsonAddress(json, "$.nativeTokenGateway"), report.gatewaysBatchReport.nativeGateway);
        assertEq(vm.parseJsonAddress(json, "$.signatureGateway"), report.gatewaysBatchReport.signatureGateway);

        // Position Managers
        assertEq(
            vm.parseJsonAddress(json, "$.giverPositionManager"), report.positionManagerBatchReport.giverPositionManager
        );
        assertEq(
            vm.parseJsonAddress(json, "$.takerPositionManager"), report.positionManagerBatchReport.takerPositionManager
        );
        assertEq(
