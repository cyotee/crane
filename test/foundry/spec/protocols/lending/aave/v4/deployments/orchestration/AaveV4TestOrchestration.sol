...
} from "@crane/contracts/protocols/lending/aave/v4/deployments/procedures/roles/AaveV4AccessManagerRolesProcedure.sol";
import {
...
import {
    AaveV4AuthorityBatch
} from "@crane/contracts/protocols/lending/aave/v4/deployments/batches/AaveV4AuthorityBatch.sol";
import {
...
} from "@crane/contracts/protocols/lending/aave/v4/deployments/batches/AaveV4SpokeInstanceBatch.sol";
import {
    TestTokensBatch
} from "@crane/test/foundry/spec/protocols/lending/aave/v4/deployments/batches/TestTokensBatch.sol";
import {
    AaveV4DeployBase
} from "@crane/contracts/protocols/lending/aave/v4/deployments/orchestration/AaveV4DeployBase.sol";
...
library AaveV4TestOrchestration {
    ...
    function deployTestTokens(TestTypes.TestTokenInput[] memory tokenInputs)
        external
        returns (TestTypes.TokenList memory)
    {
    ...
    function deployTestEnv(
        ...
        // Deploy Access Batch
        report.accessManager = AaveV4DeployBase.deployAuthorityBatch({admin: admin, salt: salt}).accessManager;

        // Deploy TreasurySpoke Batch (single instance for all hubs)
        report.treasurySpoke =
        AaveV4DeployBase.deployTreasurySpokeBatch({
            owner: treasuryAdmin, salt: keccak256(abi.encodePacked(salt, "treasurySpoke"))
        })
        .treasurySpoke;
        ...
        for (uint256 i; i < spokeCount; ++i) {
            BatchReports.SpokeInstanceBatchReport memory spokeReport = AaveV4DeployBase.deploySpokeInstanceBatch({
                proxyAdminOwner: admin,
                authority: report.accessManager,
    ...
    function deployTestTokenizationSpoke(
        ...
        BatchReports.TokenizationSpokeBatchReport memory report =
            AaveV4DeployBase.deployTokenizationSpokeBatch({
                hub: hub,
    ...
    function deployTestTreasurySpoke(address owner, bytes32 salt) external returns (address treasurySpoke) {
    ...
    function configureHubsSpokes(ConfigData.AddSpokeParams[] memory paramsList) external {
        for (uint256 i; i < paramsList.length; ++i) {
            IHub(paramsList[i].hub)
                .addSpoke({assetId: paramsList[i].assetId, spoke: paramsList[i].spoke, params: paramsList[i].config});
        }
    }
    ...
    function configureSpokes(
        ...
        for (uint256 i; i < liquidationParamsList.length; ++i) {
            ISpoke(liquidationParamsList[i].spoke).updateLiquidationConfig(liquidationParamsList[i].config);
        }
        TestTypes.SpokeReserveId[] memory spokeReserveIds = new TestTypes.SpokeReserveId[](reserveParamsList.length);
        for (uint256 i; i < reserveParamsList.length; ++i) {
            spokeReserveIds[i] = TestTypes.SpokeReserveId({
                spoke: reserveParamsList[i].spoke,
                reserveId: ISpoke(reserveParamsList[i].spoke)
                    .addReserve({
    ...
    function setRolesTestEnv(TestTypes.TestEnvReport memory report) public {
        ...
        for (uint256 i; i < report.spokeReports.length; ++i) {
            AaveV4SpokeRolesProcedure.setupSpokeAllRoles(report.accessManager, report.spokeReports[i].spoke);
        }

        // Set Configurator Roles
        AaveV4HubConfiguratorRolesProcedure.setupHubConfiguratorAllRoles(
            report.accessManager, report.configuratorReport.hubConfigurator
        );
        AaveV4SpokeConfiguratorRolesProcedure.setupSpokeConfiguratorAllRoles(
            report.accessManager, report.configuratorReport.spokeConfigurator
        );
    }

    function setupHubRolesTestEnv(TestTypes.TestHubReport memory report, address accessManager) public {
    ...
    function grantHubRolesTestEnv(TestTypes.TestEnvReport memory report, address admin, address hubAdmin) public {
        ...
        AaveV4HubRolesProcedure.grantHubRole(
            report.accessManager, Roles.HUB_CONFIGURATOR_ROLE, report.configuratorReport.hubConfigurator
        );
        ...
        AaveV4HubConfiguratorRolesProcedure.grantHubConfiguratorAllRoles(report.accessManager, hubAdmin);
    }

    function grantSpokeRolesTestEnv(TestTypes.TestEnvReport memory report, address admin, address spokeAdmin) public {
        // grant Spoke roles
        ...
        AaveV4SpokeRolesProcedure.grantSpokeRole(
            report.accessManager, Roles.SPOKE_CONFIGURATOR_ROLE, report.configuratorReport.spokeConfigurator
        );

        // grant SpokeConfigurator Admin roles (allows admin to call SpokeConfigurator functions)
        AaveV4SpokeConfiguratorRolesProcedure.grantSpokeConfiguratorAllRoles(report.accessManager, admin);
        AaveV4SpokeConfiguratorRolesProcedure.grantSpokeConfiguratorAllRoles(report.accessManager, spokeAdmin);
    }

    function configureHubsAssets(ConfigData.AddAssetParams[] memory paramsList) public returns (uint256[] memory) {
        uint256[] memory assetIds = new uint256[](paramsList.length);
        for (uint256 i; i < paramsList.length; ++i) {
            assetIds[i] = IHub(paramsList[i].hub)
                .addAsset({
    ...
    function configureHubsAssetsViaConfigurator(ConfigData.AddAssetParams[] memory paramsList, address hubConfigurator)
        public
        returns (uint256[] memory)
    {
        uint256[] memory assetIds = new uint256[](paramsList.length);
        for (uint256 i; i < paramsList.length; ++i) {
            assetIds[i] = IHubConfigurator(hubConfigurator)
    ...
    function _deployTestTokensBatch(TestTypes.TestTokenInput[] memory tokenInputs)
        internal
        returns (TestTypes.TestTokensReport memory)
    {
    ...
    function _deployTokensBatch(TestTypes.TestTokenInput[] memory tokenInputs)
        internal
        returns (TestTypes.TestTokensBatchReport memory)
    {
    ...
    function deploySpokeImplementation(address oracle, uint16 maxUserReservesLimit) internal returns (ISpokeInstance) {
        return deploySpokeImplementation(oracle, maxUserReservesLimit, "");
    }

    function deploySpokeImplementation(address oracle, uint16 maxUserReservesLimit, bytes32 salt)
        internal
        returns (ISpokeInstance)
    {
    ...
    function deployHubImplementation(bytes32 salt) internal returns (IHubInstance) {
        bytes memory initCode =
            vm.getCode("contracts/protocols/lending/aave/v4/hub/instances/HubInstance.sol:HubInstance");
    ...
    function proxify(address impl, address proxyAdminOwner, bytes memory initData) internal returns (address) {
