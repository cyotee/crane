...
// import "contracts/crane/factories/create2/callback/diamondPkg/behaviors/IDiamondFactoryPackage_Behavior.sol";
...
contract IDiamondFactoryPackage_Behavior_Test is CraneTest {
    // IDiamondFactoryPackage_Behavior private _behavior;
    ...
    function IDiamondFactoryPackage_control_facetInterfaces()
    public pure virtual
    ...
    function IDiamondFactoryPackage_control_facetCuts()
    public virtual
    ...
    function IDiamondFactoryPackage_control_diamondConfig() public returns(IDiamondFactoryPackage.DiamondConfig memory config) {
    ...
    function test_IDiamondFactoryPackage_Behavior_areValid_facetInterfaces_goodPackage() public {
        assert(
            areValid_IDiamondFactoryPackage_facetInterfaces(
                goodPackage(),
                IDiamondFactoryPackage_control_facetInterfaces(),
                goodPackage().facetInterfaces()
            )
        );
    }

    function test_IDiamondFactoryPackage_Behavior_areValid_facetInterfaces_badPackage() public {
        assertFalse(
            areValid_IDiamondFactoryPackage_facetInterfaces(
                badPackage(),
                IDiamondFactoryPackage_control_facetInterfaces(),
                badPackage().facetInterfaces()
            )
        );
    }

    // Test facetCuts
    function test_IDiamondFactoryPackage_Behavior_areValid_facetCuts_goodPackage() public {
        assert(
            areValid_IDiamondLoupe_facetCuts(
                goodPackage(),
                IDiamondFactoryPackage_control_facetCuts(),
                goodPackage().facetCuts()
            )
        );
    }

    function test_IDiamondFactoryPackage_Behavior_areValid_facetCuts_badPackage() public {
        assertFalse(
            areValid_IDiamondLoupe_facetCuts(
                badPackage(),
                IDiamondFactoryPackage_control_facetCuts(),
                badPackage().facetCuts()
            )
        );
    }

    // Test diamondConfig
    function test_IDiamondFactoryPackage_Behavior_areValid_diamondConfig_goodPackage() public {
        assert(
            areValid_IDiamondFactoryPackage_diamondConfig(
                goodPackage(),
                IDiamondFactoryPackage_control_diamondConfig(),
                goodPackage().diamondConfig()
            )
        );
    }

    function test_IDiamondFactoryPackage_Behavior_areValid_diamondConfig_badPackage() public {
        assertFalse(
            areValid_IDiamondFactoryPackage_diamondConfig(
                badPackage(),
                IDiamondFactoryPackage_control_diamondConfig(),
                badPackage().diamondConfig()
            )
        );
    }

    // Test calcSalt
    function test_IDiamondFactoryPackage_Behavior_areValid_calcSalt_goodPackage() public {
        bytes memory testArgs = abi.encode("test");
        bytes32 expectedSalt = keccak256(testArgs);

        assert(
            areValid_IDiamondFactoryPackage_calcSalt(
                vm.getLabel(address(goodPackage())),
                expectedSalt,
                goodPackage().calcSalt(testArgs)
            )
        );
    }

    function test_IDiamondFactoryPackage_Behavior_areValid_calcSalt_badPackage() public {
        bytes memory testArgs = abi.encode("test");
        bytes32 expectedSalt = keccak256(testArgs);

        assertFalse(
            areValid_IDiamondFactoryPackage_calcSalt(
                vm.getLabel(address(badPackage())),
                expectedSalt,
                badPackage().calcSalt(testArgs)
            )
        );
    }
    ...
    function test_facetInterfaces_ValidatesSingleInterface() public {
       ...
       expect_IDiamondFactoryPackage_facetInterfaces(subject, expectedInterfaces);
        assertTrue(
           hasValid_IDiamondFactoryPackage_facetInterfaces(subject),
            "Should validate single interface"
        );
    }

    // Test processArgs
    function test_IDiamondFactoryPackage_Behavior_areValid_processArgs_goodPackage() public {
        bytes memory testArgs = abi.encode("test");
        ...
        assert(
            areValid_IDiamondFactoryPackage_processArgs(
                goodPackage(),
                testArgs,
                goodPackage().processArgs(testArgs)
            )
        );
    }

    function test_IDiamondFactoryPackage_Behavior_areValid_processArgs_badPackage() public {
        bytes memory testArgs = abi.encode("test");
        ...
        assertFalse(
            areValid_IDiamondFactoryPackage_processArgs(
                badPackage(),
                testArgs,
                badPackage().processArgs(testArgs)
            )
        );
    }

    // Test expectation-based validation
    // functest_IDiamondFactoryPackage_Behavior_hasValid_diamondConfig_goodPackage

    function test_IDiamondFactoryPackage_Behavior_hasValid_diamondConfig_badPackage() public {
    ...
    function test_facetInterfaces_ValidatesMultipleInterfaces() public {
       ...
       expect_IDiamondFactoryPackage_facetInterfaces(subject, expectedInterfaces);
        assertTrue(
           hasValid_IDiamondFactoryPackage_facetInterfaces(subject),
            "Should validate multiple interfaces"
        );
    }
