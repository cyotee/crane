...
contract OpenGSNForwarder_Fork is TestBase_OpenGSNFork {
    ...
    function test_requestTypeHash_registered() public view {
        ...
        // Note: Upstream uses different field name (validUntil vs validUntilTime)
        bytes32 upstreamTypeHash = _computeUpstreamRequestTypeHash();
        assertTrue(upstreamForwarder.typeHashes(upstreamTypeHash), "Upstream type hash not registered");
    }
    ...
    function test_getNonce_incrementsAfterExecute() public {
        ...
        upstreamForwarder.execute(
            upstreamReq, upstreamDomainSeparator, _computeUpstreamRequestTypeHash(), "", upstreamSig
        );
    ...
    function test_verify_invalidSignature_reverts() public {
        ...
        vm.expectRevert("FWD: signature mismatch");
        upstreamForwarder.verify(
            upstreamReq, upstreamDomainSeparator, _computeUpstreamRequestTypeHash(), "", upstreamBadSig
        );
    }
