...
library UInt256SetRepo {
    ...
    function _addExclusive(UInt256Set storage set, uint256 value) internal returns (bool success) {
        if (!_contains(set, value)) {
            set.values.push(value);
            set.indexes[value] = set.values.length;
            return true;
    ...
    function _addExclusive(UInt256Set storage set, uint256[] memory values) internal returns (bool success) {
        for (uint256 iteration = 0; iteration < values.length; iteration++) {
