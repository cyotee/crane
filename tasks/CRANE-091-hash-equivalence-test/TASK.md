...
Add a small unit test that asserts `BetterEfficientHashLib._hash()` produces identical output to `keccak256(abi.encodePacked(...))` for representative values including negative numbers. This makes the "identical output" claim locally provable without relying solely on fork-based integration coverage.
