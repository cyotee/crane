# Gap Report for: test/foundry/spec/utils/cryptography/EIP712Repo.t.sol
...
**Follow-up alignment (post EIP712Repo LR-6):** Fixed `import EIP712Layout` and `using EIP712Repo for EIP712Layout;` -> `EIP712Repo.Storage` so the test harness compiles after EIP712Repo standardized on `struct Storage`. (LR-1/LR-7 prep.) Only edited test + this gap + GAP_REPORT. Targeted verif pending.
