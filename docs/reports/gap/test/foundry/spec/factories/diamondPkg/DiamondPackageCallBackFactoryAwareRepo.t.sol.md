# Gap Report for: test/foundry/spec/factories/diamondPkg/DiamondPackageCallBackFactoryAwareRepo.t.sol
...
**Detailed Gaps:**
- LR-7: Ensure full initialization (no address(0) facets in pkgs), exact value assertions (not just 'changed'), preview parity, use of Behavior libs, declaration tests for facets/packages.
...
**Post-gold alignment fix:** Removed obsolete `DiamondPackageCallBackFactoryAwareLayout` from import (repo standardized to `struct Storage` + duals + tags per LR-6/LR-1). Import now only `DiamondPackageCallBackFactoryAwareRepo`. This unblocks compile for LR-7 tests using the repo (same pattern as prior EIP712 consumers). Only edited this test + its gap + GAP_REPORT. Targeted list/build to verify. See main GAP.
