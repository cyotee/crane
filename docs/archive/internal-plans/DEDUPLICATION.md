...
**2026-06-17 update:** A dedicated plan now exists for the remaining large-scale Aave vendored dependency cleanup (the full `openzeppelin*` bundles under `protocols/lending/aave/v3.6/dependencies/` and the curated set in v4). See [Aave_Vendored_Dependencies_Dedup_Plan.md](Aave_Vendored_Dependencies_Dedup_Plan.md). It follows "expand `external/` first, preserve exact OZ semantics for Aave ports, many small steps" approach per current direction. This work is tracked separately from the historical dedup arc.

**2026-06-17 late update (this session):** v4: all contract + test imports migrated; 0 bundle refs remain; key suites (AccessManagerEngine 21p, AuthorityBatch 5p, Rate etc.) pass or compile cleanly. Moved to v3.6 per request: bulk import migration re-applied + verified (OZ only; solidity-utils and aave-upgradeability preserved); added events to bases; fixed one AccessControl error string expectation in test to match external v5 custom error. RateStrategy v3.6: 15/15 passed. ACLManager/Collector compile+execute using external (minor non-dep failures remain). See AAVE_DEPS_INVENTORY.md for details.
...
**See the dedicated plan:** [Aave_Vendored_Dependencies_Dedup_Plan.md](Aave_Vendored_Dependencies_Dedup_Plan.md) for the systematic "expand external first" approach to finishing the Aave vendored bundles.
