...
import {CallTargetRegistryQueryTarget} from "@crane/contracts/registries/target/CallTargetRegistryQueryTarget.sol";
import {FacetBase} from "@crane/contracts/factories/diamondPkg/FacetBase.sol";

// tag::CallTargetRegistryQueryFacet[]
/**
 * @title CallTargetRegistryQueryFacet - Reusable Diamond facet providing query access to the CallTargetRegistry (default and caller-specific targets per interface ID).
 * @author cyotee doge <not_cyotee@proton.me>
 * @dev Extends CallTargetRegistryQueryTarget for business logic (delegates to CallTargetRegistryRepo). Implements IFacet (via FacetBase) to declare
 *      supported interfaces and functions for use with Diamond loupes, DFPkgs, registries, and composition.
 * @custom:contractlistipfs
 */
contract CallTargetRegistryQueryFacet is CallTargetRegistryQueryTarget, FacetBase {
    /* -------------------------------------------------------------------------- */
    /*                                   IFacet                                   */
    /* -------------------------------------------------------------------------- */
    ...
    function facetName() public pure override returns (string memory name) {
        return type(CallTargetRegistryQueryFacet).name;
    }
    ...
    function facetInterfaces() public pure override returns (bytes4[] memory interfaces) {
        ...
        interfaces[0] = type(ICallTargetRegistryQuery).interfaceId;
    }
    ...
    function facetFuncs() public pure override returns (bytes4[] memory funcs) {
        funcs = new bytes4[](2);
        funcs[0] = ICallTargetRegistryQuery.defaultCallTargetForID.selector;
        funcs[1] = ICallTargetRegistryQuery.callTargetForIDForCaller.selector;
    }
...
// end::CallTargetRegistryQueryFacet[]
