...
abstract contract ChainlinkClient {
     ...
     */
    function _buildChainlinkRequest(bytes32 specId, address callbackAddr, bytes4 callbackFunctionSignature)
        internal
        pure
     ...
     * @return A Chainlink Request struct in memory
     */
    function _buildOperatorRequest(bytes32 specId, bytes4 callbackFunctionSignature)
        internal
        view
        returns (Chainlink.Request memory)
    {
     ...
     * @return requestId The request ID
     */
    function _sendChainlinkRequestTo(address oracleAddress, Chainlink.Request memory req, uint256 payment)
        internal
     ...
     * @return requestId The request ID
     */
    function _sendOperatorRequestTo(address oracleAddress, Chainlink.Request memory req, uint256 payment)
        internal
        returns (bytes32 requestId)
    {
        uint256 nonce = s_requestCount;
        ...
        returns (bytes32 requestId)
    {
        ...
        requestId = abi.encodePacked(this, nonce)._hash();
     ...
     */
    function _cancelChainlinkRequest(bytes32 requestId, uint256 payment, bytes4 callbackFunc, uint256 expiration)
        internal
    {
     ...
     */
    function _addChainlinkExternalRequest(address oracleAddress, bytes32 requestId)
        internal
    ...
    function _useChainlinkWithENS(address ensAddress, bytes32 node) internal {
        ...
        bytes32 linkSubnode = abi.encodePacked(s_ensNode, ENS_TOKEN_SUBNAME)._hash();
    ...
    function _updateChainlinkOracleWithENS() internal {
        ...
        bytes32 oracleSubnode = abi.encodePacked(s_ensNode, ENS_ORACLE_SUBNAME)._hash();
     ...
     */
    function _validateChainlinkCallback(bytes32 requestId)
        internal
