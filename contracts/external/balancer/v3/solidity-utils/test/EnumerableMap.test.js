...
describe('EnumerableMap', () => {
    function shouldBehaveLikeMap(mapType, keys, values) {
        const [keyA, keyB, keyC] = keys;
        const [valueA, valueB, valueC] = values;
        async function expectMembersMatch(map, keys, values) {
            (0, chai_1.expect)(keys.length).to.equal(values.length);
            await Promise.all(keys.map(async (key) => (0, chai_1.expect)(await map.contains(key)).to.equal(true)));
            (0, chai_1.expect)(await map.length()).to.equal(keys.length);
            (0, chai_1.expect)(await Promise.all(keys.map((key) => map.get(key)))).to.deep.equal(values);
            ...
            (0, chai_1.expect)(await Promise.all([...Array(keys.length).keys()].map(async (index) => {
                ...
                (0, chai_1.expect)(entryAt.key).to.equal(entryAtUnchecked.key);
                (0, chai_1.expect)(entryAt.value).to.equal(entryAtUnchecked.value);
                (0, chai_1.expect)(entryAt.value).to.equal(valueAtUnchecked);
                return [entryAt.key.toString(), entryAt.value];
            }))).to.have.same.deep.members((0, lodash_1.zip)(keys.map((k) => k), values));
        }
        let map;
        (0, sharedBeforeEach_1.sharedBeforeEach)('deploy Map', async () => {
            map = await (0, contract_1.deploy)(`Enumerable${mapType}MapMock`);
        });
        it('starts empty', async () => {
            (0, chai_1.expect)(await map.contains(keyA)).to.equal(false);
            await expectMembersMatch(map, [], []);
        });
        describe('set', () => {
            it('returns true when adding a key', async () => {
                (0, chai_1.expect)(await map.set.staticCall(keyA, valueA)).to.be.true;
            });
            it('adds a key', async () => {
            ...
            it('adds several keys', async () => {
                ...
                (0, chai_1.expect)(await map.contains(keyC)).to.equal(false);
            });
            it('returns false when adding keys already in the set', async () => {
                await map.set(keyA, valueA);
                (0, chai_1.expect)(await map.set.staticCall(keyA, valueA)).to.be.false;
            });
        ...
        describe('get', () => {
            it('returns the value for a key', async () => {
                await map.set(keyA, valueA);
                (0, chai_1.expect)(await map.get(keyA)).to.equal(valueA);
            });
            it('reverts with a custom message if the key is not in the map', async () => {
                await (0, chai_1.expect)(map.get(keyA)).to.be.revertedWithCustomError(map, 'KeyNotFound');
            });
        });
        describe('at', () => {
            it('returns the value for a key', async () => {
                await map.set(keyA, valueA);
                (0, chai_1.expect)((await map.at(0)).key).to.equal(keyA);
                (0, chai_1.expect)((await map.at(0)).value).to.equal(valueA);
            });
            it('reverts with a custom message if the index is invalid', async () => {
                await (0, chai_1.expect)(map.at(100)).to.be.revertedWithCustomError(map, 'IndexOutOfBounds');
            });
        });
        describe('indexOf', () => {
            it('returns the index of an added key', async () => {
                await map.set(keyA, valueA);
                await map.set(keyB, valueB);
                (0, chai_1.expect)(await map.indexOf(keyA)).to.equal(0);
                (0, chai_1.expect)(await map.indexOf(keyB)).to.equal(1);
            });
            it('adding and removing keys can change the index', async () => {
                ...
                (0, chai_1.expect)(await map.indexOf(keyB)).to.equal(0);
            });
            it('reverts if the key is not in the map', async () => {
                await (0, chai_1.expect)(map.indexOf(keyA)).to.be.revertedWithCustomError(map, 'KeyNotFound');
            });
        });
        describe('unchecked_indexOf', () => {
            it('returns the index of an added key, plus one', async () => {
                await map.set(keyA, valueA);
                await map.set(keyB, valueB);
                (0, chai_1.expect)(await map.unchecked_indexOf(keyA)).to.equal(0 + 1);
                (0, chai_1.expect)(await map.unchecked_indexOf(keyB)).to.equal(1 + 1);
            });
            it('adding and removing keys can change the index', async () => {
                await map.set(keyA, valueA);
                await map.set(keyB, valueB);
                await map.remove(keyA);
                // B is now the only element; its index must be 0
                (0, chai_1.expect)(await map.unchecked_indexOf(keyB)).to.equal(0 + 1);
            });
            it('returns a zero index if the key is not in the map', async () => {
                (0, chai_1.expect)(await map.unchecked_indexOf(keyA)).to.be.equal(0);
            });
        });
        describe('unchecked_setAt', () => {
            ...
            it('does not revert when setting indexes outside of the map', async () => {
                ...
                await (0, chai_1.expect)(await map.unchecked_setAt(length, valueC)).not.to.be.reverted;
            });
        });
        describe('unchecked_at', () => {
            it('returns an entry at an index', async () => {
                ...
                (0, chai_1.expect)(entry.key).to.equal(keyA);
                (0, chai_1.expect)(entry.value).to.equal(valueA);
            });
        ...
        describe('unchecked_valueAt', () => {
            it('returns a value at an index', async () => {
                ...
                (0, chai_1.expect)(value).to.equal(valueA);
            });
            it('does not revert when accessing indexes outside of the map', async () => {
                const length = await map.length();
        ...
        describe('remove', () => {
            it('returns true when removing keys in the set', async () => {
                await map.set(keyA, valueA);
                (0, chai_1.expect)(await map.remove.staticCall(keyA)).to.be.true;
            });
            ...
            it('returns false when removing keys not in the set', async () => {
                await map.set(keyA, valueA);
                await map.remove(keyA);
                (0, chai_1.expect)(await map.remove.staticCall(keyA)).to.be.false;
            });
            it('returns false when removing keys not in the set', async () => {
                (0, chai_1.expect)(await map.contains(keyA)).to.equal(false);
            });
            it('adds and removes multiple keys', async () => {
                ...
                (0, chai_1.expect)(await map.contains(keyB)).to.equal(false);
            });
        });
    }
