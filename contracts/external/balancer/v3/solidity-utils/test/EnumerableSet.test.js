...
describe('EnumerableSet', () => {
    function shouldBehaveLikeSet(setType, members) {
        const [addressA, addressB, addressC] = members;
        async function expectMembersMatch(set, members) {
            await Promise.all(members.map(async (member) => (0, chai_1.expect)(await set.contains(member)).to.equal(true)));
            (0, chai_1.expect)(await set.length()).to.equal(members.length.toString());
            await Promise.all([...Array(members.length).keys()].map(async (index) => {
                const addressAt = await set.at(index);
                (0, chai_1.expect)(addressAt).to.equal(members[index]);
            }));
        }
        let set;
        (0, sharedBeforeEach_1.sharedBeforeEach)('deploy Set', async () => {
            set = await (0, contract_1.deploy)(`Enumerable${setType}Mock`);
        });
        it('starts empty', async () => {
            (0, chai_1.expect)(await set.contains(addressA)).to.equal(false);
            await expectMembersMatch(set, []);
        });
        describe('add', () => {
            it('returns true when adding a new member', async () => {
                (0, chai_1.expect)(await set.add.staticCall(addressA)).to.be.true;
            });
            ...
            it('adds several addresses', async () => {
                ...
                (0, chai_1.expect)(await set.contains(addressC)).to.equal(false);
            });
            it('returns false when adding members already in the set', async () => {
                await set.add(addressA);
                (0, chai_1.expect)(await set.add.staticCall(addressA)).to.be.false;
                await expectMembersMatch(set, [addressA]);
            });
        });
        describe('at', () => {
            it('returns the value for an address', async () => {
                await set.add(addressA);
                (0, chai_1.expect)(await set.at(0)).to.equal(addressA);
            });
            it('reverts with a custom message if the index is invalid', async () => {
                await (0, chai_1.expect)(set.at(100)).to.be.revertedWithCustomError(set, 'IndexOutOfBounds');
            });
        });
        describe('indexOf', () => {
            it('returns the index of an added key', async () => {
                await set.add(addressA);
                await set.add(addressB);
                (0, chai_1.expect)(await set.indexOf(addressA)).to.equal(0);
                (0, chai_1.expect)(await set.indexOf(addressB)).to.equal(1);
            });
            it('adding and removing keys can change the index', async () => {
                ...
                (0, chai_1.expect)(await set.indexOf(addressB)).to.equal(0);
            });
            it('reverts if the key is not in the set', async () => {
                await (0, chai_1.expect)(set.indexOf(addressA)).to.be.revertedWithCustomError(set, 'ElementNotFound');
            });
        });
        describe('unchecked_indexOf', () => {
            it('returns the index of an added key', async () => {
                await set.add(addressA);
                await set.add(addressB);
                (0, chai_1.expect)(await set.unchecked_indexOf(addressA)).to.equal(0);
                (0, chai_1.expect)(await set.unchecked_indexOf(addressB)).to.equal(1);
            });
            it('adding and removing keys can change the index', async () => {
                await set.add(addressA);
                await set.add(addressB);
                await set.remove(addressA);
                // B is now the only element; its index must be 0
                (0, chai_1.expect)(await set.unchecked_indexOf(addressB)).to.equal(0);
            });
            it('returns 0 if the key is not in the set', async () => {
                (0, chai_1.expect)(await set.unchecked_indexOf(addressA)).to.be.equal(0);
            });
        });
        describe('remove', () => {
            it('returns true when removing members', async () => {
                await set.add(addressA);
                (0, chai_1.expect)(await set.remove.staticCall(addressA)).to.be.true;
            });
            ...
            it('returns false when removing members that used to be in the set', async () => {
                await set.add(addressA);
                await set.remove(addressA);
                (0, chai_1.expect)(await set.remove.staticCall(addressA)).to.be.false;
            });
            ...
            it('adds and removes multiple members', async () => {
                ...
                (0, chai_1.expect)(await set.contains(addressB)).to.equal(false);
            });
        });
    }
