...
class WeightedPoolBenchmark extends PoolBenchmark_behavior_1.Benchmark {
    constructor(dirname) {
        super(dirname, 'WeightedPool', {
            disableNestedPoolTests: true,
        });
        this.WEIGHTS = [(0, numbers_1.fp)(0.5), (0, numbers_1.fp)(0.5)];
    }
    async deployPool(tag, poolTokens, withRate) {
        const factory = (await (0, contract_1.deploy)('v3-pool-weighted/WeightedPoolFactory', {
        ...
        const tx = await factory.create('WeightedPool', 'Test', (0, tokenConfig_1.buildTokenConfig)(poolTokens, withRate), this.WEIGHTS, poolRoleAccounts, (0, numbers_1.fp)(0.1), constants_1.ZERO_ADDRESS, enableDonation, false, // keep support to unbalanced add/remove liquidity
        constants_1.ZERO_BYTES32);
        const receipt = await tx.wait();
        const event = expectEvent.inReceipt(receipt, 'PoolCreated');
        const pool = (await (0, contract_1.deployedAt)('v3-pool-weighted/WeightedPool', event.args.pool));
        return {
            pool: pool,
