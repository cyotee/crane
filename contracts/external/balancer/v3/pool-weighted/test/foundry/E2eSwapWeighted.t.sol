...
contract E2eSwapWeightedTest is E2eSwapTest, WeightedPoolContractsDeployer {
        ...
        returns (E2eTestState memory)
    {
        ...
        (state.swapLimits.minTokenA, state.swapLimits.maxTokenA) =
            _setSwapLimitsExactIn(newPoolBalances, state.swapLimits.minTokenA);
        (state.swapLimits.minTokenB, state.swapLimits.maxTokenB) =
            _setSwapLimitsExactOut(newPoolBalances, state.swapLimits.minTokenB);
    ...
    function _setSwapLimitsExactIn(uint256[] memory poolBalancesRaw, uint256 currentMinSwapAmountTokenA)
        private
        view
        returns (uint256 minSwapAmountTokenA, uint256 maxSwapAmountTokenA)
    {
        ...
        uint256 normalizedLiquidityTokenB =
            (poolBalancesRaw[tokenBIdx] * (10 ** decimalsTokenA)) / (10 ** decimalsTokenB);
    ...
    function _setSwapLimitsExactOut(uint256[] memory poolBalancesRaw, uint256 currentMinSwapAmountTokenB)
        private
        view
        returns (uint256 minSwapAmountTokenB, uint256 maxSwapAmountTokenB)
    {
        ...
        uint256 normalizedLiquidityTokenA =
            (poolBalancesRaw[tokenAIdx] * (10 ** decimalsTokenB)) / (10 ** decimalsTokenA);

        // 25% of tokenA or tokenB liquidity, the lowest value, to make sure the swap is executed.
        maxSwapAmountTokenB =
            (normalizedLiquidityTokenA > poolBalancesRaw[tokenBIdx]
                        ? poolBalancesRaw[tokenBIdx]
                        : normalizedLiquidityTokenA) / 4;
        // Makes sure minSwapAmount is smaller than maxSwapAmount.
    ...
    function testSwapSymmetry__Fuzz(uint256 tokenAAmountIn, uint256 weightTokenA, uint256 swapFeePercentage) public {
        ...
        // Since tokens can have different decimals and amountOut is in relation to tokenB, normalize tokenA liquidity.
        uint256 normalizedLiquidityTokenA =
            (newPoolBalances[tokenAIdx] * (10 ** decimalsTokenB)) / (10 ** decimalsTokenA);

        // Cap amount in to lowest normalized liquidity * 25%
        tokenAAmountIn =
            bound(tokenAAmountIn, 1e15, Math.min(normalizedLiquidityTokenA, newPoolBalances[tokenBIdx]) / 4);

        uint256 snapshotId = vm.snapshotState();

        vm.prank(alice);
        uint256 amountOut =
            router.swapSingleTokenExactIn(pool, tokenA, tokenB, tokenAAmountIn, 0, MAX_UINT256, false, bytes(""));

        vm.revertToState(snapshotId);

        vm.prank(alice);
        uint256 amountIn =
            router.swapSingleTokenExactOut(pool, tokenA, tokenB, amountOut, MAX_UINT128, MAX_UINT256, false, bytes(""));
    ...
    function _setPoolBalancesWithDifferentWeights(uint256 weightTokenA)
        private
    ...
    function _calculatePoolInvariant(address poolToCalculate, Rounding rounding)
        private
        view
