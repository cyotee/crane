...
describe('WeightedPool', function () {
    ...
    (0, sharedBeforeEach_1.sharedBeforeEach)('create and initialize pool', async () => {
        factory = await (0, contract_1.deploy)('WeightedPoolFactory', {
        ...
        const tokenConfig = (0, tokenConfig_1.buildTokenConfig)(poolTokens);
        const tx = await factory.create('WeightedPool', 'Test', tokenConfig, WEIGHTS, { pauseManager: constants_1.ZERO_ADDRESS, swapFeeManager: constants_1.ZERO_ADDRESS, poolCreator: constants_1.ZERO_ADDRESS }, SWAP_FEE, constants_1.ZERO_ADDRESS, false, // no donations
        false, // keep support to unbalanced add/remove liquidity
        constants_1.ZERO_BYTES32);
        const receipt = await tx.wait();
        const event = expectEvent.inReceipt(receipt, 'PoolCreated');
        pool = (await (0, contract_1.deployedAt)('WeightedPool', event.args.pool));
