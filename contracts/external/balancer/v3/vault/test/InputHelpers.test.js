...
describe('InputHelpers (token sorting)', function () {
    ...
    (0, sharedBeforeEach_1.sharedBeforeEach)(async function () {
        lib = await (0, contract_1.deploy)('InputHelpersMock');
    });
    it('ensures tokens are sorted', async () => {
        (0, chai_1.expect)(await lib.ensureSortedTokens(SORTED_TWO)).to.not.be.reverted;
        (0, chai_1.expect)(await lib.ensureSortedTokens(SORTED_THREE)).to.not.be.reverted;
        (0, chai_1.expect)(await lib.ensureSortedTokens(SORTED_FOUR)).to.not.be.reverted;
        await (0, chai_1.expect)(lib.ensureSortedTokens(UNSORTED_TWO)).to.be.revertedWithCustomError(lib, 'TokensNotSorted');
        await (0, chai_1.expect)(lib.ensureSortedTokens(UNSORTED_THREE)).to.be.revertedWithCustomError(lib, 'TokensNotSorted');
        await (0, chai_1.expect)(lib.ensureSortedTokens(UNSORTED_FOUR)).to.be.revertedWithCustomError(lib, 'TokensNotSorted');
    });
    it('sorts tokens', async () => {
        const sortedTwo = await lib.sortTokens(UNSORTED_TWO);
        (0, chai_1.expect)(sortedTwo).to.deep.equal(CORRECTED_TWO);
        const sortedThree = await lib.sortTokens(UNSORTED_THREE);
        (0, chai_1.expect)(sortedThree).to.deep.equal(CORRECTED_THREE);
        const sortedFour = await lib.sortTokens(UNSORTED_FOUR);
        (0, chai_1.expect)(sortedFour).to.deep.equal(CORRECTED_FOUR);
    });
    it('does not mutate already sorted tokens', async () => {
        const sortedTwo = await lib.sortTokens(SORTED_TWO);
        (0, chai_1.expect)(sortedTwo).to.deep.equal(SORTED_TWO);
        const sortedThree = await lib.sortTokens(SORTED_THREE);
        (0, chai_1.expect)(sortedThree).to.deep.equal(SORTED_THREE);
        const sortedFour = await lib.sortTokens(SORTED_FOUR);
        (0, chai_1.expect)(sortedFour).to.deep.equal(SORTED_FOUR);
    });
});
