...
contract Permit2Test is BaseVaultTest {
    ...
    function testPermitBatchAndCall() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) =
            getPermitSignature(IEIP712(pool), alice, address(router), bptAmountOut, 0, block.timestamp, aliceKey);
        permitSignatures[0] = abi.encodePacked(r, s, v);

        IAllowanceTransfer.PermitBatch memory permit2Batch = getPermit2Batch(
            address(router), [address(usdc), address(dai)].toMemoryArray(), uint160(DEFAULT_AMOUNT), type(uint48).max, 0
        );
        ...
        multicallData[0] = abi.encodeCall(
            IRouter.addLiquidityUnbalanced, (pool, amountsIn, DEFAULT_BPT_AMOUNT_ROUND_DOWN, false, bytes(""))
        );

        uint256[] memory minAmountsOut =
            [uint256(DEFAULT_AMOUNT_ROUND_DOWN), uint256(DEFAULT_AMOUNT_ROUND_DOWN)].toMemoryArray();
        multicallData[1] = abi.encodeCall(
            IRouter.removeLiquidityProportional, (pool, DEFAULT_BPT_AMOUNT_ROUND_DOWN, minAmountsOut, false, bytes(""))
        );
    ...
    function testPermitBatchAndCallBubbleUpRevert() public {
        ...
        bytes[] memory multicallData = new bytes[](1);
        multicallData[0] =
            abi.encodeCall(IRouter.addLiquidityUnbalanced, (pool, amountsIn, bptAmountOut, false, bytes("")));
    ...
    function testPermitBatchAndCallDos() public {
        ...
        bytes[] memory permitSignatures = new bytes[](1);
        (uint8 v, bytes32 r, bytes32 s) =
            getPermitSignature(IEIP712(pool), alice, address(router), DEFAULT_AMOUNT, 0, block.timestamp, aliceKey);
        permitSignatures[0] = abi.encodePacked(r, s, v);
    ...
    function testCustomRemoveBatchAndCall() public {
        ...
        multicallData[1] =
            abi.encodeCall(IRouter.removeLiquidityCustom, (pool, bptAmountOut, amountsOut, false, bytes("")));

        vault.manualEnableRecoveryMode(pool);

        vm.expectCall(address(router), multicallData[0]);
        vm.prank(alice);
        router.permitBatchAndCall{value: 1 ether}(permitBatch, permitSignatures, permit2Batch, bytes(""), multicallData);
    }
    ...
    function testRecoveryModeBatchAndCall() public {
        ...
        multicallData[1] =
            abi.encodeCall(IRouter.removeLiquidityRecovery, (pool, bptAmountOut, new uint256[](amountsIn.length)));
    ...
    function testPermitBatchAndCallMismatch() public {
        ...
        multicallData[0] =
            abi.encodeCall(IRouter.addLiquidityUnbalanced, (pool, new uint256[](2), bptAmountOut, false, bytes("")));
