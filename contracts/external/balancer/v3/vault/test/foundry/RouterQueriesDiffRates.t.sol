...
contract RouterQueriesDiffRatesTest is BaseVaultTest {
        ...
        returns (address newPool, bytes memory poolArgs)
    {
        ...
        PoolFactoryMock(poolFactory).registerTestPool(newPool, vault.buildTokenConfig(tokens.asIERC20(), rateProviders));
    ...
    function testQuerySwapSingleTokenExactInDiffRates__Fuzz(uint256 daiMockRate, uint256 usdcMockRate) public {
        ...
        uint256 queryAmountOut =
            router.querySwapSingleTokenExactIn(pool, dai, usdc, exactAmountIn, address(this), bytes(""));

        vm.revertToState(snapshotId);

        vm.prank(bob);
        uint256 actualAmountOut =
            router.swapSingleTokenExactIn(pool, dai, usdc, exactAmountIn, 0, MAX_UINT256, false, bytes(""));
    ...
    function testQuerySwapSingleTokenExactOutDiffRates__Fuzz(uint256 daiMockRate, uint256 usdcMockRate) public {
        ...
        uint256 queryAmountIn =
            router.querySwapSingleTokenExactOut(pool, dai, usdc, exactAmountOut, address(this), bytes(""));

        vm.revertToState(snapshotId);

        vm.prank(bob);
        uint256 actualAmountIn =
            router.swapSingleTokenExactOut(pool, dai, usdc, exactAmountOut, MAX_UINT256, MAX_UINT256, false, bytes(""));
    ...
    function testQueryAddLiquidityProportionalDiffRates__Fuzz(uint256 daiMockRate, uint256 usdcMockRate) public {
        ...
        uint256[] memory expectedAmountsIn =
            [exactBptAmountOut.divUp(2e18), exactBptAmountOut.divUp(2e18)].toMemoryArray();
        ...
        uint256[] memory actualAmountsIn =
            router.addLiquidityProportional(pool, expectedAmountsIn, exactBptAmountOut, false, bytes(""));
    ...
    function testQueryAddLiquidityUnbalancedDiffRates__Fuzz(uint256 daiMockRate, uint256 usdcMockRate) public {
        ...
        uint256[] memory exactAmountsInRaw =
            [biggerPoolInitAmount.mulUp(1e16), biggerPoolInitAmount.mulUp(0.005e18)].toMemoryArray();
        ...
        (uint256 expectedBptAmountOut,) = BasePoolMath.computeAddLiquidityUnbalanced(
            vault.getCurrentLiveBalances(pool), exactAmountsInScaled18, IERC20(pool).totalSupply(), 0, IBasePool(pool)
        );

        uint256 snapshotId = vm.snapshotState();
        _prankStaticCall();
        uint256 queryBptAmountOut =
            router.queryAddLiquidityUnbalanced(pool, exactAmountsInRaw, address(this), bytes(""));

        vm.revertToState(snapshotId);

        vm.prank(bob);
        uint256 actualBptAmountOut =
            router.addLiquidityUnbalanced(pool, exactAmountsInRaw, expectedBptAmountOut, false, bytes(""));
    ...
    function testQueryAddLiquiditySingleTokenExactOutDiffRates__Fuzz(uint256 daiMockRate, uint256 usdcMockRate) public {
        ...
        _prankStaticCall();
        uint256 queryAmountIn =
            router.queryAddLiquiditySingleTokenExactOut(pool, dai, exactBptAmountOut, address(this), bytes(""));

        vm.revertToState(snapshotId);

        vm.prank(bob);
        uint256 actualAmountIn =
            router.addLiquiditySingleTokenExactOut(pool, dai, MAX_UINT128, exactBptAmountOut, false, bytes(""));
    ...
    function testQueryAddLiquidityCustomDiffRates__Fuzz(uint256 daiMockRate, uint256 usdcMockRate) public {
        ...
        uint256[] memory maxAmountsIn =
            [expectedBptAmountOut.divUp(3e18), expectedBptAmountOut.divUp(5e18)].toMemoryArray();
        ...
        uint256 snapshotId = vm.snapshotState();
        _prankStaticCall();
        (uint256[] memory queryAmountsIn, uint256 queryBptOut,) =
            router.queryAddLiquidityCustom(pool, maxAmountsIn, expectedBptAmountOut, address(this), bytes(""));

        vm.revertToState(snapshotId);

        vm.prank(bob);
        (uint256[] memory actualAmountsIn, uint256 actualBptOut,) =
            router.addLiquidityCustom(pool, maxAmountsIn, expectedBptAmountOut, false, bytes(""));
    ...
    function testQueryRemoveLiquidityProportionalDiffRates__Fuzz(uint256 daiMockRate, uint256 usdcMockRate) public {
        ...
        uint256[] memory expectedAmountsOut =
            [exactBptAmountIn.divUp(2e18), exactBptAmountIn.divUp(2e18)].toMemoryArray();
        ...
        uint256[] memory actualAmountsOut =
            router.removeLiquidityProportional(pool, exactBptAmountIn, expectedAmountsOut, false, bytes(""));
    ...
    function testQueryRemoveLiquiditySingleTokenExactInDiffRates__Fuzz(uint256 daiMockRate, uint256 usdcMockRate)
        public
    {
        daiMockRate = bound(daiMockRate, 1e17, 1e19);
        usdcMockRate = bound(usdcMockRate, 1e17, 1e19);
        ...
        (uint256 expectedAmountOutScaled18,) = BasePoolMath.computeRemoveLiquiditySingleTokenExactIn(
            vault.getCurrentLiveBalances(pool), daiIdx, exactBptAmountIn, IERC20(pool).totalSupply(), 0, IBasePool(pool)
        );
        ...
        _prankStaticCall();
        uint256 queryAmountOut =
            router.queryRemoveLiquiditySingleTokenExactIn(pool, exactBptAmountIn, dai, address(this), bytes(""));

        vm.revertToState(snapshotId);

        vm.prank(lp);
        uint256 actualAmountOut = router.removeLiquiditySingleTokenExactIn(
            pool, exactBptAmountIn, dai, expectedAmountOutRaw, false, bytes("")
        );
    ...
    function testQueryRemoveLiquiditySingleTokenExactOutDiffRates__Fuzz(uint256 daiMockRate, uint256 usdcMockRate)
        public
    {
        daiMockRate = bound(daiMockRate, 1e17, 1e19);
        usdcMockRate = bound(usdcMockRate, 1e17, 1e19);
        ...
        uint256 snapshotId = vm.snapshotState();
        _prankStaticCall();
        uint256 queryBptAmountIn =
            router.queryRemoveLiquiditySingleTokenExactOut(pool, dai, exactAmountOut, address(this), bytes(""));

        vm.revertToState(snapshotId);

        vm.prank(lp);
        uint256 actualBptAmountIn =
            router.removeLiquiditySingleTokenExactOut(pool, expectedBptAmountIn, dai, exactAmountOut, false, bytes(""));
    ...
    function testQueryRemoveLiquidityCustomDiffRates__Fuzz(uint256 daiMockRate, uint256 usdcMockRate) public {
        ...
        uint256[] memory minAmountsOut =
            [expectedBptAmountIn.divUp(3e18), expectedBptAmountIn.divUp(5e18)].toMemoryArray();
        ...
        uint256 snapshotId = vm.snapshotState();
        _prankStaticCall();
        (uint256 queryBptIn, uint256[] memory queryAmountsOut,) =
            router.queryRemoveLiquidityCustom(pool, expectedBptAmountIn, minAmountsOut, address(this), bytes(""));

        vm.revertToState(snapshotId);

        vm.prank(lp);
        (uint256 actualBptIn, uint256[] memory actualAmountsOut,) =
            router.removeLiquidityCustom(pool, expectedBptAmountIn, minAmountsOut, false, bytes(""));
    ...
    function testQueryRemoveLiquidityRecoveryDiffRates__Fuzz(uint256 daiMockRate, uint256 usdcMockRate) public {
        ...
        vm.prank(lp);
        uint256[] memory actualAmountsOut =
            router.removeLiquidityRecovery(pool, exactBptAmountIn, new uint256[](expectedAmountsOut.length));
