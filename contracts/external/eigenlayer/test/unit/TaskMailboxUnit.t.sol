...
import {ProxyAdmin} from "@crane/contracts/external/openzeppelin-contracts-v5/proxy/transparent/ProxyAdmin.sol";
import {TransparentUpgradeableProxy} from "@crane/contracts/external/openzeppelin-contracts-v5/proxy/transparent/TransparentUpgradeableProxy.sol";
...
contract TaskMailboxUnitTests_Constructor is TaskMailboxUnitTests {
    function test_Constructor_WithCertificateVerifiers() public {
        ...
        TaskMailbox taskMailboxImpl = new TaskMailbox(bn254Verifier, ecdsaVerifier, MAX_TASK_SLA);
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            address(taskMailboxImpl),
            address(proxyAdmin),
            abi.encodeWithSelector(TaskMailbox.initialize.selector, owner, 0, feeSplitCollector)
    ...
    function test_Constructor_SetsMaxTaskSLA() public {
        ...
        TaskMailbox taskMailboxImpl = new TaskMailbox(bn254Verifier, ecdsaVerifier, customMaxTaskSLA);
        ProxyAdmin proxyAdmin = new ProxyAdmin();
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            address(taskMailboxImpl),
            address(proxyAdmin),
...
contract TaskMailboxUnitTests_submitResult is TaskMailboxUnitTests {
    ...
    function test_Revert_WhenCertificateVerificationFailed_ECDSA() public {
        ...
        ProxyAdmin proxyAdmin = new ProxyAdmin();
        TaskMailbox taskMailboxImpl =
            new TaskMailbox(address(mockBN254CertificateVerifier), address(mockECDSACertificateVerifierFailure), MAX_TASK_SLA);
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            address(taskMailboxImpl),
            address(proxyAdmin),
            abi.encodeWithSelector(TaskMailbox.initialize.selector, owner, 0, feeSplitCollector)
...
contract TaskMailboxUnitTests_Upgradeable is TaskMailboxUnitTests {
    ...
    function test_InitializerModifier_PreventsReinitialization() public {
        ...
        TransparentUpgradeableProxy uninitializedProxy = new TransparentUpgradeableProxy(
            address(new TaskMailbox(address(mockBN254CertificateVerifier), address(mockECDSACertificateVerifier), MAX_TASK_SLA)),
