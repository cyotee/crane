...
contract DisputeResolutionEngine {
    ...
    function commitVote(uint256 disputeId, uint256 lockedTokensAmount, bytes32 commitHash) external {
        ...
        require(lockedTokensAmount >= MIN_LOCK_AMOUNT_FOR_VOTING, "Insufficient locked tokens amount for voting");
        require(block.timestamp < dispute.creationTimestampSeconds + COMMIT_PERIOD_SECONDS, "Commit period ended");
    ...
    function revealVote(uint256 disputeId, bytes32 salt, bool votedForGuilty) external {
        ...
        require(
            block.timestamp < dispute.creationTimestampSeconds + COMMIT_PERIOD_SECONDS + REVEAL_PERIOD_SECONDS,
            "Reveal period ended"
        );
        ...
        require(expectedCommitHash == vote.commitHash, "Commited hash doesn't match with the revealed vote");
    ...
    function calculatePendingRewardForUser(uint256 disputeId, address userAddress) public view returns (uint256) {
        Dispute storage dispute = _disputes[disputeId];
        ...
        require(userVote.votedForGuilty || dispute.verdict == DisputeVerdict.NotGuilty, "User didn't win the dispute");
        require(!userVote.claimedReward, "User already claimed reward for this dispute");

        // Calculating reward amount for the user
        uint256 totalLockedTokenOfAllWinners =
            Math.max(dispute.revealedForGuiltyAmount, dispute.revealedForNotGuiltyAmount);
        uint256 userRewardAmount =
            (dispute.rewardPoolTokensAmount * userVote.lockedTokensAmount) / totalLockedTokenOfAllWinners;
    ...
    // return salt;
    function calculateHashForVote(uint256 disputeId, bytes32 salt, bool votedForGuilty) public pure returns (bytes32) {
    ...
    function _lockTokensAndCreateVote(uint256 disputeId, bytes32 commitHash, uint256 lockedTokensAmount) private {
        Dispute storage dispute = _disputes[disputeId];
        require(_votes[disputeId][msg.sender].lockedTokensAmount == 0, "Already locked some tokens for this dispute");
