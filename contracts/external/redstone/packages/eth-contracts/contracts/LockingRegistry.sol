...
contract LockingRegistry is Initializable {
    ...
    function initialize(IERC20 lockedTokenAddress, address authorisedSlasher, uint256 delayForUnlockingInSeconds)
        public
        initializer
    {
    ...
    function requestUnlock(uint256 amountToUnlock) external {
        ...
        require(userLockingDetails.lockedAmount >= amountToUnlock, "Can not request to unlock more than locked");

        userLockingDetails.pendingAmountToUnlock = amountToUnlock;
        userLockingDetails.unlockOpeningTimestampSeconds = block.timestamp + _delayForUnlockingInSeconds;
    ...
    function completeUnlock() external {
        UserLockingDetails storage userLockingDetails = _lockingDetailsForUsers[msg.sender];
        require(block.timestamp > userLockingDetails.unlockOpeningTimestampSeconds, "Unlocking is not opened yet");
