...
contract VestingWallet is Initializable {
    ...
    function getReleasable() public view virtual returns (uint256) {
        ...
        return token.balanceOf(address(this)) < unvestedAmount ? 0 : token.balanceOf(address(this)) - unvestedAmount;
    }
    ...
    function lock(uint256 amount) external onlyBeneficiary {
        require(token.balanceOf(address(this)) >= amount, "VestingWallet: there is not enough tokens to lock");
