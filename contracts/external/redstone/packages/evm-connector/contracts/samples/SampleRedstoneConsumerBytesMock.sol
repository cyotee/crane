...
contract SampleRedstoneConsumerBytesMock is RedstoneConsumerBytesMock {
        ...
        returns (bytes memory)
    {
        ...
        for (uint256 i = 0; i < calldataPointersForValues.length; i++) {
            bytes calldata bytesValue = getCalldataBytesFromCalldataPointer(calldataPointersForValues[i]);
    ...
    function calldataBytesToUint256(bytes calldata bytesValue) private pure returns (uint256 uintValue) {
        assembly {
