...
contract SampleProxyConnector is ProxyConnector {
    ...
    function getOracleValueUsingProxy(bytes32 dataFeedId) public view returns (uint256) {
        bytes memory encodedFunction =
            abi.encodeWithSelector(SampleRedstoneConsumerNumericMock.getValueForDataFeedId.selector, dataFeedId);

        bytes memory encodedResult = proxyCalldataView(address(sampleRedstoneConsumer), encodedFunction);
    ...
    function proxyEmptyError() public view {
        bytes memory encodedFunction =
            abi.encodeWithSelector(SampleRedstoneConsumerNumericMock.revertWithoutMessage.selector);
        proxyCalldataView(address(sampleRedstoneConsumer), encodedFunction);
    }

    function proxyTestStringError() public view {
        bytes memory encodedFunction =
            abi.encodeWithSelector(SampleRedstoneConsumerNumericMock.revertWithTestMessage.selector);
        proxyCalldataView(address(sampleRedstoneConsumer), encodedFunction);
    }
    ...
    function checkOracleValueLongEncodedFunction(bytes32 asset, uint256 price) external {
        ...
        bytes memory encodedResult =
            ProxyConnector.proxyCalldata(address(sampleRedstoneConsumer), encodedFunction, false);
    ...
    function requireValueForward() external payable {
        bytes memory encodedFunction = abi.encodeWithSelector(SampleRedstoneConsumerNumericMock.returnMsgValue.selector);
        bytes memory encodedResult =
            ProxyConnector.proxyCalldata(address(sampleRedstoneConsumer), encodedFunction, false);
        ...
        encodedResult = ProxyConnector.proxyCalldata(address(sampleRedstoneConsumer), encodedFunction, true);
