...
abstract contract MultiFeedAdapterWithoutRoundsWithReference is IMultiFeedAdapter {
     ...
     */
    function getReferenceSwitchCriteria(bytes32 dataFeedId)
        public
        view
        virtual
        returns (uint256 maxAllowedDeviationBps, uint256 maxRefBlockLag);
    ...
    function _safeGetLastUpdateDetails(IMultiFeedAdapter adapter, bytes32 dataFeedId)
        internal
        view
        returns (bool ok, uint256 lastDataTimestamp, uint256 lastBlockTimestamp, uint256 lastValue)
    {
        try adapter.getLastUpdateDetails{gas: getGasLimitForGetLastUpdateDetailsCall()}(dataFeedId) returns (
            uint256 dataTs, uint256 blockTs, uint256 value
        ) {
        ...
        returns (LastUpdateDetails[] memory detailsForFeeds)
    {
        ...
        for (uint256 i = 0; i < dataFeedIds.length;) {
            (detailsForFeeds[i].dataTimestamp, detailsForFeeds[i].blockTimestamp, detailsForFeeds[i].value) =
                getLastUpdateDetails(dataFeedIds[i]);
            unchecked {
                i++; // reduces gas costs
            }
        }
