...
} from "@crane/contracts/external/redstone/packages/evm-connector/contracts/core/RedstoneConsumerNumericBase.sol";
...
abstract contract MultiFeedAdapterWithoutRounds is RedstoneConsumerNumericBase, IMultiFeedAdapter, IPriceCalculator {
    bytes32 internal constant DATA_FEEDS_STORAGE_LOCATION =
        0x5e9fb4cb0eb3c2583734d3394f30bb14b241acb9b3a034f7e7ba1a62db4370f1; // keccak256("RedStone.MultiFeedAdapterWithoutRounds.dataFeeds");
    ...
    function updateDataFeedsValuesPartial(bytes32[] memory dataFeedsIds) public {
        (uint256[] memory oracleValues, uint256 extractedDataTimestamp) =
            getOracleNumericValuesAndTimestampFromTxMsg(dataFeedsIds);
    ...
    function _tryToUpdateDataFeed(bytes32 dataFeedId, uint256 value, uint256 dataTimestamp) internal virtual {
        (uint256 lastDataTimestamp, uint256 lastBlockTimestamp, uint256 lastValue) =
            getLastUpdateDetailsUnsafe(dataFeedId);
    ...
    /// Important! This function should not revert, it should only return bool result of the validation
    function _validateDataTimestamp(uint256 proposedDataTimestamp, uint256 lastDataTimestamp)
        internal
        view
        virtual
        returns (bool)
    ...
    function _validateValueBeforeSave(
        bytes32,
        /* dataFeedId */
    ...
    function convertDecimals(
        bytes32,
        /* dataFeedId */
        uint256 valueFromRedstonePayload
    )
        public
        view
        virtual
    ...
    function pricesOf(address[] memory assets) external view returns (uint256[] memory values) {
    ...
    function getUnderlyingPrices(address[] memory gTokens) public view returns (uint256[] memory values) {
