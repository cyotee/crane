...
abstract contract FastMultiFeedAdapter is IFastMultiFeedAdapter {
    ...
    function getMaxReadingDataStaleness() internal pure virtual returns (uint256) {
        return MAX_READING_DATA_STALENESS;
    }

    function getMaxDataTimestampDelayMicroseconds() internal pure virtual returns (uint64) {
        return MAX_DATA_TIMESTAMP_DELAY_MICROSECONDS;
    }

    function getMaxDataTimestampAheadMicroseconds() internal pure virtual returns (uint64) {
        return MAX_DATA_TIMESTAMP_AHEAD_MICROSECONDS;
    }
    ...
    // ----------------------- Storage ---------------------------------------- //
    bytes32 private constant UPDATER_LAST_PRICE_DATA_POSITION =
        keccak256("fast.multi.feed.adapter.updater.last.price.data");

    // Returns the last price update data for a given updater id and data feed id
    function updaterLastPriceData()
        private
        pure
        returns (mapping(uint256 => mapping(bytes32 => PriceData)) storage store)
    {
    ...
    function updateDataFeedsValues(uint64 priceTimestamp, PriceUpdateInput[] calldata inputs) public virtual {
        ...
        for (uint256 i = 0; i < inputs.length;) {
            ...
            PriceData memory priceData =
                PriceData({price: input.price, priceTimestamp: priceTimestamp, blockTimestamp: currentBlockTimestamp});
    ...
    function validatePriceDataValid(uint256 updaterId, bytes32 feedId, PriceData memory priceData)
        private
        returns (bool)
    {
        ...
        returns (uint256 lastDataTimestamp, uint256 lastBlockTimestamp, uint256 lastValue)
    {
        ...
        if (
            lastBlockTimestamp + getMaxReadingDataStaleness() < getBlockTimestampInMicroSecondsForStalenessCheck()
                || lastValue == 0 || lastDataTimestamp + getMaxDataTimestampDelayMicroseconds() < lastBlockTimestamp
        ...
        returns (LastUpdateDetails[] memory detailsForFeeds)
    {
        ...
        for (uint256 i = 0; i < dataFeedIds.length;) {
            (detailsForFeeds[i].dataTimestamp, detailsForFeeds[i].blockTimestamp, detailsForFeeds[i].value) =
                getLastUpdateDetailsUnsafe(dataFeedIds[i]);
            unchecked {
                i++; // reduces gas costs
            }
        }
    ...
    function getDataTimestampFromLatestUpdate(bytes32 dataFeedId)
        public
        view
        override
    ...
    function getBlockTimestampFromLatestUpdate(bytes32 dataFeedId)
        public
        view
        override
