...
abstract contract FastMultiFeedAdapterWithRounds is FastMultiFeedAdapter, IFastMultiFeedAdapterWithRounds {
    ...
    function getMaxHistorySize() internal pure virtual returns (uint256) {
        return MAX_HISTORY_SIZE;
    }

    // ----------------------- Storage ---------------------------------------- //
    bytes32 private constant LATEST_ROUND_ID_FOR_FEED_POSITION =
        keccak256("fast.multi.feed.adapter.latest.round.id.for.feed");
    ...
    function storeAggregatedPrice(
        ...
        emit RoundCreated(
            medianPrice, newPriceData.priceTimestamp, dataFeedId, newPriceData.blockTimestamp, newRoundId, updaterId
        );
    }
