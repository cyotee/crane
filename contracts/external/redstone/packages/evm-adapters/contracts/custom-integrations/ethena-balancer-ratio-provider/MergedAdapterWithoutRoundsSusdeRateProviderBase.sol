...
} from "../../price-feeds/data-services/MergedPriceFeedAdapterWithoutRoundsPrimaryProd.sol";
import {DeviationLib} from "../../libs/DeviationLib.sol";

contract MergedAdapterWithoutRoundsSusdeRateProviderBase is MergedPriceFeedAdapterWithoutRoundsPrimaryProd {
    ...
    error ProposedValueIsDeviatedTooMuch(
        uint256 latestValue, uint256 proposedNewValue, uint256 deviation, uint256 maxAllowedDeviationPercent
    );
    ...
    function _validateDeviationFromLatestAnswer(uint256 proposedValue, bytes32 dataFeedId) internal view virtual {
        uint256 latestAnswer = getValueForDataFeedUnsafe(dataFeedId);
        if (latestAnswer != 0) {
            uint256 deviation = DeviationLib.calculateAbsDeviation(proposedValue, latestAnswer, PRECISION);
            if (deviation > MAX_ALLOWED_DEVIATION_PERCENT) {
                revert ProposedValueIsDeviatedTooMuch(
                    latestAnswer, proposedValue, deviation, MAX_ALLOWED_DEVIATION_PERCENT
                );
            }
        }
    }
