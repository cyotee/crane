...
abstract contract MentoAdapterBase is RedstoneAdapterBase, Initializable {
    ...
    function getSortedOracles() public view virtual returns (ISortedOracles sortedOracles);
    ...
    function normalizeRedstoneValueForMento(uint256 valueFromRedstone) public pure returns (uint256) {
    ...
    function _validateAndUpdateDataFeedsValues(bytes32[] memory dataFeedIds, uint256[] memory values)
        ...
        override
    {
        LocationInSortedLinkedList[] memory locationsInSortedList = extractLinkedListLocationsFromCalldata();
        for (uint256 dataFeedIndex = 0; dataFeedIndex < dataFeedIds.length;) {
            ...
            getSortedOracles().report(tokenAddress, priceValue, location.lesserKey, location.greaterKey);
        ...
        returns (LocationInSortedLinkedList[] memory locationsInSortedList)
    {
        ...
        uint256 arrayLength = abi.decode(msg.data[calldataOffset:calldataOffset + STANDARD_SLOT_BS], (uint256));
    ...
    function getDataFeeds() public view returns (DataFeedDetails[] memory) {
        ...
        for (uint256 dataFeedIndex = 0; dataFeedIndex < dataFeedsCount;) {
            ...
            dataFeeds[dataFeedIndex] = DataFeedDetails({dataFeedId: dataFeedId, tokenAddress: tokenAddress});
            unchecked {
                dataFeedIndex++;
            } // reduces gas costs
        }
    ...
    function getTokenIndexByDataFeedId(bytes32 dataFeedId) public view virtual returns (uint256) {
        uint256 dataFeedsCount = getDataFeedsCount();
        for (uint256 dataFeedIndex = 0; dataFeedIndex < dataFeedsCount;) {
