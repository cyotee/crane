...
library ERC20MinterFacadeRepo {
    ...
    function _layout(
        bytes32 slot_
    ) internal pure returns(ERC20MinterFacadeLayout storage layout_) {
...
abstract contract ERC20MinterFacadeFacetDFPkgStorage {
    ...
    bytes32 private constant LAYOUT_ID
        = keccak256(abi.encode(type(ERC20MinterFacadeRepo).name));
     ...
     */
    function _erc20MinterFacade()
    internal pure virtual returns(ERC20MinterFacadeLayout storage) {
        return STORAGE_SLOT._layout();
    }
    // end::_erc20MinterFacade()[]

    function _initERC20MinterFacade(
        uint256 maxMintAmount
    ) internal {
        _erc20MinterFacade().maxMintAmount = maxMintAmount;
    }

    function _setMaxMintAmount(
        uint256 maxMintAmount
    ) internal {
        _erc20MinterFacade().maxMintAmount = maxMintAmount;
    }
...
contract ERC20MinterFacadeFacetDFPkg
is
    ...
    IERC20MinterFacadeFacetDFPkg
{
    ...
    constructor(
        ...
        SELF = this;
        ERC20MinterFacadeFacetDFPkgInit memory pkgInit = abi.decode(create3InitData.initData, (ERC20MinterFacadeFacetDFPkgInit));
    ...
    public pure override(IFacet, IDiamondFactoryPackage) returns(bytes4[] memory interfaces) {
