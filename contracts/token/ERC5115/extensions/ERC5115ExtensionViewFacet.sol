...
import { ERC5115ExtensionStorage } from "./utils/ERC5115ExtensionStorage.sol";
...
contract ERC5115ExtensionViewFacet is Create3AwareContract, ERC5115ExtensionStorage, IERC5115Extension {
    ...
    external pure returns(bytes4[] memory interfaces) {
    ...
    function facetFuncs()
    external pure returns(bytes4[] memory funcs) {
        funcs = new bytes4[](1);
        funcs[0] = IERC5115Extension.yieldTokenTypes.selector;
        return funcs;
    }

    function yieldTokenTypes()
    external view returns(bytes4[] memory yieldTokenTypes_) {
