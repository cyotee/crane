...
contract ERC5115ExtensionStorage {
    ...
    bytes32 private constant LAYOUT_ID =
        keccak256(abi.encode(type(ERC5115ExtensionRepo).name));
    ...
    bytes32 private constant STORAGE_RANGE =
        type(IERC5115Extension).interfaceId;
     ...
     */
    function _erc5115Extension()
    internal pure virtual returns (ERC5115ExtensionLayout storage) {
        return STORAGE_SLOT._layout();
    }
    // end::_erc5115Extension()[]

    function _initERC5115Extension(
        bytes4[] memory yieldTokenTypes
    ) internal {
