...
// tag::Bytes32SetRepo[]
/**
 * @title Bytes32SetRepo - Struct and atomic operations for a set of bytes32 values.
 * @author cyotee doge <doge.cyotee>
 * @dev Distinct from OpenZeppelin to allow for operations upon an array of the same type.
 * @dev Pure struct-passing utility Repo (no own STORAGE_SLOT / _layout binding); functions take Bytes32Set storage directly.
 * @dev 1-indexed to allow 0 to signify nonexistence (in the `indexes` mapping).
 * @dev Used (via `using Bytes32SetRepo for Bytes32Set;`) across introspection (e.g. ERC2535), comparators, handlers, and other collection consumers.
 */
library Bytes32SetRepo {
    using Arrays for uint256;
    using Bytes32SetRepo for Bytes32Set;

    // tag::_index(Bytes32Set-uint256)[]
    /**
     * @dev Returns the value stored at the provided index (1-based).
     * @dev Will revert if the index is not used (via BetterArrays).
     * @param set The Bytes32Set storage struct upon which this function operates.
    ...
    // end::_index(Bytes32Set-uint256)[]

    // tag::_indexOf(Bytes32Set-bytes32)[]
    /**
    ...
    // end::_indexOf(Bytes32Set-bytes32)[]

    // tag::_contains(Bytes32Set-bytes32)[]
    /**
    ...
    // end::_contains(Bytes32Set-bytes32)[]

    // tag::_length(Bytes32Set)[]
    /**
     * @dev Returns the length of the provided set.
     * @param set The Bytes32Set storage struct upon which this function operates.
     * @return length_ The "length", quantity of entries, of the provided set.
     */
    function _length(Bytes32Set storage set) internal view returns (uint256 length_) {
        return set.values.length;
    }

    // end::_length(Bytes32Set)[]

    // tag::_add(Bytes32Set-bytes32)[]
    /**
     ...
     */
    function _add(Bytes32Set storage set, bytes32 value) internal returns (bool success) {
     ...
     * @return success Boolean indicating desired set state has been achieved.
     */
    function _add(Bytes32Set storage set, bytes32[] memory values) internal returns (bool success) {
    ...
    // tag::_addAsc(Bytes32Set-bytes32)[]
    /**
     * @notice Insert a new bytes32 (or do nothing if already present)
     * @dev Keeps the `values` array sorted in ascending order.
     * @param set The Bytes32Set storage struct upon which this function operates.
     * @param value The bytes32 to insert into the set in ascending order.
     */
    function _addAsc(Bytes32Set storage set, bytes32 value) internal {
        ...
        set.values.push(bytes32(0)); // reserve space
    ...
    // end::_addAsc(Bytes32Set-bytes32)[]

    // tag::_remove(Bytes32Set-bytes32)[]
    /**
     * @dev Written to be idempotent.
     * @dev Sets care about ensuring desired state.
    ...
    // end::_remove(Bytes32Set-bytes32)[]
    ...
    // tag::_removeAsc(Bytes32Set-bytes32)[]
    /**
     * @notice Remove a bytes32 while preserving ascending order
     * @param set The Bytes32Set storage struct upon which this function operates.
     * @param value The value to remove, reordering set to maintain ascending order.
     */
    function _removeAsc(Bytes32Set storage set, bytes32 value) internal {
    ...
    // end::_removeAsc(Bytes32Set-bytes32)[]

    // tag::_asArray(Bytes32Set)[]
    /**
     * @dev Copies the set into memory as an array.
     * @param set The Bytes32Set storage struct upon which this function operates.
     * @return array The members of the set copied to memory as an array.
     */
    function _asArray(Bytes32Set storage set) internal view returns (bytes32[] memory array) {
        array = set.values;
    }

    // end::_asArray(Bytes32Set)[]

    // tag::_values(Bytes32Set)[]
    /**
    ...
    // end::_values(Bytes32Set)[]

    // tag::_range(Bytes32Set-uint256-uint256)[]
    /**
     ...
     */
    function _range(Bytes32Set storage set, uint256 start, uint256 end) internal view returns (bytes32[] memory array) {
        if (end < start) {
    ...
    // end::_range(Bytes32Set-uint256-uint256)[]

    // tag::_sortAsc(Bytes32Set)[]
    /**
     * @notice Sort the set in ascending order in-place
     * @dev Fixes both `values` array and `indexes` mapping
     * @param set The Bytes32Set storage struct to sort.
     */
    function _sortAsc(Bytes32Set storage set) internal {
    ...
    // end::_sortAsc(Bytes32Set)[]

    // tag::_quickSort(Bytes32Set)[]
    /**
     * @notice Sort the set in-place using QuickSort algorithm
     * @dev Fixes both `values` array and `indexes` mapping
     * @param set The Bytes32Set storage struct to sort.
     */
    function _quickSort(Bytes32Set storage set) internal {
        _quickSort(set, 0, int256(set.values.length) - 1);
    }

    // end::_quickSort(Bytes32Set)[]

    // tag::_quickSort(Bytes32Set-int256-int256)[]
    /**
     ...
     */
    function _quickSort(Bytes32Set storage set, int256 left, int256 right) private {
        if (left >= right) return;

        bytes32[] storage values = set.values;
        int256 i = left;
        int256 j = right;
    ...
    // end::_quickSort(Bytes32Set-int256-int256)[]

    // tag::_sort(bytes32[])[]
    /**
     * @dev Sorts an array of bytes32 in ascending order using Bubble Sort.
     * @param array The array of bytes32 to sort.
     * @return The sorted array of bytes32.
     */
    function _sort(bytes32[] memory array) internal pure returns (bytes32[] memory) {
        ...
        for (uint256 i = 1; i < array.length; i++) {
            ...
            for (uint256 j = 0; j < array.length - i; j++) {
                bytes32 next = array[j + 1];
                bytes32 actual = array[j];
                if (next < actual) {
                    array[j] = next;
                    array[j + 1] = actual;
                    swapped = true;
    ...
    // tag::_sort(bytes32[]-uint256)[]
    /**
     * @dev Helper function to recursively sort an array of bytes32 using Bubble Sort.
     * @param _arr The array of bytes32 to sort.
     * @param unsortedLen The length of the unsorted portion of the array.
     */
    function _sort(bytes32[] memory _arr, uint256 unsortedLen) internal pure {
