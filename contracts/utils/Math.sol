...
/**
 * @dev OpenZeppelin-compatible Math library using Solady's FixedPointMathLib where applicable.
 ...
 * gas-optimized implementations where possible.
 */
library Math {
    enum Rounding {
     ...
     *
     * The result is stored in two 256 variables such that sum = high * 2^256 + low.
     */
     ...
     *
     * The result is stored in two 256 variables such that product = high * 2^256 + low.
     */
    ...
    function ceilDiv(uint256 a, uint256 b) internal pure returns (uint256) {
        if (b == 0) {
            ...
            // Using Panic error code for division by zero.
            revert();
        }
    ...
    function mulDiv(uint256 x, uint256 y, uint256 denominator) internal pure returns (uint256 result) {
        return FixedPointMathLib.fullMulDiv(x, y, denominator);
    }
    ...
    /**
     * @dev Calculate x * y >> n with full precision.
     */
    function mulShr(uint256 x, uint256 y, uint8 n) internal pure returns (uint256 result) {
        (uint256 high, uint256 low) = mul512(x, y);
        if (high >= (1 << n)) revert();
        assembly ("memory-safe") {
            result := or(shr(n, low), shl(sub(256, n), high))
        }
    }

    /**
     * @dev Calculate x * y >> n, following the selected rounding direction.
     */
    function mulShr(uint256 x, uint256 y, uint8 n, Rounding rounding) internal pure returns (uint256) {
        uint256 result = mulShr(x, y, n);
        if (unsignedRoundsUp(rounding) && (x * y) & ((1 << n) - 1) > 0) {
            result += 1;
        }
        return result;
    }
    ...
    function sqrt(uint256 a) internal pure returns (uint256) {
        return FixedPointMathLib.sqrt(a);
    }
    ...
    /**
     * @dev Returns the modular exponentiation of the specified base, exponent and modulus (b ** e % m).
     * It includes a success flag indicating if the operation succeeded.
     */
