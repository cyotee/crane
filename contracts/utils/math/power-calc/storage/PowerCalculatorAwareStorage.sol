...
import {
    PowerCalculatorAwareLayout,
    PowerCalculatorAwareRepo
} from "../libs/PowerCalculatorAwareRepo.sol";
import {
...
contract PowerCalculatorAwareStorage
is
IPowerCalculatorAwareStorage
{

    using PowerCalculatorAwareRepo for bytes32;

    bytes32 private constant LAYOUT_ID
        = keccak256(abi.encode(type(PowerCalculatorAwareRepo).name));
    ...
    bytes32 private constant STORAGE_RANGE
        = type(IPowerCalculatorAware).interfaceId;
    bytes32 private constant STORAGE_SLOT
        = keccak256(abi.encode(STORAGE_RANGE, STORAGE_RANGE_OFFSET));

    function _powerCalcAware()
    internal pure virtual returns(PowerCalculatorAwareLayout storage) {
        return STORAGE_SLOT._layout();
    }

    function _power()
    internal view returns(IPower) {
        return _powerCalcAware().powerCalculator;
    }

    function _initPowerAware(
        IPowerCalculatorAwareStorage
        .PowerCalculatorAwareInit
        memory powerCalculatorAwareTargetInit
    ) internal {
