...
library AerodromeUtils {
    ...
    function _quoteWithdrawSwapWithFee(
        ...
        if (
            ownedLPAmount == 0 || lpTotalSupply == 0 || reserveOut == 0 || reserveIn == 0
                || feePercent >= AERO_FEE_DENOM
        ) {
            return (0);
        }
        ...
        (uint256 amountAWD, uint256 amountBWD) =
            ConstProdUtils._withdrawQuote(ownedLPAmount, lpTotalSupply, reserveOut, reserveIn);
