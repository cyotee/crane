...
/// @title UniswapV3ZapQuoter
/// @notice Quoting library for Uniswap V3 zap operations (zap-in and zap-out)
...
library UniswapV3ZapQuoter {
    ...
    struct ZapInQuote {
        ...
        UniswapV3Quoter.SwapQuoteResult swap; // Swap quote details
    }
    ...
    function quoteZapInSingleCore(ZapInParams memory p) internal view returns (ZapInQuote memory q) {
        require(p.amountIn > 0, "ZAPQ:ZERO_AMOUNT");
        require(p.tickLower < p.tickUpper, "ZAPQ:INVALID_RANGE");
        ...
        returns (ZapInQuote memory q)
    {
        ...
        if (swapAmount == 0) {
            ...
            q.swap = UniswapV3Quoter.SwapQuoteResult({
                amountIn: 0,
                amountOut: 0,
                feeAmount: 0,
            ...
            // Quote the swap
            UniswapV3Quoter.SwapQuoteParams memory swapParams = UniswapV3Quoter.SwapQuoteParams({
                pool: p.pool,
            ...
            q.swap = UniswapV3Quoter.quoteExactInput(swapParams);
        ...
        q.liquidity =
            UniswapV3Utils._quoteLiquidityForAmounts(priceForMint, p.tickLower, p.tickUpper, q.amount0, q.amount1);

        // Calculate amounts actually used for minting
        (uint256 used0, uint256 used1) =
            UniswapV3Utils._quoteAmountsForLiquidity(priceForMint, p.tickLower, p.tickUpper, q.liquidity);
    ...
    function createZapInParams(
        ...
        require(tokenIn == token0 || tokenIn == token1, "ZAPQ:INVALID_TOKEN");
    ...
    function quoteZapOutSingleCore(ZapOutParams memory p) internal view returns (ZapOutQuote memory q) {
        require(p.liquidity > 0, "ZAPQ:ZERO_LIQUIDITY");
        require(p.tickLower < p.tickUpper, "ZAPQ:INVALID_RANGE");
        ...
        (q.burnAmount0, q.burnAmount1) =
            UniswapV3Utils._quoteAmountsForLiquidity(sqrtPriceX96, p.tickLower, p.tickUpper, p.liquidity);
    ...
    function createZapOutParams(
        ...
        require(tokenOut == token0 || tokenOut == token1, "ZAPQ:INVALID_TOKEN");
