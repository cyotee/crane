...
import {IDAIPermit} from "@crane/contracts/interfaces/protocols/utils/permit2/IDAIPermit.sol";
import {IAllowanceTransfer} from "@crane/contracts/interfaces/protocols/utils/permit2/IAllowanceTransfer.sol";
import {SafeCast160} from "@crane/contracts/protocols/utils/permit2/SafeCast160.sol";
...
/// @dev This library provides fallback functionality to Permit2 when native permit is unavailable
library Permit2Lib {
    ...
    /// @dev Canonical deployment address across all chains
    ...
    /// @dev Falls back to Permit2 if the native transferFrom fails
    ...
    /// @notice Permit a user to spend a given amount of another user's tokens via native EIP-2612 permit
    /// if possible, falling back to Permit2 if native permit fails or is not implemented on the token
    ...
    function permit2(
        ...
        if (success) {
            // We'll use DAI's special permit if its DOMAIN_SEPARATOR matches,
            // otherwise we'll just encode a call to the standard permit function
            inputData = domainSeparator == DAI_DOMAIN_SEPARATOR
                ? abi.encodeCall(
                    IDAIPermit.permit,
                    (owner, spender, IERC20Permit(address(token)).nonces(owner), deadline, true, v, r, s)
                )
