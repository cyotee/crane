...
interface IConfigPositionManager is IPositionManagerIntentBase {
    ...
    /// @param delegatee The address of the delegatee.
    /// @param status The new permission status.
    function setCanUpdateUserRiskPremiumPermission(address spoke, address delegatee, bool status) external;

    /// @notice Sets the user dynamic config permission for a delegatee.
    /// @param spoke The address of the Spoke.
    /// @param delegatee The address of the delegatee.
    /// @param status The new permission status.
    function setCanUpdateUserDynamicConfigPermission(address spoke, address delegatee, bool status) external;
    ...
    function setGlobalPermissionWithSig(SetGlobalPermissionPermit calldata params, bytes calldata signature) external;
    ...
    function SET_CAN_UPDATE_USER_RISK_PREMIUM_PERMISSION_PERMIT_TYPEHASH() external view returns (bytes32);

    /// @notice Returns the type hash for the SetCanUpdateUserDynamicConfigPermissionPermit intent.
    function SET_CAN_UPDATE_USER_DYNAMIC_CONFIG_PERMISSION_PERMIT_TYPEHASH() external view returns (bytes32);
}
