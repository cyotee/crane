...
import {
    INativeWrapper
} from "@crane/contracts/protocols/lending/aave/v4/position-manager/interfaces/INativeWrapper.sol";
import {
    INativeTokenGateway
...
contract NativeTokenGateway is INativeTokenGateway, PositionManagerBase, ReentrancyGuardTransient {
    ...
    constructor(address nativeTokenWrapper_, address initialOwner_) PositionManagerBase(initialOwner_) {
    ...
    /// @inheritdoc INativeTokenGateway
    function supplyNative(address spoke, uint256 reserveId, uint256 amount)
        external
        payable
        nonReentrant
    ...
    /// @inheritdoc INativeTokenGateway
    function supplyAsCollateralNative(address spoke, uint256 reserveId, uint256 amount)
        external
        payable
        nonReentrant
        onlyRegisteredSpoke(spoke)
    ...
    {
        require(msg.value == amount, NativeAmountMismatch());
        (uint256 suppliedShares, uint256 suppliedAmount) = _supplyNative(spoke, reserveId, msg.sender, amount);
    ...
    /// @inheritdoc INativeTokenGateway
    function withdrawNative(address spoke, uint256 reserveId, uint256 amount)
        external
        nonReentrant
        onlyRegisteredSpoke(spoke)
        returns (uint256, uint256)
    {
        ...
        (uint256 withdrawnShares, uint256 withdrawnAmount) = ISpoke(spoke).withdraw(reserveId, amount, msg.sender);
    ...
    /// @inheritdoc INativeTokenGateway
    function borrowNative(address spoke, uint256 reserveId, uint256 amount)
        external
        nonReentrant
        onlyRegisteredSpoke(spoke)
        returns (uint256, uint256)
    {
        ...
        (uint256 borrowedShares, uint256 borrowedAmount) = ISpoke(spoke).borrow(reserveId, amount, msg.sender);
    ...
    /// @inheritdoc INativeTokenGateway
    function repayNative(address spoke, uint256 reserveId, uint256 amount)
        external
        payable
        nonReentrant
        onlyRegisteredSpoke(spoke)
        returns (uint256, uint256)
    {
        ...
        (uint256 repaidShares, uint256 repaidAmount) = ISpoke(spoke).repay(reserveId, repayAmount, msg.sender);
    ...
    function _supplyNative(address spoke, uint256 reserveId, address user, uint256 amount)
        internal
        returns (uint256, uint256)
    {
        address underlying = _getReserveUnderlying(spoke, reserveId);
