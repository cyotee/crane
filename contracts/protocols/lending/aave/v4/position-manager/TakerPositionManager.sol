...
contract TakerPositionManager is ITakerPositionManager, PositionManagerIntentBase {
    ...
    mapping(
        address spoke
            => mapping(uint256 reserveId => mapping(address owner => mapping(address spender => uint256 amount)))
        ...
        onlyRegisteredSpoke(spoke)
    {
        _updateWithdrawAllowance({
            spoke: spoke, reserveId: reserveId, owner: msg.sender, spender: spender, newAllowance: amount
        });
    }
    ...
    /// @inheritdoc ITakerPositionManager
    function approveBorrowWithSig(BorrowPermit calldata params, bytes calldata signature)
        external
        onlyRegisteredSpoke(params.spoke)
    {
        _verifyAndConsumeIntent({
    ...
    /// @inheritdoc ITakerPositionManager
    function renounceWithdrawAllowance(address spoke, uint256 reserveId, address owner)
        external
        onlyRegisteredSpoke(spoke)
    {
        if (_getWithdrawAllowance({spoke: spoke, reserveId: reserveId, owner: owner, spender: msg.sender}) == 0) {
            return;
        }
        _updateWithdrawAllowance({
            spoke: spoke, reserveId: reserveId, owner: owner, spender: msg.sender, newAllowance: 0
        });
    }

    /// @inheritdoc ITakerPositionManager
    function renounceBorrowAllowance(address spoke, uint256 reserveId, address owner)
        external
        onlyRegisteredSpoke(spoke)
    {
        if (_getBorrowAllowance({spoke: spoke, reserveId: reserveId, owner: owner, spender: msg.sender}) == 0) {
            return;
        }
        _updateBorrowAllowance({spoke: spoke, reserveId: reserveId, owner: owner, spender: msg.sender, newAllowance: 0});
    }
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 currentAllowance =
            _getWithdrawAllowance({spoke: spoke, reserveId: reserveId, owner: onBehalfOf, spender: msg.sender});
        ...
        (uint256 withdrawnShares, uint256 withdrawnAmount) = ISpoke(spoke).withdraw(reserveId, amount, onBehalfOf);
        ...
        emit WithdrawOnBehalfOf(spoke, msg.sender, onBehalfOf, reserveId, withdrawnShares, withdrawnAmount);
        ...
        returns (uint256, uint256)
    {
        IERC20 underlying = IERC20(_getReserveUnderlying(spoke, reserveId));
        uint256 currentAllowance =
            _getBorrowAllowance({spoke: spoke, reserveId: reserveId, owner: onBehalfOf, spender: msg.sender});
        ...
        (uint256 borrowedShares, uint256 borrowedAmount) = ISpoke(spoke).borrow(reserveId, amount, onBehalfOf);
    ...
    function withdrawAllowance(address spoke, uint256 reserveId, address owner, address spender)
        ...
        returns (uint256)
    {
        return _getWithdrawAllowance({spoke: spoke, reserveId: reserveId, owner: owner, spender: spender});
    }
    ...
    function borrowAllowance(address spoke, uint256 reserveId, address owner, address spender)
        ...
        returns (uint256)
    {
        return _getBorrowAllowance({spoke: spoke, reserveId: reserveId, owner: owner, spender: spender});
    }

    function _getWithdrawAllowance(address spoke, uint256 reserveId, address owner, address spender)
        internal
        view
        returns (uint256)
    {
    ...
    function _getBorrowAllowance(address spoke, uint256 reserveId, address owner, address spender)
        internal
        view
        returns (uint256)
    {
