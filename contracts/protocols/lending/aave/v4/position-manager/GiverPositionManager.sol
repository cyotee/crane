...
} from "@crane/contracts/protocols/lending/aave/v4/position-manager/interfaces/IGiverPositionManager.sol";
...
contract GiverPositionManager is IGiverPositionManager, PositionManagerBase {
        ...
        returns (uint256, uint256)
    {
        ...
        (uint256 suppliedShares, uint256 suppliedAmount) = ISpoke(spoke).supply(reserveId, amount, onBehalfOf);
        ...
        returns (uint256, uint256)
    {
        ...
        (uint256 repaidShares, uint256 repaidAmount) = ISpoke(spoke).repay(reserveId, repayAmount, onBehalfOf);
