...
} from "@crane/contracts/protocols/lending/aave/v4/position-manager/PositionManagerIntentBase.sol";
...
contract ConfigPositionManager is IConfigPositionManager, PositionManagerIntentBase {
    ...
    bytes32 public constant SET_GLOBAL_PERMISSION_PERMIT_TYPEHASH = EIP712Hash.SET_GLOBAL_PERMISSION_PERMIT_TYPEHASH;
    ...
    mapping(address spoke => mapping(address delegator => mapping(address delegatee => ConfigPermissions))) private
        _config;
    ...
    /// @inheritdoc IConfigPositionManager
    function setGlobalPermission(address spoke, address delegatee, bool status) external onlyRegisteredSpoke(spoke) {
        _setGlobalPermission({spoke: spoke, delegator: msg.sender, delegatee: delegatee, status: status});
    }

    /// @inheritdoc IConfigPositionManager
    function setCanSetUsingAsCollateralPermission(address spoke, address delegatee, bool status)
        external
        onlyRegisteredSpoke(spoke)
    {
        _setCanSetUsingAsCollateralPermission({
            spoke: spoke, delegator: msg.sender, delegatee: delegatee, status: status
        });
    }

    /// @inheritdoc IConfigPositionManager
    function setCanUpdateUserRiskPremiumPermission(address spoke, address delegatee, bool status)
        external
        onlyRegisteredSpoke(spoke)
    {
    ...
    /// @inheritdoc IConfigPositionManager
    function setCanUpdateUserDynamicConfigPermission(address spoke, address delegatee, bool status)
        external
        onlyRegisteredSpoke(spoke)
    {
    ...
    /// @inheritdoc IConfigPositionManager
    function setGlobalPermissionWithSig(SetGlobalPermissionPermit calldata params, bytes calldata signature)
        external
        onlyRegisteredSpoke(params.spoke)
    {
        ...
        _setGlobalPermission({
            spoke: params.spoke, delegator: params.delegator, delegatee: params.delegatee, status: params.status
        });
    }
    ...
    /// @inheritdoc IConfigPositionManager
    function renounceGlobalPermission(address spoke, address delegator) external onlyRegisteredSpoke(spoke) {
        ConfigPermissions oldPermissions = _getPermissions({spoke: spoke, delegator: delegator, delegatee: msg.sender});
    ...
    /// @inheritdoc IConfigPositionManager
    function renounceCanUpdateUsingAsCollateralPermission(address spoke, address delegator)
        external
        onlyRegisteredSpoke(spoke)
    {
        ConfigPermissions oldPermissions = _getPermissions({spoke: spoke, delegator: delegator, delegatee: msg.sender});
    ...
    /// @inheritdoc IConfigPositionManager
    function renounceCanUpdateUserRiskPremiumPermission(address spoke, address delegator)
        external
        onlyRegisteredSpoke(spoke)
    {
        ConfigPermissions oldPermissions = _getPermissions({spoke: spoke, delegator: delegator, delegatee: msg.sender});
    ...
    /// @inheritdoc IConfigPositionManager
    function renounceCanUpdateUserDynamicConfigPermission(address spoke, address delegator)
        external
        onlyRegisteredSpoke(spoke)
    {
        ConfigPermissions oldPermissions = _getPermissions({spoke: spoke, delegator: delegator, delegatee: msg.sender});
    ...
    function setUsingAsCollateralOnBehalfOf(
        ...
        require(
            _getPermissions({spoke: spoke, delegator: onBehalfOf, delegatee: msg.sender}).canSetUsingAsCollateral(),
            DelegateeNotAllowed()
        );
        ...
        emit SetUsingAsCollateralOnBehalfOf(spoke, msg.sender, onBehalfOf, reserveId, usingAsCollateral);
    }

    /// @inheritdoc IConfigPositionManager
    function updateUserRiskPremiumOnBehalfOf(address spoke, address onBehalfOf) external onlyRegisteredSpoke(spoke) {
        require(
            _getPermissions({spoke: spoke, delegator: onBehalfOf, delegatee: msg.sender}).canUpdateUserRiskPremium(),
            DelegateeNotAllowed()
        );
    ...
    /// @inheritdoc IConfigPositionManager
    function updateUserDynamicConfigOnBehalfOf(address spoke, address onBehalfOf) external onlyRegisteredSpoke(spoke) {
        require(
            _getPermissions({spoke: spoke, delegator: onBehalfOf, delegatee: msg.sender}).canUpdateUserDynamicConfig(),
            DelegateeNotAllowed()
        );
    ...
    {
        return _getPermissions({spoke: spoke, delegator: onBehalfOf, delegatee: delegatee}).getConfigPermissionValues();
    }
    ...
    function _setGlobalPermission(address spoke, address delegator, address delegatee, bool status) internal {
        ConfigPermissions oldPermissions = _getPermissions({spoke: spoke, delegator: delegator, delegatee: delegatee});
    ...
    /// @param delegatee The address of the delegatee.
    /// @param status The new permission status.
    function _setCanSetUsingAsCollateralPermission(address spoke, address delegator, address delegatee, bool status)
        internal
    {
        ConfigPermissions oldPermissions = _getPermissions({spoke: spoke, delegator: delegator, delegatee: delegatee});
    ...
    /// @param delegatee The address of the delegatee.
    /// @param status The new permission status.
    function _setCanUpdateUserRiskPremiumPermission(address spoke, address delegator, address delegatee, bool status)
        internal
    {
        ConfigPermissions oldPermissions = _getPermissions({spoke: spoke, delegator: delegator, delegatee: delegatee});
    ...
    /// @param delegatee The address of the delegatee.
    /// @param status The new permission status.
    function _setCanUpdateUserDynamicConfigPermission(address spoke, address delegator, address delegatee, bool status)
        internal
    {
        ConfigPermissions oldPermissions = _getPermissions({spoke: spoke, delegator: delegator, delegatee: delegatee});
    ...
    function _getPermissions(address spoke, address delegator, address delegatee)
        internal
        view
        returns (ConfigPermissions)
    {
