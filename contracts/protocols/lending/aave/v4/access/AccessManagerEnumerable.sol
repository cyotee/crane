...
import {
    AccessManager,
    IAccessManager
} from "@crane/contracts/external/openzeppelin-contracts/access/manager/AccessManager.sol";
...
contract AccessManagerEnumerable is AccessManager, IAccessManagerEnumerable {
    ...
    mapping(address target => mapping(bytes4 selector => uint64 roleId)) private _targetToSelectorToRole;
    ...
    mapping(uint64 roleId => mapping(address target => EnumerableSet.Bytes32Set)) private _roleToTargetToSelectorSet;
    ...
    function labelRole(uint64 roleId, string calldata label) public override(AccessManager, IAccessManager) {
    ...
    /// @inheritdoc IAccessManagerEnumerable
    function getRolesOfAdminRole(uint64 adminRoleId, uint256 start, uint256 end)
        external
        view
    ...
    /// @inheritdoc IAccessManagerEnumerable
    function getRoleTargets(uint64 roleId, uint256 start, uint256 end) external view returns (address[] memory) {
        return _roleToTargetSet[roleId].values(start, end);
    }

    /// @inheritdoc IAccessManagerEnumerable
    function getRoleTargetSelector(uint64 roleId, address target, uint256 index) external view returns (bytes4) {
        return bytes4(_roleToTargetToSelectorSet[roleId][target].at(index));
    }

    /// @inheritdoc IAccessManagerEnumerable
    function getRoleTargetSelectorCount(uint64 roleId, address target) external view returns (uint256) {
        return _roleToTargetToSelectorSet[roleId][target].length();
    }

    /// @inheritdoc IAccessManagerEnumerable
    function getRoleTargetSelectors(uint64 roleId, address target, uint256 start, uint256 end)
        external
        view
        returns (bytes4[] memory)
    {
        bytes32[] memory targetFunctions = _roleToTargetToSelectorSet[roleId][target].values(start, end);
