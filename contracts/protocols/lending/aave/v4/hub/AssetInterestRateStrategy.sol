...
contract AssetInterestRateStrategy is IAssetInterestRateStrategy {
    ...
    function setInterestRateData(uint256 assetId, bytes calldata data) external {
        ...
        require(
            MIN_OPTIMAL_RATIO <= rateData.optimalUsageRatio && rateData.optimalUsageRatio <= MAX_OPTIMAL_RATIO,
    ...
    function getMaxDrawnRate(uint256 assetId) external view returns (uint256) {
        return _interestRateData[assetId].baseDrawnRate + _interestRateData[assetId].rateGrowthBeforeOptimal
        ...
        returns (uint256)
    {
        ...
        if (usageRatioRay <= optimalUsageRatioRay) {
            currentDrawnRateRay += rateData.rateGrowthBeforeOptimal.bpsToRay().rayMulUp(usageRatioRay)
                .rayDivUp(optimalUsageRatioRay);
        } else {
            currentDrawnRateRay += rateData.rateGrowthBeforeOptimal.bpsToRay()
            + rateData.rateGrowthAfterOptimal.bpsToRay().rayMulUp(usageRatioRay - optimalUsageRatioRay)
