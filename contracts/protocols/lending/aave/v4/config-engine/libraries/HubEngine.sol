...
library HubEngine {
    ...
    function executeHubAssetConfigUpdates(IAaveV4ConfigEngine.AssetConfigUpdate[] calldata updates) external {
        ...
        for (uint256 i; i < length; ++i) {
            ...
            if (updateFee && updateReceiver) {
                updates[i].hubConfigurator
                    .updateFeeConfig(updates[i].hub, assetId, updates[i].liquidityFee, updates[i].feeReceiver);
            } else if (updateFee) {
                updates[i].hubConfigurator.updateLiquidityFee(updates[i].hub, assetId, updates[i].liquidityFee);
            } else if (updateReceiver) {
                updates[i].hubConfigurator.updateFeeReceiver(updates[i].hub, assetId, updates[i].feeReceiver);
            }
            ...
            if (updates[i].reinvestmentController != EngineFlags.KEEP_CURRENT_ADDRESS) {
                updates[i].hubConfigurator
                    .updateReinvestmentController(updates[i].hub, assetId, updates[i].reinvestmentController);
            }
        }
    }
    ...
    function executeHubSpokeToAssetsAdditions(IAaveV4ConfigEngine.SpokeToAssetsAddition[] calldata additions) external {
        ...
        for (uint256 i; i < length; ++i) {
            ...
            additions[i].hubConfigurator.addSpokeToAssets(additions[i].hub, additions[i].spoke, assetIds, configs);
        }
    }
    ...
    function executeHubSpokeConfigUpdates(IAaveV4ConfigEngine.SpokeConfigUpdate[] calldata updates) external {
        uint256 length = updates.length;
        for (uint256 i; i < length; ++i) {
            uint256 assetId = IHubBase(updates[i].hub).getAssetId(updates[i].underlying);
            ...
            if (updates[i].riskPremiumThreshold != EngineFlags.KEEP_CURRENT) {
                updates[i].hubConfigurator
                    .updateSpokeRiskPremiumThreshold(
                        updates[i].hub, assetId, updates[i].spoke, updates[i].riskPremiumThreshold
                    );
            }

            if (updates[i].active != EngineFlags.KEEP_CURRENT) {
                updates[i].hubConfigurator
                    .updateSpokeActive(updates[i].hub, assetId, updates[i].spoke, EngineFlags.toBool(updates[i].active));
            }
            if (updates[i].halted != EngineFlags.KEEP_CURRENT) {
                updates[i].hubConfigurator
                    .updateSpokeHalted(updates[i].hub, assetId, updates[i].spoke, EngineFlags.toBool(updates[i].halted));
            }
        }
    }
    ...
    function executeHubAssetDeactivations(IAaveV4ConfigEngine.AssetDeactivation[] calldata deactivations) external {
    ...
    function executeHubSpokeDeactivations(IAaveV4ConfigEngine.SpokeDeactivation[] calldata deactivations) external {
        uint256 length = deactivations.length;
        for (uint256 i; i < length; ++i) {
            deactivations[i].hubConfigurator.deactivateSpoke(deactivations[i].hub, deactivations[i].spoke);
        }
    }
    ...
    function _deployAndRegisterTokenizationSpoke(IAaveV4ConfigEngine.AssetListing calldata listing) private {
        ...
        if (bytes(listing.tokenization.name).length == 0 || bytes(listing.tokenization.symbol).length == 0) {
            return;
        }

        address proxy = TokenizationSpokeDeployer.deploy(
            listing.hub, listing.underlying, listing.tokenization.name, listing.tokenization.symbol
        );

        uint256 assetId = IHubBase(listing.hub).getAssetId(listing.underlying);

        listing.hubConfigurator
            .addSpoke(
                listing.hub,
                proxy,
                assetId,
    ...
    function _mergeInterestRateData(
        ...
        IAssetInterestRateStrategy.InterestRateData memory current =
            IAssetInterestRateStrategy(irStrategy).getInterestRateData(assetId);
    ...
    function _updateInterestRateStrategy(uint256 assetId, IAaveV4ConfigEngine.AssetConfigUpdate calldata update)
        private
    {
        if (update.irStrategy != EngineFlags.KEEP_CURRENT_ADDRESS) {
            ...
            update.hubConfigurator
                .updateInterestRateStrategy(update.hub, assetId, update.irStrategy, abi.encode(update.irData));
        } else {
    ...
    function _updateSpokeCaps(uint256 assetId, IAaveV4ConfigEngine.SpokeConfigUpdate calldata update) private {
        ...
        if (updateAdd && updateDraw) {
            update.hubConfigurator.updateSpokeCaps(update.hub, assetId, update.spoke, update.addCap, update.drawCap);
        } else if (updateAdd) {
