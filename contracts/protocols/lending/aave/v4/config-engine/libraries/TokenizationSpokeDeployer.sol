...
library TokenizationSpokeDeployer {
    ...
    function deploy(address hub, address underlying, string calldata name, string calldata symbol)
        external
        returns (address proxy)
    {
        ...
        bytes memory proxyCreationCode =
            abi.encodePacked(type(TransparentUpgradeableProxy).creationCode, abi.encode(impl, msg.sender, initData));
    ...
    function computeImplementationAddress(address hub, address underlying, string memory name, string memory symbol)
        external
        pure
        returns (address)
    {
    ...
    function computeProxyAddress(
        ...
        bytes memory creationCode = abi.encodePacked(
            type(TransparentUpgradeableProxy).creationCode, abi.encode(impl, proxyAdminOwner, initData)
        );
        return Create2Utils.computeCreate2Address(proxySalt, creationCode);
    }

    function _computeImplementationAddress(address hub, address underlying, string memory name, string memory symbol)
        internal
        pure
        returns (address)
    {
    ...
    function _computeImplementationSalt(address hub, address underlying, string memory name, string memory symbol)
        internal
        pure
        returns (bytes32)
    {
    ...
    function _computeProxySalt(address hub, address underlying, string memory name, string memory symbol)
        internal
        pure
        returns (bytes32)
    {
