...
library AccessManagerEngine {
    ...
    function executeRoleMemberships(IAaveV4ConfigEngine.RoleMembership[] calldata memberships) external {
        uint256 length = memberships.length;
        for (uint256 i; i < length; ++i) {
            if (memberships[i].granted) {
                IAccessManager(memberships[i].authority)
                    .grantRole(memberships[i].roleId, memberships[i].account, memberships[i].executionDelay);
            } else {
                IAccessManager(memberships[i].authority).revokeRole(memberships[i].roleId, memberships[i].account);
            }
        }
    }
    ...
    function executeTargetFunctionRoleUpdates(IAaveV4ConfigEngine.TargetFunctionRoleUpdate[] calldata updates)
        external
    {
        uint256 length = updates.length;
        for (uint256 i; i < length; ++i) {
            IAccessManager(updates[i].authority)
                .setTargetFunctionRole(updates[i].target, updates[i].selectors, updates[i].roleId);
        }
    }

    /// @notice Updates target admin delays via AccessManager.
    /// @param updates The target admin delay updates to execute.
    function executeTargetAdminDelayUpdates(IAaveV4ConfigEngine.TargetAdminDelayUpdate[] calldata updates) external {
        uint256 length = updates.length;
        for (uint256 i; i < length; ++i) {
            IAccessManager(updates[i].authority).setTargetAdminDelay(updates[i].target, updates[i].newDelay);
        }
    }
}
