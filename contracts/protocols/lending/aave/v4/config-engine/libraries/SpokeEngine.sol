...
library SpokeEngine {
    ...
    function executeSpokeReserveListings(IAaveV4ConfigEngine.ReserveListing[] calldata listings) external {
        uint256 length = listings.length;
        for (uint256 i; i < length; ++i) {
            uint256 assetId = IHubBase(listings[i].hub).getAssetId(listings[i].underlying);
            listings[i].spokeConfigurator
                .addReserve(
                    listings[i].spoke,
                    listings[i].hub,
                    assetId,
    ...
    function executeSpokeReserveConfigUpdates(IAaveV4ConfigEngine.ReserveConfigUpdate[] calldata updates) external {
        uint256 length = updates.length;
        for (uint256 i; i < length; ++i) {
            uint256 reserveId = _resolveReserveId(updates[i].spoke, updates[i].hub, updates[i].underlying);

            if (updates[i].priceSource != EngineFlags.KEEP_CURRENT_ADDRESS) {
                updates[i].spokeConfigurator
                    .updateReservePriceSource(updates[i].spoke, reserveId, updates[i].priceSource);
            }
            if (updates[i].collateralRisk != EngineFlags.KEEP_CURRENT) {
                updates[i].spokeConfigurator
                    .updateCollateralRisk(updates[i].spoke, reserveId, updates[i].collateralRisk);
            }
            if (updates[i].paused != EngineFlags.KEEP_CURRENT) {
                updates[i].spokeConfigurator
                    .updatePaused(updates[i].spoke, reserveId, EngineFlags.toBool(updates[i].paused));
            }
            if (updates[i].frozen != EngineFlags.KEEP_CURRENT) {
                updates[i].spokeConfigurator
                    .updateFrozen(updates[i].spoke, reserveId, EngineFlags.toBool(updates[i].frozen));
            }
            if (updates[i].borrowable != EngineFlags.KEEP_CURRENT) {
                updates[i].spokeConfigurator
                    .updateBorrowable(updates[i].spoke, reserveId, EngineFlags.toBool(updates[i].borrowable));
            }
            if (updates[i].receiveSharesEnabled != EngineFlags.KEEP_CURRENT) {
                updates[i].spokeConfigurator
                    .updateReceiveSharesEnabled(
                        updates[i].spoke, reserveId, EngineFlags.toBool(updates[i].receiveSharesEnabled)
                    );
            }
        }
    }
    ...
    function executeSpokeLiquidationConfigUpdates(IAaveV4ConfigEngine.LiquidationConfigUpdate[] calldata updates)
        external
    {
        uint256 length = updates.length;
        for (uint256 i; i < length; ++i) {
            ...
            if (updateTarget && updateMaxBonus && updateBonusFactor) {
                ...
                if (updateTarget) {
                    updates[i].spokeConfigurator
                        .updateLiquidationTargetHealthFactor(updates[i].spoke, updates[i].targetHealthFactor);
                }
                if (updateMaxBonus) {
                    updates[i].spokeConfigurator
                        .updateHealthFactorForMaxBonus(updates[i].spoke, updates[i].healthFactorForMaxBonus);
                }
                if (updateBonusFactor) {
                    updates[i].spokeConfigurator
                        .updateLiquidationBonusFactor(updates[i].spoke, updates[i].liquidationBonusFactor);
                }
            }
        }
    }
    ...
    function executeSpokeDynamicReserveConfigAdditions(IAaveV4ConfigEngine
                .DynamicReserveConfigAddition[] calldata additions) external {
        uint256 length = additions.length;
        for (uint256 i; i < length; ++i) {
            uint256 reserveId = _resolveReserveId(additions[i].spoke, additions[i].hub, additions[i].underlying);
            additions[i].spokeConfigurator
                .addDynamicReserveConfig(additions[i].spoke, reserveId, additions[i].dynamicConfig);
        }
    }
    ...
    function executeSpokeDynamicReserveConfigUpdates(IAaveV4ConfigEngine.DynamicReserveConfigUpdate[] calldata updates)
        external
    {
        uint256 length = updates.length;
        for (uint256 i; i < length; ++i) {
            ...
            ISpoke.DynamicReserveConfig memory current =
                ISpoke(updates[i].spoke).getDynamicReserveConfig(reserveId, updates[i].dynamicConfigKey.toUint32());
            ...
            updates[i].spokeConfigurator
                .updateDynamicReserveConfig(
                    updates[i].spoke, reserveId, updates[i].dynamicConfigKey.toUint32(), current
                );
        }
    }
    ...
    function executeSpokePositionManagerUpdates(IAaveV4ConfigEngine.PositionManagerUpdate[] calldata updates) external {
        uint256 length = updates.length;
        for (uint256 i; i < length; ++i) {
            updates[i].spokeConfigurator
                .updatePositionManager(updates[i].spoke, updates[i].positionManager, updates[i].active);
        }
    }

    /// @dev Resolves the reserve ID from spoke, hub, and underlying addresses.
    function _resolveReserveId(address spoke, address hub, address underlying) private view returns (uint256) {
