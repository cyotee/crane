...
library AaveV4DeployOrchestration {
    ...
    function deployAaveV4(
        ...
        report.authorityBatchReport =
            _deployAuthorityBatch({logger: logger, accessManagerAdmin: initialAdmin, salt: salt});
        ...
        report.treasurySpokeBatchReport = _deployTreasurySpokeBatch({
            logger: logger, treasurySpokeOwner: deployInputs.treasurySpokeOwner, salt: salt
        });
        ...
        report.spokeInstanceBatchReports = _deploySpokes({
            logger: logger, authority: accessManager, inputs: deployInputs, spokeBytecode: spokeBytecode, salt: salt
        });
        ...
        if (deployInputs.deployPositionManagers) {
            report.positionManagerBatchReport = _deployPositionManagerBatch({
                logger: logger, positionManagerOwner: deployInputs.positionManagerOwner, salt: salt
            });
        }
        ...
        if (deployInputs.grantRoles) {
            ...
            if (deployInputs.accessManagerAdmin != initialAdmin) {
                logger.logHeader1("granting AccessManager Root Admin role to", deployInputs.accessManagerAdmin);
    ...
    function _deployHub(
        ...
        hubReport.report = _deployHubInstanceBatch({
            logger: logger, proxyAdminOwner: proxyAdminOwner, authority: authority, hubBytecode: hubBytecode, salt: salt
        });
    ...
    function _deployHubInstanceBatch(
        ...
        report = AaveV4DeployBase.deployHubInstanceBatch({
            proxyAdminOwner: proxyAdminOwner, authority: authority, hubBytecode: hubBytecode, salt: salt
        });
        return report;
    }

    function _deployAuthorityBatch(Logger logger, address accessManagerAdmin, bytes32 salt)
        internal
        returns (BatchReports.AuthorityBatchReport memory report)
    {
    ...
    function _deployTreasurySpokeBatch(Logger logger, address treasurySpokeOwner, bytes32 salt)
        internal
        returns (BatchReports.TreasurySpokeBatchReport memory report)
    {
    ...
    function _deployPositionManagerBatch(Logger logger, address positionManagerOwner, bytes32 salt)
        internal
        returns (BatchReports.PositionManagerBatchReport memory report)
    {
    ...
    function _setupConfiguratorRoles(Logger logger, OrchestrationReports.FullDeploymentReport memory report) internal {
    ...
    function _setupSpokeRoles(Logger logger, BatchReports.SpokeInstanceBatchReport memory report, address accessManager)
        internal
    {
        logger.logHeader1("setting Spoke roles");
        AaveV4SpokeRolesProcedure.setupSpokeAllRoles({accessManager: accessManager, spoke: report.spokeProxy});
    }

    function _setupHubRoles(Logger logger, BatchReports.HubInstanceBatchReport memory report, address accessManager)
        internal
    {
    ...
    function _grantHubRoles(
        ...
        logger.logHeader1("granting Hub Configurator roles to", report.configuratorBatchReport.hubConfigurator);
        ...
        AaveV4HubConfiguratorRolesProcedure.grantHubConfiguratorAllRoles({
            accessManager: accessManager, admin: hubConfiguratorAdmin
        });
    }
    ...
    function _grantSpokeRoles(
        ...
        logger.logHeader1("granting Spoke Configurator roles to", report.configuratorBatchReport.spokeConfigurator);
        ...
        AaveV4SpokeConfiguratorRolesProcedure.grantSpokeConfiguratorAllRoles({
            accessManager: accessManager, admin: spokeConfiguratorAdmin
        });
    }

    function _logHubReport(Logger logger, BatchReports.HubInstanceBatchReport memory report, string memory label)
        internal
        pure
    {
        logger.log(label);
    ...
    function _logSpokeReport(Logger logger, BatchReports.SpokeInstanceBatchReport memory report, string memory label)
        internal
        pure
    {
        logger.log(label);
    ...
    function _deriveChildSalt(bytes32 baseSalt, string memory contractType, string memory label)
        internal
        pure
        returns (bytes32)
    {
