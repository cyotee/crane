...
import {
    AaveV4ConfiguratorBatch
} from "@crane/contracts/protocols/lending/aave/v4/deployments/batches/AaveV4ConfiguratorBatch.sol";
import {
    AaveV4GatewayBatch
} from "@crane/contracts/protocols/lending/aave/v4/deployments/batches/AaveV4GatewayBatch.sol";
import {
...
} from "@crane/contracts/protocols/lending/aave/v4/deployments/batches/AaveV4PositionManagerBatch.sol";
import {
...
} from "@crane/contracts/protocols/lending/aave/v4/deployments/batches/AaveV4TokenizationSpokeBatch.sol";
import {
...
library AaveV4DeployBase {
    ...
    function deployAuthorityBatch(address admin, bytes32 salt)
        internal
        returns (BatchReports.AuthorityBatchReport memory)
    {
    ...
    function deployConfiguratorBatch(address hubConfiguratorAuthority, address spokeConfiguratorAuthority, bytes32 salt)
        internal
        returns (BatchReports.ConfiguratorBatchReport memory)
    {
    ...
    function deployTreasurySpokeBatch(address owner, bytes32 salt)
        internal
        returns (BatchReports.TreasurySpokeBatchReport memory)
    {
        AaveV4TreasurySpokeBatch treasurySpokeBatch = new AaveV4TreasurySpokeBatch({owner_: owner, salt_: salt});
    ...
    function deployHubInstanceBatch(address proxyAdminOwner, address authority, bytes memory hubBytecode, bytes32 salt)
        internal
        returns (BatchReports.HubInstanceBatchReport memory)
    {
        AaveV4HubInstanceBatch hubInstanceBatch = new AaveV4HubInstanceBatch({
            proxyAdminOwner_: proxyAdminOwner, authority_: authority, hubBytecode_: hubBytecode, salt_: salt
        });
    ...
    function deployPositionManagerBatch(address owner, bytes32 salt)
        internal
        returns (BatchReports.PositionManagerBatchReport memory)
    {
        AaveV4PositionManagerBatch positionManagerBatch = new AaveV4PositionManagerBatch({owner_: owner, salt_: salt});
