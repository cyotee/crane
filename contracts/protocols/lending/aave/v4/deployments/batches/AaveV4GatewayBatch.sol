...
} from "@crane/contracts/protocols/lending/aave/v4/deployments/procedures/deploy/position-manager/AaveV4NativeTokenGatewayDeployProcedure.sol";
import {
    AaveV4SignatureGatewayDeployProcedure
} from "@crane/contracts/protocols/lending/aave/v4/deployments/procedures/deploy/position-manager/AaveV4SignatureGatewayDeployProcedure.sol";

/// @title AaveV4GatewayBatch
/// @author Aave Labs
/// @notice Deploys the NativeTokenGateway and SignatureGateway contracts, producing a batch report.
contract AaveV4GatewayBatch is AaveV4NativeTokenGatewayDeployProcedure, AaveV4SignatureGatewayDeployProcedure {
    ...
    constructor(
        ...
        if (deployNativeTokenGateway_) {
            nativeGateway = _deployNativeTokenGateway({nativeWrapper: nativeWrapper_, owner: owner_, salt: salt_});
        }
        if (deploySignatureGateway_) {
            signatureGateway = _deploySignatureGateway({owner: owner_, salt: salt_});
        }

        _report = BatchReports.GatewaysBatchReport({signatureGateway: signatureGateway, nativeGateway: nativeGateway});
    }
