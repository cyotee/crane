...
} from "@crane/contracts/protocols/lending/aave/v4/deployments/procedures/deploy/hub/AaveV4HubDeployProcedure.sol";
import {
...
contract AaveV4HubInstanceBatch is AaveV4HubDeployProcedure, AaveV4InterestRateStrategyDeployProcedure {
    ...
    constructor(address proxyAdminOwner_, address authority_, bytes memory hubBytecode_, bytes32 salt_) {
        (address hubProxy, address hubImplementation) = _deployUpgradeableHubInstance({
            proxyAdminOwner: proxyAdminOwner_, authority: authority_, hubBytecode: hubBytecode_, salt: salt_
        });
        address irStrategy = _deployInterestRateStrategy({hub: hubProxy, salt: salt_});

        _report = BatchReports.HubInstanceBatchReport({
            hubImplementation: hubImplementation, hubProxy: hubProxy, irStrategy: irStrategy
        });
    }
