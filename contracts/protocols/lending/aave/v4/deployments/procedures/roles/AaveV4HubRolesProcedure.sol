...
library AaveV4HubRolesProcedure {
    ...
    function setupHubAllRoles(address accessManager, address hub) internal {
        ...
        setupHubRole(accessManager, hub, Roles.HUB_FEE_MINTER_ROLE, Roles.getHubFeeMinterRoleSelectors());
        setupHubRole(
            accessManager, hub, Roles.HUB_DEFICIT_ELIMINATOR_ROLE, Roles.getHubDeficitEliminatorRoleSelectors()
        );
    }
    ...
    function setupHubRole(address accessManager, address hub, uint64 roleId, bytes4[] memory selectors) internal {
        require(accessManager != address(0), "invalid access manager");
        require(hub != address(0), "invalid hub");
        IAccessManager(accessManager).setTargetFunctionRole({target: hub, selectors: selectors, roleId: roleId});
    }
}
