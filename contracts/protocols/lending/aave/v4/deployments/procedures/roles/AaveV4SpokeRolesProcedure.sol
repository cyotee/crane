...
library AaveV4SpokeRolesProcedure {
    ...
    function setupSpokeRole(address accessManager, address spoke, uint64 roleId, bytes4[] memory selectors) internal {
        require(accessManager != address(0), "invalid access manager");
        require(spoke != address(0), "invalid spoke");
        IAccessManager(accessManager).setTargetFunctionRole({target: spoke, selectors: selectors, roleId: roleId});
    }
}
