...
import {
    ITokenizationSpokeInstance
} from "@crane/contracts/protocols/lending/aave/v4/deployments/utils/interfaces/ITokenizationSpokeInstance.sol";
import {
...
contract AaveV4TokenizationSpokeDeployProcedure is AaveV4DeployProcedureBase {
    ...
    function _deployUpgradeableTokenizationSpokeInstance(
        ...
        tokenizationSpokeImplementation =
            Create2Utils.create2Deploy({salt: salt, bytecode: _getTokenizationSpokeInstanceInitCode(hub, underlying)});
        ...
        require(ITokenizationSpoke(tokenizationSpokeProxy).hub() == hub, "tokenization spoke hub mismatch");
        require(
            ITokenizationSpoke(tokenizationSpokeProxy).asset() == underlying, "tokenization spoke underlying mismatch"
        );
    ...
    function _getTokenizationSpokeInstanceInitCode(address hub, address underlying)
        internal
        pure
        returns (bytes memory)
    {
        return abi.encodePacked(type(TokenizationSpokeInstance).creationCode, abi.encode(hub, underlying));
    }
}
