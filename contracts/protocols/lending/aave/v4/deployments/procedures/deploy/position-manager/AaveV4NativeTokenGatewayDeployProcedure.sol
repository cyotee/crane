...
contract AaveV4NativeTokenGatewayDeployProcedure is AaveV4DeployProcedureBase {
    ...
    function _deployNativeTokenGateway(address nativeWrapper, address owner, bytes32 salt) internal returns (address) {
        require(nativeWrapper != address(0), "invalid native wrapper");
        require(owner != address(0), "invalid owner");
        return Create2Utils.create2Deploy({
            salt: salt,
            bytecode: abi.encodePacked(type(NativeTokenGateway).creationCode, abi.encode(nativeWrapper, owner))
        });
    }
}
