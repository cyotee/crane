...
import {IMulticall} from "@crane/contracts/protocols/lending/aave/v4/interfaces/IMulticall.sol";
import {IHubBase} from "@crane/contracts/protocols/lending/aave/v4/hub/interfaces/IHubBase.sol";
import {IExtSload} from "@crane/contracts/protocols/lending/aave/v4/interfaces/IExtSload.sol";
...
interface ISpoke is IAccessManaged, IIntentConsumer, IExtSload, IMulticall {
    ...
    event AddDynamicReserveConfig(
        uint256 indexed reserveId, uint32 indexed dynamicConfigKey, DynamicReserveConfig config
    );
    ...
    event ReportDeficit(
        uint256 indexed reserveId, address indexed user, uint256 drawnShares, IHubBase.PremiumDelta premiumDelta
    );
    ...
    event SetUsingAsCollateral(
        uint256 indexed reserveId, address indexed caller, address indexed user, bool usingAsCollateral
    );
    ...
    function addDynamicReserveConfig(uint256 reserveId, DynamicReserveConfig calldata dynamicConfig)
        external
        returns (uint32 dynamicConfigKey);
    ...
    function getLiquidationBonus(uint256 reserveId, address user, uint256 healthFactor) external view returns (uint256);
