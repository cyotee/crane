...
} from "@crane/contracts/external/openzeppelin-upgradeable/access/Ownable2StepUpgradeable.sol";
...
abstract contract TreasurySpoke is ITreasurySpoke, Ownable2StepUpgradeable {
    ...
    function supply(address hub, address underlying, uint256 amount) external onlyOwner returns (uint256, uint256) {
        return _supply({hub: hub, underlying: underlying, amount: amount, skim: false});
    }

    /// @inheritdoc ITreasurySpoke
    function supplySkimmed(address hub, address underlying, uint256 amount)
        external
        onlyOwner
    ...
    /// @inheritdoc ITreasurySpoke
    function withdraw(address hub, address underlying, uint256 amount) external onlyOwner returns (uint256, uint256) {
        ...
        uint256 withdrawnAmount = MathUtils.min(amount, targetHub.getSpokeAddedAssets(assetId, address(this)));
    ...
    function _supply(address hub, address underlying, uint256 amount, bool skim) internal returns (uint256, uint256) {
        IHubBase targetHub = IHubBase(hub);
        uint256 assetId = targetHub.getAssetId(underlying);
