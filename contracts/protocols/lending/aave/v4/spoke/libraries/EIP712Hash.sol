...
library EIP712Hash {
    ...
    function hash(ISpoke.PositionManagerUpdate calldata params) internal pure returns (bytes32 digest) {
    ...
    function hash(ITokenizationSpoke.TokenizedDeposit calldata params) internal pure returns (bytes32) {
        return
        // keccak256(
        //   abi.encode(
    ...
    function hash(ITokenizationSpoke.TokenizedMint calldata params) internal pure returns (bytes32) {
        ...
        // );
        abi.encode(
                TOKENIZED_MINT_TYPEHASH, params.depositor, params.shares, params.receiver, params.nonce, params.deadline
            )._hash();
    }

    function hash(ITokenizationSpoke.TokenizedWithdraw calldata params) internal pure returns (bytes32) {
        return
        // keccak256(
        //   abi.encode(
        ...
        // );
        abi.encode(
                TOKENIZED_WITHDRAW_TYPEHASH, params.owner, params.assets, params.receiver, params.nonce, params.deadline
            )._hash();
    }

    function hash(ITokenizationSpoke.TokenizedRedeem calldata params) internal pure returns (bytes32) {
        // return
        //   keccak256(
        //     abi.encode(
        ...
        //   );
        return abi.encode(
                TOKENIZED_REDEEM_TYPEHASH, params.owner, params.shares, params.receiver, params.nonce, params.deadline
            )._hash();
    }
}
