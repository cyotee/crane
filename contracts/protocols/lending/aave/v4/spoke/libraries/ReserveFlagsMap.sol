...
library ReserveFlagsMap {
    ...
    function create(bool initPaused, bool initFrozen, bool initBorrowable, bool initReceiveSharesEnabled)
        internal
        pure
        returns (ReserveFlags)
    {
        uint8 flags = 0;
    ...
    /// @return The updated ReserveFlags.
    function setReceiveSharesEnabled(ReserveFlags flags, bool status) internal pure returns (ReserveFlags) {
        return ReserveFlags.wrap(_setStatus(ReserveFlags.unwrap(flags), RECEIVE_SHARES_ENABLED_MASK, status));
    }
