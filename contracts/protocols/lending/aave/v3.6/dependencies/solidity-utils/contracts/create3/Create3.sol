...
library Create3 {
     ...
     */
    bytes internal constant PROXY_CHILD_BYTECODE = hex"630000000980600E6000396000F3363d8037363d34f0ff";
     ...
     */
    function create3(bytes32 salt, bytes memory creationCode, uint256 value) internal returns (address) {
        // Creation code
     ...
     */
    function addressOfWithPreDeployedFactory(bytes32 salt, address preDeployedFactory) internal pure returns (address) {
        address proxy = address(
            uint160(
                uint256(keccak256(abi.encodePacked(hex"ff", preDeployedFactory, salt, KECCAK256_PROXY_CHILD_BYTECODE)))
            )
        );
