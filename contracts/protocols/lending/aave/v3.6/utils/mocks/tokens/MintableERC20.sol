...
contract MintableERC20 is IERC20WithPermit, ERC20 {
    ...
    constructor(string memory name, string memory symbol, uint8 decimals_) ERC20(name, symbol) {
        ...
        //   )
        // );
        DOMAIN_SEPARATOR = abi.encode(
                EIP712_DOMAIN, keccak256(bytes(name)), keccak256(EIP712_REVISION), chainId, address(this)
            )._hash();
        _decimals = decimals_;
    }
        ...
        override
    {
        ...
        bytes32 digest = abi.encodePacked(
                ...
                DOMAIN_SEPARATOR,
                // keccak256(abi.encode(PERMIT_TYPEHASH, owner, spender, value, currentValidNonce, deadline))
                abi.encode(PERMIT_TYPEHASH, owner, spender, value, currentValidNonce, deadline)._hash()
            )._hash();
