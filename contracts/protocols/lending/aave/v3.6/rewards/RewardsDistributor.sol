...
abstract contract RewardsDistributor is IRewardsDistributor {
    ...
    /// @inheritdoc IRewardsDistributor
    function getUserAssetIndex(address user, address asset, address reward) external view override returns (uint256) {
    ...
    function _configureAssets(RewardsDataTypes.RewardsConfigInput[] memory rewardsInput) internal {
        for (uint256 i = 0; i < rewardsInput.length; i++) {
            ...
            uint256 decimals = _assets[rewardsInput[i].asset].decimals = IERC20Metadata(rewardsInput[i].asset)
                .decimals();
