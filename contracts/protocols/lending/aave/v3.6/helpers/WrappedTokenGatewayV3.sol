...
contract WrappedTokenGatewayV3 is IWrappedTokenGatewayV3, Ownable {
    ...
    function repayETH(address, uint256 amount, address onBehalfOf) external payable override {
        uint256 paybackAmount = IERC20(POOL.getReserveVariableDebtToken(address(WETH))).balanceOf(onBehalfOf);
        ...
        POOL.repay(address(WETH), paybackAmount, uint256(DataTypes.InterestRateMode.VARIABLE), onBehalfOf);
    ...
    function borrowETH(address, uint256 amount, uint16 referralCode) external override {
        POOL.borrow(address(WETH), amount, uint256(DataTypes.InterestRateMode.VARIABLE), referralCode, msg.sender);
    ...
    function withdrawETHWithPermit(
        ...
        try aWETH.permit(msg.sender, address(this), amount, deadline, permitV, permitR, permitS) {} catch {}
        // aWETH is trusted, disabling warning
        // forge-lint: disable-next-line(erc20-unchecked-transfer)
        aWETH.transferFrom(msg.sender, address(this), amountToWithdraw);
        POOL.withdraw(address(WETH), amountToWithdraw, address(this));
