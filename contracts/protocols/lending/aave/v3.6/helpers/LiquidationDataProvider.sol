        ...
        returns (LiquidationInfo memory)
    {
        ...
        if (
            !_isReserveReadyForLiquidations({
                    reserveAsset: collateralAsset, reserveConfiguration: collateralReserveData.configuration
                })
                || !_isReserveReadyForLiquidations({
                    reserveAsset: debtAsset, reserveConfiguration: debtReserveData.configuration
                })
        ) {
            return liquidationInfo;
        }
        ...
        if (
            (liquidationInfo.maxDebtToLiquidate != 0
                ...
                || (liquidationInfo.maxCollateralToLiquidate != 0
                    && liquidationInfo.maxCollateralToLiquidate == liquidationInfo.collateralInfo.collateralBalance)
        ) {
