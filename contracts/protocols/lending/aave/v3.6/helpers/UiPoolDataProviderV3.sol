...
contract UiPoolDataProviderV3 is IUiPoolDataProviderV3 {
    ...
    function getReservesList(IPoolAddressesProvider provider) external view override returns (address[] memory) {
        ...
        returns (AggregatedReserveData[] memory, BaseCurrencyInfo memory)
    {
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            DataTypes.ReserveDataLegacy memory baseData = pool.getReserveData(reserveData.underlyingAsset);
            ...
            reserveData.availableLiquidity =
                IERC20Metadata(reserveData.underlyingAsset).balanceOf(reserveData.aTokenAddress);
            ...
            // interest rates
            try IDefaultInterestRateStrategyV2(reserveData.interestRateStrategyAddress)
                .getInterestRateData(reserveData.underlyingAsset) returns (
                IDefaultInterestRateStrategyV2.InterestRateDataRay memory res
            ) {
            ...
            reserveData.virtualUnderlyingBalance = pool.getVirtualUnderlyingBalance(reserveData.underlyingAsset);
        }
