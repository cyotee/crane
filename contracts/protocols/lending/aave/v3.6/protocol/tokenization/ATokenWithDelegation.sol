...
abstract contract ATokenWithDelegation is AToken, BaseDelegation {
    ...
    function _getDelegationState(address user) internal view override returns (DelegationState memory) {
    ...
    function _setDelegationState(address user, DelegationState memory delegationState) internal override {
     ...
     */
    function _transfer(address from, address to, uint256 amount, uint120 scaledAmount, uint256 index)
        internal
        override
    {
        ...
        super._transfer({sender: from, recipient: to, amount: amount, scaledAmount: scaledAmount, index: index});
    }
