...
library FlashLoanLogic {
    ...
    function executeFlashLoan(
        ...
        for (uint256 i = 0; i < params.assets.length; i++) {
            ...
            IAToken(reservesData[params.assets[i]].aTokenAddress)
                .transferUnderlyingTo(params.receiverAddress, vars.currentAmount);
        }

        require(
            vars.receiver
            .executeOperation(params.assets, params.amounts, vars.totalPremiums, params.user, params.params),
        ...
        for (uint256 i = 0; i < params.assets.length; i++) {
            ...
            if (DataTypes.InterestRateMode(params.interestRateModes[i]) == DataTypes.InterestRateMode.NONE) {
    ...
    function _handleFlashLoanRepayment(
        ...
        reserve.accruedToTreasury += params.totalPremium.getATokenMintScaledAmount(reserveCache.nextLiquidityIndex)
            .toUint128();
