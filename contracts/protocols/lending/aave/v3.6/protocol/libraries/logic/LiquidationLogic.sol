...
library LiquidationLogic {
    ...
    function executeEliminateDeficit(
        ...
        reserve.updateInterestRatesAndVirtualBalance(
            reserveCache, params.asset, 0, 0, params.interestRateStrategyAddress
        );
    ...
    function executeLiquidationCall(
        ...
        (vars.totalCollateralInBaseCurrency, vars.totalDebtInBaseCurrency,,, vars.healthFactor,) =
        ...
        vars.borrowerCollateralBalance = IAToken(vars.collateralReserveCache.aTokenAddress)
            .scaledBalanceOf(params.borrower).getATokenBalance(vars.collateralReserveCache.nextLiquidityIndex);
        vars.borrowerReserveDebt = IVariableDebtToken(vars.debtReserveCache.variableDebtTokenAddress)
            .scaledBalanceOf(params.borrower).getVTokenBalance(vars.debtReserveCache.nextVariableBorrowIndex);
        ...
        if (
            ...
            if (vars.borrowerReserveDebtInBaseCurrency > totalDefaultLiquidatableDebtInBaseCurrency) {
                maxLiquidatableDebt =
                    (totalDefaultLiquidatableDebtInBaseCurrency * vars.debtAssetUnit) / vars.debtAssetPrice;
            }
        }
        ...
        if (vars.collateralReserveCache.reserveConfiguration.getDebtCeiling() != 0) {
            ...
            IsolationModeLogic.updateIsolatedDebt(
                reservesData, vars.debtReserveCache, vars.actualDebtToLiquidate, params.collateralAsset
            );
        }
        ...
        if (vars.liquidationProtocolFeeAmount != 0) {
            ...
            uint256 scaledDownLiquidationProtocolFee = vars.liquidationProtocolFeeAmount
                .getATokenTransferScaledAmount(vars.collateralReserveCache.nextLiquidityIndex);
        ...
        IERC20(params.debtAsset)
            .safeTransferFrom(params.liquidator, vars.debtReserveCache.aTokenAddress, vars.actualDebtToLiquidate);
    ...
    function _burnCollateralATokens(
        ...
        IAToken(vars.collateralReserveCache.aTokenAddress)
            .burn({
                ...
                scaledAmount: vars.actualCollateralToLiquidate
                    .getATokenBurnScaledAmount(vars.collateralReserveCache.nextLiquidityIndex),
                index: vars.collateralReserveCache.nextLiquidityIndex
            });
    }
    ...
    function _calculateAvailableCollateralToLiquidate(
        ...
        vars.collateralToLiquidateInBaseCurrency =
            (vars.collateralAmount * vars.collateralAssetPrice) / collateralAssetUnit;
    ...
    function _burnBadDebt(
        ...
        while (unsafe_cachedBorrowerConfig != 0) {
            (unsafe_cachedBorrowerConfig, isBorrowed,) = UserConfiguration.getNextFlags(unsafe_cachedBorrowerConfig);
            if (isBorrowed) {
