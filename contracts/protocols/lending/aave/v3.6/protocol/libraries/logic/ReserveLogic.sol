...
library ReserveLogic {
    ...
    function getNormalizedIncome(DataTypes.ReserveData storage reserve) internal view returns (uint256) {
        ...
        if (timestamp == block.timestamp) {
            ...
            return
                MathUtils.calculateLinearInterest(reserve.currentLiquidityRate, timestamp)
    ...
    function getNormalizedDebt(DataTypes.ReserveData storage reserve) internal view returns (uint256) {
        ...
        if (timestamp == block.timestamp) {
            ...
            return MathUtils.calculateCompoundedInterest(reserve.currentVariableBorrowRate, timestamp)
        ...
        internal
    {
        ...
        uint256 totalDebtAccrued = reserveCache.currScaledVariableDebt
            .rayMulFloor(reserveCache.nextVariableBorrowIndex - reserveCache.currVariableBorrowIndex);

        uint256 amountToMint = totalDebtAccrued.percentMul(reserveCache.reserveFactor);

        if (amountToMint != 0) {
            reserve.accruedToTreasury += amountToMint.getATokenMintScaledAmount(reserveCache.nextLiquidityIndex)
                .toUint128();
        }
    }
