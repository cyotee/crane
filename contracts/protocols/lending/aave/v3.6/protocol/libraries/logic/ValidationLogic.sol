...
library ValidationLogic {
    ...
    function validateBorrow(
        ...
        if (params.userConfig.isBorrowingAny()) {
            ...
            if (vars.siloedBorrowingEnabled) {
                ...
                require(
                    !params.reserveCache.reserveConfiguration.getSiloedBorrowing(), Errors.SiloedBorrowingViolation()
                );
            }
        }
    }
    ...
    function validateLiquidationCall(
        ...
        require(borrowerConfig.isUsingAsCollateral(collateralReserve.id), Errors.CollateralCannotBeLiquidated());
    ...
    function validateHealthFactor(
        ...
        (,,,, uint256 healthFactor, bool hasZeroLtvCollateral) = GenericLogic.calculateUserAccountData(
            ...
            DataTypes.CalculateUserAccountDataParams({
                userConfig: userConfig, user: user, oracle: oracle, userEModeCategory: userEModeCategory
            })
        );
    ...
    function validateHFAndLtvzero(
        ...
        (, bool hasZeroLtvCollateral) = validateHealthFactor(
            reservesData, reservesList, eModeCategories, userConfig, from, userEModeCategory, oracle
        );
        ...
        if (hasZeroLtvCollateral) {
            require(
                getUserReserveLtv(reservesData[asset], eModeCategories[userEModeCategory], userEModeCategory) == 0,
    ...
    function validateSetUserEMode(
        ...
        unchecked {
            while (unsafe_cachedUserConfig != 0) {
                (unsafe_cachedUserConfig, isBorrowed, isEnabledAsCollateral) =
    ...
    function validateAutomaticUseAsCollateral(
        ...
        return validateUseAsCollateral(
            reservesData, reservesList, eModeCategories, userConfig, reserveConfig, asset, categoryId
        );
    }
    ...
    function getUserReserveLtv(
        ...
        if (
            categoryId != 0
                && EModeConfiguration.isReserveEnabledOnBitmap(eModeCategoryData.collateralBitmap, reserveData.id)
        ) {
            if (EModeConfiguration.isReserveEnabledOnBitmap(eModeCategoryData.ltvzeroBitmap, reserveData.id)) {
                return 0;
            } else {
