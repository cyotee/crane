...
abstract contract PoolConfigurator is VersionedInitializable, IPoolConfigurator {
    ...
    /// @inheritdoc IPoolConfigurator
    function setAssetLtvzeroInEMode(address asset, uint8 categoryId, bool ltvzero)
        external
    ...
    function _setReserveLtvzero(address asset, bool ltvZero, DataTypes.ReserveConfigurationMap memory currentConfig)
        internal
    {
        uint256 newLtv;
        ...
        emit CollateralConfigurationChanged(
            asset, newLtv, currentConfig.getLiquidationThreshold(), currentConfig.getLiquidationBonus()
        );
    }
    ...
    function _onlyRiskOrPoolAdmins() internal view {
        IACLManager aclManager = IACLManager(_addressesProvider.getACLManager());
        require(
            aclManager.isRiskAdmin(msg.sender) || aclManager.isPoolAdmin(msg.sender), Errors.CallerNotRiskOrPoolAdmin()
        );
    }
