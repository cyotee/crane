...
import {
    AaveV3TokensBatch
} from "@crane/contracts/protocols/lending/aave/v3.6/deployments/projects/aave-v3-batched/batches/AaveV3TokensBatch.sol";
import {
    AaveV3PoolBatch
} from "@crane/contracts/protocols/lending/aave/v3.6/deployments/projects/aave-v3-batched/batches/AaveV3PoolBatch.sol";
import {
    AaveV3L2PoolBatch
} from "@crane/contracts/protocols/lending/aave/v3.6/deployments/projects/aave-v3-batched/batches/AaveV3L2PoolBatch.sol";
import {
    AaveV3GettersBatchOne
} from "@crane/contracts/protocols/lending/aave/v3.6/deployments/projects/aave-v3-batched/batches/AaveV3GettersBatchOne.sol";
import {
    AaveV3GettersBatchTwo
} from "@crane/contracts/protocols/lending/aave/v3.6/deployments/projects/aave-v3-batched/batches/AaveV3GettersBatchTwo.sol";
import {
    AaveV3GettersProcedureTwo
} from "@crane/contracts/protocols/lending/aave/v3.6/deployments/procedures/AaveV3GettersProcedureTwo.sol";
import {
    AaveV3PeripheryBatch
} from "@crane/contracts/protocols/lending/aave/v3.6/deployments/projects/aave-v3-batched/batches/AaveV3PeripheryBatch.sol";
import {
    AaveV3SetupBatch
} from "@crane/contracts/protocols/lending/aave/v3.6/deployments/projects/aave-v3-batched/batches/AaveV3SetupBatch.sol";
import {
    AaveV3HelpersBatchOne
} from "@crane/contracts/protocols/lending/aave/v3.6/deployments/projects/aave-v3-batched/batches/AaveV3HelpersBatchOne.sol";
import {
    AaveV3HelpersBatchTwo
} from "@crane/contracts/protocols/lending/aave/v3.6/deployments/projects/aave-v3-batched/batches/AaveV3HelpersBatchTwo.sol";
import {
    AaveV3MiscBatch
} from "@crane/contracts/protocols/lending/aave/v3.6/deployments/projects/aave-v3-batched/batches/AaveV3MiscBatch.sol";
...
library AaveV3BatchOrchestration {
    ...
    function deployAaveV3(
        ...
        (variables.setupBatch, variables.initialReport) =
            _deploySetupContract(deployer, roles, config, deployedContracts);

        variables.gettersReport1 = _deployGettersBatch1(
            config.networkBaseTokenPriceInUsdProxyAggregator, config.marketReferenceCurrencyPriceInUsdProxyAggregator
        );

        variables.poolReport = _deployPoolImplementations(
            variables.initialReport.poolAddressesProvider, variables.initialReport.interestRateStrategy, flags
        );

        variables.peripheryReport = _deployPeripherals(
            roles, config, variables.initialReport.poolAddressesProvider, address(variables.setupBatch)
        );
        ...
        variables.setupReport = variables.setupBatch
            .setupAaveV3Market(
                roles,
                config,
        ...
        variables.tokensReport = _deployTokens(
            variables.setupReport.poolProxy, variables.setupReport.rewardsControllerProxy, variables.peripheryReport
        );

        variables.configEngineReport = _deployHelpersBatch1(
            variables.setupReport, variables.miscReport, variables.peripheryReport, variables.tokensReport
        );

        variables.staticATokenReport = _deployHelpersBatch2(
            variables.setupReport.poolProxy, variables.setupReport.rewardsControllerProxy, roles.poolAdmin
        );
    ...
    function _deployGettersBatch2(
        ...
        AaveV3GettersBatchTwo gettersBatch2 = new AaveV3GettersBatchTwo(
            poolProxy, poolAdmin, wrappedNativeToken, poolAddressesProvider, l2Flag
        );
        AaveV3GettersBatchTwo.GettersReportBatchTwo memory gettersReportTwo = gettersBatch2.getGettersReportTwo();

        if (wrappedNativeToken != address(0) || l2Flag) {
            return gettersReportTwo;
        }

        return AaveV3GettersProcedureTwo.GettersReportBatchTwo({
    ...
    function _deployHelpersBatch2(address pool, address rewardsController, address poolAdmin)
        internal
        returns (StaticATokenReport memory)
    {
        AaveV3HelpersBatchTwo helpersBatchTwo = new AaveV3HelpersBatchTwo(pool, rewardsController, poolAdmin);

        return helpersBatchTwo.staticATokenReport();
    }

    function _deployMisc(bool l2Flag, address poolAddressesProvider, address sequencerUptimeOracle, uint256 gracePeriod)
        internal
        returns (MiscReport memory)
    {
        AaveV3MiscBatch miscBatch =
            new AaveV3MiscBatch(l2Flag, poolAddressesProvider, sequencerUptimeOracle, gracePeriod);
    ...
    function _deployPeripherals(
        ...
        AaveV3PeripheryBatch peripheryBatch = new AaveV3PeripheryBatch(
            roles.poolAdmin, config, poolAddressesProvider, setupBatch
        );

        return peripheryBatch.getPeripheryReport();
    }

    function _deployTokens(address poolProxy, address rewardsControllerProxy, PeripheryReport memory peripheryReport)
        internal
        returns (AaveV3TokensBatch.TokensReport memory)
    {
        AaveV3TokensBatch tokensBatch =
            new AaveV3TokensBatch(poolProxy, rewardsControllerProxy, peripheryReport.treasury);
