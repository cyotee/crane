...
} from "@crane/contracts/protocols/lending/aave/v3.6/dependencies/solidity-utils/contracts/transparent-proxy/TransparentProxyFactory.sol";
...
contract AaveV3HelpersProcedureTwo is IErrors {
    function _deployStaticAToken(address pool, address rewardsController, address poolAdmin)
        internal
        returns (StaticATokenReport memory staticATokenReport)
    {
        ...
        staticATokenReport.staticATokenImplementation =
            address(new StataTokenV2(IPool(pool), IRewardsController(rewardsController)));
