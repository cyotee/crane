...
import {
    PriceOracleSentinel,
    ISequencerOracle
} from "@crane/contracts/protocols/lending/aave/v3.6/misc/PriceOracleSentinel.sol";
import {
        ...
        if (l2Flag && sequencerUptimeOracle != address(0) && gracePeriod != 0) {
            miscReport.priceOracleSentinel = address(
                new PriceOracleSentinel(
                    IPoolAddressesProvider(poolAddressesProvider), ISequencerOracle(sequencerUptimeOracle), gracePeriod
                )
            );
        }
