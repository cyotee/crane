...
} from "@crane/contracts/protocols/lending/aave/v3.6/protocol/configuration/PoolAddressesProviderRegistry.sol";
...
contract AaveV3SetupProcedure {
    ...
    function _initialDeployment(
        ...
        report.interestRateStrategy = address(new DefaultReserveInterestRateStrategyV2(report.poolAddressesProvider));
        report.poolAddressesProviderRegistry = _deployPoolAddressesProviderRegistry(
            marketOwner, providerRegistry, report.poolAddressesProvider, providerId
        );
    ...
    function _setupAaveV3Market(
        ...
        report.aclManager = _setupACL(
            roles, initialReport.poolAddressesProvider, report.poolConfiguratorProxy, config.flashLoanPremium
        );
    ...
    function _deployPoolAddressesProviderRegistry(
        ...
        if (providerRegistry == address(0)) {
            ...
            PoolAddressesProviderRegistry(poolAddressesProviderRegistry)
                .registerAddressesProvider(poolAddressesProvider, providerId);
    ...
    function _setupPoolAddressesProvider(AddressProviderInput memory input) internal returns (SetupReport memory) {
        SetupReport memory report;

        IPoolAddressesProvider provider = IPoolAddressesProvider(input.initialReport.poolAddressesProvider);
        ...
        if (input.rewardsControllerProxy == address(0)) {
            ...
            IEmissionManager emissionManager =
                IEmissionManager(IRewardsController(report.rewardsControllerProxy).EMISSION_MANAGER());
    ...
    function _setProtocolDataProvider(InitialReport memory initialReport, address protocolDataProvider) internal {
    ...
    function _configureFlashloanParams(ACLManager manager, address poolConfiguratorProxy, uint128 flashLoanPremium)
        internal
    {
