...
contract AaveV3TreasuryProcedure {
    ...
    function _deployAaveV3Treasury(address poolAdmin, bytes32 collectorSalt) internal returns (TreasuryReport memory) {
        ...
        if (salt != "") {
            ...
            treasuryReport.dustBin =
                address(new TransparentUpgradeableProxy{salt: salt}(treasuryReport.emptyImplementation, poolAdmin, ""));
        } else {
            ...
            treasuryReport.dustBin =
                address(new TransparentUpgradeableProxy(treasuryReport.emptyImplementation, poolAdmin, ""));
        }
