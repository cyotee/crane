...
library MarketReportUtils {
    function toContractsReport(MarketReport memory report)
        internal
        pure
        returns (ContractsReport memory contractsReport)
    {
        contractsReport.poolAddressesProviderRegistry =
            IPoolAddressesProviderRegistry(report.poolAddressesProviderRegistry);
        ...
        contractsReport.poolConfiguratorImplementation = IPoolConfigurator(report.poolConfiguratorImplementation);
        ...
        contractsReport.defaultInterestRateStrategy = IDefaultInterestRateStrategyV2(report.defaultInterestRateStrategy);
        ...
        contractsReport.walletBalanceProvider = WalletBalanceProvider(payable(report.walletBalanceProvider));
        contractsReport.uiIncentiveDataProvider = UiIncentiveDataProviderV3(report.uiIncentiveDataProvider);
        ...
        contractsReport.rewardsControllerImplementation = IRewardsController(report.rewardsControllerImplementation);
