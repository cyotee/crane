...
library RateEngine {
    ...
    function executeRateStrategiesUpdate(
        ...
        (address[] memory ids, IEngine.InterestRateInputData[] memory rates) = _unpackRatesUpdate(updates);
    ...
    function _configRateStrategies(
        ...
        for (uint256 i = 0; i < strategiesParams.length; i++) {
            ...
            if (atLeastOneKeepCurrent) {
                IDefaultInterestRateStrategyV2.InterestRateData memory currentStrategyData =
                    IDefaultInterestRateStrategyV2(rateStrategy).getInterestRateDataBps(asset);
    ...
    function _unpackRatesUpdate(IEngine.RateStrategyUpdate[] memory updates)
        internal
        pure
        returns (address[] memory, IEngine.InterestRateInputData[] memory)
    {
        address[] memory ids = new address[](updates.length);
        IEngine.InterestRateInputData[] memory rates = new IEngine.InterestRateInputData[](updates.length);
