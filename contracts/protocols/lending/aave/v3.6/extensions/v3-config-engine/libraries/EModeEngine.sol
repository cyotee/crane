...
library EModeEngine {
    ...
    function executeEModeCategoriesCreate(
        ...
        for (uint256 i; i < creations.length; i++) {
            require(
                keccak256(abi.encode(creations[i].label)) != keccak256(abi.encode(EngineFlags.KEEP_CURRENT_STRING)),
                "INVALID_LABEL"
            );
            ...
            for (uint256 j; j < creations[i].collaterals.length; j++) {
                engineConstants.poolConfigurator
                    .setAssetCollateralInEMode(creations[i].collaterals[j], categoryId, true);
            }
            for (uint256 k; k < creations[i].borrowables.length; k++) {
                engineConstants.poolConfigurator
                    .setAssetBorrowableInEMode(creations[i].borrowables[k], categoryId, true);
            }
        }
    }
    ...
    function _configAssetsEMode(
        ...
        for (uint256 i = 0; i < updates.length; i++) {
            DataTypes.CollateralConfig memory cfg = pool.getEModeCategoryCollateralConfig(updates[i].eModeCategory);
            ...
            if (updates[i].collateral != EngineFlags.KEEP_CURRENT) {
                poolConfigurator.setAssetCollateralInEMode(
                    updates[i].asset, updates[i].eModeCategory, EngineFlags.toBool(updates[i].collateral)
                );
            }
            if (updates[i].borrowable != EngineFlags.KEEP_CURRENT) {
                poolConfigurator.setAssetBorrowableInEMode(
                    updates[i].asset, updates[i].eModeCategory, EngineFlags.toBool(updates[i].borrowable)
                );
            }
            if (updates[i].ltvzero != EngineFlags.KEEP_CURRENT) {
                ...
                poolConfigurator.setAssetLtvzeroInEMode(
                    updates[i].asset, updates[i].eModeCategory, EngineFlags.toBool(updates[i].ltvzero)
                );
            }
        }
    }
    ...
    function _configEModeCategories(
        ...
        for (uint256 i = 0; i < updates.length; i++) {
            ...
            if (notAllKeepCurrent) {
                ...
                require(
                    updates[i].liqThreshold.percentMul(100_00 + updates[i].liqBonus) <= 100_00, "INVALID_LT_LB_RATIO"
                );
