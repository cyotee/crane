...
library ListingEngine {
    ...
    function executeCustomAssetListing(
        ...
        engineLibraries.priceFeedEngine
            .functionDelegateCall(
                abi.encodeWithSelector(
                    PriceFeedEngine.executePriceFeedsUpdate.selector, engineConstants, repacked.priceFeedsUpdates
                )
            );

        _initAssets(context, engineConstants.poolConfigurator, repacked.ids, repacked.basics, repacked.rates);

        engineLibraries.capsEngine
            .functionDelegateCall(
                abi.encodeWithSelector(CapsEngine.executeCapsUpdate.selector, engineConstants, repacked.capsUpdates)
            );

        engineLibraries.borrowEngine
            .functionDelegateCall(
                abi.encodeWithSelector(
                    BorrowEngine.executeBorrowSide.selector, engineConstants, repacked.borrowsUpdates
                )
            );

        engineLibraries.collateralEngine
                .functionDelegateCall(
                abi.encodeWithSelector(
                    CollateralEngine.executeCollateralSide.selector, engineConstants, repacked.collateralsUpdates
                )
            );
    }

    function _repackListing(IEngine.ListingWithCustomImpl[] calldata listings)
        internal
        pure
        returns (IEngine.RepackedListings memory)
    {
        ...
        IEngine.CollateralUpdate[] memory collateralsUpdates = new IEngine.CollateralUpdate[](listings.length);
        IEngine.PriceFeedUpdate[] memory priceFeedsUpdates = new IEngine.PriceFeedUpdate[](listings.length);
        ...
        IDefaultInterestRateStrategyV2.InterestRateData[] memory rates =
            new IDefaultInterestRateStrategyV2.InterestRateData[](listings.length);

        for (uint256 i = 0; i < listings.length; i++) {
            require(listings[i].base.asset != address(0), "INVALID_ASSET");
            ids[i] = listings[i].base.asset;
            basics[i] = IEngine.Basic({
                assetSymbol: listings[i].base.assetSymbol, implementations: listings[i].implementations
            });
            priceFeedsUpdates[i] =
                IEngine.PriceFeedUpdate({asset: listings[i].base.asset, priceFeed: listings[i].base.priceFeed});
            ...
            rates[i] = IDefaultInterestRateStrategyV2.InterestRateData({
                ...
                baseVariableBorrowRate: listings[i].base.rateStrategyParams.baseVariableBorrowRate.toUint32(),
        ...
        return IEngine.RepackedListings(
            ids, basics, borrowsUpdates, collateralsUpdates, priceFeedsUpdates, capsUpdates, rates
        );
    }
    ...
    function _initAssets(
        ...
        ConfiguratorInputTypes.InitReserveInput[] memory initReserveInputs =
            new ConfiguratorInputTypes.InitReserveInput[](ids.length);
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            initReserveInputs[i] = ConfiguratorInputTypes.InitReserveInput({
                ...
                variableDebtTokenName: string.concat(
                    "Aave ", context.networkName, " Variable Debt ", basics[i].assetSymbol
                ),
                variableDebtTokenSymbol: string.concat(
                    "variableDebt", context.networkAbbreviation, basics[i].assetSymbol
                ),
                params: bytes("")
            });
        }
