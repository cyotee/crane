...
library PriceFeedEngine {
    ...
    function _setPriceFeeds(IAaveOracle oracle, IEngine.PriceFeedUpdate[] memory updates) internal {
        ...
        for (uint256 i = 0; i < updates.length; i++) {
            ...
            require(AggregatorInterface(updates[i].priceFeed).latestAnswer() > 0, "FEED_SHOULD_RETURN_POSITIVE_PRICE");
            require(AggregatorInterface(updates[i].priceFeed).decimals() == 8, "FEED_MUST_USE_8_DECIMALS");
