...
abstract contract ERC20AaveLMUpgradeable is ERC20Upgradeable, IERC20AaveLM {
     ...
     */
    function _getPendingRewards(uint256 balance, uint256 rewardsIndexOnLastInteraction, uint256 currentRewardsIndex)
        internal
        view
        returns (uint256)
    {
     ...
     */
    function _claimRewardsOnBehalf(address onBehalfOf, address receiver, address[] memory rewards) internal virtual {
        for (uint256 i = 0; i < rewards.length; i++) {
            ...
            if (userReward > 0) {
                ...
                $._userRewardsData[onBehalfOf][rewards[i]].rewardsIndexOnLastInteraction =
                    currentRewardsIndex.toUint128();
