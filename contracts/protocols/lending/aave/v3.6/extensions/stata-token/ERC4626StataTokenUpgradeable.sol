...
import {
    ...
    Math,
    IERC4626
} from "@crane/contracts/external/openzeppelin-upgradeable/token/ERC20/extensions/ERC4626Upgradeable.sol";
...
abstract contract ERC4626StataTokenUpgradeable is ERC4626Upgradeable, IERC4626StataToken {
    ...
    function _getERC4626StataTokenStorage() private pure returns (ERC4626StataTokenStorage storage $) {
        assembly {
    ...
    function __ERC4626StataToken_init_unchained(address newAToken) internal onlyInitializing returns (IERC20) {
    ...
    function depositWithPermit(
        ...
        try IERC20Permit(assetToDeposit).permit(_msgSender(), address(this), assets, deadline, sig.v, sig.r, sig.s) {}
            catch {}
    ...
    function maxDeposit(address) public view override returns (uint256) {
        ...
        .mulDiv(_rate(), RAY, Math.Rounding.Ceil);
        return currentSupply >= supplyCap ? 0 : supplyCap - currentSupply;
    }

    ///@inheritdoc IERC4626StataToken
    function latestAnswer() external view returns (int256) {
        uint256 aTokenUnderlyingAssetPrice =
            IAaveOracle(POOL_ADDRESSES_PROVIDER.getPriceOracle()).getAssetPrice(asset());
        // @notice aTokenUnderlyingAssetPrice * rate / RAY
        return int256(aTokenUnderlyingAssetPrice.mulDiv(_rate(), RAY, Math.Rounding.Floor));
    }

    function _deposit(address caller, address receiver, uint256 assets, uint256 shares, bool depositToAave)
        internal
        virtual
    {
        if (shares == 0) {
