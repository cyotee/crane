...
import "@crane/contracts/external/openzeppelin-upgradeable/utils/cryptography/draft-EIP712Upgradeable.sol";
    ...
    TokenHelper
{
    ...
    function _emitEvents(
        ...
        for (uint256 i = 0; i < len; i++) {
            _emitOrderFilledV2(hashes[i], params[i].order, fromMakers[i], toMakers[i], netFees[i], notionalVolumes[i]);
        }
    }

    function _emitOrderFilledV2(
        bytes32 hash,
        Order memory order,
        uint256 fromMaker,
        uint256 toMaker,
