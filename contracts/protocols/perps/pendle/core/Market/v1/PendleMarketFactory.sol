...
import {Errors as PendleErrors} from "@crane/contracts/protocols/perps/pendle/core/libraries/Errors.sol";
...
contract PendleMarketFactory is BoringOwnableUpgradeable, IPMarketFactory {
    ...
    function createNewMarket(address PT, int256 scalarRoot, int256 initialAnchor) external returns (address market) {
        if (!IPYieldContractFactory(yieldContractFactory).isPT(PT)) revert PendleErrors.MarketFactoryInvalidPt();
        if (IPPrincipalToken(PT).isExpired()) revert PendleErrors.MarketFactoryExpiredPt();
    ...
    function setTreasury(address newTreasury) public onlyOwner {
        if (newTreasury == address(0)) revert PendleErrors.MarketFactoryZeroTreasury();
