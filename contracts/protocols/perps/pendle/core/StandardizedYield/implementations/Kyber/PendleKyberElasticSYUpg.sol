...
contract PendleKyberElasticSYUpg is SYBaseWithRewardsUpg, KyberNftManagerBaseUpg {
    ...
    function _previewDeposit(address tokenIn, uint256 amountTokenToDeposit)
        ...
        returns (uint256 amountSharesOut)
    {
        return IKyberMathHelper(kyberMathHelper)
            .previewDeposit(pool, tickLower, tickUpper, tokenIn == token0, amountTokenToDeposit);
    }
    ...
    {
        return IKyberMathHelper(kyberMathHelper)
            .previewRedeem(pool, tickLower, tickUpper, tokenOut == token0, amountSharesToRedeem);
    }
