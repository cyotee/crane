...
contract LinearPreview is BoringOwnableUpgradeable, UUPSUpgradeable {
        ...
        returns (uint256 amountOut)
    {
        ...
        for (uint256 i = 0; i < tokens.length;) {
            if (address(tokens[i]) == address(mainToken)) {
                ...
                imd._mainIndex = i;
            } else if (address(tokens[i]) == address(pool)) {
                imd.pool = pool;
                imd._BPT_INDEX = i;
            } else {
            ...
            if (address(tokens[i]) == address(tokenIn)) {
                indexIn = i;
            } else if (address(tokens[i]) == address(tokenOut)) {
                indexOut = i;
            }
            unchecked {
                i++;
    ...
    function _onSwapGeneral(
        ...
        LinearMath.Params memory params = LinearMath.Params({
            fee: IERC4626LinearPool(imd.pool).getSwapFeePercentage(), lowerTarget: lowerTarget, upperTarget: upperTarget
        });
    ...
    function _onSwapGivenIn(
        ...
        } else if (address(request.tokenIn) == address(imd._wrappedToken)) {
    ...
    function _swapGivenBptIn(
        ...
        // );
        return (address(request.tokenOut) == address(imd._mainToken)
    ...
    function _swapGivenMainIn(
        ...
        //     Errors.INVALID_TOKEN
        // );
        return address(request.tokenOut) == address(imd.pool)
