...
contract ActionSwapYTV3 is CallbackHelper, IPActionSwapYTV3, ActionBase {
    ...
    function swapExactPtForYt(
        ...
        (netYtOut, totalPtToSwap, netSyFee) =
            _readMarket(market).approxSwapExactPtForYt(YT.newIndex(), exactPtIn, block.timestamp, guessTotalPtToSwap);

        _transferFrom(IERC20(PT), msg.sender, market, exactPtIn);

        IPMarket(market)
            .swapExactPtForSy(address(YT), totalPtToSwap, _encodeSwapExactPtForYt(receiver, exactPtIn, minYtOut, YT));
    ...
    function swapExactYtForPt(
        ...
        (netPtOut, totalPtFromSwap, netSyFee) =
            _readMarket(market).approxSwapExactYtForPt(YT.newIndex(), exactYtIn, block.timestamp, guessTotalPtFromSwap);

        if (netPtOut < minPtOut) revert("Slippage: INSUFFICIENT_PT_OUT");

        _transferFrom(IERC20(YT), msg.sender, address(YT), exactYtIn);
        IPMarket(market)
            .swapSyForExactPt(address(this), totalPtFromSwap, _encodeSwapExactYtForPt(receiver, netPtOut, PT, YT));
