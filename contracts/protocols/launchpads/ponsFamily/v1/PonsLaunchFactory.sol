 ...
 */
 ...
 emit WhitelistedLauncherUpdated(launcher, enabled);
    ...
    /// @dev Intermediate launch state — Crane stack-depth adaptation (no behavior change).
    struct LaunchCtx {
        DexConfig dex;
        LaunchConfig config;
        address token;
    ...
    {
        LaunchCtx memory ctx = _prepareLaunch(params, launchConfigId, dexId, salt);
        token = ctx.token;
        _seedPoolAndPosition(ctx);
        _recordLockAndEmit(params, ctx);

        if (ctx.initialBuyAmount != 0) {
            if (ctx.dex.swapRouter == address(0)) revert RouterNotSet();
            // Temporarily unlock the seed recipient for same-block anti-snipe rules.
            PonsLauncherToken(token).setInitialBuyRecipient(ctx.initialBuyRecipient);
            _executeInitialBuy(ctx.dex, ctx.config, token, ctx.initialBuyRecipient, ctx.initialBuyAmount);
    ...
    function _prepareLaunch(
        ...
        uint256 dexId,
        bytes32 salt
    ) private returns (LaunchCtx memory ctx) {
        ...
        if (!ctx.dex.enabled) revert DexDisabled();
        ...
        ctx.initialBuyRecipient = params.feeWallet == address(0) ? msg.sender : params.feeWallet;
        ctx.initialBuyAmount = msg.value - launchFee;

        bytes memory creationCode = _buildTokenCreationCode(params, ctx.config, ctx.dex, msg.sender);
        bytes32 initCodeHash = keccak256(creationCode);

        // Ensure the deployed token address ends in ...bbbb.
        (bytes32 deploySalt,) = _resolveVanitySalt(
            salt, initCodeHash, ctx.config.pairToken, ctx.dex.factory, ctx.dex.poolFee
        );

        _payLaunchFee();
        ctx.token = _deployToken(deploySalt, creationCode);
        emit TokenDeployed(
            ctx.token, msg.sender, ctx.dex.factory, ctx.config.pairToken, dexId, launchConfigId
        );
    }

    function _seedPoolAndPosition(LaunchCtx memory ctx) private {
        // Token ordering flips the concentrated range; mirror the tick accordingly.
        ctx.isToken0 = ctx.token < ctx.config.pairToken;
        int24 poolTick = ctx.isToken0 ? ctx.config.initialTick : -ctx.config.initialTick;
        (int24 tickLower, int24 tickUpper) =
            _positionRange(ctx.isToken0, ctx.config.initialTick, ctx.dex.tickSpacing);

        INonfungiblePositionManagerLike manager =
            INonfungiblePositionManagerLike(ctx.dex.positionManager);
        (address token0, address token1) = ctx.isToken0
            ? (ctx.token, ctx.config.pairToken)
            : (ctx.config.pairToken, ctx.token);
        ctx.pool = manager.createAndInitializePoolIfNecessary(
            token0, token1, ctx.dex.poolFee, PonsTickMath.getSqrtRatioAtTick(poolTick)
        );

        // Deposit the full fixed supply as one-sided liquidity, then clear the allowance.
        IERC20(ctx.token).forceApprove(ctx.dex.positionManager, ctx.config.supply);
        (uint256 positionId,,,) = manager.mint(
            INonfungiblePositionManagerLike.MintParams({
                ...
                tickLower: tickLower,
                tickUpper: tickUpper,
                amount0Desired: ctx.isToken0 ? ctx.config.supply : 0,
                amount1Desired: ctx.isToken0 ? 0 : ctx.config.supply,
                amount0Min: 0,
                amount1Min: 0,
        ...
        IERC20(ctx.token).forceApprove(ctx.dex.positionManager, 0);
        ctx.positionId = positionId;
        ctx.restrictionEndBlock = PonsLauncherToken(ctx.token).restrictionEndBlock();
    }

    function _recordLockAndEmit(TokenParams calldata params, LaunchCtx memory ctx) private {
        _launchedTokens[ctx.token] = LaunchedToken({
            token: ctx.token,
            deployer: msg.sender,
            ...
            restrictionsEndBlock: ctx.restrictionEndBlock,
            supply: ctx.config.supply,
            isToken0: ctx.isToken0,
            poolFee: ctx.dex.poolFee,
            exists: true,
            initialBuyAmount: ctx.initialBuyAmount
        });
        _graduationThresholds[ctx.token] = ctx.config.graduationThreshold;

        // Locker owns the NFT permanently; factory only retains the launch record.
        INonfungiblePositionManagerLike(ctx.dex.positionManager).safeTransferFrom(
            address(this), locker, ctx.positionId
        );
        IPonsLaunchLocker(locker).lockPosition(ctx.token);
        // Point LP fee claims at the creator wallet when the UI supplies one.
        if (params.feeWallet != address(0)) {
            IPonsLaunchLocker(locker).setFeeRedirect(ctx.token, params.feeWallet);
        }
        ...
        emit TokenLaunched(
            ...
            msg.sender,
            ctx.dex.factory,
            ctx.config.pairToken,
            ctx.pool,
            ctx.dexId,
            ctx.launchConfigId,
            ctx.positionId,
            ctx.restrictionEndBlock,
            ctx.initialBuyAmount
        );
    }
 ...
 */
 ...
 return _hasRequiredSuffix(token);
    ...
    function _buildTokenCreationCode(
        ...
        // Crane adaptation: encode `PonsLauncherToken.Init` (struct) so legacy codegen
        // can compile without viaIR; field set matches upstream flat constructor args.
        PonsLauncherToken.Init memory init = PonsLauncherToken.Init({
            name: params.name,
            symbol: params.symbol,
            logo: params.logo,
            description: params.description,
            socials: PonsLauncherToken.Socials({
        ...
        return abi.encodePacked(type(PonsLauncherToken).creationCode, abi.encode(init));
    }
