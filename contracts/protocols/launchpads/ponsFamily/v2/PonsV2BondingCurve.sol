...
/**
 * @notice Constructor args for `PonsV2BondingCurve`, bundled so the constructor
 * (and CREATE2 arg encoding) stays under the 16-slot stack limit without viaIR.
 * Field order matches the original flat constructor ABI members for encoding
 * compatibility with a single-tuple `abi.encode`.
 */
struct PonsV2CurveInit {
    address pairToken;
    address deployer;
    address factory;
...
contract PonsV2BondingCurve is ReentrancyGuard {
    ...
    /**
     * @param init Bundled constructor args (see `PonsV2CurveInit`). Single-struct
     * form is required so legacy (non-IR) codegen can compile the curve; a flat
     * 12-arg constructor overflows the 16-slot stack even with an empty body.
     */
    constructor(PonsV2CurveInit memory init) {
        if (init.deployer == address(0) || init.factory == address(0)) revert ZeroAddress();
        if (address(init.feePolicy) == address(0) || address(init.feeEscrow) == address(0)) revert ZeroAddress();
        if (address(init.buybackVault) == address(0)) revert ZeroAddress();
        if (
            init.policy.protocolFeeRecipient == address(0) || init.policy.protocolFeeShareBps > BASIS_POINTS
                || init.policy.buybackBurnBps > BASIS_POINTS || init.policy.maxInternalPriceImpactBps == 0
                || init.policy.maxInternalPriceImpactBps >= BASIS_POINTS
        ) {
        ...
        if (init.feeBps + init.creatorTaxBps > MAX_TOTAL_TRADE_FEE_BPS) revert InvalidFeePolicy();
        ...
        feePolicy = init.feePolicy;
        feeEscrow = init.feeEscrow;
        buybackVault = init.buybackVault;
        protocolFeeRecipient = init.policy.protocolFeeRecipient;
        buybackCreatorRecipient = init.deployer;
        protocolFeeShareBps = init.policy.protocolFeeShareBps;
        buybackBurnBps = init.policy.buybackBurnBps;
        maxInternalPriceImpactBps = init.policy.maxInternalPriceImpactBps;
        phantomQuote = init.phantomQuote;
        feeBps = init.feeBps;
        creatorTaxBps = init.creatorTaxBps;
        buybackEnabled = init.buybackEnabled;
     ...
     */
    /// @dev Scratch space for `buy` so legacy codegen stays under the stack limit.
    struct BuyCtx {
        address recipient;
        ...
        returns (uint256 tokensOut)
    {
        ...
        ctx.minTokensOut = minTokensOut;
        ctx.received = _receiveQuote(quoteIn);
        if (ctx.received == 0) revert ZeroAmount();
        ...
        // its own.
        _accrueFees(ctx.fee + ctx.snipeTax, ctx.tax);
        trackedQuote += ctx.spent;
        trackedTokens -= tokensOut;
        IERC20(token).safeTransfer(ctx.recipient, tokensOut);

        uint256 refund = ctx.received - ctx.spent;
        if (refund != 0) {
            emit CurveBuyRefunded(msg.sender, refund);
            _sendQuote(msg.sender, refund);
        }

        if (ctx.snipeTax != 0) emit SnipeTaxCharged(ctx.recipient, ctx.snipeTax);
        emit CurveBuy(msg.sender, ctx.recipient, ctx.spent, tokensOut, ctx.fee + ctx.snipeTax, ctx.tax);
        _tryAutoGraduate();
    }

    /**
     * @dev Prices the buy (including partial-fill clamp and slippage check).
     * Split from `buy` so locals live on a memory struct under non-IR codegen.
     */
    function _buyPriceAndFill(BuyCtx memory ctx) private view {
        ctx.quoteReserveBefore = phantomQuote + trackedQuote - quoteFeeBalance - creatorTaxBalance;
        ctx.tokenReserveBefore = trackedTokens;
        ...
        ctx.snipeTaxBps = currentSnipeTaxBps(ctx.recipient);
        if (ctx.snipeTaxBps != 0) {
            uint256 maxSnipeTaxBps = BASIS_POINTS - feeBps - creatorTaxBps - 100;
            if (ctx.snipeTaxBps > maxSnipeTaxBps) ctx.snipeTaxBps = maxSnipeTaxBps;
        }

        ctx.spent = ctx.received;
        ctx.fee = (ctx.spent * feeBps) / BASIS_POINTS;
        ctx.tax = (ctx.spent * creatorTaxBps) / BASIS_POINTS;
        ctx.snipeTax = (ctx.spent * ctx.snipeTaxBps) / BASIS_POINTS;
        ctx.tokensOut = PonsV2BondingCurveMath.getAmountOut(
            ctx.spent - ctx.fee - ctx.tax - ctx.snipeTax, ctx.quoteReserveBefore, ctx.tokenReserveBefore, 0
        );

        ctx.sellable =
            ctx.tokenReserveBefore > reservedTokens ? ctx.tokenReserveBefore - reservedTokens : 0;
        if (ctx.sellable == 0) revert CurveGraduated();

        if (ctx.tokensOut > ctx.sellable) {
            ctx.tokensOut = ctx.sellable;
            // Price the clamped fill from the token side, then gross the
            // result back up so the fee legs still come out of the input.
            uint256 net = PonsV2BondingCurveMath.getAmountIn(
                ctx.sellable, ctx.quoteReserveBefore, ctx.tokenReserveBefore, 0
            );
            ctx.spent = Math.min(
                Math.mulDiv(
                    net,
                    BASIS_POINTS,
                    BASIS_POINTS - feeBps - creatorTaxBps - ctx.snipeTaxBps,
                    Math.Rounding.Ceil
                ),
                ctx.received
            );
        ...
        if (ctx.spent * ctx.minTokensOut > ctx.received * ctx.tokensOut) {
            revert SlippageExceeded(ctx.tokensOut, ctx.minTokensOut);
        }
    }
     ...
     */
    /// @dev Scratch space for `_sweepFees` so legacy codegen stays under the stack limit.
    struct SweepCtx {
        uint256 pending;
        uint256 tax;
        uint256 protocolAmount;
    ...
    function _sweepFees(uint256 minBuybackTokensOut, bool executeBuyback) private {
        SweepCtx memory ctx;
        ctx.pending = quoteFeeBalance;
        ctx.tax = creatorTaxBalance;
        if (ctx.pending == 0 && ctx.tax == 0) return;

        ctx.minBuybackTokensOut = minBuybackTokensOut;
        ctx.executeBuyback = executeBuyback;
        ctx.protocolAmount = (ctx.pending * protocolFeeShareBps) / BASIS_POINTS;
        ctx.creatorBucket = ctx.pending - ctx.protocolAmount;
        ...
        ctx.buybackAmount = ctx.executeBuyback ? Math.min(buybackQuoteBalance, ctx.creatorBucket) : 0;
        ...
        ctx.creatorAmount = ctx.creatorBucket - ctx.buybackAmount + ctx.tax;

        _sweepTryBuyback(ctx);
        ...
        trackedQuote -= ctx.protocolAmount + ctx.creatorAmount;
        ...
        if (ctx.tokensLocked != 0) {
            ...
            trackedTokens -= ctx.tokensLocked;
            IERC20(token).forceApprove(address(buybackVault), ctx.tokensLocked);
            buybackVault.lock(
                token, ctx.tokensLocked, buybackCreatorRecipient, protocolFeeRecipient, protocolFeeShareBps
            );
            emit BuybackLocked(ctx.buybackAmount, ctx.tokensLocked);
        }
        if (ctx.protocolAmount != 0) {
            _creditQuote(protocolFeeRecipient, ctx.protocolAmount);
        }
        if (ctx.creatorAmount != 0) {
            _creditQuote(deployer, ctx.creatorAmount);
        }

        emit FeesSwept(ctx.protocolAmount, ctx.buybackAmount, ctx.creatorAmount);
    }

    /**
     * @dev Optional buyback leg of a fee sweep. Mutates `ctx` only.
     */
    function _sweepTryBuyback(SweepCtx memory ctx) private view {
        if (ctx.buybackAmount == 0) return;
        ...
        uint256 reserveMovementBps = (ctx.buybackAmount * BASIS_POINTS) / (quoteReserve_ + ctx.buybackAmount);
        if (reserveMovementBps <= maxInternalPriceImpactBps) {
            ...
            uint256 tokensOut =
                PonsV2BondingCurveMath.quoteAmountOut(ctx.buybackAmount, quoteReserve_, tokenReserve_, 0);
            ...
            if (tokensOut != 0 && tokensOut <= sellableTokens()) {
                ctx.tokensLocked = tokensOut;
            }
        }
        if (ctx.tokensLocked == 0) {
            // Curve too shallow or the buyback would move its price too
            // far; fold it back into the creator's payout instead.
            ctx.creatorAmount += ctx.buybackAmount;
            ctx.buybackAmount = 0;
        } else if (ctx.tokensLocked < ctx.minBuybackTokensOut) {
