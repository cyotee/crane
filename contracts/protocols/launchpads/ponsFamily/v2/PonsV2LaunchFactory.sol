...
contract PonsV2LaunchFactory is Ownable2Step, ReentrancyGuard, IPonsV2LaunchFactory {
     ...
     */
    /// @dev Locals for `_createPoolAndMintPosition` under non-IR codegen.
    struct MintPositionCtx {
        address token;
        Currency currency0;
        Currency currency1;
    ...
    function _createPoolAndMintPosition(
        ...
        MintPositionCtx memory ctx;
        ctx.token = token;
        (ctx.currency0, ctx.currency1, ctx.memecoinIsCurrency0) = _sortCurrencies(token, launch.pairToken);
        ctx.key = PoolKey({
            currency0: ctx.currency0,
            currency1: ctx.currency1,
            fee: launch.poolFee,
            tickSpacing: launch.tickSpacing,
            hooks: IHooks(address(memeHook))
        });

        (ctx.amount0, ctx.amount1) =
            ctx.memecoinIsCurrency0 ? (tokenAmount, pairTokenAmount) : (pairTokenAmount, tokenAmount);
        if (ctx.amount0 > type(uint128).max || ctx.amount1 > type(uint128).max) revert GraduationSeedNotViable();
        ctx.sqrtPriceX96 = PonsV2GraduationMath.sqrtPriceX96FromAmounts(ctx.amount0, ctx.amount1);
        if (ctx.sqrtPriceX96 <= TickMath.MIN_SQRT_PRICE || ctx.sqrtPriceX96 >= TickMath.MAX_SQRT_PRICE) {
            revert SqrtPriceOutOfBounds();
        }

        poolManager.initialize(ctx.key, ctx.sqrtPriceX96);
        ctx.policy = _launchFeePolicies[token];
        ...
        ctx.buybackCreatorRecipient = PonsV2BondingCurve(launch.curve).buybackCreatorRecipient();
        memeHook.registerPool(
            ctx.key,
            token,
            launch.creatorFeeRecipient,
            ctx.buybackCreatorRecipient,
            launch.creatorTaxBps,
            launch.buybackEnabled,
            ctx.policy
        );

        (ctx.tickLower, ctx.tickUpper) = _fullRangeTicks(launch.tickSpacing);
        positionId = positionManager.nextTokenId();

        ctx.nativeValue = _fundGraduationExecutor(ctx.currency0, ctx.currency1, ctx.amount0, ctx.amount1);
        graduationExecutor.mintFullRangePosition{value: ctx.nativeValue}(
            token,
            ctx.key,
            ctx.tickLower,
            ctx.tickUpper,
            ...
            ctx.currency0,
            ctx.currency1,
            ctx.policy.protocolFeeRecipient
        );
