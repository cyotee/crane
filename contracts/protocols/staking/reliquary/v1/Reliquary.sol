...
contract Reliquary is IReliquary, Multicall, ERC721, AccessControlEnumerable, ReentrancyGuard {
    ...
    function setEmissionRate(uint256 _emissionRate) external onlyRole(EMISSION_RATE) {
        ReliquaryService._massUpdatePools(poolInfo, emissionRate, totalAllocPoint);
        emissionRate = _emissionRate;
    ...
    function updatePool(uint8 _poolId) external nonReentrant {
        ReliquaryService._updatePool(poolInfo[_poolId], emissionRate, totalAllocPoint);
    }
    ...
    function emergencyWithdraw(uint256 _relicId) external nonReentrant {
        ...
        ReliquaryService._updatePool(pool, emissionRate, totalAllocPoint);
    ...
    function shift(uint256 _fromId, uint256 _toId, uint256 _amount) public nonReentrant {
        ...
        toPosition.entry = ReliquaryService._weightEntry(
        ...
        vars_.newToLevel = ReliquaryService._updateLevel(toPosition, vars_.oldToLevel);
        ...
        returns (uint8 poolId_)
    {
        ...
        uint256 received_ = ReliquaryService._updateRelic(
            position,
            poolInfo[poolId_],
            _kind,
            _relicId,
