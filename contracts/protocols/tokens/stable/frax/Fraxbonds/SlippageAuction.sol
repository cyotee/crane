...
contract SlippageAuction {
    ...
    function createAuction(
        ...
        uint128 _startPrice,
        uint128 _priceDecay,
        uint128 _priceSlippage
    ) external returns (uint256 _auctionNo) {
    ...
    function buy(uint256 _auctionNo, uint128 _amountIn, uint256 _minAmountOut) external returns (uint128 amountOut) {
    ...
    function getAmountOut(uint256 _auctionNo, uint128 _amountIn) external view returns (uint128 amountOut) {
        // Get the auction info
        Auction memory auction = auctions[_auctionNo];
        if (auction.exited) revert AuctionAlreadyExited();
    ...
    error MinAmountOut(uint256 _min_out, uint128 _actual_out);
    ...
    event AuctionExited(uint256 auctionNo);

    /// @dev When someone buys from an auction
    event Buy(uint256 auctionNo, address buyToken, address sellToken, uint128 amountIn, uint128 amountOut);
