...
import "@crane/contracts/protocols/tokens/stable/frax/Misc_AMOs/kyberswap_v2/IFactory.sol";
...
import "@crane/contracts/protocols/tokens/stable/frax/Misc_AMOs/kyberswap_v2/periphery/IBasePositionManager.sol";
import "@crane/contracts/protocols/tokens/stable/frax/Misc_AMOs/kyberswap_v2/IPool.sol";
import "@crane/contracts/protocols/tokens/stable/frax/Misc_AMOs/kyberswap_v2/periphery/IRouter.sol";
import "@crane/contracts/protocols/tokens/stable/frax/Misc_AMOs/kyberswap_v2/ITickFeesReader.sol";
...
contract ComboOracle_KyberSwapElasticV2 is Owned {
    ...
    function getNFTBasicInfo(uint256 token_id) public view returns (NFTBasicInfo memory) {
        ...
        (IBasePositionManager.Position memory pos, IBasePositionManager.PoolInfo memory info) =
    ...
    function getNFTValuationMiddleInputs(uint256 token_id)
        public
        view
        returns (NFTValuationMiddleInputs memory midInputs)
    {
    ...
    function getNFTValueInfo(uint256 token_id) public view returns (NFTValueInfo memory nftvi) {
        ...
        // Tick math
        {
            ...
            nftvi.token0_val_usd = (token0_1pm_amt * liq_pricing_divisor * token0_precise_price * token0_miss_dec_mult)
                / PRECISE_PRICE_PRECISION;
            nftvi.token1_val_usd = (token1_1pm_amt * liq_pricing_divisor * token1_precise_price * token1_miss_dec_mult)
                / PRECISE_PRICE_PRECISION;

            // Get the pool TVL
            nftvi.pool_tvl_usd =
                (ERC20(lp_basic_info.token0).balanceOf(pool_address) * token0_precise_price * token0_miss_dec_mult)
                    / PRECISE_PRICE_PRECISION;
            nftvi.pool_tvl_usd += (ERC20(lp_basic_info.token1).balanceOf(pool_address)
                    * token1_precise_price
                    * token1_miss_dec_mult) / PRECISE_PRICE_PRECISION;
        }
        ...
        returns (uint256 liquidity, int24 tick_lower, int24 tick_upper)
    {
        ...
        (IBasePositionManager.Position memory pos_seed, IBasePositionManager.PoolInfo memory info_seed) =
            kyber_positions_mgr.positions(seed_nft_id);

        // Get the test NFT info
        (IBasePositionManager.Position memory pos_test, IBasePositionManager.PoolInfo memory info_test) =
