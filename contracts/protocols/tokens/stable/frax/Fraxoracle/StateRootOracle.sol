...
contract StateRootOracle is IStateRootOracle {
    ...
    constructor(IBlockhashProvider[] memory _providers, uint256 _minProviders) {
        owner = msg.sender;
    ...
    function setMinProviders(uint256 _minProviders) external {
    ...
    function removeProvider(IBlockhashProvider provider) external {
        if (msg.sender != owner) revert("Not owner");
        for (uint256 i = 0; i < providers.length; i++) {
            if (address(providers[i]) == address(provider)) {
