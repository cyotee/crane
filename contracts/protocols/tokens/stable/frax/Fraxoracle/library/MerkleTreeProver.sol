...
library MerkleTreeProver {
   ...
   function proofStorageRoot(bytes32 stateRootHash, address proofAddress, bytes[] memory _proofBytesArray) internal pure returns (Verifier.Account memory accountPool) {
   ...
   function proofStorageSlotValue(bytes32 storageRoot, bytes32 slot, bytes[] memory _proofBytesArray) internal pure returns (Verifier.SlotValue memory slotValue) {
      RLPReader.RLPItem[] memory proof = new RLPReader.RLPItem[](_proofBytesArray.length);
      for (uint i=0;i<_proofBytesArray.length;i++) proof[i] = _proofBytesArray[i].toRlpItem();
