...
contract KashiAMO is Owned {
    ...
    // Key Kashi Pairs
    IKashiPairMediumRiskV1 private wEthFraxKashiPair =
        IKashiPairMediumRiskV1(0x9f357F839d8bB4EcC179A20eb10bB77B888686ff);
    ...
    /// @return allocations Array: [0]=Unallocated, [1]=Allocated, [2]=Total (TODOs noted in body)
    function showAllocations() public view returns (uint256[3] memory allocations) {
        ...
        // allocations[3] = 0;
        // Frax in Kashi pairs
        for (uint256 i = 0; i < kashipairs_array.length; i++) {
    ...
    function fraxToKashipairFraction(address kashipair_address, uint256 frax_amount) public view returns (uint256) {
    ...
    /// @return frax amount
    function kashipairFractionToFrax(address kashipair_address, uint256 fraction) public view returns (uint256) {
        require(kashipairs_check[kashipair_address], "KashiAMO: Add Kashipair address to AMO");
        IKashiPairMediumRiskV1 newKashipair = IKashiPairMediumRiskV1(kashipair_address);
        (uint256 kashipair_totalAsset_elastic, uint256 kashipair_totalAsset_base) = newKashipair.totalAsset();
    ...
    function getPairGeneralInfo(address kashipair_address)
        public
        view
        returns (address[5] memory kashipair_important_addresses)
    {
        require(kashipairs_check[kashipair_address], "KashiAMO: Add Kashipair address to AMO");
        IKashiPairMediumRiskV1 newKashipair = IKashiPairMediumRiskV1(kashipair_address);
    ...
    function getPairFinancialInfo(address kashipair_address)
        public
        view
        returns (uint256[7] memory kashipair_financial_info)
    {
        require(kashipairs_check[kashipair_address], "KashiAMO: Add Kashipair address to AMO");
        IKashiPairMediumRiskV1 newKashipair = IKashiPairMediumRiskV1(kashipair_address);
        ...
        uint256 fullAssetAmount =
            SUSHIBentoBox.toAmount(fraxAddress, _totalAsset_elastic, false) + (_totalBorrow_elastic);
        uint256 utilization = (uint256(_totalBorrow_elastic) * (UTILIZATION_PRECISION)) / fullAssetAmount;
    ...
    function depositToPair(address kashipair_address, uint256 frax_amount) public onlyByOwnGov {
        require(kashipairs_check[kashipair_address], "KashiAMO: Add Kashipair address to AMO");
        IKashiPairMediumRiskV1 newKashipair = IKashiPairMediumRiskV1(kashipair_address);
        uint256 _share = 0;
        ...
        if (!SUSHIBentoBox.masterContractApproved(address(newKashipair.masterContract()), address(this))) {
            ...
            SUSHIBentoBox.setMasterContractApproval(
                address(this), address(newKashipair.masterContract()), true, v, r, s
            );
        }
    ...
    function withdrawFromPair(address kashipair_address, uint256 frax_amount) public onlyByOwnGov {
        require(kashipairs_check[kashipair_address], "KashiAMO: Add Kashipair address to AMO");
        IKashiPairMediumRiskV1 newKashipair = IKashiPairMediumRiskV1(kashipair_address);
    ...
    /// @param frax_amount Amount of Frax to be withdrawed
    function withdrawFromBentobox(uint256 frax_amount) public onlyByOwnGov {
        require(
            SUSHIBentoBox.toAmount(fraxAddress, SUSHIBentoBox.balanceOf(fraxAddress, address(this)), false)
                >= frax_amount,
            "KashiAMO: KashiAMO fund in Bentobox is low"
        );
    ...
    function createNewPair(address collateralAddress, address collatralOracleAddress, uint256 _decimals)
        public
        onlyByOwnGov
        returns (address)
    {
