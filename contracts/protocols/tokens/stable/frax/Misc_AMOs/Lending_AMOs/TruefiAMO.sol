...
contract TruefiAMO is Owned {
    ...
    function showAllocations() public view returns (uint256[8] memory allocations) {
        ...
        for (uint256 i = 0; i < frax_based_truefi_managedportfolio_array.length; i++) {
            ...
            if (frax_based_truefi_managedportfolios[pool_address]) {
                sum_frax_in_managed_portfolio =
                    sum_frax_in_managed_portfolio + (balanceInManagedPortfolio(pool_address));
            }
        }
        ...
        for (uint256 i = 0; i < usdc_based_truefi_managedportfolio_array.length; i++) {
            ...
            if (usdc_based_truefi_managedportfolios[pool_address]) {
                sum_usdc_in_managed_portfolio =
                    sum_usdc_in_managed_portfolio + (balanceInManagedPortfolio(pool_address));
            }
        }
    ...
    // E18
    function truefiDepositManagedPortfolioFRAX(uint256 frax_amount, address pool_address)
        public
        onlyByOwnGovCust
        validFRAXManagedPortfolio(pool_address)
    {
    ...
    // E18
    function truefiWithdrawManagedPortfolioFRAX(uint256 share, address pool_address)
        public
        onlyByOwnGovCust
        validFRAXManagedPortfolio(pool_address)
    {
    ...
    // E6
    function truefiDepositManagedPortfolioUSDC(uint256 usdc_amount, address pool_address)
        public
        onlyByOwnGovCust
        validUSDCManagedPortfolio(pool_address)
    {
    ...
    // E6
    function truefiWithdrawManagedPortfolioUSDC(uint256 share, address pool_address)
        public
        onlyByOwnGovCust
        validUSDCManagedPortfolio(pool_address)
    {
