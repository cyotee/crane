...
interface IKSReinvestmentTokenPool {
    ...
    function mint(
        ...
        uint128 qty,
        bytes memory data
    ) external returns (uint256 qty0, uint256 qty1, uint256 feeGrowthInsideLast);
    function name() external view returns (string memory);
    function poolOracle() external view returns (address);
    function swap(address recipient, int256 swapQty, bool isToken0, uint160 limitSqrtP, bytes memory data)
        external
