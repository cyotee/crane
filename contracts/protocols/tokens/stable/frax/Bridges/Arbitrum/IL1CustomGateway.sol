...
interface IL1CustomGateway {
    ...
    function getExternalCall(uint256 _exitNum, address _initialDestination, bytes calldata _initialData)
        external
        view
        returns (address target, bytes calldata data);
    function getOutboundCalldata(address _l1Token, address _from, address _to, uint256 _amount, bytes calldata _data)
        external
        view
        returns (bytes calldata outboundCalldata);
    ...
    ) external payable returns (bytes calldata res);
