...
import "@crane/contracts/external/uniswap/v3-core/contracts/libraries/SafeCast.sol";
import "@crane/contracts/external/uniswap/v2-periphery/contracts/interfaces/IWETH.sol";
import "@crane/contracts/external/uniswap/v3-periphery/contracts/libraries/TransferHelper.sol";
import "@crane/contracts/external/uniswap/v3-core/contracts/interfaces/callback/IUniswapV3SwapCallback.sol";
import "@crane/contracts/external/uniswap/v3-core/contracts/interfaces/IUniswapV3Pool.sol";
import "@crane/contracts/external/uniswap/v2-core/contracts/interfaces/IUniswapV2Pair.sol";
...
contract FraxswapRouterMultihop is ReentrancyGuard, Ownable {
        ...
        returns (uint256 amountOut)
    {
        if (params.tokenIn == address(0)) {
            ...
            if (params.v != 0) {
                ...
                IERC20Permit(params.tokenIn)
                    .permit(msg.sender, address(this), amount, params.deadline, params.v, params.r, params.s);
            }
    ...
    function uniswapV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata _data) external {
        ...
        if (!data.directFundThisPool) {
            ...
            TransferHelper.safeTransfer(
                data.tokenIn, msg.sender, uint256(amount0Delta > 0 ? amount0Delta : amount1Delta)
            );
        }
    }
    ...
    function executeSwap(FraxswapRoute memory prevRoute, FraxswapRoute memory route, FraxswapStepData memory step)
        internal
        returns (uint256 amountOut)
    {
        ...
        if (step.swapType < 2) {
            ...
            // Uni v3
            bool zeroForOne = prevRoute.tokenOut < step.tokenOut;
            (int256 amount0, int256 amount1) = IUniswapV3Pool(step.pool)
                .swap(
                    step.directFundNextPool == 1 ? getNextDirectFundingPool(route) : address(this),
                    zeroForOne,
                    amountIn.toInt256(),
                    zeroForOne ? 4295128740 : 1461446703485210103287273052203988822378723970341, // Do not fail because of price
                    abi.encode(
                        SwapCallbackData({
                            tokenIn: prevRoute.tokenOut, directFundThisPool: step.directFundThisPool == 1
                        })
                    )
                );
            ...
            amountOut = PoolInterface(step.pool).exchange{value: value}(
                int128(int256(step.extraParam1)), int128(int256(step.extraParam2)), amountIn, 0
            );
            ...
            TransferHelper.safeApprove(prevRoute.tokenOut, step.pool, amountIn);
            amountOut = PoolInterface(step.pool)
                .exchange_underlying(int128(int256(step.extraParam1)), int128(int256(step.extraParam2)), amountIn, 0);
        } else if (step.swapType == 6) {
            // Saddle
            TransferHelper.safeApprove(prevRoute.tokenOut, step.pool, amountIn);
            amountOut = PoolInterface(step.pool)
                .swap(uint8(step.extraParam1), uint8(step.extraParam2), amountIn, 0, block.timestamp);
    ...
    function encodeRoute(address tokenOut, uint256 percentOfHop, bytes[] memory steps, bytes[] memory nextHops)
        external
        pure
    ...
    function getAmountForPct(uint256 pctOfHop1, uint256 pctOfHop2, uint256 amountIn)
        internal
        pure
        returns (uint256 amountOut)
    {
...
interface PoolInterface {
    ...
    function swap(address tokenIn, address tokenOut, uint256 amountIn, address target)
        external
        returns (uint256 amountOut);
}
