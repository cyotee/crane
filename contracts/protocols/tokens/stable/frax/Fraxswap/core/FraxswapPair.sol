...
import "@crane/contracts/protocols/tokens/stable/frax/Fraxswap/twamm/LongTermOrders.sol";
...
contract FraxswapPair is IUniswapV2PairPartialV5, FraxswapERC20 {
    ...
    event WithdrawProceedsFromLongTermOrder(
        address indexed addr, uint256 orderId, address indexed proceedToken, uint256 proceeds, bool orderExpired
    );
    ...
    function price0CumulativeLast() public view override returns (uint256) {
        return TWAPObservationHistory.length > 0
    ...
    function getDetailedOrdersForUser(address user, uint256 offset, uint256 limit)
        external
        view
        returns (LongTermOrdersLib.Order[] memory detailed_orders)
    {
    ...
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1, uint32 timeElapsed)
        private
    {
        if (!(balance0 + twammReserve0 <= type(uint112).max && balance1 + twammReserve1 <= type(uint112).max)) {
            revert Uint112Overflow(); // OVERFLOW
        }
        uint32 blockTimestamp = uint32(block.timestamp % 2 ** 32);
        ...
        execVirtualOrders
    {
        ...
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
            uint256 minusFee = 10000 - fee;
            uint256 balance0Adjusted = (balance0 * 10000) - (amount0In * minusFee);
            uint256 balance1Adjusted = (balance1 * 10000) - (amount1In * minusFee);
            if (!(balance0Adjusted * balance1Adjusted >= uint256(_reserve0) * _reserve1 * (10000 ** 2))) {
                revert KConstantError(); // K
            }
        }
    ...
    function transferAmountIn(address token, uint256 amountIn) internal returns (uint256) {
        ...
        (bool success, bytes memory data) =
            token.call(abi.encodeWithSelector(0x23b872dd, msg.sender, address(this), amountIn));
    ...
    function cancelLongTermSwap(uint256 orderId) external lock execVirtualOrders {
        (address sellToken, uint256 unsoldAmount, address buyToken, uint256 purchasedAmount) =
            longTermOrders.cancelLongTermSwap(orderId);
    ...
    {
        (address proceedToken, uint256 proceeds, bool orderExpired) =
            longTermOrders.withdrawProceedsFromLongTermSwap(orderId);
    ...
    function executeVirtualOrdersInternal(uint256 blockTimestamp) internal {
        ...
        LongTermOrdersLib.ExecuteVirtualOrdersResult memory result =
            LongTermOrdersLib.ExecuteVirtualOrdersResult(reserve0, reserve1, twammReserve0, twammReserve1, fee);
    ...
    function getTwammRewardFactor(uint256 _blockTimestamp)
        ...
        view
        override
        returns (uint256 rewardFactorPool0AtTimestamp, uint256 rewardFactorPool1AtTimestamp)
    {
        uint256 lastExpiryTimestamp = _blockTimestamp - (_blockTimestamp % orderTimeInterval);
    ...
    {
        require(orderId < longTermOrders.orderId); // INVALID ORDERID
        LongTermOrdersLib.OrderPool storage orderPool =
            LongTermOrdersLib.getOrderPool(longTermOrders, longTermOrders.orderMap[orderId].sellTokenAddr);
