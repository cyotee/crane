...
import {
    IUniswapV2PairV5
} from "@crane/contracts/protocols/tokens/stable/frax/Fraxswap/core/interfaces/IUniswapV2PairV5.sol";
import {FraxswapERC20} from "@crane/contracts/protocols/tokens/stable/frax/Fraxswap/core/FraxswapERC20.sol";
import {Math} from "@crane/contracts/protocols/tokens/stable/frax/Math/Math.sol";
import {UQ112x112} from "@crane/contracts/protocols/tokens/stable/frax/Math/UQ112x112.sol";
import {IERC20V5} from "@crane/contracts/protocols/tokens/stable/frax/Fraxswap/core/interfaces/IERC20V5.sol";
import {
    IUniswapV2FactoryV5
} from "@crane/contracts/protocols/tokens/stable/frax/Fraxswap/core/interfaces/IUniswapV2FactoryV5.sol";
import {
    IUniswapV2CalleeV5
} from "@crane/contracts/protocols/tokens/stable/frax/Fraxswap/core/interfaces/IUniswapV2CalleeV5.sol";

contract FraxswapRangePair is FraxswapERC20 {
    ...
    function initialize(address _token0, address _token1, uint256 _centerPrice, uint256 _rangeSize, uint256 _fee)
        external
    {
    ...
    function rangeSize() public view returns (uint256 size) {
    ...
    function _update(
        ...
        require(balance0 <= uint112(type(uint256).max) && balance1 <= uint112(type(uint256).max), "UniswapV2: OVERFLOW");
        ...
        if (
            forceUpdateVirtualReserves || K * (10000 - fee) / 10000 > lastVirtualReserveUpdateK
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        uint256 amount0 = balance0 + (virtualReserve0) - (_reserve0);
        uint256 amount1 = balance1 + (virtualReserve1) - (_reserve1);

        bool feeOn = _mintFee(_reserve0, _reserve1);
        uint256 _totalSupply = totalSupply; // gas savings, must be defined here since totalSupply can update in _mintFee
        if (_totalSupply == 0) {
            liquidity = Math.sqrt(uint256(_reserve0) * (uint256(_reserve1))) - (MINIMUM_LIQUIDITY);
            _mint(address(0), MINIMUM_LIQUIDITY); // permanently lock the first MINIMUM_LIQUIDITY tokens
        } else {
            liquidity = Math.min(
                amount0 * (_totalSupply) / (_reserve0 - virtualReserve0),
                amount1 * (_totalSupply) / (_reserve1 - virtualReserve1)
            );
        }
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external lock {
        ...
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
            ...
            uint256 balance0Adjusted = balance0 * (10000) - (amount0In * (_fee));
            uint256 balance1Adjusted = balance1 * (10000) - (amount1In * (_fee));
            require(
                balance0Adjusted * (balance1Adjusted) >= uint256(_reserve0) * (_reserve1) * (10000 ** 2), "UniswapV2: K"
            );
        }
    ...
    function sync() external lock {
        _update(
            IERC20V5(token0).balanceOf(address(this)) + (virtualReserve0),
            IERC20V5(token1).balanceOf(address(this)) + (virtualReserve1),
            reserve0,
            reserve1,
            true
        );
