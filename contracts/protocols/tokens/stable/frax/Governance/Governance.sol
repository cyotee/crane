...
contract GovernorAlpha {
    ...
    function quorumVotes() public pure returns (uint256) {
        return 4000000e18; // 4,000,000 = 4% of FXS
    }

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public pure returns (uint256) {
        return 1000000e18; // 1,000,000 = 1% of FXS
    }
    ...
    constructor(address timelock_, address fxs_, address guardian_) {
    ...
    function propose(
        ...
        emit ProposalCreated(
            proposalId, msg.sender, targets, values, signatures, calldatas, startBlock, endBlock, description
        );
        return proposalId;
    }
    ...
    function castVoteBySig(uint256 proposalId, bool support, uint8 v, bytes32 r, bytes32 s) public {
        ...
        bytes32 domainSeparator =
            abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(name)), getChainId(), address(this))._hash();
