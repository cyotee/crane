...
contract SigRelayer2 {
	...
	function relayBySigsGST(DelegationSig[] memory s1, VoteSig[] memory s2) public discountGST2 {
		for (uint i = 0; i < s1.length; i++) {
			...
			compToken.functionCall(abi.encodeWithSignature("delegateBySig(address,uint256,uint256,uint8,bytes32,bytes32)", sig.delegatee, sig.nonce, sig.expiry, sig.v, sig.r, sig.s));
		}
		for (uint i = 0; i < s2.length; i++) {
			...
			governorBravo.functionCall(abi.encodeWithSignature("castVoteBySig(uint256,uint8,uint8,bytes32,bytes32)", sig.proposalId,sig.support,sig.v,sig.r,sig.s));
		}
	}
	...
	function signatoryFromVoteSig(uint proposalId, uint8 support, uint8 v, bytes32 r, bytes32 s) public view returns (address) {
	    ...
	    bytes32 domainSeparator = abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(name2)), getChainId(), governorBravo)._hash();
