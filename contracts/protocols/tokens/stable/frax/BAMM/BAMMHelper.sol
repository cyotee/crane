...
contract BAMMHelper {
    ...
    function getAmountOut(uint112 reserveIn, uint112 reserveOut, uint256 fee, uint256 amountIn)
        internal
        pure
        returns (uint256)
    {
    ...
    function getSwapAmount(int256 RES_A, int256 RES_B, int256 TKN_A_AMT, int256 TKN_B_AMT, int256 fee)
        public
        view
        returns (int256 result)
    {
        ...
        if (TKN_A_AMT * RES_B >= TKN_B_AMT * RES_A) {
            ...
            int256 XA = RES_A + TKN_A_AMT;
            int256 YB = RES_B + TKN_B_AMT;
            int256 resultOut;
            int256 diffResult;
    ...
    function getSwapAmountSolve(int256 RES_A, int256 RES_B, int256 TKN_A_AMT, int256 TKN_B_AMT, int256 fee)
        public
        pure
        returns (int256 result)
    {
    ...
    /// @param TKN_B_AMT Amount of token B coming in
    /// @return result The amount that needs to be swapped. Positive = tokenA is swapped out, Negative = tokenB is swapped out.
    function _getSwapAmountSolve(int256 RES_A, int256 RES_B, int256 TKN_A_AMT, int256 TKN_B_AMT, int256 fee)
        internal
        pure
        returns (int256 result)
    {
        ...
        uint256 div_c;
        {
            (int256 c1, uint256 div_c1) = mul(RES_A * RES_A, TKN_B_AMT);
            (int256 c2, uint256 div_c2) = mul(RES_A * RES_B, TKN_A_AMT);
            if (div_c1 > div_c2) {
        ...
        (int256 b2, uint256 div_b2) = mul(b, b);
        (int256 ac4, uint256 div_ac4) = mul(4 * a, c);
        div_ac4 += div_c;
        int256 s;
        uint256 div_s;
        if (div_b2 > div_ac4) {
    ...
    function mul(int256 a, int256 b) internal pure returns (int256 result, uint256 div) {
        uint256 log_a = log2(a);
        uint256 log_b = log2(b);
        if ((log_a + log_b) > 252) {
            div = log_a + log_b - 252;
            uint256 div_a = (log_a * div) / (log_a + log_b);
            uint256 div_b = div - div_a;
