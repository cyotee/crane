...
import "@crane/contracts/external/openzeppelin-contracts/utils/ReentrancyGuard.sol";
...
contract BAMM is ERC20, ERC20Burnable, ReentrancyGuard {
    ...
    uint256 public immutable pairFee;
    ...
    uint256 public constant MIN_RATE = 1 * PRECISION; // 1%

    /// @notice Interest rate after the first kink (70% utility)
    uint256 public constant KINK1_RATE = 10 * PRECISION; // 10%

    /// @notice Interest rate after the second kink (80% utility)
    uint256 public constant KINK2_RATE = 100 * PRECISION; // 100%

    /// @notice The maximum interest rate
    uint256 public constant MAX_RATE = 730 * PRECISION; // 730% = 2% per day

    /// @notice Percent above which the position is considered insolvent
    uint256 public constant SOLVENCY_THRESHOLD = (98 * PRECISION) / 100; // 98%

    /// @notice Protocol's cut of the interest rate
    uint256 public constant FEE_SHARE = (10 * 10000) / 100; // 10%
    ...
    struct Action {
        ...
        uint256 minToken0Amount; // Minimum amount of token 0 expected
        uint256 minToken1Amount; // Minimum amount of token 1 expected
    ...
    event LoanRepaid(
        address indexed borrower, int256 rent, uint256 token0ToAddToLP, uint256 token1ToAddToLP, bool closePosition
    );
    ...
    function mint(address to, uint256 amount) external nonReentrant returns (uint256 bamm_out) {
        ...
        uint256 sqrtRentedReal = (sqrtRented * rentedMultiplier) / PRECISION;

        // Take the LP from the sender and mint them BAMM wrapper tokens
        bamm_out = ((sqrtBalance + sqrtRentedReal) == 0)
            ? sqrtAmount
            : ((sqrtAmount * totalSupply()) / (sqrtBalance + sqrtRentedReal));
    ...
    function executeActionsAndSwap(Action memory action, FraxswapRouterMultihop.FraxswapParams memory swapParams)
        public
        nonReentrant
    {
        ...
        if (action.rent > 0) {
            ...
            uint256 sqrtAmountRented = (uint256(action.rent > 0 ? action.rent : -action.rent) * rentedMultiplier) / 1e18;
            uint256 lpTokenAmount = (sqrtAmountRented * pairTotalSupply) / Math.sqrt(uint256(reserve0) * reserve1);
            uint256 token0Amount = (reserve0 * lpTokenAmount) / pairTotalSupply;
            uint256 token1Amount = (reserve1 * lpTokenAmount) / pairTotalSupply;
            ...
            sqrtRented += uint256(action.rent);
        ...
        if (action.rent < 0) {
            ...
            uint256 sqrtAmountRented = (uint256(-action.rent) * rentedMultiplier) / 1e18;
            uint256 lpTokenAmount = (sqrtAmountRented * pairTotalSupply) / Math.sqrt(uint256(reserve0) * reserve1);
            ...
            require(
                (token0ToAddToLP > action.minToken0Amount) && (token1ToAddToLP > action.minToken1Amount),
                "Min amount used"
            );
        ...
        if (action.token0Amount < 0) {
            ...
            SafeERC20.safeTransfer(
                ...
                (action.to == address(0) ? msg.sender : action.to),
                uint256(-action.token0Amount)
            );
    ...
    function liquidate(address user) external nonReentrant returns (uint256 liq_out, uint256 liq_fee) {
        // Sync the LP, then add the interest
        (uint112 reserve0, uint112 reserve1, uint256 pairTotalSupply) = _addInterest();
        ...
        SafeERC20.safeApprove(token0, address(bammHelper), uint256(vault.token0));
        SafeERC20.safeApprove(token1, address(bammHelper), uint256(vault.token1));

        // Give token0 and token1 to bammHelper, which will interact with the LP and mint LP to this contract
        uint256 liquidity = bammHelper.addLiquidityUnbalanced(
            uint256(vault.token0),
            uint256(vault.token1),
            0,
            pair,
            (isFraxswapPair ? pair.fee() : pairFee),
            isFraxswapPair
        );
        ...
        // Update the total rented liquidity
        sqrtRented -= uint256(vault.rented);
    ...
    function getInterestRate(uint256 utilityRate) public pure returns (uint256) {
    ...
    function calcRent(address tkn_desired_addr, uint256 amt_tkn_desired)
        external
        returns (int256 rent, uint256 lp_unwound, uint256 amt_tkn_other)
    {
        // Sync the LP, then add the interest
        (uint112 reserve0, uint112 reserve1, uint256 pairTotalSupply) = _addInterest();
    ...
    function _addTokensToVault(Vault memory vault, Action memory action) internal {
        ...
        if (action.v != 0 && (action.token0Amount > 0 || action.token1Amount > 0)) {
            ...
            // Do the permit
            uint256 amount = action.approveMax
                ? type(uint256).max
                : uint256(action.token0Amount > 0 ? action.token0Amount : action.token1Amount);
            IERC20Permit(address(token))
                .permit(msg.sender, address(this), amount, action.deadline, action.v, action.r, action.s);
        }
    ...
    function _addInterest() internal returns (uint112 reserve0, uint112 reserve1, uint256 pairTotalSupply) {
        ...
        if (period > 0) {
            ...
            if (sqrtRented > 0) {
                ...
                uint256 feeMintAmount =
                    (fee * totalSupply()) / (sqrtBalance + ((sqrtRented * rentedMultiplier) / PRECISION));
    ...
    function _solvent(Vault memory vault) internal view returns (bool) {
        ...
        if (vault.rented == 0) {
            ...
            // Check the LTV
            uint256 ltv = (uint256(vault.rented) * rentedMultiplier) / Math.sqrt(uint256(vault.token0 * vault.token1));
    ...
    function _checkMaxUtility(uint112 reserve0, uint112 reserve1, uint256 pairTotalSupply) internal view {
        //console.log("sqrtRented", sqrtRented);
        if (sqrtRented > 0) {
            uint256 balance = pair.balanceOf(address(this));
            uint256 sqrtBalance =
    ...
    function ammPriceCheck(uint112 reserve0, uint112 reserve1) internal {
        ...
        (uint256 result0, uint256 result1) = fraxswapOracle.getPrice(IFraxswapPair(address(pair)), 60 * 30, 10, 10000);
        result0 = 1e36 / result0;
        uint256 spotPrice = (uint256(reserve0) * 1e18) / reserve1;
