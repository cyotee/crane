...
contract LeveragePool {
    ...
    constructor(IERC20 _collateralToken, IOracle _oracle, IFundingRateModel _fundingRateModel, uint256 _inception) {
    ...
    function deposit(int256 amount, uint256 pool) external {
        ...
        if (transferAmount > 0 && address(collateralToken) != address(0x0)) {
            require(collateralToken.transferFrom(msg.sender, address(this), uint256(transferAmount)), "Transfer failed");
        }
    ...
    function withdrawCollateral(int256 amount) external {
        ...
        if (amount > 0 && address(collateralToken) != address(0x0)) {
            require(collateralToken.transfer(msg.sender, uint256(amount)), "Transfer failed");
        }
    }
    ...
    function withdrawAdminFees(int256 amount) external {
        ...
        adminFees -= amount;
        if (address(collateralToken) != address(0x0)) {
            require(collateralToken.transfer(msg.sender, uint256(amount)), "Transfer failed");
        }
    }
        ...
        if (block.timestamp >= epochStartTime + epochPeriod) {
            ...
            { // Apply the price change to all the pools
                ...
                while (!lastLoop) {
                    ...
                    if (!lastLoop) {
                        // Change was capped, continue with appliying the rest of the change
            ...
            { // Handle the funding and rebalance rates
                ...
                for (uint256 i = 0; i < pools.length; i++) {
                    Pool storage pool = pools[i];
                    if (pool.isLiquidityPool) {
                        pool.collateral -= (pool.collateral
                                * pool.leverage
                                * rates.liquidityPoolFundingRate
                                * int256(epochPeriod)
                                / (SECONDS_PER_YEAR * PRECISION)) + (pool.collateral * pool.leverage)
                            * rates.rebalanceLiquidityPoolRate * int256(epochPeriod) / (SECONDS_PER_YEAR * PRECISION);
                    } else if (pool.leverage > 0) {
                        pool.collateral -= (pool.collateral
                                * pool.leverage
                                * rates.longFundingRate
                                * int256(epochPeriod)
                                / (SECONDS_PER_YEAR * PRECISION)) + (pool.collateral * pool.rebalanceMultiplier)
                            * rates.rebalanceRate * int256(epochPeriod) / (SECONDS_PER_YEAR * PRECISION);
                    } else {
                        pool.collateral -= (-pool.collateral
                                * pool.leverage
                                * rates.shortFundingRate
                                * int256(epochPeriod)
                                / (SECONDS_PER_YEAR * PRECISION)) + (pool.collateral * pool.rebalanceMultiplier)
                            * rates.rebalanceRate * int256(epochPeriod) / (SECONDS_PER_YEAR * PRECISION);
                    }
            ...
            { // Do the deposits/withdrawels
                ...
                for (uint256 i = 0; i < pools.length; i++) {
                    ...
                    if (depositAmount > 0) {
                        ...
                        if (pool.shares > 0) {
                            data.sharesPerCollateralDeposit =
                                (pool.shares * PRECISION / pool.collateral) * (depositAmount - fees) / depositAmount;
                        } else {
                            data.sharesPerCollateralDeposit = PRECISION * (depositAmount - fees) / depositAmount;
                        }
            ...
            { // Distribute the fees among the liquidity pools and the admin
                ...
                if (amounts.liquidityPoolAmount > 0) {
                    ...
                    for (uint256 i = 0; i < pools.length; i++) {
                        Pool storage pool = pools[i];
                        if (pool.isLiquidityPool) {
                            pool.collateral += liquidityPoolFees * pool.collateral * pool.leverage
                                / amounts.liquidityPoolAmount;
                        }
                    }
                } else {
                    adminFees += sumFees;
    ...
    function _calculatePoolAmounts() internal view returns (PoolAmounts memory amounts) {
        ...
        if (amounts.longAmount > amounts.shortAmount) {
            ...
            if (missingAmount < amounts.liquidityPoolAmount) {
                ...
                if (amounts.longAmount > 0) {
                    amounts.longLeverage =
                        (amounts.shortAmount + amounts.liquidityPoolAmount) * PRECISION / amounts.longAmount;
                } else {
                    amounts.longLeverage = PRECISION;
                }
            ...
            if (missingAmount < amounts.liquidityPoolAmount) {
                ...
                if (amounts.shortAmount > 0) {
                    amounts.shortLeverage =
                        (amounts.longAmount + amounts.liquidityPoolAmount) * PRECISION / amounts.shortAmount;
                } else {
                    amounts.shortLeverage = PRECISION;
                }
     ...
     *
     */
    function _getNextEpoch() internal view returns (uint256 nextEpoch) {
    ...
    function getRates(PoolAmounts memory amounts) internal view returns (Rates memory rates) {
        if (address(fundingRateModel) != address(0x0)) {
            (
                ...
                rates.liquidityPoolFundingRate,
                rates.rebalanceRate,
                rates.rebalanceLiquidityPoolRate
            ) =
                fundingRateModel.getFundingRate(
                    amounts.longAmount, amounts.shortAmount, amounts.liquidityPoolAmount, amounts.rebalanceAmount
                );
        }
    }
...
interface IFundingRateModel {
    function getFundingRate(
        int256 _longAmount,
        int256 _shortAmount,
        int256 _liquidityPoolAmount,
        int256 _rebalanceAmount
    ) external view returns (int256, int256, int256, int256, int256);
}
...
contract BaseInterestRateModel is IFundingRateModel {
    ...
    {
        if (_shortAmount > 0 && _longAmount > _shortAmount) {
            // longs pay shorts
            ...
            if (missingAmount > _liquidityPoolAmount) {
                shortFundingRate = liquidityPoolFundingRate = -longFundingRate * _longAmount
                    / (_shortAmount + _liquidityPoolAmount);
            } else {
                shortFundingRate = -longFundingRate;
                if (_liquidityPoolAmount > 0) {
                    liquidityPoolFundingRate = shortFundingRate * missingAmount / _liquidityPoolAmount;
                }
            }
            ...
            if (missingAmount > _liquidityPoolAmount) {
                longFundingRate = liquidityPoolFundingRate = -shortFundingRate * _shortAmount
                    / (_longAmount + _liquidityPoolAmount);
            } else {
                longFundingRate = -shortFundingRate;
                if (_liquidityPoolAmount > 0) {
                    liquidityPoolFundingRate = longFundingRate * missingAmount / _liquidityPoolAmount;
                }
            }
        }
        if (_liquidityPoolAmount > 0) {
            // calculate the rebalancing rates
