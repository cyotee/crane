...
import "@crane/contracts/protocols/tokens/stable/frax/Uniswap/TransferHelperV2.sol";
import "@crane/contracts/protocols/tokens/stable/frax/Fraxoracle/interface/IStateRootOracle.sol";
import "@crane/contracts/protocols/tokens/stable/frax/Fraxoracle/library/MerkleTreeProver.sol";
...
contract FerryOnL2 {
    ...
    uint256 public constant EXIT_WAIT_TIME = 60 * 60 * 24 * 2; // 48 hours
    ...
    event Embark(address indexed sender, uint32 indexed index, uint256 amount, uint256 tip, uint256 timestamp);
    event IncreaseTip(uint32 indexed index, uint256 tip);
    event Disembark(address indexed captain, uint32 index, address indexed recipient, uint256 amount);
    ...
    function captainDeposit(uint256 amount) external {
    ...
    function captainInitiateWithdraw(uint256 amount) external {
        CaptainData storage data = captainData[msg.sender];
    ...
    function captainWithdraw() external {
        ...
        uint256 toWithdraw = data.withdrawAmount > data.balance ? data.balance : data.withdrawAmount;
    ...
    function disembark(
        ...
        for (uint256 i = 0; i < _proofValue.length; i++) {
            bytes32 slot = bytes32(
                uint256(keccak256(abi.encodePacked(keccak256(abi.encodePacked(abi.encode(captain), bytes32(0))))))
                    + index + i
            );
            uint256 value =
                uint256(MerkleTreeProver.proofStorageSlotValue(accountPool.storageRoot, slot, _proofValue[i]).value);
            if (value == 0) revert("Empty slot");
            uint256 amount = uint64(value >> 160) * REDUCED_DECIMALS;
    ...
    function embarkWithRecipient(uint256 amount, uint256 tip, address recipient, uint32 deadline) public {
    ...
    function increaseTip(uint32 index, uint256 tip) external {
    ...
    function collect(uint32 index, uint256 blockNumber, bytes[] memory _proofAccount, bytes[] memory _proofValue)
        external
    {
        ...
        bytes32 hash = abi.encodePacked(transaction.amount, transaction.recipient, index)._hash();
        //   bytes32 slot = keccak256(abi.encodePacked(hash,bytes32(uint(1))));
        bytes32 slot = abi.encodePacked(hash, bytes32(uint256(1)))._hash();
        uint256 value = MerkleTreeProver.proofStorageSlotValue(accountPool.storageRoot, slot, _proofValue).value;
        if (value == 0) {
            // Not disembarked, return the funds
