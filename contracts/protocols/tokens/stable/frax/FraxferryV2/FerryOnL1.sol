...
contract FerryOnL1 {
    ...
    event Disembark(address indexed captain, uint32 indexed index, address indexed recipient, uint256 amount);
    ...
    // ************* L1 => L2
    function embarkWithRecipient(uint256 amount, address recipient, address _captain, uint256 maxCumulativeAmount)
        public
    {
        CaptainInfo memory captain = captains[_captain];
        if (captain.cumulativeAmount > maxCumulativeAmount) revert("maxCumulativeAmount");
        ...
        uint256 fee;
        if (fee_exempt_addrs[_captain][msg.sender]) {
            fee = 0;
        } else {
            if (block.timestamp > captain.newFeeActiveTimestamp) {
                fee = Math.min(Math.max(captain.newMinFee, amount * captain.newFeeRate / 10000), captain.newMaxFee);
            } else {
                fee = Math.min(Math.max(captain.minFee, amount * captain.feeRate / 10000), captain.maxFee);
            }
        }
        ...
        emit Embark(
            recipient,
            transactionsIn[_captain].length,
            amount,
            amountAfterFee * REDUCED_DECIMALS,
            block.timestamp,
            _captain
        );
        transactionsIn[_captain].push(TransactionIn(recipient, amountAfterFee, uint32(block.timestamp)));
        captains[_captain].cumulativeAmount += amountAfterFee;
    ...
    function withdraw() external {
        CaptainInfo storage captain = captains[msg.sender];
        uint256 withdrawAmount = captain.balance;
        captain.balance = 0;
        TransferHelperV2.safeTransfer(address(token), msg.sender, withdrawAmount);
    }

    // ************* L2 => L1
    function disembark(uint256 amount, address recipient, uint32 index, address captain, uint32 deadline) public {
        if (block.timestamp >= deadline) revert("Deadline");
        // bytes32 hash = keccak256(abi.encodePacked(amount,recipient,index));
        bytes32 hash = abi.encodePacked(amount, recipient, index)._hash();
