...
contract Fraxferry {
    ...
    uint256 public MIN_WAIT_PERIOD_EXECUTE = 79200; // Minimal 22 hour waiting
    ...
    function embarkWithRecipient(uint256 amount, address recipient) public notPaused {
        ...
        uint256 fee;
        if (fee_exempt_addrs[msg.sender]) {
            fee = 0;
        } else {
    ...
    function disembark(BatchData calldata batchData) external notPaused isFirstOfficer {
        ...
        bytes32 hash = abi.encodePacked(targetChain, targetToken, chainid, token, batch.start)._hash();
        for (uint256 i = 0; i < batchData.transactions.length; ++i) {
            if (!cancelled[batch.start + i]) {
                TransferHelper.safeTransfer(
                    address(token), batchData.transactions[i].user, batchData.transactions[i].amount * REDUCED_DECIMALS
                );
            }
            //  hash = keccak256(abi.encodePacked(hash, batchData.transactions[i].user,batchData.transactions[i].amount));
            hash = abi.encodePacked(hash, batchData.transactions[i].user, batchData.transactions[i].amount)._hash();
        }
    ...
    function setMinWaitPeriods(uint256 _MIN_WAIT_PERIOD_ADD, uint256 _MIN_WAIT_PERIOD_EXECUTE) external isOwner {
        ...
        emit SetMinWaitPeriods(
            MIN_WAIT_PERIOD_ADD, MIN_WAIT_PERIOD_EXECUTE, _MIN_WAIT_PERIOD_ADD, _MIN_WAIT_PERIOD_EXECUTE
        );
    ...
    function getTransactionsHash(uint256 start, uint256 end) public view returns (bytes32) {
        ...
        bytes32 result = abi.encodePacked(chainid, token, targetChain, targetToken, uint64(start))._hash();
        for (uint256 i = start; i <= end; ++i) {
            //  result = keccak256(abi.encodePacked(result, transactions[i].user,transactions[i].amount));
            result = abi.encodePacked(result, transactions[i].user, transactions[i].amount)._hash();
        }
        return result;
    }
