...
import {UniswapV2Library} from "@crane/contracts/protocols/tokens/stable/frax/Uniswap/UniswapV2Library.sol";
...
contract FraxPoolvAMM is AccessControl {
    ...
    function getAmountOut(uint256 amountIn, uint256 reserveIn, uint256 reserveOut, uint256 the_fee)
        public
        pure
        ...
        )
    {
            ...
            if (drift_time > 0) {
                ...
                if (drift_collat_positive > 0) {
                    current_collat_virtual_reserves =
                        current_collat_virtual_reserves.add(drift_collat_positive.mul(drift_time));
                } else {
                    current_collat_virtual_reserves =
                        current_collat_virtual_reserves.sub(drift_collat_negative.mul(drift_time));
                }
            }
        }
        ...
        if (time_elapsed > drift_time && drift_time > 0) {
            average_fxs_virtual_reserves = average_fxs_virtual_reserves.mul(drift_time)
                .add(current_fxs_virtual_reserves.mul(time_elapsed.sub(drift_time))).div(time_elapsed);
            average_collat_virtual_reserves = average_collat_virtual_reserves.mul(drift_time)
                .add(current_collat_virtual_reserves.mul(time_elapsed.sub(drift_time))).div(time_elapsed);
        }
    }
    ...
    function refreshDrift() external {
        ...
        uint256 targetK = internal_k > ext_k
        ...
        if (collat_reserves_average_price < ext_collat_reserves_average_price) {
            drift_collat_positive =
                (ext_collat_reserves_average_price - collat_reserves_average_price).div(drift_refresh_period);
            drift_collat_negative = 0;
        } else {
            drift_collat_positive = 0;
            drift_collat_negative =
                (collat_reserves_average_price - ext_collat_reserves_average_price).div(drift_refresh_period);
        }
    ...
    function collatDollarBalance() public view returns (uint256) {
        return (collateral_token.balanceOf(address(this)).add(collateral_invested).sub(unclaimedPoolCollateral))
    ...
    function _update(
        ...
        uint256 time_elapsed = block.timestamp - last_update_time;
        if (time_elapsed > 0) {
            fxs_price_cumulative += average_fxs_virtual_reserves.mul(1e18).div(average_collat_virtual_reserves)
                .mul(time_elapsed);
        }
        ...
        returns (uint256, uint256, uint256)
    {
        ...
        require(
            total_frax_mint
                <= collateral_amount.mul(10 ** missing_decimals).mul(uint256(1e6).add(max_drift_band))
