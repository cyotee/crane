...
import '@crane/contracts/external/reactive/abstract-base/AbstractCallback.sol';
...
import '@crane/contracts/external/uniswap/v2-periphery/contracts/interfaces/IUniswapV2Router02.sol';
...
contract UniswapDemoStopTakeProfitCallback is AbstractCallback, RescuableBase {
    ...
    function createStopOrder(
        ...
        // Stack-depth split for Crane (viaIR=false); behavior unchanged from upstream.
        require(pair != address(0), "Invalid pair address");
        require(amount >= MIN_AMOUNT, "Amount too small");
        require(coefficient > 0 && threshold > 0, "Invalid price parameters");
        _assertPairLiquidity(pair);
        return _storeStopOrder(pair, sellToken0, amount, coefficient, threshold, orderType);
    }

    function _assertPairLiquidity(address pair) private view {
        (uint112 reserve0, uint112 reserve1,) = IUniswapV2Pair(pair).getReserves();
        require(reserve0 > 0 && reserve1 > 0, "Pair has no liquidity");
    }

    function _storeStopOrder(
        address pair,
        bool sellToken0,
        uint256 amount,
        uint256 coefficient,
        ...
        address tokenSell = sellToken0 ? IUniswapV2Pair(pair).token0() : IUniswapV2Pair(pair).token1();
        address tokenBuy = sellToken0 ? IUniswapV2Pair(pair).token1() : IUniswapV2Pair(pair).token0();
