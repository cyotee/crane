...
/// @title CustomSwapFeeModule
/// @notice Fee module that allows custom swap fees to be set per pool
...
contract CustomSwapFeeModule is ICustomFeeModule {
    ...
    /// @notice Maximum swap fee allowed (3%)
    uint256 public constant MAX_FEE = 30_000;
    ...
    /// @notice Creates the CustomSwapFeeModule
    ...
    function setCustomFee(address _pool, uint24 _fee) external override {
        require(msg.sender == factory.swapFeeManager(), "Not swap fee manager");
    ...
    function getFee(address _pool) external view override returns (uint24) {
        ...
        // Otherwise return default fee for tick spacing
        if (fee == ZERO_FEE_INDICATOR) {
            return 0;
        } else if (fee != 0) {
            return fee;
        } else {
            return factory.tickSpacingToFee(tickSpacing);
        }
    }
}
