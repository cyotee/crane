...
/// @dev Key difference from Uniswap V3: has `staked` parameter to handle gauge integration
library Position {
    ...
    /// @dev When staked is true, fees are NOT accumulated to the position (handled by gauge instead)
    function update(
        Info storage self,
        ...
        if (!staked) {
            ...
            // Fee growth values use modular arithmetic and can wrap around
            unchecked {
                tokensOwed0 = uint128(
                    FullMath.mulDiv(
