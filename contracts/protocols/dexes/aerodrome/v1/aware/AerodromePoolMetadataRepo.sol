// SPDX-License-Identifier: AGPL-3.0-or-later
pragma solidity ^0.8.0;

import {IPoolFactory} from "@crane/contracts/interfaces/protocols/dexes/aerodrome/IPoolFactory.sol";

// tag::AerodromePoolMetadataRepo[]
/**
 * @title AerodromePoolMetadataRepo - Storage library for Aerodrome V1 pool metadata dependency injection (Aware pattern).
 * @author cyotee doge <not_cyotee@proton.me>
 * @dev Storage library (Repo) for holding Aerodrome V1 pool metadata (IPoolFactory reference + isStable flag).
 * @dev Provides dual (parameterized + default) overloads for initialization and getters.
 * @dev Follows the gold standard from DeployedAddressesRepo, OperableRepo, MultiStepOwnableRepo, ERC2535Repo, Create3FactoryAwareRepo, DiamondPackageCallBackFactoryAwareRepo, BalancerV3VaultAwareRepo, CamelotV2RouterAwareRepo, UniswapV2FactoryAwareRepo, UniswapV2RouterAwareRepo, AerodromeRouterAwareRepo
 *      (rich NatSpec, exact // tag:: / end:: include tags, @dev "The Storage struct to operate on.", ERC1967 slot).
 * @dev Used by Aerodrome V1 protocol ports (services, test bases, etc.) for dependency injection of pool metadata (factory + stable vs volatile classification).
 */
library AerodromePoolMetadataRepo {
    // tag::STORAGE_SLOT[]
    /**
     * @dev ERC1967-compliant storage slot.
     *      Computed as bytes32(uint256(keccak256(abi.encode("protocols.dexes.aerodrome.v1.pool.metadata.repo"))) - 1).
     *      This follows the canonical pattern used by OperableRepo, ERC2535Repo, MultiStepOwnableRepo, DeployedAddressesRepo, Create3FactoryAwareRepo, DiamondPackageCallBackFactoryAwareRepo, BalancerV3VaultAwareRepo, CamelotV2RouterAwareRepo, UniswapV2* and the just-closed AerodromeRouterAwareRepo
     *      gold-standard Repos for collision-resistant deterministic storage binding.
     */
    bytes32 internal constant STORAGE_SLOT =
        bytes32(uint256(keccak256(abi.encode("protocols.dexes.aerodrome.v1.pool.metadata.repo"))) - 1);

    // end::STORAGE_SLOT[]

    // tag::Storage[]
    /**
     * @dev Standardized storage layout for aerodrome v1 pool metadata reference (Aware).
     *      factory: reference to IPoolFactory.
     *      isStable: flag indicating stable vs volatile pool type.
     */
    struct Storage {
        IPoolFactory factory;
        bool isStable;
    }

    // end::Storage[]

    // tag::_layoutStruct(bytes32)[]
    /**
     * @dev Argumented version of _layoutStruct to allow for custom storage slot usage.
     * @param slot_ The storage slot to bind.
     * @return layoutStruct The Storage struct bound to the provided slot.
     */
    function _layoutStruct(bytes32 slot_) internal pure returns (Storage storage layoutStruct) {
        assembly {
            layoutStruct.slot := slot_
        }
    }

    // end::_layoutStruct(bytes32)[]

    // tag::_layoutStruct()[]
    /**
     * @dev Default _layoutStruct binding to the canonical ERC1967 STORAGE_SLOT.
     * @return layoutStruct The Storage struct bound to STORAGE_SLOT.
     */
    function _layoutStruct() internal pure returns (Storage storage layoutStruct) {
        return _layoutStruct(STORAGE_SLOT);
    }

    // end::_layoutStruct()[]

    // tag::_initialize(Storage-IPoolFactory-bool)[]
    /**
     * @dev Argumented version of _initialize to allow direct Storage access.
     * @dev The Storage struct to operate on.
     * @param layoutStruct The Storage struct to operate on.
     * @param factory_ The IPoolFactory instance to inject.
     * @param isStable_ The stable/volatile flag for the pool.
     */
    function _initialize(Storage storage layoutStruct, IPoolFactory factory_, bool isStable_) internal {
        layoutStruct.factory = factory_;
        layoutStruct.isStable = isStable_;
    }

    // end::_initialize(Storage-IPoolFactory-bool)[]

    // tag::_initialize(IPoolFactory-bool)[]
    /**
     * @dev Default version of _initialize binding to the standard STORAGE_SLOT.
     * @param factory_ The IPoolFactory instance to inject.
     * @param isStable_ The stable/volatile flag for the pool.
     */
    function _initialize(IPoolFactory factory_, bool isStable_) internal {
        _initialize(_layoutStruct(), factory_, isStable_);
    }

    // end::_initialize(IPoolFactory-bool)[]

    // tag::_factory(Storage)[]
    /**
     * @dev Argumented version of _factory to allow direct Storage access.
     * @dev The Storage struct to operate on.
     * @param layoutStruct The Storage struct to operate on.
     * @return factory_ The stored IPoolFactory (or zero if not initialized).
     */
    function _factory(Storage storage layoutStruct) internal view returns (IPoolFactory factory_) {
        return layoutStruct.factory;
    }

    // end::_factory(Storage)[]

    // tag::_factory()[]
    /**
     * @dev Default version of _factory binding to the standard STORAGE_SLOT.
     * @return factory_ The stored IPoolFactory.
     */
    function _factory() internal view returns (IPoolFactory factory_) {
        return _factory(_layoutStruct());
    }

    // end::_factory()[]

    // tag::_isStable(Storage)[]
    /**
     * @dev Argumented version of _isStable to allow direct Storage access.
     * @dev The Storage struct to operate on.
     * @param layoutStruct The Storage struct to operate on.
     * @return isStable_ The stored isStable flag.
     */
    function _isStable(Storage storage layoutStruct) internal view returns (bool isStable_) {
        return layoutStruct.isStable;
    }

    // end::_isStable(Storage)[]

    // tag::_isStable()[]
    /**
     * @dev Default version of _isStable binding to the standard STORAGE_SLOT.
     * @return isStable_ The stored isStable flag.
     */
    function _isStable() internal view returns (bool isStable_) {
        return _isStable(_layoutStruct());
    }
    // end::_isStable()[]
}
// end::AerodromePoolMetadataRepo[]
