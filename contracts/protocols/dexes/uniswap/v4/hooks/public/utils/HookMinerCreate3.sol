...
library HookMinerCreate3 {
        ...
        returns (address, bytes32)
    {
        ...
        for (uint256 salt; salt < MAX_LOOP; salt++) {
            hookAddress = computeAddress(deployer, salt);
    ...
    function computeAddress(address deployer, uint256 salt) internal pure returns (address hookAddress) {
    ...
    function findWithPrefix(
        ...
        for (uint256 i; i < MAX_LOOP; i++) {
            ...
            bytes32 salt = abi.encodePacked(saltPrefix, i)._hash();
            hookAddress = computeAddress(deployer, uint256(salt));
