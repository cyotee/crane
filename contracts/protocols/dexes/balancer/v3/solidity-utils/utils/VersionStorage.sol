...
library VersionRepo {
    ...
    ) internal pure returns(VersionLayout storage layout_) {
...
contract VersionStorage {
    ...
    bytes32 private constant LAYOUT_ID
        = keccak256(abi.encode(type(VersionRepo).name));
