...
library FactoryWidePauseWindowRepo {
    ...
    function _layout(
        bytes32 slot_
    ) internal pure returns(FactoryWidePauseWindowLayout storage layout_) {
...
contract FactoryWidePauseWindowStorage {
    ...
    bytes32 private constant LAYOUT_ID
        = keccak256(abi.encode(type(FactoryWidePauseWindowRepo).name));
     ...
     */
    function _factoryPauseWindow()
    internal pure virtual returns(FactoryWidePauseWindowLayout storage) {
    ...
    function _initFactoryWidePauseWindow(
