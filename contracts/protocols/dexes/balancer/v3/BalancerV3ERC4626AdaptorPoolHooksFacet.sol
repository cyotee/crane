    ...
    BalancerV3HooksFacet
{
    ...
    function onRegister(
        ...
        if (liquidityManagement.enableRemoveLiquidityCustom) {
            // Short circuit the evaluation by returning false.
            return false;
        }
        ...
        for ( uint256 cursor = 0; cursor < tokenConfig.length; cursor++ ) {
            ...
            if (
                tokenConfig[cursor].paysYieldFees
            ) {
                // Short circuit the evaluation by returning false.
    ...
    function onBeforeSwap(
        ...
        if(params.kind == SwapKind.EXACT_IN) {
            ...
            if (params.indexIn == wrapperIndex) {
        ...
        if (params.kind == SwapKind.EXACT_OUT) {
            ...
            if (params.indexOut == wrapperIndex) {
    ...
    returns (bool success, uint256 hookAdjustedAmountCalculatedRaw) {
        ...
        if (params.kind == SwapKind.EXACT_IN) {
            ...
            if (address(params.tokenIn) == address(wrapper)) {
        ...
        if (params.kind == SwapKind.EXACT_OUT) {
            ...
            if (address(params.tokenOut) == address(wrapper)) {
