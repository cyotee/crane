...
contract BalancerV3ConstantProductPoolTarget is IBalancerV3Pool {
    ...
    {
        if (balancesLiveScaled18.length != 2) {
            revert BalancerV3Pool_RequiresTwoTokens(balancesLiveScaled18.length);
        }
        ...
        returns (uint256 newBalance)
    {
        ...
        if (tokenInIndex > 1) {
            revert BalancerV3Pool_TokenIndexOutOfBounds(tokenInIndex);
        }
        ...
        // Use 512-bit intermediate to prevent overflow with large invariants
        ...
        returns (uint256 amountCalculatedScaled18)
    {
        ...
        if (params.kind == SwapKind.EXACT_IN) {
            ...
            // Use 512-bit intermediate to prevent overflow with large balances
