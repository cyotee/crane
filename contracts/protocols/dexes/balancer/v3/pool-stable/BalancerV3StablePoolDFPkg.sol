...
interface IBalancerV3StablePoolDFPkg {
    struct PkgInit {
        ...
        IFacet balancerV3StablePoolFacet;
    ...
    function deployPool(TokenConfig[] calldata tokenConfigs, uint256 amplificationParameter, address hooksContract)
        external
        returns (address vault);
}

/**
 * @title BalancerV3StablePoolDFPkg
 * @notice Diamond Factory Package for deploying Balancer V3 stable pools.
 * @dev Stable pools are optimized for assets that trade near parity (e.g., stablecoins).
 * Maximum 5 tokens per pool (enforced by StableMath).
 *
 * Key differences from weighted pools:
 * - Uses amplification parameter instead of weights
 * - Maximum 5 tokens (not 8)
 * - Uses StableMath for all calculations
 */
contract BalancerV3StablePoolDFPkg is BalancerV3BasePoolFactory, IDiamondFactoryPackage, IBalancerV3StablePoolDFPkg {
    using Address for address[];
    using BetterEfficientHashLib for bytes;
    using SafeERC20 for IERC20;
    using TokenConfigUtils for TokenConfig[];

    // Stable pools have same fee bounds as weighted pools
    uint256 private constant _MIN_SWAP_FEE_PERCENTAGE = 1e12; // 0.0001%
    uint256 private constant _MAX_SWAP_FEE_PERCENTAGE = 0.1e18; // 10%

    // Stable pools use tighter invariant ratio bounds (from StableMath)
    uint256 private constant _MIN_INVARIANT_RATIO = StableMath.MIN_INVARIANT_RATIO;
    uint256 private constant _MAX_INVARIANT_RATIO = StableMath.MAX_INVARIANT_RATIO;

    // Stable pools support 2-5 tokens (StableMath limitation)
    ...
    IFacet public immutable BALANCER_V3_STABLE_POOL_FACET;

    error InvalidTokensLength(uint256 maxLength, uint256 minLength, uint256 providedLength);
    error InvalidAmplificationParameter(uint256 minAmp, uint256 maxAmp, uint256 providedAmp);
    ...
    constructor(PkgInit memory pkgInit) {
        ...
        BALANCER_V3_STABLE_POOL_FACET = pkgInit.balancerV3StablePoolFacet;
    ...
    /**
     * @notice Deploy a new stable pool.
     * @param tokenConfigs_ Token configurations (2-5 tokens).
     ...
     */
    function deployPool(TokenConfig[] calldata tokenConfigs_, uint256 amplificationParameter_, address hooksContract)
        public
        returns (address vault)
    {
    ...
    function packageName() public pure returns (string memory name_) {
        return type(BalancerV3StablePoolDFPkg).name;
    }
    ...
    function facetInterfaces() public pure returns (bytes4[] memory interfaces) {
        ...
        interfaces[11] = type(IBalancerV3StablePool).interfaceId;
        return interfaces;
    }
    ...
    function facetAddresses() public view returns (address[] memory facetAddresses_) {
        ...
        facetAddresses_[4] = address(BALANCER_V3_STABLE_POOL_FACET);
    }
    ...
    function facetCuts() public view returns (IDiamond.FacetCut[] memory facetCuts_) {
        ...
        facetCuts_[4] = IDiamond.FacetCut({
            facetAddress: address(BALANCER_V3_STABLE_POOL_FACET),
            action: IDiamond.FacetCutAction.Add,
            functionSelectors: BALANCER_V3_STABLE_POOL_FACET.facetFuncs()
        });
     ...
     * @dev Includes all configuration parameters to ensure unique pools.
     */
    function calcSalt(bytes memory pkgArgs) public pure returns (bytes32 salt) {
        PkgArgs memory decodedArgs = abi.decode(pkgArgs, (PkgArgs));

        uint256 tokensLen = decodedArgs.tokenConfigs.length;
        if (tokensLen < _MIN_TOKENS || tokensLen > _MAX_STABLE_TOKENS) {
            revert InvalidTokensLength(_MAX_STABLE_TOKENS, _MIN_TOKENS, tokensLen);
        }

        // Validate amplification bounds
        if (
            decodedArgs.amplificationParameter < StableMath.MIN_AMP
                || decodedArgs.amplificationParameter > StableMath.MAX_AMP
        ) {
            revert InvalidAmplificationParameter(
                StableMath.MIN_AMP, StableMath.MAX_AMP, decodedArgs.amplificationParameter
            );
        }
        ...
        // Include ALL parameters in salt calculation (learned from CRANE-179)
        // This prevents salt collisions when only amp or hooks differ
        bytes memory saltData =
            abi.encode(decodedArgs.tokenConfigs, decodedArgs.amplificationParameter, decodedArgs.hooksContract);
    ...
    function initAccount(bytes memory initArgs) public {
        ...
        BalancerV3StablePoolRepo._initialize(decodedArgs.amplificationParameter);
        BalancerV3AuthenticationRepo._initialize(keccak256(abi.encode(address(this))));
    }
    ...
    function _buildPoolName(address[] memory tokens) internal view returns (string memory) {
        if (tokens.length == 2) {
            return string.concat(
                "BV3Stable of (", IERC20Metadata(tokens[0]).name(), " / ", IERC20Metadata(tokens[1]).name(), ")"
            );
        } else {
            string memory result = "BV3Stable of (";
    ...
    function postDeploy(address proxy) public returns (bool) {
        _registerPoolWithBalV3Vault(
            ...
            5e16, // 5% initial swap fee (same as weighted)
