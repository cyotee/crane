...
interface IBalancerV3Gyro2CLPPoolDFPkg {
    ...
    function deployPool(TokenConfig[] calldata tokenConfigs, uint256 sqrtAlpha, uint256 sqrtBeta, address hooksContract)
        external
    ...
    IBalancerV3Gyro2CLPPoolDFPkg
{
    ...
    function calcSalt(bytes memory pkgArgs) public pure returns (bytes32 salt) {
        ...
        bytes memory saltData = abi.encode(
            decodedArgs.tokenConfigs, decodedArgs.sqrtAlpha, decodedArgs.sqrtBeta, decodedArgs.hooksContract
        );
    ...
    function initAccount(bytes memory initArgs) public {
        ...
        BalancerV3Gyro2CLPPoolRepo._initialize(decodedArgs.sqrtAlpha, decodedArgs.sqrtBeta);
        BalancerV3AuthenticationRepo._initialize(keccak256(abi.encode(address(this))));
    }
    ...
    function _buildPoolName(address[] memory tokens) internal view returns (string memory) {
        return
            string.concat(
                "BV3-2CLP of (", IERC20Metadata(tokens[0]).name(), " / ", IERC20Metadata(tokens[1]).name(), ")"
            );
    }
