...
library IntrospectionFacetFactoryService {
    ...
    function deployERC165Facet(ICreate3FactoryProxy create3Factory) internal returns (IFacet erc165Facet) {
        erc165Facet =
            create3Factory.deployFacet(type(ERC165Facet).creationCode, abi.encode(type(ERC165Facet).name)._hash());
        vm.label(address(erc165Facet), type(ERC165Facet).name);
    }

    function deployDiamondLoupeFacet(ICreate3FactoryProxy create3Factory) internal returns (IFacet diamondLoupeFacet) {
        diamondLoupeFacet = create3Factory.deployFacet(
            type(DiamondLoupeFacet).creationCode, abi.encode(type(DiamondLoupeFacet).name)._hash()
        );
        vm.label(address(diamondLoupeFacet), type(DiamondLoupeFacet).name);
    }

    function deployDiamondCutFacet(ICreate3FactoryProxy create3Factory) internal returns (IFacet diamondCutFacet) {
