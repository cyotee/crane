...
interface IBalancerV3Pool {
    /**
     * @notice Thrown when a function expecting exactly 2 token balances receives a different count.
     * @param length The actual number of balances provided.
     * @custom:signature BalancerV3Pool_RequiresTwoTokens(uint256)
     * @custom:selector 0x832fa110
     */
    error BalancerV3Pool_RequiresTwoTokens(uint256 length);

    /**
     * @notice Thrown when a token index is out of bounds for a 2-token pool.
     * @param index The out-of-bounds token index.
     * @custom:signature BalancerV3Pool_TokenIndexOutOfBounds(uint256)
     * @custom:selector 0x63da7284
     */
    error BalancerV3Pool_TokenIndexOutOfBounds(uint256 index);
    /**
     *
     *                                Invariant
     *
