...
library ERC2612Repo {
     ...
     */
    function _layout(bytes32 slot_) internal pure returns (ERC2612Layout storage layout_) {
        assembly {
            layout_.slot := slot_
        }
    }
    ...
    function _layout() internal pure returns (ERC2612Layout storage layout) {
     ...
     */
    function _useNonce(ERC2612Layout storage layout, address owner) internal returns (uint256) {
     ...
     */
    function _useCheckedNonce(ERC2612Layout storage layout, address owner, uint256 nonce) internal {
    ...
    function _nonces(ERC2612Layout storage layout, address owner) internal view returns (uint256) {
        return layout.nonces[owner];
    }
