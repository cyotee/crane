...
```solidity
// First, deploy the Create2CallBackFactory
Create2CallBackFactory create2Factory = new Create2CallBackFactory();
...
#### Create2CallBackFactory API

The `Create2CallBackFactory` contract provides the following key methods:
...
1. **Create the Factory**: Deploy a Create2CallBackFactory instance.
...
```solidity
// 1. Deploy the Create2CallBackFactory
Create2CallBackFactory create2Factory = new Create2CallBackFactory();
...
### Using the Create3 Pattern (Preferred for Non-Diamond Deployments)

The Crane Framework now supports and recommends the **Create3** deployment pattern for deploying new contracts that are not Diamond proxies. Create3 is an evolution of deterministic contract deployment, providing a more robust and flexible approach than Create2 for most use cases outside of Diamond proxy deployment.

**Key Contracts:**

- `Create2CallBackFactory`: The main factory contract, now includes `create3` methods.
- `ICreate3Aware` and `Create3AwareContract`: Interfaces and base contracts for Create3-aware deployments.

**How Create3 Works:**

- Contracts are deployed using the `create3` function in `Create2CallBackFactory`.
- Initialization data and a salt are passed to the contract at deployment.
- Contracts should inherit from `Create3AwareContract` (or implement `ICreate3Aware`) to access their initialization data and deployment metadata.
...
import { Create2CallBackFactory } from "crane/factories/create2/callback/Create2CallBackFactory.sol";
import { Create3AwareContract } from "crane/factories/create2/aware/Create3AwareContract.sol";
import { ICreate3Aware } from "crane/interfaces/ICreate3Aware.sol";

// 1. Deploy the Create2CallBackFactory
Create2CallBackFactory factory = new Create2CallBackFactory();

// 2. Prepare initialization data and salt
ICreate3Aware.CREATE3InitData memory init = ICreate3Aware.CREATE3InitData({
    salt: keccak256("my-unique-salt"),
    initData: abi.encode("MyName", msg.sender)
});

// 3. Deploy your contract using create3
address deployed = factory.create3(
    type(MyContract).creationCode,
    abi.encode(init),
    init.salt
);

// 4. Inherit from Create3AwareContract to access initData in your contract
contract MyContract is Create3AwareContract {
    string public name;
    address public owner;

    constructor(ICreate3Aware.CREATE3InitData memory create3InitData)
...
- **Preferred for Non-Diamond Deployments:** Use Create3 for all new contract deployments unless you are specifically deploying a Diamond proxy.
- **Deterministic Addresses:** Like Create2, but with improved flexibility and metadata support.
- **Initialization Data:** Contracts can access their initialization data directly in the constructor.
- **Salted Deployments:** Use a unique salt for each deployment to ensure address uniqueness and predictability.

**Best Practices:**

- Always inherit from `Create3AwareContract` for contracts deployed via Create3.
- Use the `create3` function in `Create2CallBackFactory` for all non-Diamond deployments.
- For Diamond proxies, continue to use the Diamond-specific deployment flow.

**Note:** The older Create2-based deployment pattern is still supported for backward compatibility and for Diamond proxy deployments, but Create3 is now the recommended approach for all other contract deployments.
...
3. **Fixture_Crane Implementation**:

   ```solidity
   contract Fixture_Crane is Fixture {
       // Provides access to core framework contracts
       function factory() public returns (Create2CallBackFactory);
...
4. **Using Fixtures in Tests**:
   ...
   ```solidity
   contract MyTest is Test_Crane {
       function setUp() public {
           // Test_Crane inherits Fixture_Crane, providing access to all framework contracts
