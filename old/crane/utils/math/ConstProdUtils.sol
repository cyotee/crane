...
library ConstProdUtils {
    ...
    /**
     * @dev Calculates the yield (in LP tokens) attributable to a holder's owned LP amount.
     * Wrapper version: computes newK and returns (lpOfYield, newK).
        * @notice DEPRECATED: this wrapper is deprecated and retained for compatibility only.
        * @deprecated Do not rely on this helper in new code. It may be removed in a future cleanup.
     */
    function _calculateYieldForOwnedLP(
        ...
        lpOfYield = _calculateYieldForOwnedLP(reserveA, reserveB, totalSupply, lastK, newK, ownedLP);
        return (lpOfYield, newK);
    }

    /**
     * @dev Core calculation: given newK, compute LP yield for ownedLP based on growth in sqrt(K).
        * @notice DEPRECATED: this core helper is deprecated and retained for compatibility only.
        * @deprecated Do not rely on this helper in new code. It may be removed in a future cleanup.
     */
    function _calculateYieldForOwnedLP(
        uint256 reserveA,
