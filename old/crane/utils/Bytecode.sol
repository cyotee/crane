...
library Bytecode {
     ...
     */
    function _codeSizeOf(address addr) internal view returns (uint256 size) {
        assembly {
            size := extcodesize(addr)
        }
    }

    function _codeAt(address addr) internal view returns (bytes memory code) {
        assembly {
     ...
     */
    function _codeAt(address addr, uint256 start, uint256 end) internal view returns (bytes memory oCode) {
     ...
     */
    function _create(bytes memory initCode) internal returns (address deployment) {
        assembly {
     ...
     */
    function _createWithArgs(bytes memory initCode, bytes memory constructorArgs)
        internal
        returns (address deployment)
    {
        deployment = _create(_encodeInitArgs(initCode, constructorArgs));
    }
     ...
     */
    function _create2WithArgs(bytes memory initCode, bytes32 salt, bytes memory initArgs)
        internal
        returns (address deployment)
    {
     ...
     * @return deployment addr of the deployed contract, reverts on error
     */
    function _create3(bytes memory initCode, bytes32 salt, uint256 value) internal returns (address deployment) {
        // // Creation code
     ...
     */
    function _create3(bytes memory initCode, bytes32 salt) internal returns (address addr) {
