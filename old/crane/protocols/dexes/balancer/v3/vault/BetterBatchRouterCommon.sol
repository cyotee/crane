...
import {BetterRouterCommon} from "contracts/crane/protocols/dexes/balancer/v3/vault/BetterRouterCommon.sol";
...
abstract contract BetterBatchRouterCommon is BetterRouterCommon {
    ...
    function _initBetterBatchRouterCommon(IVault vault, IWETH weth, IPermit2 permit2, string memory routerVersion)
        internal
    {
        _initBetterRouterCommon(vault, weth, permit2, routerVersion);
    }
    ...
    function _calculateBatchRouterStorageSlot(string memory key) internal pure returns (bytes32) {
        return TransientStorageHelpers.calculateSlot(type(BetterBatchRouterCommon).name, key);
    }
