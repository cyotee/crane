...
library BetterInputHelpers {
    ...
    function castToOZIERC20(address[] memory tokens) internal pure returns (OZIERC20[] memory) {
        OZIERC20[] memory ozTokens = new OZIERC20[](tokens.length);
        for (uint256 i = 0; i < tokens.length; ++i) {
            ozTokens[i] = OZIERC20(address(tokens[i]));
        }
        return ozTokens;
    }

    function castToOZIERC20(IERC20[] memory tokens) internal pure returns (OZIERC20[] memory) {
        OZIERC20[] memory ozTokens = new OZIERC20[](tokens.length);
        for (uint256 i = 0; i < tokens.length; ++i) {
            ozTokens[i] = OZIERC20(address(tokens[i]));
        }
    ...
    function castToIERC20(OZIERC20[] memory tokens) internal pure returns (IERC20[] memory) {
        IERC20[] memory iTokens = new IERC20[](tokens.length);
        for (uint256 i = 0; i < tokens.length; ++i) {
            iTokens[i] = IERC20(address(tokens[i]));
        }
        return iTokens;
    }
}
